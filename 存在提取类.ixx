...
private:
    ...
    static inline void 打包二值图_8x8块(const std::vector<std::uint8_t>& maskN, int N, VecIU64& outBlocks)
    {
        outBlocks.clear();
        if (N <= 0 || (N % 8) != 0) return;
        ...
        const int BS = N / 8;
        outBlocks.assign((std::size_t)BS * (std::size_t)BS, 0ull);
        ...
        for (int by = 0; by < BS; ++by) {
            for (int bx = 0; bx < BS; ++bx) {
                ...
                outBlocks[(std::size_t)by * (std::size_t)BS + (std::size_t)bx] = bits;
            }
        }
    }

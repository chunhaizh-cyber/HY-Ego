...
/// 与 I64 体系兼容的无符号指数趋近下限（0）。
    /// - 当前值越大，下降越快；intensity 越大，下降越快；k 越大，下降越快。
    /// - 返回值保证：0 <= out <= 当前值。
export inline std::uint64_t 非负64位_指数趋近下限(std::uint64_t 当前值, double intensity, double k) noexcept
    {
        if (当前值 == 0) return 0;
...
/// 与 I64 体系兼容的无符号指数趋近上限（maxInclusive），并保留 1 的间隙（永远不达到 maxInclusive）。
    /// - 最高返回：maxInclusive-1（若 maxInclusive==0 则返回 0）。
    /// - 返回值保证：当前值 <= out <= maxInclusive-1（在可提升范围内）。
export inline std::uint64_t 非负64位_指数趋近上限_保留1间隙(
        std::uint64_t 当前值,
...
export inline std::uint64_t 非负双精度转无符号64位饱和(double 数值) noexcept
    {
    ...
    export inline std::int64_t 非负双精度转I64饱和(double 数值) noexcept
    {
        if (!std::isfinite(数值) || 数值 <= 0.0) {
            return 0;
        }

        const long double 上限 = static_cast<long double>((std::numeric_limits<std::int64_t>::max)());
        const long double 当前 = static_cast<long double>(数值);
        if (当前 >= 上限) {
            return (std::numeric_limits<std::int64_t>::max)();
        }

        return static_cast<std::int64_t>(std::llround(当前));
    }
    // -------------------- 小工具：遍历同层环（安全防御版） --------------------

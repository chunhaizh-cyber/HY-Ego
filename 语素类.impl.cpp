...
词节点类* 语素类::添加词(const std::string& 词字符串) {
    if (词字符串.empty()) {
        ...
        词节点类* 当前游标 =语素链.根指针;   // 第 0 层在根环，其后在“上一命中”的子链
        词节点类* 当前节点 = nullptr;
        ...
        for (size_t i = 0; i < 词字符串.length(); ++i) {
            ...
            // 查找当前前缀是否已存在
            当前节点 = (当前游标 == 语素链.根指针) ?
                语素链.查找节点_当前链(当前游标, &临时主信息, 枚举_比较字段::语素_词_词, 枚举_比较条件::相等) :
                语素链.查找节点_当前链(当前游标->子, &临时主信息, 枚举_比较字段::语素_词_词, 枚举_比较条件::相等);
            ...
            if (!当前节点) {
                ...
                if (当前游标 == 语素链.根指针) {
                    当前节点 = 语素链.添加节点(当前游标, 新词节点);
                }
                else {
                    当前节点 = 语素链.添加子节点(当前游标, 新词节点);
                }
                if (!当前节点) {
                    // 处理添加节点失败的情况
                    delete 新词节点->主信息;
...
词节点类* 语素类::添加二次特征词(语素节点类* 参照物, 语素节点类* 比较对象, 语素节点类* 比较类型) { return nullptr; }
 ...
 词节点类* 语素类::查找词(const std::string& 词字符串) {
     ...
     for (char ch : 词字符串) {
         ...
         auto* 节点 = (当前游标 == 语素链.根指针)
             ? 语素链.查找节点_当前链(当前游标, &临时, 枚举_比较字段::语素_词_词, 枚举_比较条件::相等)
             : 语素链.查找节点_当前链(当前游标->子, &临时, 枚举_比较字段::语素_词_词, 枚举_比较条件::相等);

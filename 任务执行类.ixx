...
namespace 任务执行模块_detail {
    inline const 词性节点类* 特征_尝试学习任务事实(const std::string& 键) {
        return 任务类::取任务事实特征(键);
    }

    inline const 词性节点类* 动作_推进一步_() {
        static const 词性节点类* type = 语素集.添加词性词("推进一步", "动词");
        return type;
    }

    inline const 词性节点类* 动作_自动筹办任务_() {
        static const 词性节点类* type = 语素集.添加词性词("自动筹办任务", "动词");
        return type;
    }

    inline const 词性节点类* 动作_确保步骤补条件子任务_() {
        static const 词性节点类* type = 语素集.添加词性词("确保步骤补条件子任务", "动词");
        return type;
    }

    inline const 词性节点类* 动作_推进步骤子任务_() {
        static const 词性节点类* type = 语素集.添加词性词("推进步骤子任务", "动词");
        return type;
    }

    inline const 词性节点类* 动作_执行步骤_() {
        static const 词性节点类* type = 语素集.添加词性词("执行步骤", "动词");
        return type;
    }
    ...
    struct 结构_任务动作事件守卫 {
        ...
        结构_任务动作事件守卫(
            ...
            时间戳 now,
            std::string cp)
            : 主信息(mi), 动作词(动作), 输入场景(输入), 输出场景(输出), 开始时间(now ? now : 结构体_时间戳::当前_微秒()), 调用点(std::move(cp))
        {
            if (!主信息 || !动作词) return;
            (void)任务类::记录任务动作动态(
                主信息,
        ...
        void 设置输出场景(场景节点类* 输出) noexcept { 输出场景 = 输出; }
    ...
    inline constexpr std::int64_t 任务错误码_无可执行方法 = 1001;
    inline constexpr std::int64_t 任务错误码_步骤等待子任务 = 1002;
    inline constexpr std::int64_t 任务错误码_结果写入失败 = 1003;
    inline constexpr std::int64_t 任务错误码_未注入学习分支 = 1004;
...
public:
    ...
    bool 推进一步(任务节点类* 任务头结点, 结构_任务返回消息* 返回消息 = nullptr) {
        ...
        const 时间戳 推进一步时间 = 结构体_时间戳::当前_微秒();
        结构_任务动作事件守卫 动作守卫(
            任务头结点->主信息,
            动作_推进一步_(),
            headInfo->场景,
        ...
        if (headInfo->状态 == 枚举_任务状态::未定义
            ...
            任务类::轻量设置任务状态(任务头结点, 枚举_任务状态::运行中, "任务执行器::推进一步/置运行中");
        }
        // 调度侧流水在“标记任务推进”统一写入，执行侧流水再交给“标记任务执行开始/结束”补齐。
        const bool ok = 私有_执行步骤(任务头结点, stepNode, 返回消息);
        动作守卫.设置结果(ok, ok ? 0 : 任务错误码_无可执行方法);
        return ok;
    }
...
private:
    ...
    任务节点类* 私有_自动筹办任务(任务节点类* 任务头结点) {
        ...
        auto* headInfo = 取头信息(任务头结点);
        if (!headInfo || !任务头结点->主信息) return nullptr;
        const 时间戳 自动筹办时间 = 结构体_时间戳::当前_微秒();
        结构_任务动作事件守卫 动作守卫(
            任务头结点->主信息,
            动作_自动筹办任务_(),
            headInfo->场景,
        ...
        动作守卫.设置结果(best != nullptr, best ? 0 : 任务错误码_无可执行方法);
        return best;
    }
    ...
    bool 私有_先推进步骤子任务(任务节点类* 任务头结点, 任务节点类* 步骤节点, bool& 子任务已全部完成, 结构_任务返回消息* 返回消息 = nullptr) {
        ...
        auto* stepInfo = 取步骤信息(步骤节点);
        if (!headInfo || !stepInfo || !任务头结点->主信息 || !步骤节点->主信息) return false;
        const 时间戳 推进子任务时间 = 结构体_时间戳::当前_微秒();
        结构_任务动作事件守卫 动作守卫(
            步骤节点->主信息,
            动作_推进步骤子任务_(),
            stepInfo->场景,
    ...
    bool 私有_执行步骤(任务节点类* 任务头结点, 任务节点类* 步骤节点, 结构_任务返回消息* 返回消息 = nullptr) {
        ...
        const 时间戳 任务执行开始时间 = 结构体_时间戳::当前_微秒();
        日志::运行f(
            "[任务执行器][调试] 执行步骤创建动作守卫前: 任务={}, 步骤={}",
            (void*)任务头结点,
            (void*)步骤节点);
        结构_任务动作事件守卫 动作守卫(
            步骤节点->主信息,
            ...
            "任务执行器::执行步骤/动作动态");
        日志::运行f(
            "[任务执行器][调试] 执行步骤创建动作守卫后: 任务={}, 步骤={}",
            (void*)任务头结点,
            (void*)步骤节点);
        日志::运行f(
            "[任务执行器][调试] 执行步骤设置当前步骤前: 任务={}, 步骤={}",
            (void*)任务头结点,
            (void*)步骤节点);
        (void)任务类::设置任务当前步骤(任务头结点->主信息, 步骤节点, 任务执行开始时间, "任务执行器::执行步骤/设置当前步骤");
        日志::运行f(
            "[任务执行器][调试] 执行步骤设置当前步骤后: 任务={}, 步骤={}",
            (void*)任务头结点,
            (void*)步骤节点);
        ...
        auto* 本次执行方法首节点 = stepInfo->当前选中方法首节点;
        日志::运行f(
            "[任务执行器][调试] 执行步骤记账开始前: 任务={}, 步骤={}, 方法={}",
            (void*)任务头结点,
            (void*)步骤节点,
            (void*)本次执行方法首节点);
        任务类::标记任务执行开始(
            ...
            "任务执行器::执行步骤/任务记账开始");
        日志::运行f(
            "[任务执行器][调试] 执行步骤记账开始后: 任务={}, 步骤={}, 方法={}",
            (void*)任务头结点,
            (void*)步骤节点,
            (void*)本次执行方法首节点);

        auto 结束任务记账 = [&](枚举_任务执行记账结果 结果, std::int64_t 错误码, 任务节点类* 最近结果节点 = nullptr, bool 转入学习 = false) {
            任务类::标记任务执行结束(
                任务头结点->主信息,
                结果,
                错误码,
        ...
        if (!stepInfo->当前选中方法首节点) {
            ...
            结束任务记账(枚举_任务执行记账结果::失败, 任务执行模块_detail::任务错误码_无可执行方法);
        ...
        if (私有_先推进步骤子任务(任务头结点, 步骤节点, 子任务已全部完成, 返回消息)) {
            ...
            if (!子任务已全部完成) {
                ...
                结束任务记账(枚举_任务执行记账结果::未完成, 任务执行模块_detail::任务错误码_步骤等待子任务);
                动作守卫.设置结果(true, 0);
                return true;
            }
        }
        ...
        日志::运行f(
            ...
            (void*)任务头结点,
            (void*)步骤节点);
        任务类::轻量设置任务状态(步骤节点, 枚举_任务状态::执行中, "任务执行器::执行步骤/步骤执行中");
        日志::运行f(
        ...
        动作守卫.设置输出场景(r.结果场景 ? r.结果场景 : headInfo->场景);
        动作守卫.设置结果(r.成功, static_cast<std::int64_t>(r.失败类型));
        ...
        if (!actualResultNode) {
            结束任务记账(枚举_任务执行记账结果::失败, 任务执行模块_detail::任务错误码_结果写入失败);
            写返回消息(枚举_任务返回消息类型::步骤失败, false, false, false, "步骤执行结果未能写入结果节点");
            动作守卫.设置结果(false, 任务错误码_结果写入失败);
            return false;
        }

        // 1210: 叶子热路径先只更新内存态镜像，结果指针与分支动作的虚拟存在写回延后到低频整理。
        headInfo->最近实际结果节点 = actualResultNode;
        任务类::轻量设置任务状态(步骤节点, r.成功 ? 枚举_任务状态::完成 : 枚举_任务状态::失败, "任务执行器::执行步骤/步骤结束");
        ...
        if (nextStep) {
            ...
            结束任务记账(
                r.成功 ? 枚举_任务执行记账结果::成功 : 枚举_任务执行记账结果::失败,
                r.成功 ? 0 : static_cast<std::int64_t>(r.失败类型),
                actualResultNode);
            写返回消息(枚举_任务返回消息类型::步骤完成, false, true, false, r.摘要.empty() ? "步骤完成，已进入下一阶段" : r.摘要);
            动作守卫.设置结果(r.成功, r.成功 ? 0 : static_cast<std::int64_t>(r.失败类型));
            return true;
        }
        ...
        if (!r.成功) {
            ...
            if (action == 枚举_分支选择动作::回跳重试 && stepInfo->已重试次数 < retryLimit) {
                结束任务记账(枚举_任务执行记账结果::失败, static_cast<std::int64_t>(r.失败类型), actualResultNode);
                ++stepInfo->已重试次数;
                (void)私有_切换到下一个方法(stepInfo);
                任务类::设置任务状态(步骤节点, 枚举_任务状态::就绪, "任务执行器::执行步骤/重试回到就绪");
                任务类::设置任务状态(任务头结点, 枚举_任务状态::运行中, "任务执行器::执行步骤/任务等待重试");
                动作守卫.设置结果(false, static_cast<std::int64_t>(r.失败类型));
                return true;
            }

            if (action == 枚举_分支选择动作::转入尝试学习) {
                auto* 尝试学习任务 = 私有_确保尝试学习任务();
                if (!尝试学习任务 || !尝试学习任务->主信息) {
                    headInfo->等待学习唤醒 = false;
                    headInfo->等待学习方法首节点 = nullptr;
                    任务类::设置任务状态(任务头结点, 枚举_任务状态::失败, "任务执行器::执行步骤/缺少尝试学习分支任务");
                    结束任务记账(
                        枚举_任务执行记账结果::失败,
                        任务执行模块_detail::任务错误码_未注入学习分支,
                        actualResultNode);
                    写返回消息(
                        枚举_任务返回消息类型::步骤失败,
                        false,
                        ...
                        false,
                        r.摘要.empty()
                        ? "步骤执行失败，且未注入学习分支，无法转入尝试学习"
                        : (r.摘要 + "；未注入学习分支，无法转入尝试学习"));
                    动作守卫.设置结果(false, 任务错误码_未注入学习分支);
                    return true;
                }
                const 时间戳 now = 结构体_时间戳::当前_微秒();
                auto* 当前学习方法 = stepInfo->当前选中方法首节点;
                if (!当前学习方法 && !stepInfo->可用方法首节点列表.empty()) {
                    当前学习方法 = stepInfo->可用方法首节点列表.front();
                }
                headInfo->等待学习唤醒 = true;
                headInfo->等待学习方法首节点 = 当前学习方法;
                if (尝试学习任务 && 尝试学习任务->主信息) {
                    auto* 学习头信息 = dynamic_cast<任务头结点信息*>(尝试学习任务->主信息);
                    if (学习头信息) {
                        const auto* 待处理特征 = 任务执行模块_detail::特征_尝试学习任务事实("尝试学习_待处理");
                        const auto* 请求时间特征 = 任务执行模块_detail::特征_尝试学习任务事实("尝试学习_请求时间");
                        const auto* 请求次数特征 = 任务执行模块_detail::特征_尝试学习任务事实("尝试学习_请求次数");
                        const auto* 来源优先级特征 = 任务执行模块_detail::特征_尝试学习任务事实("尝试学习_来源优先级");
                        const auto* 错误码特征 = 任务执行模块_detail::特征_尝试学习任务事实("尝试学习_错误码");
                        const auto* 来源任务特征 = 任务执行模块_detail::特征_尝试学习任务事实("尝试学习_来源任务头指针");
                        const auto* 来源步骤特征 = 任务执行模块_detail::特征_尝试学习任务事实("尝试学习_来源步骤指针");
                        const auto* 来源方法特征 = 任务执行模块_detail::特征_尝试学习任务事实("尝试学习_来源方法指针");
                        const auto* 来源结果特征 = 任务执行模块_detail::特征_尝试学习任务事实("尝试学习_来源结果指针");

                        const auto 请求次数 = 学习头信息->任务虚拟存在
                            ? 世界树.读取特征_I64(
                                学习头信息->任务虚拟存在,
                                请求次数特征,
                                "任务执行器::执行步骤/读取尝试学习请求次数").value_or(0)
                            : 0;
                        const std::int64_t 来源优先级 = (std::max)(
                            headInfo->调度优先级,
                            static_cast<std::int64_t>(stepInfo->基准优先级 + stepInfo->局部优先级偏移));
                        ...
                        (void)任务类::写任务虚拟存在_I64特征(
                            ...
                            来源优先级,
                            now,
                            "任务执行器::执行步骤/尝试学习来源优先级");
                        (void)任务类::写任务虚拟存在_I64特征(
                            学习头信息,
                            错误码特征,
                            static_cast<I64>(r.失败类型),
                            now,
                            "任务执行器::执行步骤/尝试学习错误码");
                        (void)任务类::写任务虚拟存在_指针特征(
                            学习头信息,
                            来源任务特征,
                            reinterpret_cast<std::uintptr_t>(任务头结点),
                            now,
                            "任务执行器::执行步骤/尝试学习来源任务");
                        (void)任务类::写任务虚拟存在_指针特征(
                            学习头信息,
                            来源步骤特征,
                            reinterpret_cast<std::uintptr_t>(步骤节点),
                            now,
                            "任务执行器::执行步骤/尝试学习来源步骤");
                        (void)任务类::写任务虚拟存在_指针特征(
                            学习头信息,
                            来源方法特征,
                            reinterpret_cast<std::uintptr_t>(当前学习方法),
                            now,
                            "任务执行器::执行步骤/尝试学习来源方法");
                        (void)任务类::写任务虚拟存在_指针特征(
                            学习头信息,
                            来源结果特征,
                            reinterpret_cast<std::uintptr_t>(actualResultNode),
                            now,
                            "任务执行器::执行步骤/尝试学习来源结果");
                        if (学习头信息->状态 == 枚举_任务状态::挂起
                            || 学习头信息->状态 == 枚举_任务状态::未启动
                            || 学习头信息->状态 == 枚举_任务状态::完成
                            || 学习头信息->状态 == 枚举_任务状态::失败
                            || 学习头信息->状态 == 枚举_任务状态::无法执行
                            || 学习头信息->状态 == 枚举_任务状态::待重筹办
                            || 学习头信息->状态 == 枚举_任务状态::未定义) {
                            任务类::设置任务状态(
                                尝试学习任务,
                                枚举_任务状态::就绪,
                                "任务执行器::执行步骤/尝试学习任务复位为就绪");
                        }
                        (void)任务类::设置任务基准优先级(
                            学习头信息,
                            来源优先级,
                            now,
                            "任务执行器::执行步骤/尝试学习任务基准优先级");
                        (void)任务类::设置任务调度优先级(
                            学习头信息,
                            来源优先级,
                            now,
                            "任务执行器::执行步骤/尝试学习任务调度优先级");
                    }
                }
                任务类::设置任务状态(任务头结点, 枚举_任务状态::待重筹办, "任务执行器::执行步骤/转入尝试学习待重筹办");
                结束任务记账(
                    枚举_任务执行记账结果::失败,
                    static_cast<std::int64_t>(r.失败类型),
                    actualResultNode,
                    true);
                写返回消息(
                    枚举_任务返回消息类型::步骤无法执行,
        ...
        if (需要重筹办) {
            ...
            结束任务记账(枚举_任务执行记账结果::成功, 0, actualResultNode);
    ...
    任务节点类* 私有_确保尝试学习任务() {
        ...
        日志::运行_错误("[任务执行器] 未注入尝试学习分支；禁止自动创建独立尝试学习任务头");
        return nullptr;
    }

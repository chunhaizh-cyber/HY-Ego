...
public:
    using 节点类型 = 基础信息节点类;
    场景节点类* 现实世界 = nullptr;  // 现实世界根场景（在 宇宙链 上的一棵子树）   
    场景节点类* 文本世界 = nullptr;
    场景节点类* 想象世界 = nullptr;
    场景节点类* 记忆世界 = nullptr;
    场景节点类* 推理世界 = nullptr;
...
public:
    ...
    void 初始化默认世界() {     
        ...
        现实世界 =宇宙链.添加节点(宇宙链.根指针, 主信息);
         主信息 = new 场景节点主信息类(); 
         文本世界 = 宇宙链.添加节点(宇宙链.根指针, 主信息);
         主信息 = new 场景节点主信息类();  
         想象世界 = 宇宙链.添加节点(宇宙链.根指针, 主信息);
         主信息 = new 场景节点主信息类();  
         记忆世界 = 宇宙链.添加节点(宇宙链.根指针, 主信息);
         主信息 = new 场景节点主信息类();  
         推理世界 = 宇宙链.添加节点(宇宙链.根指针, 主信息);
         主信息 = new 场景节点主信息类();  
         附属世界 = 宇宙链.添加节点(宇宙链.根指针, 主信息);
        主信息 = new 场景节点主信息类();  
        虚拟世界 = 宇宙链.添加节点(宇宙链.根指针, 主信息);

	//	auto* 自我词性节点 = 语素集.添加词性词("", "代词");
        主信息 = new 场景节点主信息类();
        auto* 自我存在主信息 = new 存在节点主信息类();
        自我所在场景=宇宙链.添加子节点(现实世界, 主信息);//添加自我所在场景
        自我指针=宇宙链.添加子节点(自我所在场景,自我存在主信息);//添加自我存在节点           
       
    }

    //生成一个空白场景,并放在世界树根链上,可以有许多,会定期清除
    场景节点类* 添加场景(场景节点类* 上级场景=nullptr) {
        if(上级场景==nullptr){
            上级场景=宇宙链.根指针;
		}
        auto* 主信息 = new 场景节点主信息类();         
        auto* 节点 = 宇宙链.添加节点(上级场景, 主信息);
        return 节点;       
	}
	//根据名称和上级场景生成一个新场景

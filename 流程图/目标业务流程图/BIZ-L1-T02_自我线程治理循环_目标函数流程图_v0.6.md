...
flowchart TD
    ...
    N24A{"N24A【指令-判断】新任务承接意图生产门是否仍开放且与当前运行代次一致"}
    ...
    N07 -->|内部错误| N30
    N08 -->|内部错误| N30
    N10 -->|停止优先且无其它待交付输入；本批无需业务下行| N27
    N10 -->|拒绝/内部错误| N30
    N10 -->|已路由| N11
...
| N21—N27 | 判断 / 函数调用 / 提交 | 当前批次全部结果和新任务意图生产门 | 需求重判、正差距、治理后继、下行意图或停产期间未下行结果 | 只经具名owner写入；线程只写私有游标 | 全批完成或具名重试 | 形成D意图后、分配任何新键前重读门；冻结时零下行并完成当前批次 |

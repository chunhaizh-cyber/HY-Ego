...
| N06—N10 | 函数调用 | 冻结批次与共享G0 | 一致事实、维护、调度和有序路由 | 只经具名owner读写；线程无领域写权 | 全部完整后进入逐项路由 | 全部来源截止互证 |
...
- 恢复002-02一致事实、003-03执行权限、002-06显式需求核算父链回流、004-01正差距和002-07全局治理后继在根循环中的明确位置。

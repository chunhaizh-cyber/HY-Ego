...
被调函数流程图：BIZ-L3-004-06-02—04需按 v0.2 重基线为目标与fresh当前事实读取、统一比较、需求独立结算/服务输入投影
...
flowchart TD
    ...
    N02["N02【函数调用】同G0读取需求目标合同与目标宿主当前事实 -> fresh材料"]
    N03{"N03【指令-判断】目标、当前事实、宿主、特征、场景、来源和版本是否完整同义"}
    N04["N04【函数调用】按注册比较合同裁决目标是否命中 -> 比较结果"]
    N05{"N05【指令-判断】当前状态是否满足本需求目标"}
    N06["N06【指令-构造】形成零动作已满足核算候选；可选来源引用只作线索/归因"]
    N07["N07【函数调用】需求owner提交或重放独立正式结算 -> 结算结果"]
    N08["N08【函数调用】独立读回本需求结算并逐字段互证 -> 读回结果"]
    N09{"N09【指令-判断】是否需要形成服务生存聚合输入"}
    N10["N10【函数调用】按正式服务绑定投影值式服务生存聚合输入 -> 投影结果"]
...
| N09/N10 | 判断/投影 | 正式需求结算和服务绑定 | 可选服务生存输入 | 不写服务值/合同/动态 | 非服务需求、合同失效、准备绑定 |

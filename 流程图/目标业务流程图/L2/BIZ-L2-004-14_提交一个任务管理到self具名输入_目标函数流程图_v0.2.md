...
本图完整替代 v0.1“提交自我治理消息”。旧图允许任务结果、等待、许可等模糊载荷，并让桥保存第二份待重发状态和自定义消息优先级；v0.2每次只提交一个由 T03 已经形成的具名强类型输入，重试材料由 T03 原处理槽持有，邮箱只按停止优先和稳定到达顺序冻结批次。
...
参数：四类受支持载荷中的恰一具名强类型载荷、运行代次和输入幂等身份；载荷内部保存自身正式截止和来源版本

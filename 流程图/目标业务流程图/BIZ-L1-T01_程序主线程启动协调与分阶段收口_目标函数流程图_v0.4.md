...
flowchart TD
    subgraph S1["承接 BIZ-L0-01 系统启动"]
        ...
        N12["N12【函数调用】启动任务管理线程(已进入T04线程池租约) -> T03启动结果"]
...
| --- | --- | --- | --- |
| BIZ-L0-01 系统启动 | N01—N16 | 001-03完成初始化与T02停门；支持旁路先启动；再启动T04消费者池、T03生产者、T06，开放T02治理门并按模式启动T05 | HEAD内联001-03子步骤，并由运行包先启动T03、后启动一个旧结果尾部线程；消费者—生产者顺序与T04能力均漂移，支持、T06、T05仍为空壳 |
...
- T04全部进入时只停在关闭接收门后的等待态，不能抢跑消费；T03取得完整池租约并成功进入后，才拥有后继派发和开放接收条件的资格。

...
flowchart TD
    subgraph S1["承接 BIZ-L0-01 系统启动"]
        ...
        N09{"N09【指令-判断】信号、程序控制停止来源和致命线程故障停止来源租约是否完整"}
    ...
    subgraph S3["承接 BIZ-L0-03 运行模块结束"]
        ...
        N23["N23【指令-读取/构造】非破坏性重读三类来源当前首次锁存；汇总首要原因、各收口结果、可得的首次致命补充材料和未释放租约"]
...
- N18只在三类来源中锁存本次值式观察结果内第一项验真通过的当前代次请求；通知只唤醒，进程信号锁存、程序控制停止锁存及其首次记录、致命线程故障停止锁存及其首次记录必须分别非破坏性重读并互证。本地观察批标记不进入任何来源。停止阶段不可撤销或倒退。
...
| N23 | 停止来源末次重读非成功 | 保留首要停止原因和既有收口结果，标记末次来源材料不完整；不得重开或倒退停止阶段，也不得补造全部后到故障历史 |
...
- 三类停止来源租约覆盖完整收口，N23在返回前再次读取各来源当前首次锁存。若致命来源此前为空、收口期间首次锁存，可把该一份材料作为补充；不得宣称枚举全部后到故障或按严重度选择。后到历史语义受 `DRIFT-BIZ-HOST-FATAL-FAULT-CLASSIFIER-AND-SOURCE-ABI-UNFROZEN` 限制。

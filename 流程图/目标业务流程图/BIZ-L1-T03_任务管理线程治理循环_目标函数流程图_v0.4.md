# BIZ-L1-T03 任务管理线程治理循环目标函数流程图 v0.4
...
flowchart TD
    ...
    N05["N05【函数调用】BIZ-L2-004-02 按具体需求D建立或复用当前任务 -> 任务承接结果"]
    N06{"N06【指令-判断】任务树结果"}
...
| --- | --- | --- | --- |
| 具体需求D任务承接意图 | 独立复核D、G0、批次和选择见证；正式读取D所属L，以L查询0..1当前任务后调用task owner承接 | 当前任务材料完整后进入N14任务主阶段治理 | 不把消息发送成功当任务已建立；不从列表其它成员、旧任务缓存或事实变化推断D |
...
当前 `BIZ-L2-004-02 v0.1` 子图仍把输入写成“正差距 -> 建立或复用需求任务树”，与本图已经冻结的具体 D 承接接口不一致。该子树排入后续 `BIZ-L2-004-02-D-ONLY-TASK-ACCEPTANCE-REBASE` 校正：只能读取 D 所属 L、按 L 查询 0..1 当前任务并由 task owner 建立 / 复用 `T→D`，不得重新建立需求或从正差距补造 D。本项是后继图校正，不改变本图的 T03 输入职责。
...
- T02输入由模糊消息组改为每次一个具名强类型输入；当前只分流具体D承接意图与self后继决议定位。

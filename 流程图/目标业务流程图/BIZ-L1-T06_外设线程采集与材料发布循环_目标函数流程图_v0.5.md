...
已发布源码证据：启动头/源 `a01c6351` / `a29a493b`、模拟/空材料线程头 `29697f13`、D455协议头/源 `32a5c7a8` / `cd1fcfe9`、
D455采集器头/源 `a4d3248f` / `a84835ea`、D455缓存头 `3785f58c`、D455上行桥头 `a0714adc`、线程生命周期数据头 `71bd6068`、投影服务头 `3dab32c1`
```
...
当前发布源码：生产T06类、模块和根函数不存在；海中鱼巣/启动.应用程序.h、海中鱼巣/启动.应用程序.cpp中的bool启动外设线程组()和void收口并连接外设线程组()均为空壳
...
| D455真实采集适配器 | `海中鱼巣/适配/采集器.D455相机.h`、`海中鱼巣/适配/采集器.D455相机.cpp`具备打开、同步帧采样、停止和状态快照；工程默认关闭RealSense编译，且生产启动链没有构造调用 | 特定外设采集基础存在但未接线 |

...
flowchart TD
    ...
    subgraph S2["承接 BIZ-L0-02 运行模块运行"]
        ...
        N18["N18【函数调用】等待程序停止请求(已冻结来源读取组, T01停止阶段锁存端) -> 来源已互证且已锁存停止结果"]
    end
...
| BIZ-L0-02 运行模块运行 | N17—N18 | T02—T06并发运行；主线程等待正式闭合的停止来源组并按冻结仲裁锁存一个停止请求 | 当前代码只等待进程信号，来源宇宙、仲裁和T01停止阶段锁存均未闭合 |

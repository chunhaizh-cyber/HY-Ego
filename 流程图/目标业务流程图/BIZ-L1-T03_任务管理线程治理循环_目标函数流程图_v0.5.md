...
被调函数流程图：BIZ-L2-003-03 v0.4、004-02、004-04、004-08—14、004-16、004-17，共12个直接根
```
...
flowchart TD
    ...
    N30["N30【函数调用】BIZ-L2-004-17 v0.1 形成合法无执行短路结果并收束当前任务轮次 -> 零动作收束结果"]
    N31{"N31【指令-判断】零动作结果、轮次收束、0..1上级调用和self定位是否完整"}
    ...
    N16 -->|冲突或内部错误| N26
    N17 --> N18
    N18 -->|需要初次筹办或同轮后继筹办| N19 --> N20
    N18 -->|执行冻结完成并待普通派发| N27
    N18 -->|需要零动作收束| N30 --> N31
    N18 -->|等待、缺口或现实授权请求| N21
...
- 004-11 v0.2、004-12 v0.2、004-13 v0.2按正式T→E/当前轮次/ARCH-L2阶段、完整T→D逐需求材料、单项具名回流和阶段裁决分层串联；004-04只消费已冻结工作包，004-17收束零动作，004-14只提交值式self消息。旧任务目标副本、治理存在和治理状态不得进入N13。

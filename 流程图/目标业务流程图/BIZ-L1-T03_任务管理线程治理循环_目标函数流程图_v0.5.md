...
flowchart TD
    ...
    N21["N21【指令-构造】形成任务轮次结算定位、自我现实执行授权请求或任务治理缺口定位恰一"]
...
| 外设材料 | 只消费6340 v0.2正式承接结果；区分任务 / 工作项材料、动作验证、派生需求三件套、结构化缺口和普通处置 | 前两类唤醒既有任务治理；后两类形成任务治理缺口定位交T02；普通处置收口 | 不从报告直接建立需求或任务；派生需求三件套只是self后继候选，不是已建立需求 |
...
- 004-10后继校正：按6340补入活跃需求，并把承接产物分成任务 / 工作项材料引用、派生需求三件套、动作验证输入、结构化缺口和普通处置；派生候选只经任务治理缺口定位交self，不由T03建立需求或任务。

...
被调函数流程图：BIZ-L2-003-03 v0.3、004-02、004-04、004-08—14、004-16，共11个直接根
```
...
flowchart TD
    ...
    N27["N27【函数调用】BIZ-L2-003-03 v0.3 选择一个冻结后当前可普通派发任务 -> 派发选择结果"]
...
| 主阶段治理槽 | 调用唯一主阶段治理provider并按结构化状态路由；筹办 / 重筹办可直接形成相应工作包，执行冻结完成只进入正式待普通派发候选 | T04筹办工作包、003-03 v0.3普通派发选择或T02定位消息 | 不在管理线程同步筹办、执行方法动作或从单个槽临场拼调度证据 |
...
- `003-03 v0.3`只在执行冻结完成后、真实派发前先读取 task owner 中性当前任务全集及集合版本，再逐任务读取正式阶段、冻结和排队事实形成完整候选；只有返回已选择才构造执行工作包。控制态关闭、全部零调度资格、材料不足或漂移均不得退回队列到达顺序直接派发。
...
- 把 BIZ-L2-003-03 v0.3 作为第11个直接根接入任务管理线程；旧 T02 每批全局权限调用边继续退出。
...
- T03 与 L2 父级接线保持不变；003-03 v0.3 已把候选读取继续展开到中性当前任务全集、任务集合版本和逐任务BIZ候选事实两项L4读取叶。

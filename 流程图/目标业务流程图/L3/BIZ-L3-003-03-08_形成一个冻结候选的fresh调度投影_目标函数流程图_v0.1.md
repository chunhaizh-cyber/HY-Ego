...
归属模块 / 文件 / 目标分解深度：任务普通派发选择 / 目标候选：海中鱼巣/领域/内部治理/服务.任务普通派发选择.h 与 海中鱼巣/领域/内部治理/服务.任务普通派发选择.cpp（当前物理文件不存在） / BIZ-L3
可见性：module-private纯值函数；只由BIZ-L2-003-03 v0.5根组合器逐候选调用
...
- 本函数继续只是纯值投影：任务级安全调度适用场景、T→D、基础优先级及安全层来源必须先由BIZ-L3-003-03-07按同一Gnow完整形成；任一来源缺失只返回材料不足，不能由本函数从动作场景、self位置或队列补造。
- 当前发布源码没有本函数；6150/6170现有纯值片段只构成复用候选，不证明本目标函数已经接线。

...
业务主链承接：BIZ-MAIN-001_自我治理到需求任务方法结果结算_目标业务流程图_v0.5.md
```
...
flowchart TD
    subgraph L001["L0-01【系统启动】"]
        ...
        S04["按目标依赖顺序启动其它普通模式模块<br/>支持线程组 -> T04消费者池关闭门进入 -> T03生产者进入并开放T04门 -> 外设线程组"]
        S05["复核前述必需模块租约、进入见证和运行门结果<br/>成功后开放自我线程治理运行门"]
        S06["按模式进入控制面板窗口宿主或无窗口等待<br/>物理线程拓扑待下层冻结"]
...
-> 复核前述启动结果
-> 开放自我线程治理运行门
-> 按模式进入控制面板窗口宿主或无窗口等待（当前普通模式仍错误进入无窗口宿主）
-> 全部必需模块成功后进入 L0-02
```
...
| 控制面板运行宿主 | `启动.应用程序.ixx` | 普通模式窗口宿主或无窗口等待按冻结合同进入 | 当前函数只返回模式判断，普通模式仍走无窗口宿主；主线程/独立线程拓扑未冻结 |

HEAD当前仍是“任务管理线程先于单个旧结果尾部线程”，只属于实现事实，不能覆盖目标。`BIZ-L1-T01 v0.5`、`BIZ-L2-001-05/06`现冻结为：T04消费者池先全部进入关闭接收门后的等待态，T03生产者再进入；只有T03进入见证成立，001-06才原子开放T04接收门并形成完整T03/T04启动材料。L0只冻结共同门禁：任一必需模块未进入、T04门未合法开放、启动结果失败或自我治理门未开放，都不得进入L0-02。

## 2026-08-27 T03/T04父级接线校正

- 撤回“按当前代码顺序”和“已接纳先T03后T04为目标”的旧句；目标流程不得被现状反序覆盖。
- L0只展示消费者—生产者依赖：T04关闭门进入、T03进入、001-06开放T04门。候选创建、进入等待、失败回收和门事务留在L2—L4展开。
- 上述校正不改变“当前普通模式代码的实际顺序”证据段；该段继续用于登记HEAD漂移。
...
`BIZ-L1-T01 v0.5`已明确以唯一根函数`运行普通程序(启动模式)`分别承接普通模式的L0-01、L0-02和L0-03；`BIZ-L1-T02—T06`是L0-02中获准并发运行的线程入口，不是六个并列的L0子节点。支持线程组是T01下的旁路启动/收口子能力，不构造一个虚假的统一业务根循环。L0-04是终止接口，不再拆分。

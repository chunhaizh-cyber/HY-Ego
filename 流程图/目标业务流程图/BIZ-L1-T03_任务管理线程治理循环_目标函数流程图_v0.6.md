...
| 主阶段治理槽 | 调用唯一主阶段治理provider并按结构化状态路由；初次 / 同轮后继筹办形成相应工作包，执行冻结完成只进入正式待普通派发候选，筹办期目标已满足进入004-17零动作收束 | T04筹办工作包、003-03 v0.4普通派发选择、004-17零动作收束；只有根任务轮次已收束才可形成T02定位消息 | 不在管理线程同步筹办、执行方法动作或从单个槽临场拼调度证据；不把零动作伪装成执行回执；不把子任务送入self |
...
- T03 与 L2 父级接线保持不变；003-03 v0.4及其读取叶表达共同作用场景目标互证，但HEAD正式6150/5230与代码尚未同步；不得恢复self位置、动作目标或公共祖先推导，也不得把目标图写成实现完成。

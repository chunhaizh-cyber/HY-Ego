...
- S0 时 `main == origin/main`、divergence `0/0`、index clean，#380 是唯一可执行候选。
- 最近 #380 WIP `2934596b9f673d01a3b72d468942f6912bf4cda4` 至 S0 的 14 个代码 / 工程文件与两份专属记录零变化。
- 验证期间基线前移到 `2c38ec4017e38b2e541b93c8c9ae038856f849c7`；前移提交只属于流程图维护切片，#380 代码、工程、计划、接口和记录均未被修改。

### 通过

1. 静态与构建
   - `git diff --check`：通过。
   - `python .\tools\check_specs.py --strict`：正式规范目录 `101/101`。
   - 四宏全关 Debug x64 fresh Rebuild：0 warning、0 error，约 47.45 秒。
   - 入口领域关键字与 stdout/stderr/printf：零命中。
   - 自检 import 只有设计允许的 `启动.应用程序.ixx` 两项。
2. V14 固定无效 SQL UI
   - 两轮均设置 `HY_EGO_SQL_SERVER=tcp:127.0.0.1,1`，取得非零窗口句柄，`CloseMainWindow()` 返回 true，并自然 `exit 0`。
   - 第 1 轮窗口约 16.262 秒出现，关闭后约 173 毫秒退出，总时长约 16.441 秒。
   - 第 2 轮窗口约 16.113 秒出现，关闭后约 241 毫秒退出，总时长约 16.361 秒。
   - 两轮均在固定 180 秒边界内完成；旧 90 / 180 秒不退出只保留为历史高延迟 / 高方差证据。

### 未闭合

1. 关系仓库性能专项稳定性
   - 使用上述四宏全关 fresh Debug 构建，同一 `--warehouse-performance-self-test-exit` 命令连续运行。
   - 第 1 轮约 266.443 秒，`exit 1`，stderr 为空。
   - 第 2 轮约 296.350 秒，`exit 0`，stderr 为空。
   - 同一构建、同一命令的连续退出码不一致，不能以单次成功覆盖失败，也不能声明性能专项稳定完成。
2. 性能专项实现、阈值和其失败来源不属于 #380 白名单；本计划不得修改来源或降低验收门限。

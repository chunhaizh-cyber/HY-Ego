...
`提交相关概念组合事务`在概念服务互斥范围内先用概念请求的owner幂等键读取私有概念端口首次材料。首次材料未找到时，才以参与者M+1重新调用准备函数；只有状态`已准备`且概念写集owner、参与者、代次和owner键精确匹配时才组装新请求。首次材料成功时，必须验证owner、owner键、首次H、首次结果和请求概念的权威读回，再把首次规范化写集无损转换为参与者M+1的v3写集，用于原组合键重放；首次材料残缺、错owner、错键或读回不同义返回内部不一致且不提交。这样发布后的同键重放不依赖在新当前代次重新准备，也不把当前已经存在的挂接误判成无需重放。

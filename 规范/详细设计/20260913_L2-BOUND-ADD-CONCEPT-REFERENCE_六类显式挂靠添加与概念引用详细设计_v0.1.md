...
状态：历史设计；共同出生及L2创建概念方向已被用户纠正，不具当前施工资格

## 0. 当前资格与替代顺序

本文原有“L2协调概念参与片段，与世界信息共同出生”的设计已经退出当前施工资格。下文相关DTO、参与者、共同事务、P1—P6路线和完成条件只保留形成历史，不得继续实施、局部恢复或用于解释当前WIP；已发布P0代码事实不因本文退出而改写。

当前正确顺序为：世界树类从唯一全局世界树根导航并取得明确挂靠；世界树持有待建信息，先调用概念树类按完整建点材料精确查询概念，确实无匹配时才调用概念树类创建概念；概念身份发布并读回后，世界树把该身份和显式挂靠交给对应L2建立世界信息节点、挂靠及概念引用。概念定义可以先于未来世界实例创建，定义不冒充世界支持证据；L2不创建概念，也不协调概念与世界信息共同出生。

已核实的实现阻碍只有边界事实：现有旧概念创建路径仍要求世界来源、上位或实例宿主，P0窄参与路径也要求稳定世界挂靠；这些入口不能直接承担“先概念、后世界”。所需新查询、独立概念创建及L2引用的精确DTO、owner映射、事务和迁移必须由后续设计另行冻结，本文不作裁决，也不生成新后继序列。
...
struct 相关概念待发布存在挂靠 { L1有限N分区原子参与者身份_v3 参与者; L1所有者范围写集本地键 本地键; };
struct 相关概念参与请求 { std::uint32_t 版本=1; std::uint64_t G0=0; L1所有者范围写入幂等身份 幂等身份; 相关概念方案 方案; 相关概念世界挂靠 挂靠; 相关概念读取预算 预算; std::optional<相关概念待发布存在挂靠> 待发布存在挂靠; };
...
`相关概念参与请求::待发布存在挂靠`是 P1 的追加字段。稳定`挂靠`有效时该字段必须为空；使用待发布挂靠时稳定`挂靠`必须为默认空值。待发布参与者必须严格早于概念参与者，目标必须是该前序写集中节点种类为普通的 E 本地键；P1 固定引用参与者1、本地键1。概念片段只可把形成端点写为该具名跨参与者引用。其它类别、反向引用、同参与者引用和不存在的键均返回`挂靠无效`且零写。提交成功后由 L2 从事务映射解析 E，构造只含稳定`概念树存在引用{E}`的读回请求；待发布引用不得进入持久事实或成功结果。
...
`提交相关概念组合事务`在概念服务互斥范围内先用概念请求的owner幂等键读取私有概念端口首次材料。首次材料未找到时，才以参与者M+1重新调用准备函数；只有状态`已准备`且概念写集owner、参与者、代次和owner键精确匹配时才组装新请求。稳定挂靠请求命中首次材料时，必须验证owner、owner键、首次H、首次结果和请求概念的权威读回，再把首次规范化owner写集机械转换为参与者M+1的v3写集：合同、代次、owner键、本地键、材料、退出事实和项顺序原样保留，稳定引用转为同一稳定引用，本owner本地引用使用v3事实引用联合中的plain `L1所有者范围写集本地键`分支并保持同一本地键，禁止附加参与者M+1编号。待发布存在挂靠请求命中首次材料时不单独重造组合写集，返回`已可能发布`且`已进入L1=false`，由L2按所有owner原键完成整体恢复。除请求中唯一合法的待发布存在挂靠外，概念写集禁止生成带编号跨参与者引用。反向规范化把plain本地键恢复为同一本owner本地引用并必须与首次材料逐字段相等，否则返回内部不一致且不提交。首次材料残缺、错owner、错键或读回不同义同样返回内部不一致且不提交。
...
1. P0：概念唯一 owner 的无总根窄参与合同及独立验证，已由结果提交`e825e764a576f12d347c8d232ec2500c85bd157f`发布。
2. P1：存在类和场景类公开添加入口及待发布存在挂靠，设计见同目录`20260913_L2-BOUND-ADD-SCENE-EXISTENCE-P1_场景存在显式挂靠纯添加详细设计_v0.1.md`。世界树消费者切换仍留P6。

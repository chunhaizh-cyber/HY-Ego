...
日期：2026-09-13
版本：v0.5
设计基线：`f8b47fefdd11b97da1da420b64de64a6fc8f4192`
状态：总体设计冻结；第一代码叶执行退回的状态、模块依赖、受限原子提交与本地引用编码合同缺口已修订
...
`提交相关概念组合事务`在概念服务互斥范围内先用概念请求的owner幂等键读取私有概念端口首次材料。首次材料未找到时，才以参与者M+1重新调用准备函数；只有状态`已准备`且概念写集owner、参与者、代次和owner键精确匹配时才组装新请求。首次材料成功时，必须验证owner、owner键、首次H、首次结果和请求概念的权威读回，再把首次规范化owner写集机械转换为参与者M+1的v3写集：合同、代次、owner键、本地键、材料、退出事实和项顺序原样保留，稳定引用转为同一稳定引用，本owner本地引用使用v3事实引用联合中的plain `L1所有者范围写集本地键`分支并保持同一本地键，禁止附加参与者M+1编号。L1 v3的带编号跨参与者引用只允许指向当前写集之前的参与者1..M；本概念写集不消费前序参与者的新建本地事实，因而继续禁止生成任何带编号跨参与者引用。反向规范化把plain本地键恢复为同一本owner本地引用并必须与首次材料逐字段相等，否则返回内部不一致且不提交。该写集只用于原组合键重放；首次材料残缺、错owner、错键或读回不同义同样返回内部不一致且不提交。这样发布后的同键重放不依赖在新当前代次重新准备，也不把当前已经存在的挂接误判成无需重放。

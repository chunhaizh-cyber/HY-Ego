# REMOVE-LEGACY-ACCEPTANCE-CODE 生产内旧验收体系物理退出详细设计 v0.1
...
正式基线：`main == origin/main == 40c110656c401d9bb2cd3d64e2764bb255427d19`，Git index 空。
...
1. 名称含“自检”的源码共 72 个；71 个进入 `海中鱼巣.vcxproj` 和 `海中鱼巣.vcxproj.filters`，`海中鱼巣/领域/自检.分层安全维护与任务权限.ixx` 是未编译残留。
...
| `海中鱼巣.vcxproj.filters` | 删除与上述 71 个编译项一一对应的 filter 节点；其它 filter 不动 |
...
4. 索引发布后，`REMOVE-LEGACY-ACCEPTANCE-CODE v0.1` 必须是唯一 `可执行` 计划；其它既有状态除 R1 退出外不改变。

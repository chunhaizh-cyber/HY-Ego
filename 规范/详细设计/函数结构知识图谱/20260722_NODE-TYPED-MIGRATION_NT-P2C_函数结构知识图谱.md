...
自我结构退役提供者
  -> 形成自我结构退役参与包
  -> 共享许可内读取全部关系审计并冻结全部历史谱系

自我结构退役参与包
  -> 执行一次
  -> 独占许可内重新枚举并比较历史角色 1 / 2 谱系
  -> P2C 关系写集恒为空，不再次失效历史关系
...
-> 退役提供者::形成自我结构退役参与包
   -> 读取全部关系审计并按关系编号分组
   -> 每个角色 1 谱系最高版本当前头必须已失效
   -> 冻结全部历史内部世界和角色 2 审计组
   -> 任一有效角色 1 / 2：内部不一致，禁止借退役修复
   -> 冻结其它当前关系和索引影响面
-> 自我结构退役参与包::执行一次
   -> 独占许可内重读历史谱系，P2C 关系写集为空
   -> #346 回调登记其它关系失效、索引移除、自我节点删除、读回和请求提交
   -> 核心执行器无额外参与者重载
-> 提交前失败：逆序撤销
-> 已提交：读回节点已删除、历史关系版本未变化、其它关系与索引已收口
-> 发布后读回失败：保留已提交，返回权威读回内部错误
```

当前自我、世界根、世界根内部世界场景普通退役拒绝；历史内部世界、成员及其它固定身份都不是本包删除目标。跨请求只有节点删除审计、全部关系 21 失效头、无有效相关关系和索引物理移除证明完全匹配时返回 `已退役幂等读回`；完全无节点审计才是合法未找到。
...
逻辑内返回
  ...
  退役幂等读回：节点已删除且历史关系 / 索引审计完全匹配

内部逻辑错误
  已发布域零个或多个竞争当前自我
  自我缺/多内部世界、自我是自身成员、成员重复或成环
  关系 21/22/19/20 角色、端点、基数或版本异常
  历史自我仍有有效角色 1 / 2，或历史谱系最高版本头不是已失效

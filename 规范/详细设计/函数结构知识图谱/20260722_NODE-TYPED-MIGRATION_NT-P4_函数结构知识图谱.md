...
flowchart LR
    ...
    ISO --> FREEZE["NT-C4/v0.4 冻结协调器::形成统一权威冻结材料"]
    ...
    CAP --> FIRST["NT-C5/v0.4 运行期上下文宿主::尝试首次发布已验证上下文"]
    FIRST --> EMPTY["仅空宿主单次原子发布"]
    EMPTY --> READBACK["正式服务发布后读回"]
    READBACK --> SELF["完整自我恢复证据：内部世界、成员、槽位、状态、动态"]

    OLD["旧快照只读材料：来源 / span / 长度 / SHA-256"] --> IDENTIFY["识别旧快照格式"]
    IDENTIFY --> CURRENT["HZYCSNP2 / 2 / 2：当前格式，不迁移"]
    IDENTIFY --> DAMAGE["损坏：布局证据不足"]
    IDENTIFY --> EMPTYSET["支持集为空：未知且无版本身份"]
    IDENTIFY --> LEGACY["未来已登记版本：离线迁移器::迁移旧快照到当前格式"]
    LEGACY --> FILE
    LEGACY --> REJECT["按六拒绝码拒绝；无歧义映射不成立"]
...
`NT-C4/v0.4` 与 `NT-C5/v0.4` 已冻结完整中文签名；九段记录、恢复 DTO 和旧格式 ABI 唯一读取 `规范/详细设计/NODE-TYPED-MIGRATION_NT-P4_九段记录ABI严格恢复与旧格式拒绝详细设计.md`：
...
`NT-C4-LEGACY/v1` 的当前函数图固定为：

```text
旧快照只读材料
-> 来源 / 长度 / SHA-256 自洽
-> HZYCSNP2 / 2 / 2：当前格式，版本身份为空，不迁移
-> 当前头截断 / 尾随 / 越界 / 溢出：损坏 + 布局证据不足
-> 其它材料：当前支持集合为空 + 版本身份为空
-> 未来合同升版且精确登记后，才进入字段 / 历史 / 跨段映射
-> 映射完整后调用 #349 编码，并强制交 #350 严格恢复验证
```

旧格式识别、迁移与报告完整 DTO、枚举值、字段顺序、六拒绝映射和借用生命周期读取九段详细设计 v0.3 与 #351 v0.6。当前不建立可解析旧布局夹具，也不把人读报告当迁移事实。
...
`NT-C4/v0.4` 格式节点固定为：魔数 `48 5A 59 43 53 4E 50 32`；容器版本 2；小端标识 1；容器头 104 字节；段头 64 字节；段清单版本 2；九段记录 ABI 版本 1；摘要算法 SHA-256/版本 1；冻结规则版本 1；稳定主键命名域 ABI 1；九段按类型 1—9 升序且各恰一段。上限为文件 `8,589,934,592` 字节、单段 `2,147,483,648` 字节、每段 `4,194,304` 项、单记录 `16,777,216` 字节、字符串和单向量各 `1,048,576`。容器摘要计算时将头偏移 72—103 视为零。
...
```text
P4-FREEZE 独占；提供 NT-C4/v0.4 格式 / 冻结 / 编码
...
P4-RESTORE 独占；按 NT-C4/v0.4 提供解析 / 恢复 / 验证
...
P4-LEGACY 独占；按 NT-C4/v0.4 提供旧材料识别 / 拒绝 / 审计
  旧格式只读结构
  离线迁移器
  旧格式迁移 / 拒绝自检

P4-CUTOVER 唯一共享接线所有者；提供 NT-C5/v0.4
  装配.运行期业务.ixx
  启动.运行期上下文.ixx
  启动.生产运行期.ixx
  生产运行期配置.数据.h
...
```mermaid
flowchart TD
    CONTRACT["NT-C1/v0.3 + NT-C1Q/v0.1 + NT-C2/v0.4 + NT-C3/v0.5 + NT-C4/v0.4"] --> F["P4-FREEZE 候选"]
    ...
    R --> PROVIDERS
    L --> PROVIDERS
    PROVIDERS --> ACCEPT["CUTOVER-ACCEPT/v2"]
    MAP["CUTOVER-MAP/v2"] --> GATE["CUTOVER-GATE/v2"]
    ACCEPT --> GATE
    DELETE["CUTOVER-DELETE/v1"] --> GATE
    WHITE["CUTOVER-WHITELIST/v1"] --> GATE
    GATE --> C["P4-CUTOVER / NT-C5/v0.4"]
    C --> ONE["一个最终切换提交"]
    ONE --> A["默认装配改指新域"]
    ONE --> B["旧生产入口不可达"]
    ONE --> S["影子权威读取不可达"]
...
六个 `CUTOVER-*` 节点是静态证据合同，不是当前 PASS。`CUTOVER-GATE/v2` 只有在共同集成基线绑定提供者固定集、合同接受矩阵、调用点映射、最终删除集和执行白名单的真实内容 blob 后才激活；白名单扫描只构成设计期候选上界。
...
冻结与格式
  -> 同代次全段冻结
  -> HZYCSNP2 / 容器头104 / 段头64 / 九段1—9 / 固定小端 / SHA-256 数值 ABI
...
最终切换
  -> CUTOVER-PROVIDERS / ACCEPT / MAP / DELETE / WHITELIST 五证据与共同基线由 CUTOVER-GATE/v2 闭合
...
| 日期 | 版本 | 修订内容 |
| --- | --- | --- |
| 2026-07-25 | v0.2 | 同步 `NT-C4-LEGACY/v1` 当前格式 / 损坏 / 空支持集识别链、完整 DTO 路由和未来六拒绝分支。 |
| 2026-07-22 | v0.1 | 建立 NT-P4 冻结、恢复、旧格式与最终切换函数结构图谱。 |

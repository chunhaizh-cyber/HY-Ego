...
inline void 自我线程类::自我初始化() {
    ...
    日志::运行f("[自我线程] 自我初始化完成: self={}, 待学习方法数量={}", (void*)自我指针, 待学习方法数量_);
}
...
inline void 自我线程类::处理消息_任务(const 任务消息& m) {
    // 说明：
    // - 本轮开始补齐“异步长等待回执”：外部回执到达后，把等待中的任务置回就绪。
    // - 其它任务消息暂以日志为主（后续你可接到任务树/门控/安全模型）。
...
inline void 自我线程类::初始化_初始需求与任务() {
    ...
    if (!学习任务节点_) {
        ...
        info->状态 = 枚举_任务状态::未启动;
        info->创建时间 = now;
        info->优先级 = (待学习方法数量_ > 0) ? 0 : (std::numeric_limits<std::int64_t>::min)();
    ...
    学习任务节点_->主信息->优先级 = (待学习方法数量_ > 0) ? 0 : (std::numeric_limits<std::int64_t>::min)();
...
inline void 自我线程类::学习线程入口() {
    while (!stop_.load()) {
        ...
        if (item.节点 == 学习任务节点_) {
            ...
            info->优先级 = (after > 0) ? 0 : (std::numeric_limits<std::int64_t>::min)();
            info->创建时间 = now;
            info->状态 = 枚举_任务状态::就绪;

...
inline 场景节点类* 自我线程类::私有_确保动作线程输出缓存场景(const std::string& 调用点) {
    if (动作线程输出缓存场景_ && 动作线程输出缓存场景_->主信息) {
        return 动作线程输出缓存场景_;
    }
    ...
    if (auto* 已有 = 私有_查找外设观察桥场景("自我线程_动作输出缓存")) {
        动作线程输出缓存场景_ = 已有;
        return 动作线程输出缓存场景_;
    }

    auto* mi = new 场景节点主信息类();
    mi->名称 = 语素集.添加词性词("自我线程_动作输出缓存", "名词");
    mi->类型 = 语素集.添加词性词("任务步骤执行场景", "名词");
    mi->最后观测时间 = 结构体_时间戳::当前_微秒();
    动作线程输出缓存场景_ = 世界树.创建场景(parent, mi, 调用点);
    return 动作线程输出缓存场景_;
}
...
inline 任务执行器::结构_叶子执行结果 自我线程类::私有_执行任务叶子步骤(任务节点类* 步骤节点) {
    ...
    auto 尝试学习兜底补候选 = [&]() {
        ...
        ctx.安全值_Q10000 = 饱和转I64(自我核心_.安全值());
        ctx.服务值_Q10000 = 饱和转I64(自我核心_.服务值());
...
inline void 自我线程类::校验根约束不变量(const std::string& 调用点) {
    const I64 安全值 = 自我核心_.安全值();
    const I64 服务值 = 自我核心_.服务值();
...
inline void 自我线程类::周期性维护() {
    const I64 安全前 = 自我核心_.安全值();
    const I64 服务前 = 自我核心_.服务值();
    ...
    私有_消费前端人类输入桥();
    私有_消费外设观察桥观察();
    私有_冲刷延迟任务事实队列("自我线程类::周期性维护/延迟任务事实", now);
    ...
    const I64 安全后 = 自我核心_.安全值();
    const I64 服务后 = 自我核心_.服务值();
...
inline I64 自我线程类::私有_读任务事实I64(const 任务信息基类* info, const std::string& 键) const {
    ...
    return 世界树.读取特征_I64(
        ...
        "自我线程类::私有_读任务事实I64/" + 键).value_or(0);
}
...
inline I64 自我线程类::私有_读任务方向I64(const 任务信息基类* info, const std::string& 键) const {
    ...
    return 世界树.读取特征_I64(
        ...
        "自我线程类::私有_读任务方向I64/" + 键).value_or(0);
}
...
inline 结构_结构B因果预测结果 自我线程类::私有_估计任务因果预测(任务节点类* 任务节点, bool 作为学习任务) const {
    ...
    累加方向(out.预测ΔS, "安全值", 1.8);
    累加方向(out.预测ΔS, "待机状态", 0.8);
    累加方向(out.预测ΔS, "时序正向步长", 0.4);
    累加方向(out.预测ΔS, "时序反向步长", -0.6);
    累加方向(out.预测ΔS, "服务时序衰减步长", -0.8);
    累加方向(out.预测ΔS, "试探动作_已停止", 0.7);
    累加方向(out.预测ΔS, "保守态_请求", 0.9);
    累加方向(out.预测ΔS, "需求对齐_安全", 0.5);
    累加方向(out.预测ΔS, "需求对齐_安全服务", 0.3);
    累加事实(out.预测ΔS, "风险级别", -0.6);

    累加方向(out.预测ΔV, "服务值", 1.8);
    累加方向(out.预测ΔV, "服务动作_输出有效", 1.0);
    累加方向(out.预测ΔV, "服务动作_已确认收到", 0.9);
    累加方向(out.预测ΔV, "服务动作_已请求补充信息", 0.4);
    累加方向(out.预测ΔV, "人类输入_存在", 0.3);
    累加方向(out.预测ΔV, "需求对齐_服务", 0.5);
    累加方向(out.预测ΔV, "需求对齐_安全服务", 0.2);
    ...
    out.置信度 = std::clamp(0.15 + static_cast<double>(证据数) * 0.08, 0.0, 0.95);
    return out;
}
...
inline void 自我线程类::私有_确保学习任务阶段节点(时间戳 now) {
    ...
    auto* 头信息 = dynamic_cast<任务头结点信息*>(学习任务节点_->主信息);
    if (!头信息) return;
    ...
    auto 创建步骤 = [&](任务节点类*& 槽位, const char* 名称, const char* 类型, std::int32_t 步骤序号) {
        ...
        参数.所属任务头结点 = 学习任务节点_;
        参数.步骤场景 = 头信息->场景 ? 头信息->场景 : 世界树.虚拟世界;
        参数.创建时间 = now;
        参数.步骤序号 = 步骤序号;
        参数.允许切换方法 = false;
        参数.初始状态 = 枚举_任务状态::就绪;
...
inline 自我线程类::结构_学习候选方法 自我线程类::私有_选择学习候选方法() const {
    ...
    auto 填充候选 = [&](方法节点类* 方法首节点, 结构_学习候选方法* 输出) -> bool {
        ...
        输出->缺条件 = !hasCondition;
        输出->缺结果 = !hasResult;
        输出->动作键 = hmi->动作名 ? hmi->动作名->获取主键() : std::string{};
        return true;
    };
    ...
    if (私有_学习任务有待处理请求()) {
        if (const auto 来源方法指针 = 私有_读取任务事实指针(学习任务节点_, "尝试学习_来源方法指针");
        ...
        if (const auto 来源步骤指针 = 私有_读取任务事实指针(学习任务节点_, "尝试学习_来源步骤指针");
            来源步骤指针.has_value() && *来源步骤指针 != 0) {
            auto 定向候选 = 从步骤提取候选(reinterpret_cast<任务节点类*>(*来源步骤指针));
            if (定向候选.head) {
                日志::运行f(
                    "[自我线程] 学习任务采用来源步骤候选: 步骤={}, 方法={}, 动作={}",
                    reinterpret_cast<void*>(*来源步骤指针),
                    (void*)定向候选.head,
                    定向候选.动作键.empty() ? std::string("<unknown>") : 定向候选.动作键);
                return 定向候选;
            }
        ...
        if (const auto 来源任务指针 = 私有_读取任务事实指针(学习任务节点_, "尝试学习_来源任务头指针");
            来源任务指针.has_value() && *来源任务指针 != 0) {
            auto* 来源任务头 = reinterpret_cast<任务节点类*>(*来源任务指针);
...
inline bool 自我线程类::私有_推进学习验证阶段(时间戳 now) {
    ...
    const bool 通过 = 学习工作流状态_.当前候选.head && 学习工作流状态_.上次有进展;
...
inline void 自我线程类::私有_推进学习提交阶段(时间戳 now) {
    ...
    if (学习工作流状态_.需要唤醒等待任务 && 学习工作流状态_.当前候选.head) {
        私有_唤醒等待学习的任务(学习工作流状态_.当前候选.head, "学习提交阶段");
    }

    日志::运行f(
        "[自我线程] 学习提交阶段完成: 方法={}, 候选={}, 更新={}, 新建={}, 观察成功={}",
        (void*)学习工作流状态_.当前候选.head,
...
inline void 自我线程类::私有_推进学习回滚阶段(时间戳 now) {
    ...
    日志::运行f(
        "[自我线程] 学习回滚阶段结束: 方法={}, 候选={}, 更新={}, 新建={}, 原因=验证未通过或无进展",
        (void*)学习工作流状态_.当前候选.head,
        static_cast<long long>(学习工作流状态_.候选数量),
        static_cast<long long>(学习工作流状态_.更新数量),
        static_cast<long long>(学习工作流状态_.新建数量));
...
inline bool 自我线程类::私有_学习任务有待处理请求() const {
    return 学习任务节点_
        && 学习任务节点_->主信息
        && 私有_读取任务事实I64(学习任务节点_, "尝试学习_待处理").value_or(0) > 0;
}
...
inline void 自我线程类::私有_排队任务事实指针(任务信息基类* info, const std::string& 键, std::uintptr_t ptr, 时间戳 now) {
    if (!info || !info->任务虚拟存在 || ptr == 0) return;
    std::lock_guard<std::mutex> lk(延迟任务事实锁_);
    延迟任务事实队列_.push_back(待写回任务事实项{
        info,
        键,
...
inline void 自我线程类::初始化_初始需求与任务() {
    ...
    调度器_.设置调度上下文查询回调([this]() {
        ...
        结构_结构B调度上下文 ctx{};
        ctx.当前安全值 = 归一化百分值(自我核心_.安全值());
        ctx.当前服务值 = 归一化百分值(自我核心_.服务值());
        ctx.安全下限 = 1.0;
        ctx.安全回归中心 = 50.0;
        return ctx;
    });
    ...
    if (启用学习线程_) {
        ...
        const auto 预算 = 调度器_.计算结构B预算(每次心跳最大派发数_, 1u);
        const auto 调制 = 调度器_.刷新任务结构B调制(
            学习任务节点_,
            true,
...
inline void 自我线程类::心跳调度_启动就绪任务() {
    ...
    if (!启用学习线程_) return;
    if (!学习任务节点_ || !学习任务节点_->主信息) return;
    const bool 有待处理请求 = 私有_学习任务有待处理请求();
    if (!有待处理请求 && 学习工作流状态_.当前阶段 == 枚举_学习任务阶段::空闲) return;
    if (预算.学习预算 == 0) return;
    ...
    const 时间戳 now = 结构体_时间戳::当前_微秒();
    const auto 来源优先级 = 私有_读取任务事实I64(学习任务节点_, "尝试学习_来源优先级").value_or(0);
    info->调度优先级 = 有待处理请求
        ? 来源优先级
        : ((待学习方法数量_ > 0) ? 0 : (std::numeric_limits<std::int64_t>::min)());
    info->创建时间 = now;
    if (调度器已初始化_.load()) {
        const auto 调制 = 调度器_.刷新任务结构B调制(
            学习任务节点_,
...
inline void 自我线程类::学习线程入口() {
    while (!stop_.load()) {
        ...
        if (item.节点 == 学习任务节点_) {
            ...
            if (待学习方法数量_ <= 0 && 学习工作流状态_.当前阶段 == 枚举_学习任务阶段::空闲) {

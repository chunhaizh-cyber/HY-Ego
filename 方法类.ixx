...
public:
        ...
        const std::string& 调用点 = "方法类::取或创建_方法虚拟存在")
    {
        ...
        // 缓存 miss 时优先信任方法头上已经挂好的虚拟存在根，避免同一方法重复造根。
        if (auto* base = dynamic_cast<方法信息基类*>(方法首节点->主信息)) {
            if (base->方法虚拟存在根) {
                std::unique_lock lk(元缓存_mtx_);
                cache_首到虚拟存在_[方法首节点] = base->方法虚拟存在根;
                return base->方法虚拟存在根;
            }
        }
        ...
        const std::string& 调用点 = "方法类::初始化方法虚拟存在信息")
    {
        ...
        if (hmi->动作句柄.类型 == 枚举_动作句柄类型::本能函数ID) {
            (void)记录方法虚拟存在特征(方法首节点, 特征_方法本能动作ID_, 特征快照值{ hmi->动作句柄.本能ID }, now, 调用点);
        }
        ...
        const std::string& 调用点 = "方法类::标记方法调用开始")
    {
        ...
        if (!输入场景) {
            // 1210: 最小试探热路径不在叶子执行前同步方法虚拟存在，避免与任务链/世界树写回形成嵌套热点。
            return;
        }
        ...
        同步场景特征到方法虚拟存在(方法首节点, 输入场景, now, 调用点 + "/输入");
        ...
        std::int64_t 耗时微秒 = 0)
    {
        ...
        if (!输出场景) {
            // 1210: 与开始记账对齐，最小试探热路径的结束账本延后到低频整理，不在动作线程叶子回调内写方法虚拟存在。
            return;
        }
    ...
    运行时动态主信息记录* 记录方法调用动态(
        ...
        const std::string& 调用点 = "方法类::记录方法调用动态")
    {
        using namespace 方法模块_detail;
        if (!方法首节点) return nullptr;
        私有_确保元词_();
        ...
        初始化方法虚拟存在信息(方法首节点, 开始时间, 调用点);
        auto* 方法存在 = 取或创建_方法虚拟存在(方法首节点, 结束时间, 调用点);
        auto* 状态场景 = 取或创建_方法内部状态场景(方法首节点, 结束时间, 调用点);
        if (!方法存在 || !状态场景) return nullptr;

        auto* 方法首信息 = MI首(方法首节点);
        const auto* 动作词 = (方法首信息 && 方法首信息->动作名)
            ? 方法首信息->动作名
            : 特征_方法动作_;
        auto* 方法动作特征 = 世界树.确保特征(
            方法存在,
            动作词,
            动作词,
            调用点 + "/动作特征");
        if (!方法动作特征) return nullptr;
        ...
        auto 填充来源信息 = [&](运行时动态主信息记录* 信息) {
            if (!信息) return;
            信息->来源类型 = 枚举_动态来源类型::方法调用;
            信息->来源方法首节点 = 方法首节点;
            信息->来源动作名 = 方法首信息 ? 方法首信息->动作名 : nullptr;
            信息->来源动作相位 = 动作相位;
            信息->来源输入场景 = 输入场景;
            信息->来源输出场景 = 输出场景;
            信息->来源执行成功 = 成功;
        ...
        // 因果当前按场景、且回退路径按“动态主体”做时序提炼。
        // 为了让“方法动作 -> 安全/服务状态”真正进入同一条自我内部时序，
        // 这里额外把动作镜像到“自我内部世界”根场景，并把主体挂到“自我存在”上。
        auto* 自我存在 = 世界树.自我指针;
        auto* 自我内部世界 = 世界树.取内部世界();
        if (!自我内部世界 && 自我存在) {
            auto* 名称 = 语素集.添加词性词("自我内部世界", "专有名词");
            自我内部世界 = 世界树.取或创建存在内部世界(
                自我存在,
                结束时间,
                名称,
                调用点 + "/统一事件流/内部世界");
        ...
        if (自我存在 && 自我内部世界) {
            auto* 自我动作特征 = 世界树.确保特征(
                自我存在,
                动作词,
                动作词,
                调用点 + "/统一事件流/动作特征");
            if (自我动作特征) {
                if (auto* 镜像动态 = 动态集.创建动作动态主信息(
                    自我内部世界,
                    自我存在,
                    自我动作特征,
                    开始时间,
                    ...
                    填充来源信息(镜像动态);
                    // 这里只把方法动作镜像进统一事件流。
                    // 同步方法调用路径里如果立刻触发因果/二次特征刷新，容易与当前场景写入形成重入卡死。
                    // 后续状态变化仍会读取这条事件动态并继续提炼因果。
                }
            }
        }
        return 动态信息;
        ...
        const std::string& 调用点 = "方法类::生成尝试学习参数")
    {
...

        {
            ...
            out.参数场景 = 世界树.创建场景(模板场景, smi, 调用点 + "/创建参数场景");
        }
        ...
        const std::string& 调用点 = "方法类::写入_运行证据_到节点虚拟存在")
    {
        ...
        auto 写签名并记录 = [&](存在节点类* host, const 词性节点类* featType, std::uint64_t value) {
            const auto vmax = static_cast<std::uint64_t>((std::numeric_limits<I64>::max)());
    ...
    void 注册本能函数(
        ...
        本能函数 fn,
        方法节点类* 方法首节点 = nullptr,
        const std::string& 调用点 = "方法类::注册本能函数")
    {
        if (本能ID == 0 || !fn) return;
        ...
        方法节点类* head = 方法首节点;
        if (!head) {
            head = 查找或创建_本能方法首节点(本能ID, now, 调用点);
        }
        if (head) {
            初始化方法虚拟存在信息(head, now, 调用点);
        }
    ...
    方法节点类* 注册本能函数(
        枚举_本能动作ID 本能ID,
        ::本能函数 场景本能函数,
        const std::string& 函数名称,
        时间戳 now = 0,
        const std::string& 调用点 = "方法类::注册本能函数(场景签名)")
    {
        if (本能ID == 枚举_本能动作ID::未定义 || !场景本能函数) return nullptr;
        if (now == 0) now = 结构体_时间戳::当前_微秒();

        const auto id64 = static_cast<std::uint64_t>(本能ID);
        const std::string 动作名文本 = 函数名称.empty()
            ? (std::string("本能_") + std::to_string(id64))
            : 函数名称;

        结构体_动作句柄 h{};
        h.类型 = 枚举_动作句柄类型::本能函数ID;
        h.本能ID = static_cast<I64>(id64);

        const 词性节点类* 动作名 = 语素集.添加词性词(动作名文本, "动词");
        auto* head = 注册方法首节点(动作名, h, 枚举_方法沉淀来源::未定义, {}, 调用点);
        if (head) {
            初始化方法虚拟存在信息(head, now, 调用点);
        }

        ::本能函数 包装后的场景本能函数 = [this, 场景本能函数 = std::move(场景本能函数), head, 调用点](场景节点类* 输入场景, 场景节点类* 输出场景) -> bool {
            ...
            try {
                (void)记录方法调用动态(
                    ...
                    枚举_动作事件相位::开始运行,
                    调用点 + "/动作动态/开始");
                const bool ok = 场景本能函数(输入场景, 输出场景);
        ...
        本能函数 适配方法本能 = [包装后的场景本能函数](结构_方法上下文& ctx, std::atomic_bool& cancel) -> 结构_方法执行结果 {
            结构_方法执行结果 out{};
            if (cancel.load()) {
                out.成功 = false;
                out.成功码 = -2;
            ...
            场景节点类* 输入场景 = ctx.输入观测包场景
                ? ctx.输入观测包场景
                : const_cast<场景节点类*>(ctx.当前场景);
            if (!输入场景) {
                输入场景 = ctx.任务条件场景;
            }
            场景节点类* 输出场景 = ctx.调用覆盖场景 ? ctx.调用覆盖场景 : 输入场景;

            const bool ok = 包装后的场景本能函数(输入场景, 输出场景);
            out.成功 = ok;
            out.成功码 = ok ? 0 : -1;
            out.结果存在集场景 = 输出场景;
            return out;
        ...
        注册本能函数(id64, std::move(适配方法本能), head, 调用点);
        (void)本能集.注册(本能ID, 包装后的场景本能函数, 动作名文本, head);
        return head;
    }
    ...
    结构_方法执行结果 调用本能函数(std::uint64_t 本能ID, 结构_方法上下文& ctx, std::atomic_bool& cancel) {
        本能函数 fn;
        方法节点类* 已注册方法首节点 = nullptr;
        {
            std::lock_guard<std::mutex> g(本能_mtx_);
            auto it = 本能表_.find(本能ID);
            if (it == 本能表_.end()) return {};
            fn = it->second;
            auto itHead = 本能方法首节点表_.find(本能ID);
            if (itHead != 本能方法首节点表_.end()) {
                已注册方法首节点 = itHead->second;
            }
        }

        if (!ctx.方法首节点 && 已注册方法首节点) {
            ctx.方法首节点 = 已注册方法首节点;
        }
        ...
        const 场景节点类* 当前场景 = nullptr)
    {
        ...
        ctx.调用覆盖场景 = 输出场景;
        ctx.方法首节点 = 方法首节点;
        ctx.当前场景 = 当前场景 ? 当前场景 : 输入场景;
        ctx.条件场景 = 输入场景;
        return 调用本能函数(static_cast<std::uint64_t>(本能ID), ctx, cancel);
    }
        ...
        const 场景节点类* 当前场景 = nullptr)
    {
        ...
        return 调用本能函数(本能ID, 输入场景, 输出场景, cancel, 方法首节点, 当前场景);
    }
...
private:
    ...
    // 桶/证据特征类型
    inline static const 词性节点类* 特征_桶签名_ = nullptr;
    inline static const 词性节点类* 特征_方法运行状态_ = nullptr;
    inline static const 词性节点类* 特征_方法动作_ = nullptr;
    ...
    static void 私有_确保元词_()
    {
        ...
        // 桶/证据特征（写入到桶虚拟存在）
        if (!特征_桶签名_) 特征_桶签名_ = 语素集.添加词性词("桶_签名", "名词");
        if (!特征_方法运行状态_) 特征_方法运行状态_ = 语素集.添加词性词("方法_运行状态", "名词");
        if (!特征_方法动作_) 特征_方法动作_ = 语素集.添加词性词("方法_动作", "名词");
    ...
    std::unordered_map<std::uint64_t, 方法节点类*> 本能方法首节点表_{};

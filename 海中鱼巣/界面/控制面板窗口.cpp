...
struct 控制面板窗口::窗口实现 {
    ...
    bool 重建当前树视图() {
        ...
        if (当前分类索引 == 0) {
            根项 = 插入控制面板树节点(TVI_ROOT, 当前快照.世界树结构材料.根节点, 当前分类索引);
        } else if (当前分类索引 == 2) {
            根项 = 插入控制面板树节点(TVI_ROOT, 当前快照.语素树结构材料.根节点, 当前分类索引);
        } else {
            根项 = 插入分类树根(当前分类索引);
        }
    ...
    void 更新系统信息页() const {
        ...
        if (当前系统信息是(系统信息导航::线程信息)) {
            ...
            文本 << L"仓库信息"
                ...
                << L"\r\n\r\n"
                << L"显示边界：仓库信息只做人读统计；节点、关系、索引仍以内存仓库结构为准。";

...
struct 控制面板窗口::窗口实现 {
    ...
    bool 读取只读材料() {
        ...
        if (!总览有效 || !树材料有效 || !统计快照有效 || !世界树材料有效) {
            标记追根因错误(L"控制面板只读材料、世界树快照或启动结构统计不符合已确认边界");
            return false;
        }
        当前快照.总览材料 = 总览材料;
        当前快照.树视图材料组 = 树材料组;
    ...
    HTREEITEM 插入世界树节点(HTREEITEM 父项, const 控制面板树节点材料& 节点材料) {
        ...
        插入材料.item.lParam = static_cast<LPARAM>(0);
        const auto 新项 = TreeView_InsertItem(树视图, &插入材料);
        if (新项 == nullptr) {
            标记追根因错误(L"控制面板世界树节点创建失败");
            return nullptr;
        }
        for (const auto& 子节点 : 节点材料.子节点组) {
            if (插入世界树节点(新项, 子节点) == nullptr) {
                return nullptr;
            }
        }
    ...
    bool 创建控件() {
        ...
        for (std::uint32_t 索引 = 0; 索引 < 树分类名称.size(); ++索引) {
            ...
            if (索引 == 0 && 插入世界树节点(新项, 当前快照.世界树结构材料.根节点) == nullptr) {
                return false;
            }
            TreeView_Expand(树视图, 新项, TVE_EXPAND);
            if (首项 == nullptr) {
    ...
    void 更新详情控件() const {
        ...
        if (树材料.分类 == 控制面板树分类::世界树) {
            ...
            文本 << L"第一轮边界：当前分类只显示树根入口，不生成虚假子节点。真实领域树子节点由后续只读 DTO 专项承接。";
        }
...
控制面板窗口::控制面板窗口(const 控制面板服务& 控制面板, const SQL数据库适配& 数据库,
    结构统计快照 启动结构统计, 控制面板树结构材料 世界树结构材料)
    : 实现(std::make_unique<窗口实现>(控制面板, 数据库, 启动结构统计, std::move(世界树结构材料))) {
}

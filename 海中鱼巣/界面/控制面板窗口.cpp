...
enum class 日志信息导航 : std::uint32_t {
    运行日志 = 0,
    调试日志 = 1,
    逻辑错误日志 = 2,
    事件日志 = 3
...
const std::array<const wchar_t*, 4> 日志信息导航名称{
    L"运行日志",
    L"调试日志",
    L"逻辑错误日志",
    L"事件日志"
...
const wchar_t* 树分类说明(控制面板树分类 分类) {
    ...
    case 控制面板树分类::概念树:
        return L"概念结构的只读导航入口。显示内容来自活动图四根抽象树投影。";
    case 控制面板树分类::语素树:
...
std::uint64_t 计算树节点数量(const 控制面板树结构材料& 树材料) {
    std::uint64_t 数量 = 0;
...
bool 树结构显示语义有效(const 控制面板树结构材料& 树材料, 控制面板树分类 预期分类) {
    return 树材料.状态 == 控制面板请求状态::可展示
        && 树材料.分类 == 预期分类
        && 树材料.可展示
        && 树材料.只读快照
        && 树材料.来源版本 != 0
        && !树材料.根节点组.empty()
        && 树材料.真实节点投影数量 == 计算树节点数量(树材料)
        && std::all_of(
            树材料.根节点组.begin(),
            树材料.根节点组.end(),
            [](const 控制面板树节点材料& 根节点) {
                return 根节点.关系角色 == 控制面板树关系角色::真实根
                    && 树节点显示语义有效(根节点);
            });
}
...
struct 控制面板窗口::窗口实现 {
    ...
    std::uint32_t 当前导航项数量() const {
        switch (当前分页) {
        case 控制面板分页::信息树:
            return static_cast<std::uint32_t>(树分类名称.size());
        case 控制面板分页::日志信息:
            return static_cast<std::uint32_t>(日志信息导航名称.size());
        case 控制面板分页::系统信息:
            return static_cast<std::uint32_t>(系统信息导航名称.size());
        }
        return 0;
    }

    std::uint32_t 当前导航索引() const {
        switch (当前分页) {
        case 控制面板分页::信息树:
            return 当前分类索引;
        case 控制面板分页::日志信息:
    ...
    const wchar_t* 当前导航名称(std::uint32_t 索引) const {
        switch (当前分页) {
        case 控制面板分页::信息树:
            return 索引 < 树分类名称.size() ? 树分类名称[索引] : L"";
        case 控制面板分页::日志信息:
            return 索引 < 日志信息导航名称.size() ? 日志信息导航名称[索引] : L"";
        case 控制面板分页::系统信息:
            return 索引 < 系统信息导航名称.size() ? 系统信息导航名称[索引] : L"";
        }
        return L"";
    }

    bool 当前系统信息是(系统信息导航 导航项) const {
        return 当前系统信息索引 == static_cast<std::uint32_t>(导航项);
    }

    bool 当前日志信息是(日志信息导航 导航项) const {
        return 当前日志信息索引 == static_cast<std::uint32_t>(导航项);
    }

    bool 重建导航栏() {
        if (导航栏 == nullptr) {
            标记追根因错误(L"控制面板导航栏重建入口无效");
            return false;
        }
        SendMessageW(导航栏, LB_RESETCONTENT, 0, 0);
        const std::uint32_t 数量 = 当前导航项数量();
        for (std::uint32_t 索引 = 0; 索引 < 数量; ++索引) {
            if (SendMessageW(导航栏, LB_ADDSTRING, 0, reinterpret_cast<LPARAM>(当前导航名称(索引))) == LB_ERR) {
                标记追根因错误(L"控制面板导航栏条目创建失败");
                return false;
            }
        }
        const std::uint32_t 选中索引 = 当前导航索引();
        if (数量 == 0 || 选中索引 >= 数量) {
            标记追根因错误(L"控制面板导航栏选中索引越界");
            return false;
        }
        SendMessageW(导航栏, LB_SETCURSEL, static_cast<WPARAM>(选中索引), 0);
        return true;
    }
    ...
    bool 重建全部树视图() {
        ...
        当前树项引用序号 = static_cast<std::size_t>(当前项.lParam);
        return true;
    }
    ...
    void 更新系统信息页() const {
        ...
        if (当前系统信息是(系统信息导航::线程信息)) {
            文本 << L"线程信息"
                << L"\r\n\r\n"
                << L"当前控制面板只显示线程设计和启动样例的人读摘要，不把线程状态作为动作来源。"
                << L"\r\n"
                << L"运行宿主线程：负责运行消息调度壳，线程本身不生成动作动态。"
                << L"\r\n"
                << L"任务管理线程：承接任务执行请求材料，入队不裁决任务完成。"
                << L"\r\n"
                << L"任务工作线程：消费请求并生成回执材料，回执仍需领域服务复核。"
                << L"\r\n"
                << L"缓存统计线程：只刷新非权威缓存统计，不改变权威仓库结构。"
                << L"\r\n"
                << L"事件日志线程：只消费人读审计材料，不恢复运行期结构。"
                << L"\r\n"
                << L"外设采样材料线程：当前只承载模拟或空外部材料，不接真实外设。";
        } else if (当前系统信息是(系统信息导航::仓库信息)) {
            文本 << L"仓库信息"
                << L"\r\n\r\n"
                << L"节点仓库数量：" << 当前快照.启动结构统计.节点数
                << L"\r\n"
                << L"关系仓库数量：" << 当前快照.启动结构统计.关系数
                << L"\r\n"
            ...
            文本 << L"数据库审计"
                ...
                << L"下方列表显示最近结构统计审计记录。"
                << L"\r\n"
                << L"数据库只作快照、事件日志、恢复和审计适配，不直接裁决运行期事实。";
        } else {
            文本 << L"控制面板信息"
                << L"\r\n\r\n"
                << L"控制面板模式：只读显示"
    ...
    void 更新日志页() const {
        SetWindowTextW(数据库标题文本, 日志信息导航名称[当前日志信息索引]);
        std::wostringstream 文本;
        文本 << 日志信息导航名称[当前日志信息索引]
            << L"\r\n\r\n"
            ...
            << L"\r\n\r\n";
        if (当前日志信息是(日志信息导航::运行日志)) {
            追加日志文件行(文本, 日志类别::运行);
        } else if (当前日志信息是(日志信息导航::逻辑错误日志)) {
            追加日志文件行(文本, 日志类别::逻辑错误);
        } else if (当前日志信息是(日志信息导航::事件日志)) {
            追加日志文件行(文本, 日志类别::事件);
        } else {
            文本 << L"调试日志："
                << (日志目录路径() / L"调试_<切片>.log").wstring()
                << L"\r\n";
            文本 << L"状态：按调试切片宏启用，默认不作为常驻显示依据。"
                << L"\r\n"
                << L"说明：调试日志必须在切片调试完成后关闭，防止无用日志泛滥。";
        }
        SetWindowTextW(详情文本, 文本.str().c_str());
    }
    ...
    bool 处理导航选择(std::uint32_t 选中索引) {
        ...
        case 控制面板分页::日志信息:
            ...
            return true;
        case 控制面板分页::系统信息:
            if (选中索引 >= 系统信息导航名称.size()) {
                标记追根因错误(L"控制面板系统信息导航索引越界");
                return false;
            }
            当前系统信息索引 = 选中索引;
            应用分页控件可见性();
    ...
    void 更新详情控件() const {
        ...
        if (树项 != nullptr && 树项->是结构节点 && 树项->节点材料 != nullptr) {
            ...
            文本 << L"所属信息树：" << 树分类名称[树项->分类索引]
                ...
                << L"\r\n"
                << L"投影关系角色：" << 控制面板树关系角色文本(节点材料.关系角色)
                << L"\r\n"
                << L"重复投影：" << (节点材料.重复投影 ? L"是" : L"否")
                << L"\r\n"
                << L"可读名称入口数：" << 节点材料.名称语素入口组.size()
                << L"\r\n"
            ...
            文本 << L"当前选中：" << 树分类名称[树项->分类索引]
                ...
                << 树分类说明(树材料.分类)
                << L"\r\n\r\n"
                << L"真实根数量：" << 树结构.根节点组.size()
                << L"\r\n"
                << L"真实投影项数量：" << 树结构.真实节点投影数量
                << L"\r\n"
                << L"来源版本：" << 树结构.来源版本
                << L"\r\n"
                << L"快照序号：" << 当前快照.六树候选序号
    ...
    static LRESULT CALLBACK 窗口过程(HWND 窗口, UINT 消息, WPARAM 参数一, LPARAM 参数二) {
        ...
        case WM_NOTIFY: {
            ...
            if (通知 != nullptr
                && 通知->hwndFrom == 当前实现->树视图
                && 通知->code == TVN_SELCHANGEDW
                && !当前实现->正在重建树视图) {
        ...
        case WM_COMMAND:
            if (reinterpret_cast<HWND>(参数二) == 当前实现->导航栏 && HIWORD(参数一) == LBN_SELCHANGE) {
                ...
                if (选中项 != LB_ERR) {
                    if (!当前实现->处理导航选择(static_cast<std::uint32_t>(选中项))) {
                        DestroyWindow(窗口);
                    }
                }
                return 0;

...
private:
  ...
  自我治理循环退出结果 运行治理循环() noexcept {
    ...
    while (true) {
      ...
      治理批次分类请求 分类请求;
      分类请求.治理批次身份 = 当前批次.批次序号;
      分类请求.当前单调时间 = 当前完整秒边界;
      分类请求.运行代次 = 运行代次_;
      auto 分类结果 = 复核并分类治理批次(分类请求);
      当前阶段 = 治理批次处理阶段::输入处理;
      if (分类结果.状态 == 治理批次分类状态::存在具名拒绝 ||
          分类结果.状态 == 治理批次分类状态::内部错误) {
        return 自我治理循环退出结果::领域治理终止;
      }
      // N08 → 空输入组 → N14；非空 → N09循环
      // 002-03 真实业务逻辑（逐项复核）仍属 §不授权范围 §2，骨架返回空批次

      // N09-N13: 输入处理循环（BIZ-L2-002-04/05, L2-004-06）
      // 空批次，跳过输入处理循环
      // TODO 待激活：按治理输入种类调用复核/裁决/结算

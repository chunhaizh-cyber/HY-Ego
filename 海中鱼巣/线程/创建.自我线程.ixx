...
private:
  ...
  自我治理循环退出结果 运行治理循环() noexcept {
    ...
    while (true) {
      ...
      分类请求.运行代次 = 运行代次_;
      const auto 分类结果 =
          批次路由_->复核并分类治理批次(分类请求);
      当前阶段 = 治理批次处理阶段::输入处理;
      if (分类结果.状态 == 治理批次分类状态::入口拒绝 ||
          分类结果.状态 == 治理批次分类状态::存在时效拒绝 ||
          分类结果.状态 == 治理批次分类状态::内部错误) {
        return 自我治理循环退出结果::领域治理终止;
      }
      ...
      // N09-N13: 输入处理循环（BIZ-L2-002-04/05, L2-004-06）
      for (const auto& 治理输入 : 分类结果.有序治理输入组) {
        (void)治理输入;
        治理输入事实复核请求 输入复核请求;
        输入复核请求.治理批次身份 = 当前批次.批次序号;
        输入复核请求.事实截止版本 = 当前共享事实截止;
        输入复核请求.运行代次 = 运行代次_;
        const auto 输入复核结果 = 复核治理输入绑定事实(输入复核请求);
        if (输入复核结果.状态 != 治理输入事实复核状态::可消费) {
          // 002-04 尚未实现；不得把已分类输入伪造成已消费事实。
          return 自我治理循环退出结果::领域治理终止;
        }
      }

      // N14: 重判需求父链与活动集（BIZ-L2-002-06）
      需求父链重判请求 重判请求;
      重判请求.治理批次身份 = 当前批次.批次序号;
      重判请求.事实截止版本 = 当前共享事实截止;
      auto 重判结果 = 重判需求父链与活动集(重判请求);
      当前阶段 = 治理批次处理阶段::正差距确认;
      if (重判结果.状态 != 需求父链重判状态::无变化 &&
          重判结果.状态 != 需求父链重判状态::已更新 &&
          重判结果.状态 != 需求父链重判状态::合法等待) {
        return 自我治理循环退出结果::领域治理终止;
      }

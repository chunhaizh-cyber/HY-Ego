...
public:
  explicit 自我线程(
      线程生命周期发布端口 *生命周期发布端口,
      const L2存在结构服务 *存在结构服务 = nullptr) noexcept
      : 生命周期发布端口_(生命周期发布端口),
        存在结构服务_(存在结构服务) {}
...
private:
  ...
  const L2存在结构服务 *存在结构服务_ = nullptr;
  ...
  自我治理循环退出结果 运行治理循环() noexcept {
    ...
    while (true) {
      ...
      一致事实请求.读取前共享事实截止 = 当前批次.共享事实截止;
      一致事实请求.完整秒边界 = 当前批次.完整秒.完整秒边界;

...
海中鱼巣_定义自我线程身份(自我线程逻辑身份);
海中鱼巣_定义自我线程身份(自我线程运行代次);
...
enum class 自我线程操作状态 : std::uint8_t {
    ...
    队列已满 = 6, 消息冲突 = 7, 运行代次不匹配 = 8, 等待超时 = 9,
    资源失败 = 10, 内部错误 = 11,
};
...
struct 自我线程正式上下文请求_v1 final {
    ...
    自我线程上下文读取预算_v1 预算;
    [[nodiscard]] bool 完整() const noexcept;
    friend bool operator==(const 自我线程正式上下文请求_v1&, const 自我线程正式上下文请求_v1&) = default;
};
...
struct 自我线程根需求复核消息_v1 final {
    ...
    自我线程复核触发根 触发根 = 自我线程复核触发根::双根;
    自我线程需求身份 正式需求定位;
    friend bool operator==(const 自我线程根需求复核消息_v1&, const 自我线程根需求复核消息_v1&) = default;
};

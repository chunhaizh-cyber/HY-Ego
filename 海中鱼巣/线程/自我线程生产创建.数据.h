...
// 预冻结 BIZ 候选：真实自我形成 DTO 与线程生命周期发布端口的最终 ABI
// 发布后必须机械匹配。本文件不定义 DATA 事实，也不把候选源码视为已编译。
namespace 海中鱼巣 {
...
struct 自我线程逻辑身份 final {
  std::uint64_t 值 = 0;
  friend bool operator==(const 自我线程逻辑身份 &,
                         const 自我线程逻辑身份 &) = default;
};

enum class 自我线程生命周期投影 : std::uint8_t {
  创建 = 1,
  启动中 = 2,
  等待治理运行门 = 3,
  停止请求中 = 4,
...
struct 自我线程生命周期发布请求 final {
  std::uint32_t 合同版本 = 自我线程生产创建合同版本;
  自我线程逻辑身份 线程逻辑身份;
  std::uint64_t 运行代次 = 0;
  自我线程生命周期投影 当前投影 = 自我线程生命周期投影::创建;
  std::optional<自我线程生命周期投影> 前一投影;
  std::uint64_t 原因键 = 0;
};

enum class 自我线程生命周期发布状态 : std::uint8_t {
  已发布 = 1,
  发布失败 = 2,
  端口未提供 = 3
};

struct 自我线程生命周期发布结果 final {
  自我线程生命周期发布状态 状态 =
      自我线程生命周期发布状态::端口未提供;
};
...
class 自我线程生命周期发布端口 {
public:
  virtual ~自我线程生命周期发布端口() = default;
  virtual 自我线程生命周期发布结果 发布(
      const 自我线程生命周期发布请求 &请求) noexcept = 0;
};

...
public:
    ...
    服务合同事实权威服务& 取得服务合同事实权威服务() noexcept {
        return *服务合同事实权威服务_;
    }

    const 服务合同事实权威服务&
    取得服务合同事实权威服务() const noexcept {
        return *服务合同事实权威服务_;
    }
...
private:
    ...
    普通应用上下文(L1事实基座运行包&& 运行包,
        ...
        std::unique_ptr<安全根定义与当前值服务>&& 安全根定义与当前值服务,
        std::unique_ptr<服务合同事实权威服务>&& 服务合同事实权威服务) noexcept
        : 运行包_(std::move(运行包)), 材料服务_(std::move(材料服务)),
          ...
          服务合同事实权威服务_(std::move(服务合同事实权威服务)) {}
    ...
    std::unique_ptr<服务合同事实权威服务> 服务合同事实权威服务_;
        ...
        auto 服务合同事实原始交付 =
            运行包.所有者范围签发器().建立所有者范围(
                {L1所有者范围CRUD合同版本,
                    服务合同事实权威所有者建立身份_v1,
                    L1所有者范围种类::独占结构范围});
        if ((服务合同事实原始交付.建立结果.状态
                    != L1所有者范围管理状态::成功
                && 服务合同事实原始交付.建立结果.状态
                    != L1所有者范围管理状态::精确重复)
            || !服务合同事实原始交付.建立结果.所有者事实
            || !服务合同事实原始交付.写入端口)
            return {普通应用装配状态::服务合同事实所有者范围建立失败,
                nullptr};
        auto 服务合同事实交付 = 尝试形成服务合同事实权威所有者交付_v1(
            运行包.读取服务(), std::move(服务合同事实原始交付));
        if (!服务合同事实交付)
            return {普通应用装配状态::服务合同事实所有者交付形成失败,
                nullptr};
        std::unique_ptr<服务合同事实权威服务> 服务合同事实服务;
        try {
            服务合同事实服务 = std::make_unique<服务合同事实权威服务>(
                运行包.读取服务(), std::move(*服务合同事实交付));
        } catch (...) {
            return {普通应用装配状态::服务合同事实权威服务构造失败,
                nullptr};
        }

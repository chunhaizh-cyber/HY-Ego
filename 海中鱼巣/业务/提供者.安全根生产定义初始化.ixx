...
public:
    ...
    void ARCH_注入下层来源规则读取状态(
        主动安全结算来源规则读取状态_v1 状态,
        std::uint8_t 次数 = 1) noexcept {
        ARCH_来源规则读取状态_ = 状态;
        ARCH_来源规则读取剩余次数_ = 次数;
    }

    void ARCH_注入下层来源规则发布状态(
    ...
    安全根生产定义初始化结果_v1 初始化并读取安全根生产定义_v1(
        ...
        try {
            ...
            for (std::uint8_t 尝试 = 0; 尝试 != 2; ++尝试) {
                ...
                const auto G规则 = 状态服务_.读取中性当前事实代次_v1();
                if (!G规则)
                    return 失败(安全根生产定义初始化状态_v1::内部错误);
                const 发布主动安全结算来源规则请求_v1 规则发布请求{
                    安全根定义合同版本_v1,
                    {L2结构合同版本, *G规则},
                    主动安全结算来源规则发布幂等身份_v1,
                    请求.本能根锚点,
                ...
                if (!来源规则需要同键重放) {
                    ...
                    if (当前规则.状态
                        ...
                        return 失败(映射来源规则读取失败(当前规则.状态));
                }

                const auto 规则发布 = 发布来源规则(规则发布请求);
                if (规则发布.成功()) {
                    if (!规则发布.规则
                        || !主动安全结算生产来源规则匹配_v1(
                            *规则发布.规则, 请求.本能根锚点))
...
private:
    安全根生产定义初始化结果_v1 形成成功(
        ...
        if (!读回.定义 || !安全根生产定义匹配_v1(*读回.定义, 锚点))
            return 失败(安全根生产定义初始化状态_v1::定义冲突);
        const auto 规则读回 = 读取当前来源规则({
            安全根定义合同版本_v1, {L2结构合同版本, *Gread}, 锚点});
        if (!规则读回.成功())
            return 失败(映射来源规则读取失败(规则读回.状态));
        if (!规则读回.规则
            || !主动安全结算生产来源规则匹配_v1(
                *规则读回.规则, 锚点)
            || 规则读回.规则->自我 != 读回.定义->自我
    ...
    static 安全根生产定义初始化状态_v1 映射来源规则读取失败(
    ...
    static 安全根生产定义初始化状态_v1 映射来源规则发布失败(
        主动安全结算来源规则发布状态_v1 状态) noexcept {
        switch (状态) {
        case 主动安全结算来源规则发布状态_v1::入口拒绝:
            return 安全根生产定义初始化状态_v1::入口拒绝;
        case 主动安全结算来源规则发布状态_v1::许可拒绝:
            return 安全根生产定义初始化状态_v1::许可拒绝;
        case 主动安全结算来源规则发布状态_v1::当前性漂移:
            return 安全根生产定义初始化状态_v1::当前性漂移;
        case 主动安全结算来源规则发布状态_v1::版本漂移:
        case 主动安全结算来源规则发布状态_v1::幂等冲突:
        case 主动安全结算来源规则发布状态_v1::引用冲突:
            return 安全根生产定义初始化状态_v1::定义冲突;
        case 主动安全结算来源规则发布状态_v1::资源失败:
            return 安全根生产定义初始化状态_v1::资源失败;
        case 主动安全结算来源规则发布状态_v1::已可能发布:
            return 安全根生产定义初始化状态_v1::已可能发布;
        default:
            return 安全根生产定义初始化状态_v1::内部错误;
        }
    ...
    主动安全结算来源规则读取结果_v1 读取当前来源规则(
        const 主动安全结算来源规则读取请求_v1& 请求) const noexcept {
#if defined(ARCH_INSTINCT_PRODUCTION_SAFETY_DEFINITION_VALIDATION)
        if (ARCH_来源规则读取剩余次数_ != 0) {
            --ARCH_来源规则读取剩余次数_;
            主动安全结算来源规则读取结果_v1 结果;
            结果.状态 = ARCH_来源规则读取状态_;
            return 结果;
        }
#endif
        return 定义服务_.读取当前主动安全结算来源规则_v1(请求);
    }

    主动安全结算来源规则发布结果_v1 发布来源规则(
        const 发布主动安全结算来源规则请求_v1& 请求) noexcept {
#if defined(ARCH_INSTINCT_PRODUCTION_SAFETY_DEFINITION_VALIDATION)
        if (ARCH_来源规则发布剩余次数_ != 0) {
            --ARCH_来源规则发布剩余次数_;
            主动安全结算来源规则发布结果_v1 结果;
            结果.状态 = ARCH_来源规则发布状态_;
            结果.幂等身份 = 请求.幂等身份;
            return 结果;
        }
#endif
        return 定义服务_.发布主动安全结算来源规则_v1(请求);
    }
    ...
    mutable 主动安全结算来源规则读取状态_v1 ARCH_来源规则读取状态_ =
        主动安全结算来源规则读取状态_v1::内部错误;
    mutable std::uint8_t ARCH_来源规则读取剩余次数_ = 0;
    主动安全结算来源规则发布状态_v1 ARCH_来源规则发布状态_ =
        主动安全结算来源规则发布状态_v1::内部错误;
    std::uint8_t ARCH_来源规则发布剩余次数_ = 0;
#endif
};

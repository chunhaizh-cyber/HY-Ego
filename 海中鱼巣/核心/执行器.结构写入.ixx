...
public:
    ...
    结构写入结果 执行(const std::function<void(结构写入会话&)>& 回调) const {
        ...
        try {
            ...
            const bool 已隔离 = 撤销完成(撤销) || 标记撤销失败隔离(许可);
    ...
    结构写入结果 执行确认前故障自检(
        ...
        const auto 会话确认 = 会话.完成确认();
        const auto 参与者确认 = 会话确认.成功() ? 安全确认参与者(参与者) : 会话确认;
        try { 观察回调(); } catch (...) {}
        const auto 参与者撤销 = 安全完成参与者撤销(参与者);
        const auto 会话撤销 = 会话.完成撤销();
        return {结构写入状态::内部不一致, 0, 0,
            会话确认.成功() && 参与者确认.成功() && 参与者撤销.成功() && 撤销完成(会话撤销) ? 0U : 1U};
    }
#endif
...
private:
    ...
    结构写入结果 隔离后内部不一致(const 结构事务许可& 许可) const noexcept {
        return {结构写入状态::内部不一致, 0, 0, 标记撤销失败隔离(许可) ? 0U : 1U};
    }

...
public:
    ...
    结构写入结果 绑定主键(std::uint64_t 主键, 节点句柄 预期节点) {
        const auto 请求 = 形成未知兼容索引绑定请求(主键, 预期节点);
        if (!可继续写入()) return {};
        auto 结果 = 索引_.结构化绑定主键(请求, 令牌_);
        if (!结果.成功()) {
            记录失败(结果);
            return 结果;
        }
    ...
    结构写入结果 绑定主键(const 索引绑定请求& 请求) {
        if (!可继续写入() || !请求.显式完整()) return {};
        auto 结果 = 索引_.结构化绑定主键(请求, 令牌_);
        if (!结果.成功()) {
            记录失败(结果);
            return 结果;
        ...
        try {
            ...
            (void)索引_.严格删除主键(请求.最终物理键, 请求.目标, 令牌_);
            结果.状态 = 结构写入状态::内部不一致;
            记录失败(结果);
        }
        return 结果;
    ...
    bool 主键绑定匹配(std::uint64_t 主键, 节点句柄 预期节点) {
        if (!当前线程可访问()) return false;
        const 索引绑定记录* 写入记录 = nullptr;
        for (const auto& 记录 : 索引写集_) {
            if (记录.请求.最终物理键 == 主键 && 记录.请求.目标 == 预期节点) {
                写入记录 = &记录;
                break;
            }
        }
        const auto 当前记录 = 索引_.读取主键绑定记录(主键, 令牌_);
        const bool 匹配 = 当前记录.has_value() && 当前记录->节点 == 预期节点
            && (写入记录 == nullptr || *当前记录 == 主键绑定记录{
                写入记录->请求.最终物理键,
                写入记录->请求.目标,
                写入记录->请求.所有者声明,
                写入记录->请求.探测序号});
        ...
        for (auto& 记录 : 索引写集_) {
            if (记录.请求.最终物理键 == 主键 && 记录.请求.目标 == 预期节点) 记录.已读回 = true;
        }
        return true;
    }

    bool 主键绑定匹配(const 索引绑定请求& 请求) {
        if (!当前线程可访问() || !请求.显式完整()) return false;
        const auto 当前记录 = 索引_.读取主键绑定记录(请求.最终物理键, 令牌_);
        const bool 匹配 = 当前记录 == std::optional<主键绑定记录>{主键绑定记录{
            请求.最终物理键, 请求.目标, 请求.所有者声明, 请求.探测序号}};
        if (!匹配) {
            记录失败({结构写入状态::内部不一致, 请求.最终物理键, 请求.目标.版本号, 0});
            return false;
        }
        for (auto& 记录 : 索引写集_) {
            if (记录.请求 == 请求) 记录.已读回 = true;
        }
        return true;
    }
...
private:
    ...
    结构写入结果 完成撤销() {
        ...
        for (auto 位置 = 索引写集_.rbegin(); 位置 != 索引写集_.rend(); ++位置) {
            完整 = 索引_.严格删除主键(
                位置->请求.最终物理键, 位置->请求.目标, 令牌_).成功() && 完整;
        }

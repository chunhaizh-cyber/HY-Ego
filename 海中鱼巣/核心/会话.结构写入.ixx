...
public:
    ...
    enum class 类型索引投影状态 : std::uint8_t { 已形成, 入口拒绝, 内部不一致 };
    ...
    结构类型索引投影结果 读取有效节点与索引投影(节点类型 类型) {
        结构类型索引投影结果 输出;
        if (!可查询写前材料() || 类型 == 节点类型::未分类) return 输出;
        const auto 节点组 = 节点_.读取指定类型有效节点记录组(类型, 令牌_);
        const auto 全部绑定组 = 索引_.读取全部主键绑定组(令牌_);
        for (const auto& 绑定 : 全部绑定组) {
            if (!绑定.完整()) {

...
L1中性值事实 转换中性值(const 值事实& 事实) {
    return {事实.编码, 事实.所属节点, 事实.属性类型节点,
        转换中性材料(事实.材料), 事实.来源节点, 事实.创建事实代次};
}
...
建立L1事实基座持久运行包_v1(
    ...
    try {
        ...
        const bool 已建立空仓 =
            结果.恢复.状态 == L1事实基座持久恢复状态_v1::已建立空仓
            && !结果.恢复.恢复见证;
        const bool 已恢复格式4 =
            结果.恢复.状态 == L1事实基座持久恢复状态_v1::已恢复
            && 结果.恢复.恢复见证
            && 结果.恢复.恢复见证->格式版本 == L1事实基座持久快照格式版本_v1
            && 结果.恢复.恢复见证->快照序号 != 0
            && 结果.恢复.恢复见证->事实代次 != 0
            && std::any_of(结果.恢复.恢复见证->载荷SHA256.begin(),
                结果.恢复.恢复见证->载荷SHA256.end(),
                [](std::uint8_t 值) { return 值 != 0; });
        if (!已建立空仓 && !已恢复格式4) return 结果;

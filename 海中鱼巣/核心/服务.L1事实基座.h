...
// 诊断责任：无适用错误分支；只投影L1权威墓碑的完整值材料。
L1中性物理清理墓碑 转换中性物理清理墓碑(
    const L1物理清理墓碑& 墓碑) noexcept {
    return {墓碑.编码,
        static_cast<L1中性物理清理事实种类>(
...
public:
    ...
    L1中性写入首次结果读取结果 读取中性写入首次结果(
        ...
        return 仓库_.读取中性写入首次结果(请求);
    }

    L1物理清理结果 执行L1物理清理(
        const L1物理清理请求& 请求) noexcept {
        return 仓库_.执行L1物理清理(请求);
    }
    ...
    L1中性节点读取结果 读取中性当前节点(const L1中性事实读取请求& 请求) const {
        ...
        try {
            ...
            结果.读取事实代次 = 读取.读取事实代次;
            if (读取.墓碑)
                结果.物理清理事实代次 = 读取.墓碑->物理清理事实代次;
            if (读取.墓碑)
                结果.物理清理墓碑 = 转换中性物理清理墓碑(*读取.墓碑);
    ...
    L1所有者范围有界历史关系组读取结果 读取所有者范围有界历史关系组(
        const L1所有者范围有界历史关系组读取请求& 请求) const noexcept {
        try {
            return 仓库_.读取所有者范围有界历史关系组(请求);
        } catch (const std::bad_alloc&) {
            return {L1所有者范围读取状态::资源失败,
    ...
    L1所有者范围有界历史属性值组读取结果 读取所有者范围有界历史属性值组(
        const L1所有者范围有界历史属性值组读取请求& 请求) const noexcept {
        try {
            return 仓库_.读取所有者范围有界历史属性值组(请求);
        } catch (const std::bad_alloc&) {
            return {L1所有者范围读取状态::资源失败,
...
public:
    ...
    L1跨所有者原子事务结果 提交跨所有者原子事务(
        ...
        // 唯一能力顺序与请求的具名参与者一致：接收者授权状态 owner，
        // 参数授权动态 owner。普通服务引用不能替代任一写端口。
    ...
    L1三分区原子事务结果_v2 提交三分区原子事务_v2(
        ...
        const L1三分区原子参与者写集_v2* 写集[3]{};
        for (const auto& 项 : 请求.参与者写集组) {
            if (项.参与者.值 < 1 || 项.参与者.值 > 3
                || 写集[项.参与者.值 - 1])
                return 失败(L1三分区原子事务状态_v2::入口拒绝);
            写集[项.参与者.值 - 1] = &项;
        }
        ...
        const auto 状态 = 状态_.lock();
        const auto 第二状态 = 第二参与者.状态_.lock();
        const auto 第三状态 = 第三参与者.状态_.lock();
        if (!状态 || !第二状态 || !第三状态
            || 状态.get() != 第二状态.get() || 状态.get() != 第三状态.get()
            || !有效() || !第二参与者.有效() || !第三参与者.有效())
            return 失败(L1三分区原子事务状态_v2::内部不一致);
        if (所有者_ != 写集[0]->所有者
            || 第二参与者.所有者_ != 写集[1]->所有者
            || 第三参与者.所有者_ != 写集[2]->所有者)
            return 失败(L1三分区原子事务状态_v2::引用冲突);
        return 状态->仓库.提交三分区原子事务_v2(请求);
    }
    ...
    L1有限N分区原子事务结果_v3 提交有限N分区原子事务_v3(
        ...
        try {
            ...
            std::vector<const L1有限N分区原子参与者写集_v3*> 写集(
                参与者数, nullptr);
            for (const auto& 项 : 请求.参与者写集组) {
                if (项.参与者.值 < 1 || 项.参与者.值 > 参与者数
                    || 写集[项.参与者.值 - 1])
                    return 失败(L1有限N分区原子事务状态_v3::入口拒绝);
                写集[项.参与者.值 - 1] = &项;
            }
            if (std::ranges::any_of(写集,
                    [](const auto* 项) { return 项 == nullptr; }))
                return 失败(L1有限N分区原子事务状态_v3::入口拒绝);
            ...
            std::unordered_set<const L1所有者范围写端口*> 端口地址组;
            端口地址组.reserve(其余参与者.size() + 1);
            端口地址组.insert(this);
            for (std::size_t 序号 = 0; 序号 != 其余参与者.size(); ++序号) {
                const auto* 端口 = 其余参与者[序号];
                if (!端口 || !端口地址组.insert(端口).second)
                    return 失败(L1有限N分区原子事务状态_v3::入口拒绝);
                const auto 参与者状态 = 端口->状态_.lock();
                if (!参与者状态 || 参与者状态.get() != 状态.get()
                    || !端口->有效())
                    return 失败(L1有限N分区原子事务状态_v3::内部不一致,
                        L1所有者范围重试边界::不适用);
                if (端口->所有者_ != 写集[序号 + 1]->所有者)
                    return 失败(L1有限N分区原子事务状态_v3::引用冲突);
            }
            return 状态->仓库.提交有限N分区原子事务_v3(请求);
        } catch (const std::bad_alloc&) {
            return 失败(L1有限N分区原子事务状态_v3::资源失败,
                L1所有者范围重试边界::原请求可重试);
        } catch (...) {
            return 失败(L1有限N分区原子事务状态_v3::内部不一致,
                L1所有者范围重试边界::不适用);
    ...
    // 诊断责任：向上送出；失效内部写会话映射内部不一致，其它仓库状态透明返回。
...
public:
    ...
    L1所有者范围交付 重新签发所有者范围写端口(
        ...
        try {
            std::lock_guard<std::mutex> 锁(状态->端口锁);
            交付.重入结果 = 状态->仓库.验证所有者范围重入(请求);
            if (交付.重入结果.状态 != L1所有者范围管理状态::成功)
                return 交付;
            if (状态->活动端口所有者.contains(请求.所有者.编码.值)) {
                交付.重入结果.状态 = L1所有者范围管理状态::内部不一致;
                交付.重入结果.是否形成内存权威发布 = false;
                交付.重入结果.重试边界 =
                    L1所有者范围重试边界::原请求可重试;
                return 交付;
            }
            已登记活动端口 = 状态->活动端口所有者.insert(
                请求.所有者.编码.值).second;
            if (!已登记活动端口)
                return 失败(L1所有者范围管理状态::内部不一致,
                    交付.重入结果.事实代次,
                    L1所有者范围重试边界::原请求可重试);
            交付.写入端口 = L1所有者范围写端口{状态, 请求.所有者};
            return 交付;
        } catch (const std::bad_alloc&) {
            if (已登记活动端口) {
                try {
                    std::lock_guard<std::mutex> 回滚锁(状态->端口锁);
                    状态->活动端口所有者.erase(请求.所有者.编码.值);
                } catch (...) {}
            }
...
inline L1所有者范围空域读取结果 L1事实基座服务::读取所有者范围空域(
    const L1所有者范围空域读取请求& 请求) const noexcept {
    if (请求.合同版本 != L1所有者范围空域读取合同版本
        || !有效(请求.所有者) || 请求.期望事实代次 == 0
        || 请求.最大核验事实数 == 0)
        return {L1所有者范围空域读取状态::入口拒绝,
            L1所有者范围空域读取合同版本, 请求.所有者,
            请求.期望事实代次, 0, std::nullopt};
    return 仓库_.读取所有者范围空域(请求);
}
inline L1节点当前引用读取结果 L1事实基座服务::读取节点全部当前引用(
    const L1节点当前引用读取请求& 请求) const noexcept {
    if (请求.合同版本 != L1节点当前引用读取合同版本
        || !有效(请求.节点) || 请求.期望事实代次 == 0 || 请求.最大数量 == 0)
        return {L1节点当前引用读取状态::入口拒绝,
            L1节点当前引用读取合同版本, 请求.节点,
            请求.期望事实代次, 0, {}};
    return 仓库_.读取节点全部当前引用(请求);
}

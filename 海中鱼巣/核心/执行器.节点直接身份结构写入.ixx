...
private:
    friend void 自检设置下一次混合原子写故障(
        节点直接身份结构写入执行器&,
        节点直接混合原子写自检故障) noexcept;
    ...
    节点直接类型化结构数据操作结果 执行带参与者类型化结构事务_(
        ...
        if (固定混合形状) {
            const auto 故障编码 = 下一次混合原子写故障_.exchange(0, std::memory_order_acq_rel);
            const auto 故障阶段 = static_cast<节点直接混合原子写自检阶段>(故障编码 >> 8);
            const auto 故障对象 = static_cast<std::uint8_t>(故障编码 & 0xFFU);
            const auto 命中故障 = [&](节点直接混合原子写自检阶段 阶段,
                std::uint8_t 对象) noexcept {
                return 故障阶段 == 阶段 && 故障对象 == 对象;
            };
            ...
            for (std::size_t 序号 = 0; 候选完整 && 序号 < 5; ++序号) {
                ...
                if (命中故障(节点直接混合原子写自检阶段::候选建立后,
                        static_cast<std::uint8_t>(序号 + 4))) {
                    候选完整 = false;
                    首次失败 = 节点直接类型化结构数据操作状态::资源失败;
                }
            }
            ...
            if (候选完整) {
                ...
                if (命中故障(节点直接混合原子写自检阶段::候选建立后, 9)) {
                    候选完整 = false;
                    首次失败 = 节点直接类型化结构数据操作状态::资源失败;
                }
            }

            if (候选完整 && 故障阶段 == 节点直接混合原子写自检阶段::候选撤销前) {
                候选完整 = false;
                首次失败 = 节点直接类型化结构数据操作状态::内部不一致;
            }

            if (候选完整 && 合同候选) {
                if (命中故障(节点直接混合原子写自检阶段::候选确认前, 1)
                    || 类型合同_->确认候选(*合同候选, 许可.事务序号())
                        != 节点直接仓候选操作状态::已确认待发布) 候选完整 = false;
            }
            if (候选完整 && (命中故障(节点直接混合原子写自检阶段::候选确认前, 2)
                    || 节点_->确认节点候选(*节点候选, 许可.事务序号()).状态
                        != 节点直接身份写入状态::已确认待发布)) 候选完整 = false;
            if (候选完整 && (命中故障(节点直接混合原子写自检阶段::候选确认前, 3)
                    || 类型化值_->确认候选(*值候选, 许可.事务序号())
                        != 节点直接仓候选操作状态::已确认待发布)) 候选完整 = false;
            for (std::size_t 序号 = 0; 候选完整 && 序号 < 关系候选组.size(); ++序号) {
                if (命中故障(节点直接混合原子写自检阶段::候选确认前,
                        static_cast<std::uint8_t>(序号 + 4))
                    || 关系_->确认候选(关系候选组[序号], 许可.事务序号())
                        != 正式关系操作状态::已确认待发布) 候选完整 = false;
            }
            if (候选完整 && (命中故障(节点直接混合原子写自检阶段::候选确认前, 9)
            ...
            auto 撤销业务候选 = [&]() noexcept {
                bool 完整 = true;
                if (幂等候选) {
                    const bool 注入 = 命中故障(节点直接混合原子写自检阶段::候选撤销前, 9);
                    完整 = 幂等_->撤销候选(*幂等候选, 许可.事务序号())
                        == 节点直接仓候选操作状态::已撤销 && !注入 && 完整;
                }
                for (std::size_t 序号 = 关系候选组.size(); 序号 > 0; --序号) {
                    const bool 注入 = 命中故障(节点直接混合原子写自检阶段::候选撤销前,
                        static_cast<std::uint8_t>(序号 + 3));
                    完整 = 关系_->撤销候选(关系候选组[序号 - 1], 许可.事务序号())
                        == 正式关系操作状态::已撤销 && !注入 && 完整;
                }
                if (值候选) {
                    const bool 注入 = 命中故障(节点直接混合原子写自检阶段::候选撤销前, 3);
                    完整 = 类型化值_->撤销候选(*值候选, 许可.事务序号())
                        == 节点直接仓候选操作状态::已撤销 && !注入 && 完整;
                }
                if (节点候选) {
                    const bool 注入 = 命中故障(节点直接混合原子写自检阶段::候选撤销前, 2);
                    完整 = 节点_->撤销节点候选(*节点候选, 许可.事务序号()).状态
                        == 节点直接身份写入状态::已撤销 && !注入 && 完整;
                }
                if (合同候选) {
                    const bool 注入 = 命中故障(节点直接混合原子写自检阶段::候选撤销前, 1);
            ...
            bool 已发布对象 = false;
            auto 发布前失败 = [&](std::uint8_t 对象) {
                if (!命中故障(节点直接混合原子写自检阶段::候选发布前, 对象)) return false;
                if (已发布对象) {
                    许可.标记隔离();
                    结果.状态 = 节点直接类型化结构数据操作状态::内部不一致;
                } else {
            ...
            if (合同候选) {
                if (发布前失败(1)) { if (!候选完整) return 撤销并返回(); return 结果; }
            ...
            if (发布前失败(2)) { if (!候选完整) return 撤销并返回(); return 结果; }
            节点_->完成发布节点候选(*节点候选, 许可.事务序号()); 已发布对象 = true;
            if (发布前失败(3)) return 结果;
            if (类型化值_->完成发布(*值候选, 许可.事务序号())
                != 节点直接仓候选操作状态::已发布) {
                许可.标记隔离(); 结果.状态 = 节点直接类型化结构数据操作状态::内部不一致; return 结果;
            }
            for (std::size_t 序号 = 0; 序号 < 关系候选组.size(); ++序号) {
                if (发布前失败(static_cast<std::uint8_t>(序号 + 4))) return 结果;
                关系_->完成发布(关系候选组[序号], 许可.事务序号());
            }
            if (发布前失败(9)) return 结果;
            ...
            if (命中故障(节点直接混合原子写自检阶段::发布后读回前, 1)) 读回一致 = false;
            ...
            for (std::size_t 序号 = 0; 序号 < 关系见证组.size(); ++序号) {
                ...
                读回一致 = 最终关系.状态 == 稳定关系当前读取状态::当前有效
                    ...
                    && 读回一致;
                if (命中故障(节点直接混合原子写自检阶段::发布后读回前,
                        static_cast<std::uint8_t>(序号 + 4))) 读回一致 = false;
            }
    ...
    mutable std::atomic<std::uint16_t> 下一次混合原子写故障_{0};
};

inline void 自检设置下一次混合原子写故障(
    节点直接身份结构写入执行器& 执行器,
    节点直接混合原子写自检故障 故障) noexcept {
    const auto 阶段 = static_cast<std::uint8_t>(故障.阶段);
    const bool 阶段有效 = 阶段 >= static_cast<std::uint8_t>(
            节点直接混合原子写自检阶段::候选建立后)
        && 阶段 <= static_cast<std::uint8_t>(节点直接混合原子写自检阶段::发布后读回前);
    const bool 对象有效 = 故障.对象序号 >= 1 && 故障.对象序号 <= 10
        && (故障.阶段 != 节点直接混合原子写自检阶段::发布后读回前
            || 故障.对象序号 <= 8);
    const auto 编码 = 阶段有效 && 对象有效
        ? static_cast<std::uint16_t>((static_cast<std::uint16_t>(阶段) << 8)
            | 故障.对象序号)
        : 0;
    执行器.下一次混合原子写故障_.store(编码, std::memory_order_release);
}

...
bool 绑定主键_已加锁(
    std::unordered_map<std::uint64_t, 主键绑定记录>& 主键索引,
    std::unordered_map<std::uint64_t, std::vector<std::uint64_t>>& 节点主键组,
    std::unordered_map<std::uint64_t, 主键绑定记录>& 永久保留主键组,
    const 索引绑定请求& 请求) {
    const 主键绑定记录 新记录{
        请求.最终物理键, 请求.目标, 请求.所有者声明, 请求.探测序号};
    const auto 现有 = 主键索引.find(请求.最终物理键);
    if (现有 != 主键索引.end()) return 现有->second == 新记录;
    if (永久保留主键组.contains(请求.最终物理键)) return false;

    bool 新建永久保留 = false;
    if (请求.所有者声明.键保留策略 == 索引键保留策略::删除后永久保留) {
        const auto [位置, 已插入] = 永久保留主键组.emplace(请求.最终物理键, 新记录);
        if (!已插入) return false;
        新建永久保留 = true;
    }
    try {
        auto [反向位置, 新建反向项] = 节点主键组.try_emplace(请求.目标.节点编号);
        反向位置->second.push_back(请求.最终物理键);
        try {
            const auto [位置, 已插入] = 主键索引.emplace(请求.最终物理键, 新记录);
            if (!已插入) {
                反向位置->second.pop_back();
                if (新建反向项 && 反向位置->second.empty()) 节点主键组.erase(反向位置);
                if (新建永久保留) 永久保留主键组.erase(请求.最终物理键);
                return 位置->second == 新记录;
            }
        } catch (...) {
            if (!反向位置->second.empty() && 反向位置->second.back() == 请求.最终物理键) {
                反向位置->second.pop_back();
            }
            if (新建反向项 && 反向位置->second.empty()) 节点主键组.erase(反向位置);
            throw;
...
bool 删除主键_已加锁(
    ...
    const auto 节点编号 = 位置->second.节点.节点编号;
...
bool 索引仓库::绑定主键(std::uint64_t 主键, 节点句柄 节点) {
    ...
    return 绑定主键_已加锁(
        主键索引_, 节点主键组_, 永久保留主键组_, 形成未知兼容索引绑定请求(主键, 节点));
}
...
结构写入结果 索引仓库::结构化绑定主键(
    ...
    节点句柄 节点,
    const 结构事务令牌& 令牌) {
    return 结构化绑定主键(形成未知兼容索引绑定请求(主键, 节点), 令牌);
}
...
结构写入结果 索引仓库::结构化绑定主键(
    ...
    const 结构事务令牌& 令牌) {
    结构写入结果 结果{
        结构写入状态::入口拒绝, 请求.最终物理键, 请求.目标.版本号, 0};
    if (!验证独占令牌(事务接线_, 令牌)) {
        结果.状态 = 结构写入状态::许可拒绝;
        return 结果;
    }
    if (!请求.兼容完整() || !节点_.节点是否有效(请求.目标, 令牌)) {
        return 结果;
    }
    std::unique_lock<std::shared_mutex> 锁(仓库锁_);
    const auto 现有 = 主键索引_.find(请求.最终物理键);
    if (现有 != 主键索引_.end()) {
        结果.当前版本 = 现有->second.节点.版本号;
        const 主键绑定记录 请求记录{
            请求.最终物理键, 请求.目标, 请求.所有者声明, 请求.探测序号};
        if (现有->second != 请求记录) {
            const bool 节点同一身份 = 现有->second.节点.仓库编号 == 请求.目标.仓库编号
                && 现有->second.节点.节点编号 == 请求.目标.节点编号;
            结果.状态 = 节点同一身份 && 现有->second.节点 != 请求.目标
                ? 结构写入状态::版本漂移 : 结构写入状态::入口拒绝;
            return 结果;
        }
        const auto 反向位置 = 节点主键组_.find(请求.目标.节点编号);
        if (反向位置 == 节点主键组_.end()
            || std::find(反向位置->second.begin(), 反向位置->second.end(), 请求.最终物理键)
                == 反向位置->second.end()) {
            结果.状态 = 结构写入状态::内部不一致;
            return 结果;
        }
    ...
    if (!绑定主键_已加锁(主键索引_, 节点主键组_, 永久保留主键组_, 请求)) {
        结果.状态 = 永久保留主键组_.contains(请求.最终物理键)
            ? 结构写入状态::入口拒绝 : 结构写入状态::内部不一致;
        return 结果;
    }
    结果.状态 = 结构写入状态::已提交;
    结果.当前版本 = 请求.目标.版本号;
...
主键绑定组读取结果 索引仓库::读取全部主键绑定组(const 结构事务令牌& 令牌) const {
    ...
    try {
        ...
        for (const auto& [主键, 记录] : 主键索引_) {
            const auto 反向位置 = 节点主键组_.find(记录.节点.节点编号);
...
结构写入结果 索引仓库::严格删除主键(
    ...
    结果.当前版本 = 位置->second.节点.版本号;
    if (位置->second.节点 != 预期节点) {
        const bool 同一身份 = 位置->second.节点.仓库编号 == 预期节点.仓库编号
            && 位置->second.节点.节点编号 == 预期节点.节点编号;
...
std::uint64_t 索引仓库::有效主键数量() const {
    ...
    std::vector<节点句柄> 候选组;
    {
        ...
        for (const auto& 条目 : 主键索引_) 候选组.push_back(条目.second.节点);
    }
    std::uint64_t 数量 = 0;
...
仓库权威导出结果<索引仓库权威材料> 索引仓库::导出权威状态(
    const 结构事务令牌& 令牌) const {
    if (!验证共享令牌(事务接线_, 令牌)) {
        return {仓库权威导出状态::入口拒绝, {}};
    }
    const auto 节点仓库编号 = 节点_.仓库编号();
    if (节点仓库编号 == 0) {
    ...
    索引仓库权威材料 材料{节点仓库编号, {}, {}, {}};
    try {
        材料.当前绑定组.reserve(主键索引_.size());
        for (const auto& [主键, 记录] : 主键索引_) {
            if (主键 == 0 || 记录.主键 != 主键 || !记录.完整()
                || 记录.节点.仓库编号 != 节点仓库编号) {
                return {仓库权威导出状态::内部不一致, {}};
            }
            const auto 反向位置 = 节点主键组_.find(记录.节点.节点编号);
            if (反向位置 == 节点主键组_.end()
                || std::count(反向位置->second.begin(), 反向位置->second.end(), 主键) != 1) {
                return {仓库权威导出状态::内部不一致, {}};
            }
            if (记录.所有者声明.键保留策略 == 索引键保留策略::删除后永久保留) {
                const auto 保留位置 = 永久保留主键组_.find(主键);
                if (保留位置 == 永久保留主键组_.end() || 保留位置->second != 记录) {
                    return {仓库权威导出状态::内部不一致, {}};
                }
            }
            材料.当前绑定组.push_back(
                {记录.主键, 记录.节点, 记录.所有者声明, 记录.探测序号});
        }
        材料.永久保留绑定组.reserve(永久保留主键组_.size());
        for (const auto& [主键, 记录] : 永久保留主键组_) {
            if (主键 == 0 || 记录.主键 != 主键 || !记录.完整()
                || 记录.节点.仓库编号 != 节点仓库编号
                || 记录.所有者声明.键保留策略 != 索引键保留策略::删除后永久保留) {
                return {仓库权威导出状态::内部不一致, {}};
            }
            const auto 当前位置 = 主键索引_.find(主键);
            if (当前位置 != 主键索引_.end() && 当前位置->second != 记录) {
                return {仓库权威导出状态::内部不一致, {}};
            }
            材料.永久保留绑定组.push_back(
                {记录.主键, 记录.节点, 记录.所有者声明, 记录.探测序号});
        }
        材料.反向绑定组.reserve(节点主键组_.size());
        for (const auto& [节点编号, 主键组] : 节点主键组_) {
            if (节点编号 == 0 || 主键组.empty()) {
                return {仓库权威导出状态::内部不一致, {}};
            }
            for (std::size_t 序号 = 0; 序号 < 主键组.size(); ++序号) {
                const auto 主键 = 主键组[序号];
                const auto 正向位置 = 主键索引_.find(主键);
                if (主键 == 0 || 正向位置 == 主键索引_.end()
                    || 正向位置->second.节点.节点编号 != 节点编号
                    || std::count(主键组.begin(), 主键组.end(), 主键) != 1) {
                    return {仓库权威导出状态::内部不一致, {}};
                }
            }
            材料.反向绑定组.push_back({节点编号, 主键组});
        }
    } catch (...) {

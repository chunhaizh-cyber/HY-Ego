...
class 节点直接类型合同恢复候选 final {
public:
    节点直接类型合同恢复候选(const 节点直接类型合同恢复候选&) = delete;
    节点直接类型合同恢复候选& operator=(const 节点直接类型合同恢复候选&) = delete;
    节点直接类型合同恢复候选(节点直接类型合同恢复候选&& 其它) noexcept
    ...
    节点直接类型合同恢复候选& operator=(节点直接类型合同恢复候选&&) = delete;
    节点直接仓候选阶段 读取阶段() const noexcept { return 阶段_; }

private:
    friend class 节点直接类型合同仓库;
    节点直接类型合同恢复候选(节点直接类型合同仓库* 仓库,
        std::uint64_t 事务序号, 节点直接类型合同仓库权威材料 材料)
...
public:
    ...
    节点直接类型合同恢复候选结果 结构化建立恢复未发布候选(
        节点直接类型合同仓库权威材料 材料, std::uint64_t 事务序号) {
        节点直接类型合同恢复候选结果 结果;
...
private:
    ...
    static bool 恢复材料完整_(节点直接类型合同仓库权威材料& 材料) {
        std::sort(材料.每域高水位.begin(), 材料.每域高水位.end(), [](const auto& 左, const auto& 右) {
            return 左.命名域 < 右.命名域;
        });
        std::sort(材料.历史占用.begin(), 材料.历史占用.end(), [](const auto& 左, const auto& 右) {
        ...
        for (std::size_t 序号 = 0; 序号 < 材料.记录组.size(); ++序号) {
            ...
            if (!合同完整_(记录)
                || (序号 && 材料.记录组[序号 - 1].合同身份 == 记录.合同身份
                    && 材料.记录组[序号 - 1].合同版本 == 记录.合同版本)
                || std::none_of(材料.历史占用.begin(), 材料.历史占用.end(),
                    [&](const auto& 项) { return 项.身份 == 记录.合同身份; })) return false;
        }
        return true;
    }
    ...
    bool 恢复候选匹配_(const 节点直接类型合同恢复候选& 候选,

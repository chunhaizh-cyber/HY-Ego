...
inline constexpr std::uint32_t L1所有者范围有界历史组读取合同版本 = 1;
...
inline constexpr std::uint32_t L1所有者范围历史完整关系组读取合同版本 = 2;
inline constexpr std::uint32_t L1所有者范围历史完整属性值组读取合同版本 = 2;
...
struct L1所有者范围有界历史关系组读取请求 final {
    std::uint32_t 合同版本 = L1所有者范围有界历史组读取合同版本;
    L1所有者范围关系端点方向 方向 = L1所有者范围关系端点方向::源;
    稳定编码 端点节点;
    稳定编码 关系类型节点;
    std::uint64_t 期望读取事实代次 = 0;
    std::uint64_t 历史截止事实代次 = 0;
    std::uint64_t 最大数量 = 0;
    friend bool operator==(const L1所有者范围有界历史关系组读取请求&,
        const L1所有者范围有界历史关系组读取请求&) = default;
};

struct L1所有者范围有界历史关系组读取结果 final {
    L1所有者范围读取状态 状态 = L1所有者范围读取状态::入口拒绝;
    std::uint32_t 合同版本 = L1所有者范围有界历史组读取合同版本;
    L1所有者范围关系端点方向 方向 = L1所有者范围关系端点方向::源;
    稳定编码 端点节点;
    ...
    std::uint64_t 读取事实代次 = 0;
    std::vector<L1所有者范围关系事实> 关系组;
    bool 成功(const L1所有者范围有界历史关系组读取请求& 请求) const noexcept {
        if (状态 != L1所有者范围读取状态::成功
            || 合同版本 != L1所有者范围有界历史组读取合同版本
            || 请求.合同版本 != 合同版本 || 方向 != 请求.方向
            || 端点节点 != 请求.端点节点
            || 关系类型节点 != 请求.关系类型节点
            || 期望读取事实代次 != 请求.期望读取事实代次
            || 历史截止事实代次 != 请求.历史截止事实代次
            || 最大数量 != 请求.最大数量
            || 读取事实代次 != 请求.期望读取事实代次
            || 关系组.size() > 最大数量) return false;
        std::uint64_t 前一编码 = 0;
        for (const auto& 事实 : 关系组) {
            if (!事实.编码.值 || 事实.编码.值 <= 前一编码
                ...
                || 事实.关系类型节点 != 关系类型节点
                || (方向 == L1所有者范围关系端点方向::源
                    ? 事实.源节点 != 端点节点 : 事实.目标节点 != 端点节点))
                return false;
            前一编码 = 事实.编码.值;
        }
        return true;
    }
    friend bool operator==(const L1所有者范围有界历史关系组读取结果&,
        const L1所有者范围有界历史关系组读取结果&) = default;
};

struct L1所有者范围有界历史属性值组读取请求 final {
    std::uint32_t 合同版本 = L1所有者范围有界历史组读取合同版本;
    稳定编码 所属节点;
    std::uint64_t 期望读取事实代次 = 0;
    std::uint64_t 历史截止事实代次 = 0;
    std::uint64_t 最大数量 = 0;
    friend bool operator==(const L1所有者范围有界历史属性值组读取请求&,
        const L1所有者范围有界历史属性值组读取请求&) = default;
};

struct L1所有者范围有界历史属性值组读取结果 final {
    L1所有者范围读取状态 状态 = L1所有者范围读取状态::入口拒绝;
    std::uint32_t 合同版本 = L1所有者范围有界历史组读取合同版本;
    稳定编码 所属节点;
    std::uint64_t 期望读取事实代次 = 0;
    ...
    std::uint64_t 读取事实代次 = 0;
    std::vector<L1所有者范围值事实> 属性值组;
    bool 成功(const L1所有者范围有界历史属性值组读取请求& 请求) const noexcept {
        if (状态 != L1所有者范围读取状态::成功
            || 合同版本 != L1所有者范围有界历史组读取合同版本
            || 请求.合同版本 != 合同版本 || 所属节点 != 请求.所属节点
            || 期望读取事实代次 != 请求.期望读取事实代次
            || 历史截止事实代次 != 请求.历史截止事实代次
            || 最大数量 != 请求.最大数量
            || 读取事实代次 != 请求.期望读取事实代次
            || 属性值组.size() > 最大数量) return false;
        std::uint64_t 前一编码 = 0;
        for (const auto& 事实 : 属性值组) {
            if (!事实.编码.值 || 事实.编码.值 <= 前一编码
                || !事实.写入所有者.编码.值 || !事实.创建事实代次
                || 事实.创建事实代次 > 历史截止事实代次
                || (事实.退出事实代次
                    && 历史截止事实代次 >= *事实.退出事实代次)
                || 事实.所属节点 != 所属节点) return false;
            前一编码 = 事实.编码.值;
        }
        return true;
    }
    friend bool operator==(const L1所有者范围有界历史属性值组读取结果&,
        const L1所有者范围有界历史属性值组读取结果&) = default;
};
...
struct L1所有者范围来源当前完整值组读取结果_v2 final {
    ...
    std::vector<L1所有者范围值事实> 当前值;
    bool 成功(const L1所有者范围来源当前完整值组读取请求_v2&) const noexcept;
...
struct L1所有者范围属性类型当前完整值组读取结果_v2 final {
    ...
    std::uint64_t 期望事实代次 = 0, 读取事实代次 = 0;
    std::vector<L1所有者范围值事实> 当前值;
    bool 成功(const L1所有者范围属性类型当前完整值组读取请求_v2&) const noexcept;
...
struct L1所有者范围空域完整读取结果_v2 final {
    ...
    std::uint64_t 期望事实代次 = 0, 读取事实代次 = 0;
    std::optional<bool> 空域{};
    bool 成功(const L1所有者范围空域完整读取请求_v2&) const noexcept;
    bool 确认空域(const L1所有者范围空域完整读取请求_v2&) const noexcept;
...
struct L1节点当前完整引用读取结果_v2 final {
    ...
    std::uint64_t 期望事实代次 = 0, 读取事实代次 = 0;
    std::vector<L1节点当前引用事实> 引用{};
    bool 成功(const L1节点当前完整引用读取请求_v2&) const noexcept;
    friend bool operator==(const L1节点当前完整引用读取结果_v2&,
        const L1节点当前完整引用读取结果_v2&) = default;
};

enum class L1所有者范围历史完整关系组读取状态_v2 : std::uint8_t {
    成功 = 1, 入口拒绝 = 2, 许可拒绝 = 3, 未找到 = 4,
    已退出 = 5, 属性未设置 = 6, 事实代次漂移 = 7,
    资源失败 = 8, 内部不一致 = 9, 历史材料已清理 = 10
};

struct L1所有者范围历史完整关系组读取请求_v2 final {
    std::uint32_t 合同版本 = L1所有者范围历史完整关系组读取合同版本;
    L1所有者范围关系端点方向 方向 = L1所有者范围关系端点方向::源;
    稳定编码 端点节点, 关系类型节点;
    std::uint64_t 期望读取事实代次 = 0, 历史截止事实代次 = 0;
    friend bool operator==(const L1所有者范围历史完整关系组读取请求_v2&,
        const L1所有者范围历史完整关系组读取请求_v2&) = default;
};

struct L1所有者范围历史完整关系组读取结果_v2 final {
    L1所有者范围历史完整关系组读取状态_v2 状态 =
        L1所有者范围历史完整关系组读取状态_v2::入口拒绝;
    std::uint32_t 合同版本 = L1所有者范围历史完整关系组读取合同版本;
    L1所有者范围关系端点方向 方向 = L1所有者范围关系端点方向::源;
    稳定编码 端点节点, 关系类型节点;
    std::uint64_t 期望读取事实代次 = 0, 历史截止事实代次 = 0,
        读取事实代次 = 0;
    std::vector<L1所有者范围关系事实> 关系组;
    bool 成功(const L1所有者范围历史完整关系组读取请求_v2&) const noexcept;
    friend bool operator==(const L1所有者范围历史完整关系组读取结果_v2&,
        const L1所有者范围历史完整关系组读取结果_v2&) = default;
};

enum class L1所有者范围历史完整属性值组读取状态_v2 : std::uint8_t {
    成功 = 1, 入口拒绝 = 2, 许可拒绝 = 3, 未找到 = 4,
    已退出 = 5, 属性未设置 = 6, 事实代次漂移 = 7,
    资源失败 = 8, 内部不一致 = 9, 历史材料已清理 = 10
};

struct L1所有者范围历史完整属性值组读取请求_v2 final {
    std::uint32_t 合同版本 = L1所有者范围历史完整属性值组读取合同版本;
    稳定编码 所属节点;
    std::uint64_t 期望读取事实代次 = 0, 历史截止事实代次 = 0;
    friend bool operator==(const L1所有者范围历史完整属性值组读取请求_v2&,
        const L1所有者范围历史完整属性值组读取请求_v2&) = default;
};

struct L1所有者范围历史完整属性值组读取结果_v2 final {
    L1所有者范围历史完整属性值组读取状态_v2 状态 =
        L1所有者范围历史完整属性值组读取状态_v2::入口拒绝;
    std::uint32_t 合同版本 = L1所有者范围历史完整属性值组读取合同版本;
    稳定编码 所属节点;
    std::uint64_t 期望读取事实代次 = 0, 历史截止事实代次 = 0,
        读取事实代次 = 0;
    std::vector<L1所有者范围值事实> 属性值组;
    bool 成功(const L1所有者范围历史完整属性值组读取请求_v2&) const noexcept;
    friend bool operator==(const L1所有者范围历史完整属性值组读取结果_v2&,
        const L1所有者范围历史完整属性值组读取结果_v2&) = default;
};
...
inline bool L1所有者范围来源当前完整值组读取结果_v2::成功(
    const L1所有者范围来源当前完整值组读取请求_v2& 请求) const noexcept {
    if (请求.合同版本 != L1所有者范围来源当前完整值组读取合同版本
        || !有效(请求.所有者) || !有效(请求.来源节点)
        || 请求.期望事实代次 == 0
        || 状态 != L1所有者范围来源当前完整值组读取状态_v2::成功
        || 合同版本 != 请求.合同版本 || 所有者 != 请求.所有者
        || 来源节点 != 请求.来源节点
        || 期望事实代次 != 请求.期望事实代次
        || 读取事实代次 != 请求.期望事实代次) return false;
    std::uint64_t 前一编码 = 0;
    for (const auto& 事实 : 当前值) {
        if (!事实.编码.值 || 事实.编码.值 <= 前一编码
            || 事实.写入所有者 != 所有者 || 事实.来源节点 != 来源节点
            || !有效(事实.所属节点) || !有效(事实.属性类型节点)
            || 事实.材料.valueless_by_exception()
            || !L1所有者范围原始材料完整(事实.材料)
            || !事实.创建事实代次 || 事实.创建事实代次 > 读取事实代次
...
inline bool L1所有者范围空域完整读取结果_v2::成功(
    const L1所有者范围空域完整读取请求_v2& 请求) const noexcept {
    return 请求.合同版本 == L1所有者范围空域完整读取合同版本
        && 有效(请求.所有者) && 请求.期望事实代次 != 0
        && 状态 == L1所有者范围空域完整读取状态_v2::成功
        && 合同版本 == 请求.合同版本 && 所有者 == 请求.所有者
        && 期望事实代次 == 请求.期望事实代次
        && 读取事实代次 == 请求.期望事实代次 && 空域.has_value();
}

inline bool L1所有者范围空域完整读取结果_v2::确认空域(
    const L1所有者范围空域完整读取请求_v2& 请求) const noexcept {
    return 成功(请求) && *空域;
}

inline bool L1节点当前完整引用读取结果_v2::成功(
    const L1节点当前完整引用读取请求_v2& 请求) const noexcept {
    if (请求.合同版本 != L1节点当前完整引用读取合同版本
        || !有效(请求.节点) || !请求.期望事实代次
        || 状态 != L1节点当前完整引用读取状态_v2::成功
...
inline bool L1所有者范围历史完整关系组读取结果_v2::成功(
    const L1所有者范围历史完整关系组读取请求_v2& 请求) const noexcept {
    const bool 方向有效 = 请求.方向 == L1所有者范围关系端点方向::源
        || 请求.方向 == L1所有者范围关系端点方向::目标;
    if (请求.合同版本 != L1所有者范围历史完整关系组读取合同版本
        || !方向有效 || !有效(请求.端点节点) || !有效(请求.关系类型节点)
        || !请求.期望读取事实代次 || !请求.历史截止事实代次
        || 请求.历史截止事实代次 > 请求.期望读取事实代次
        || 状态 != L1所有者范围历史完整关系组读取状态_v2::成功
        || 合同版本 != L1所有者范围历史完整关系组读取合同版本
        || 请求.合同版本 != 合同版本 || 方向 != 请求.方向
        || 端点节点 != 请求.端点节点 || 关系类型节点 != 请求.关系类型节点
        || 期望读取事实代次 != 请求.期望读取事实代次
        || 历史截止事实代次 != 请求.历史截止事实代次
        || 读取事实代次 != 请求.期望读取事实代次) return false;
    std::uint64_t 前一编码 = 0;
    for (const auto& 事实 : 关系组) {
        if (!事实.编码.值 || 事实.编码.值 <= 前一编码
            || !有效(事实.写入所有者) || !有效(事实.源节点)
            || !有效(事实.目标节点) || !有效(事实.关系类型节点)
            || !事实.创建事实代次
            || 事实.创建事实代次 > 历史截止事实代次
            || (事实.退出事实代次 && *事实.退出事实代次 > 读取事实代次)
            || (事实.退出事实代次 && 历史截止事实代次 >= *事实.退出事实代次)
            || 事实.关系类型节点 != 关系类型节点
            || (方向 == L1所有者范围关系端点方向::源
                ? 事实.源节点 != 端点节点 : 事实.目标节点 != 端点节点)) return false;
        前一编码 = 事实.编码.值;
    }
    return true;
}

inline bool L1所有者范围历史完整属性值组读取结果_v2::成功(
    const L1所有者范围历史完整属性值组读取请求_v2& 请求) const noexcept {
    if (请求.合同版本 != L1所有者范围历史完整属性值组读取合同版本
        || !有效(请求.所属节点) || !请求.期望读取事实代次
        || !请求.历史截止事实代次
        || 请求.历史截止事实代次 > 请求.期望读取事实代次
        || 状态 != L1所有者范围历史完整属性值组读取状态_v2::成功
        || 合同版本 != L1所有者范围历史完整属性值组读取合同版本
        || 请求.合同版本 != 合同版本 || 所属节点 != 请求.所属节点
        || 期望读取事实代次 != 请求.期望读取事实代次
        || 历史截止事实代次 != 请求.历史截止事实代次
        || 读取事实代次 != 请求.期望读取事实代次) return false;

...
std::vector<节点记录> 节点仓库::读取指定类型有效节点记录组(
    节点类型 类型,
    const 结构事务令牌& 令牌) const {
    if (!验证共享令牌(事务接线_, 令牌) || !节点类型已定义(类型)) return {};
    std::vector<节点记录> 结果;
    std::shared_lock<std::shared_mutex> 锁(仓库锁_);
    try {
        结果.reserve(节点表_.size());
        for (const auto& [_, 记录] : 节点表_) {
            if (记录.状态 == 记录状态::有效 && 记录.类型 == 类型) 结果.push_back(记录);
        }
    } catch (...) {
        return {};
    }
    std::sort(结果.begin(), 结果.end(), [](const 节点记录& 左, const 节点记录& 右) {

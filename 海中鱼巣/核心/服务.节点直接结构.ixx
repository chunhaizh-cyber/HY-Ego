...
节点直接目标角色源投影结果 节点直接结构查询服务::读取目标角色命中源的当前关系与类型化值(
    const 节点直接目标角色源投影请求& 请求) const noexcept {
    static_assert(noexcept(节点直接目标角色源投影结果{
        {节点直接结构服务状态::资源失败, 0}, {}, {}, std::nullopt}));
    static_assert(noexcept(节点直接目标角色源投影结果{
        {节点直接结构服务状态::内部不一致, 0}, {}, {}, std::nullopt}));
    ...
    auto 节点顺序 = [](const 节点稳定身份见证& 左,
        ...
        if (左.稳定主键.键值 != 右.稳定主键.键值)
            return 左.稳定主键.键值 < 右.稳定主键.键值;
        using 节点类型值 = std::underlying_type_t<节点类型>;
        if (左.类型 != 右.类型)
            return static_cast<节点类型值>(左.类型) < static_cast<节点类型值>(右.类型);
        return 左.身份版本 < 右.身份版本;
    };
    auto 关系顺序 = [](const 关系稳定身份见证& 左,
        const 关系稳定身份见证& 右) noexcept {
    ...
    auto 值顺序 = [](const 类型化值读回& 左, const 类型化值读回& 右) noexcept {
        ...
        if (左.所属身份.稳定主键.键值 != 右.所属身份.稳定主键.键值)
            return 左.所属身份.稳定主键.键值 < 右.所属身份.稳定主键.键值;
        using 节点类型值 = std::underlying_type_t<节点类型>;
        if (左.所属身份.类型 != 右.所属身份.类型)
            return static_cast<节点类型值>(左.所属身份.类型)
                < static_cast<节点类型值>(右.所属身份.类型);
        if (左.所属身份.身份版本 != 右.所属身份.身份版本)
            return 左.所属身份.身份版本 < 右.所属身份.身份版本;
        if (左.类型合同身份.命名域 != 右.类型合同身份.命名域)
            return 左.类型合同身份.命名域 < 右.类型合同身份.命名域;
        if (左.类型合同身份.键值 != 右.类型合同身份.键值)
            return 左.类型合同身份.键值 < 右.类型合同身份.键值;
    ...
    try {
        if (请求.合同版本 != 节点直接目标角色源投影合同版本
            || 请求.必须节点组.empty() || 请求.筛选组.empty()) {
            return 空结果(节点直接结构服务状态::入口拒绝);
        }
        for (std::size_t 序号 = 0; 序号 < 请求.必须节点组.size(); ++序号) {
            const auto& 节点 = 请求.必须节点组[序号];
            if (!节点稳定身份见证完整(节点)) return 空结果(节点直接结构服务状态::入口拒绝);
            for (std::size_t 后序 = 序号 + 1; 后序 < 请求.必须节点组.size(); ++后序) {
                if (节点.稳定主键 == 请求.必须节点组[后序].稳定主键)
                    return 空结果(节点直接结构服务状态::入口拒绝);
            }
        }
        for (std::size_t 序号 = 0; 序号 < 请求.筛选组.size(); ++序号) {
            const auto& 筛选 = 请求.筛选组[序号];
            if ((筛选.关系种类 != 关系类型::状态事实角色
                    && 筛选.关系种类 != 关系类型::动态事实角色)
                || !节点稳定身份见证完整(筛选.目标)
                || std::none_of(请求.必须节点组.begin(), 请求.必须节点组.end(),
                    [&](const auto& 节点) { return 节点 == 筛选.目标; })) {
                return 空结果(节点直接结构服务状态::入口拒绝);
            }
            for (std::size_t 后序 = 序号 + 1; 后序 < 请求.筛选组.size(); ++后序) {
                const auto& 另一 = 请求.筛选组[后序];
                if (筛选.关系种类 == 另一.关系种类
                    && 筛选.目标.稳定主键 == 另一.目标.稳定主键
                    && 筛选.目标角色 == 另一.目标角色) {
                    return 空结果(节点直接结构服务状态::入口拒绝);
                }
            }
        ...
        auto 筛选组 = 请求.筛选组;
        std::sort(必须节点组.begin(), 必须节点组.end(), 节点顺序);
        std::sort(筛选组.begin(), 筛选组.end(), [&](const auto& 左, const auto& 右) {
            using 关系类型值 = std::underlying_type_t<关系类型>;
            if (左.关系种类 != 右.关系种类)
                return static_cast<关系类型值>(左.关系种类)
                    < static_cast<关系类型值>(右.关系种类);
            if (左.目标.稳定主键.命名域 != 右.目标.稳定主键.命名域)
                return 左.目标.稳定主键.命名域 < 右.目标.稳定主键.命名域;
            if (左.目标.稳定主键.键值 != 右.目标.稳定主键.键值)
                return 左.目标.稳定主键.键值 < 右.目标.稳定主键.键值;
            if (左.目标角色 != 右.目标角色) return 左.目标角色 < 右.目标角色;
            return 节点顺序(左.目标, 右.目标);
        });
        ...
        auto 查找句柄 = [&](const 节点稳定身份见证& 见证) -> std::optional<节点句柄> {
            const auto 位置 = std::find_if(当前节点组.begin(), 当前节点组.end(),
                [&](const auto& 项) { return 项.见证 == 见证; });
            return 位置 == 当前节点组.end() ? std::nullopt
                                               : std::optional<节点句柄>{位置->句柄};
        };

        struct 源候选 { 关系类型 类型; 节点稳定身份见证 源; };
        std::vector<源候选> 源候选组;
        for (const auto& 筛选 : 筛选组) {
            const auto 目标句柄 = 查找句柄(筛选.目标);
            if (!目标句柄) return 空结果(节点直接结构服务状态::内部不一致);
            const auto 读取 = 关系_.读取有效目标关系组(*目标句柄, 筛选.关系种类);
            if (读取.状态 != 稳定关系组读取状态::已读取)
                return 空结果(节点直接结构服务状态::内部不一致);
            for (const auto& 记录 : 读取.关系组) {
                if (记录.目标节点 != *目标句柄 || 记录.顺序号 != 筛选.目标角色) continue;
                const auto 见证 = 形成当前关系见证_(记录);
                if (!见证 || 见证->类型 != 筛选.关系种类 || 见证->目标端 != 筛选.目标)
                    return 空结果(节点直接结构服务状态::内部不一致);
                const auto 位置 = std::find_if(源候选组.begin(), 源候选组.end(), [&](const auto& 项) {
                    return 项.类型 == 筛选.关系种类
                        && 项.源.稳定主键 == 见证->源端.稳定主键;
                });
                if (位置 == 源候选组.end()) 源候选组.push_back({筛选.关系种类, 见证->源端});
                else if (位置->源 != 见证->源端)
                    return 空结果(节点直接结构服务状态::内部不一致);
            }
        }
        ...
        for (const auto& 源候选项 : 源候选组) {
            const auto 当前 = 节点_.读取稳定主键当前身份(源候选项.源.稳定主键);
            if (当前.状态 != 稳定主键当前身份状态::当前有效
                || !当前.当前身份 || !当前.当前记录
                || 形成节点见证_(*当前.当前记录) != 源候选项.源) {
                return 空结果(节点直接结构服务状态::内部不一致);
            }
            const auto 关系读取 = 关系_.读取有效源关系组(*当前.当前身份, 源候选项.类型);
            if (关系读取.状态 != 稳定关系组读取状态::已读取 || 关系读取.关系组.empty())
                return 空结果(节点直接结构服务状态::内部不一致);
            节点直接当前关系源投影 投影;
            投影.关系种类 = 源候选项.类型;
            投影.源 = 源候选项.源;
            for (const auto& 记录 : 关系读取.关系组) {
                const auto 见证 = 形成当前关系见证_(记录);
                if (!见证 || 见证->类型 != 源候选项.类型 || 见证->源端 != 源候选项.源)
                    return 空结果(节点直接结构服务状态::内部不一致);
                const auto 位置 = std::find_if(投影.当前关系组.begin(), 投影.当前关系组.end(),
                    [&](const auto& 项) { return 项.稳定主键 == 见证->稳定主键; });
                if (位置 == 投影.当前关系组.end()) 投影.当前关系组.push_back(*见证);
                else if (*位置 != *见证) return 空结果(节点直接结构服务状态::内部不一致);
            }
            auto 当前值组 = 类型化值_.读取所属身份当前值组(源候选项.源.稳定主键);
            for (const auto& 值 : 当前值组) {
                if (!类型化值记录完整_(值) || 值.所属身份 != 源候选项.源 || !值.当前
                    || 值.首次发布代次 > 代次 || 值.当前状态发布代次 > 代次) {
                    return 空结果(节点直接结构服务状态::内部不一致);
                }
                const auto 同身份 = std::find_if(投影.当前类型化值组.begin(), 投影.当前类型化值组.end(),
                    [&](const auto& 项) { return 项.值记录身份 == 值.值记录身份; });
                if (同身份 != 投影.当前类型化值组.end()) {
                    if (*同身份 != 值) return 空结果(节点直接结构服务状态::内部不一致);
                    continue;
                }
                const auto 同合同 = std::find_if(投影.当前类型化值组.begin(), 投影.当前类型化值组.end(),
                    [&](const auto& 项) {
                        return 项.所属身份 == 值.所属身份
                            && 项.类型合同身份 == 值.类型合同身份
                            && 项.类型合同版本 == 值.类型合同版本;
                    });
                if (同合同 != 投影.当前类型化值组.end())
                    return 空结果(节点直接结构服务状态::内部不一致);
                投影.当前类型化值组.push_back(值);
            }
            std::sort(投影.当前关系组.begin(), 投影.当前关系组.end(), 关系顺序);
            std::sort(投影.当前类型化值组.begin(), 投影.当前类型化值组.end(), 值顺序);
            源组.push_back(std::move(投影));
        }
        std::sort(源组.begin(), 源组.end(), [&](const auto& 左, const auto& 右) {
            using 关系类型值 = std::underlying_type_t<关系类型>;
            if (左.关系种类 != 右.关系种类)
                return static_cast<关系类型值>(左.关系种类)
                    < static_cast<关系类型值>(右.关系种类);
        ...
        return {{节点直接结构服务状态::成功, 代次},
            std::move(已解析必须节点组), std::move(源组), std::nullopt};
    } catch (const std::bad_alloc&) {
        return {{节点直接结构服务状态::资源失败, 0}, {}, {}, std::nullopt};
    } catch (const std::length_error&) {
        return {{节点直接结构服务状态::资源失败, 0}, {}, {}, std::nullopt};

...
class 正式关系恢复候选 final {
public:
    正式关系恢复候选(const 正式关系恢复候选&) = delete;
    正式关系恢复候选& operator=(const 正式关系恢复候选&) = delete;
    正式关系恢复候选(正式关系恢复候选&& 其它) noexcept
    ...
    正式关系恢复候选& operator=(正式关系恢复候选&&) = delete;
    节点直接仓候选阶段 读取阶段() const noexcept { return 阶段_; }
private:
    friend class 正式关系仓库;
    正式关系恢复候选(正式关系仓库* 仓库, std::uint64_t 事务序号,
        正式关系仓库权威材料 材料)
...
public:
    ...
    正式关系恢复候选结果 结构化建立恢复未发布候选(
        正式关系仓库权威材料 材料, std::uint64_t 事务序号) {
        正式关系恢复候选结果 结果;
        if (!有效() || 事务序号 == 0 || !恢复材料完整_(材料)) {
        ...
        if (!关系表_.empty() || !主键占用表_.empty() || !每域高水位_.empty()) {
            结果.状态 = 节点直接恢复仓操作状态::版本漂移;
            return 结果;
        }
        try {
            恢复事务序号_ = 事务序号;
            结果.候选.emplace(正式关系恢复候选{this, 事务序号, std::move(材料)});
    ...
    节点直接恢复仓操作状态 撤销恢复候选(
        正式关系恢复候选& 候选, std::uint64_t 事务序号) noexcept {
        try {
            std::unique_lock 锁(仓库锁_);
    ...
    节点直接恢复仓操作状态 完成发布恢复候选(
        ...
        try {
            decltype(关系表_) 新关系表;
            decltype(主键占用表_) 新占用表;
            decltype(每域高水位_) 新高水位;
            std::uint64_t 新下个关系编号 = 1;
            for (const auto& 高水位 : 候选.材料_.每域高水位)
                新高水位.emplace(static_cast<std::uint64_t>(高水位.命名域), 高水位.高水位);
            for (const auto& 占用 : 候选.材料_.历史占用)
                新占用表.emplace(占用.稳定主键, 主键占用条目{0, true});
            for (const auto& 记录 : 候选.材料_.记录组) {
                if (!节点_.读取节点(记录.源节点).has_value()
                    || !节点_.读取节点(记录.目标节点).has_value())
                    return 节点直接恢复仓操作状态::关系矛盾;
                auto [位置, 新建] = 新关系表.try_emplace(记录.关系编号,
                    关系条目{记录, {}, true, 0});
                if (!新建) {
                    if (位置->second.当前记录.稳定主键 != 记录.稳定主键
                        || 记录.版本号 != 位置->second.当前记录.版本号 + 1)
                        return 节点直接恢复仓操作状态::版本漂移;
                    位置->second.历史记录组.push_back(位置->second.当前记录);
                    位置->second.当前记录 = 记录;
                }
                auto 占用 = 新占用表.find(记录.稳定主键);
                if (占用 == 新占用表.end()) return 节点直接恢复仓操作状态::身份冲突;
                if (占用->second.关系编号 != 0 && 占用->second.关系编号 != 记录.关系编号)
                    return 节点直接恢复仓操作状态::身份冲突;
                占用->second.关系编号 = 记录.关系编号;
                if (记录.关系编号 == std::numeric_limits<std::uint64_t>::max())
                    return 节点直接恢复仓操作状态::版本漂移;
                新下个关系编号 = std::max(新下个关系编号, 记录.关系编号 + 1);
            }
            std::unique_lock 锁(仓库锁_);
            if (!恢复候选匹配_(候选, 事务序号, 节点直接仓候选阶段::已确认待发布)
                || !关系表_.empty() || !主键占用表_.empty() || !每域高水位_.empty())
...
private:
    ...
    static bool 恢复材料完整_(正式关系仓库权威材料& 材料) {
        std::sort(材料.每域高水位.begin(), 材料.每域高水位.end(), [](const auto& 左, const auto& 右) {
            return static_cast<std::uint64_t>(左.命名域) < static_cast<std::uint64_t>(右.命名域);
        });
        std::sort(材料.历史占用.begin(), 材料.历史占用.end(), [](const auto& 左, const auto& 右) {
            return 左.稳定主键.命名域 != 右.稳定主键.命名域
                ? 左.稳定主键.命名域 < 右.稳定主键.命名域
                : 左.稳定主键.键值 < 右.稳定主键.键值;
        });
        ...
        for (std::size_t 序号 = 0; 序号 < 材料.每域高水位.size(); ++序号) {
            if (材料.每域高水位[序号].命名域 != 关系稳定主键命名域::正式关系
                || 材料.每域高水位[序号].高水位 == 0 || 序号 != 0) return false;
        }
        for (std::size_t 序号 = 0; 序号 < 材料.历史占用.size(); ++序号) {
            const auto& 主键 = 材料.历史占用[序号].稳定主键;
            if (!关系稳定主键完整(主键)
                || (序号 && 材料.历史占用[序号 - 1].稳定主键 == 主键)
                || 材料.每域高水位.empty() || 材料.每域高水位.front().高水位 < 主键.键值) return false;
        }
        for (std::size_t 序号 = 0; 序号 < 材料.记录组.size(); ++序号) {
            const auto& 记录 = 材料.记录组[序号];
            if (!关系稳定主键完整(记录.稳定主键) || 记录.关系编号 == 0
                || static_cast<std::uint32_t>(记录.类型) >= 正式关系类型ABI数量
                || !句柄有效(记录.源节点) || !句柄有效(记录.目标节点)
                || 记录.版本号 == 0
                || std::none_of(材料.历史占用.begin(), 材料.历史占用.end(),
                    [&](const auto& 项) { return 项.稳定主键 == 记录.稳定主键; })) return false;
            if (序号 && 材料.记录组[序号 - 1].关系编号 == 记录.关系编号
                && 材料.记录组[序号 - 1].版本号 + 1 != 记录.版本号) return false;
        }
        return 材料.记录组.empty() == 材料.历史占用.empty();
    }
    ...
    bool 恢复候选匹配_(const 正式关系恢复候选& 候选,

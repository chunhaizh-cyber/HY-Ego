...
class 节点直接身份恢复候选 final {
public:
    节点直接身份恢复候选(const 节点直接身份恢复候选&) = delete;
    节点直接身份恢复候选& operator=(const 节点直接身份恢复候选&) = delete;
    节点直接身份恢复候选(节点直接身份恢复候选&& 其它) noexcept
        : 仓库_(其它.仓库_), 事务序号_(其它.事务序号_), 材料_(std::move(其它.材料_)),
          投影组_(std::move(其它.投影组_)), 阶段_(其它.阶段_) {
        其它.仓库_ = nullptr;
        其它.事务序号_ = 0;
        其它.阶段_ = 阶段::已移动;
    }
    节点直接身份恢复候选& operator=(节点直接身份恢复候选&&) = delete;

    std::vector<节点直接恢复节点内部投影> 读取本事务节点投影组() const {
        return 投影组_;
    }

private:
    friend class 节点直接身份仓库;
    enum class 阶段 : std::uint8_t { 未发布, 已确认待发布, 已撤销, 已发布, 已移动 };
    节点直接身份恢复候选(节点直接身份仓库* 仓库, std::uint64_t 事务序号,
        节点直接身份仓库权威材料 材料,
        std::vector<节点直接恢复节点内部投影> 投影组) noexcept
        : 仓库_(仓库), 事务序号_(事务序号), 材料_(std::move(材料)),
          投影组_(std::move(投影组)) {}

    节点直接身份仓库* 仓库_ = nullptr;
    std::uint64_t 事务序号_ = 0;
    节点直接身份仓库权威材料 材料_;
    std::vector<节点直接恢复节点内部投影> 投影组_;
    阶段 阶段_ = 阶段::未发布;
};
...
public:
    ...
    节点直接身份恢复候选结果 结构化建立恢复未发布候选(
        const 节点直接身份仓库权威材料& 材料, std::uint64_t 事务序号) {
        节点直接身份恢复候选结果 结果;
        if (事务序号 == 0 || 材料.仓库编号 != 仓库编号_
            || 材料.下个节点编号 == 0 || 材料.下个创建序号 == 0) return 结果;

        std::vector<节点直接身份记录> 记录组;
        std::vector<节点稳定主键历史占用> 占用组;
        std::vector<节点稳定主键高水位> 高水位组;
        std::vector<节点直接恢复节点内部投影> 投影组;
        try {
            记录组 = 材料.记录组;
            占用组 = 材料.历史占用;
            高水位组 = 材料.每域高水位;
            ...
            std::sort(占用组.begin(), 占用组.end(), [](const auto& 左, const auto& 右) {
                if (左.稳定主键.命名域 != 右.稳定主键.命名域)
                    return 左.稳定主键.命名域 < 右.稳定主键.命名域;
                return 左.稳定主键.键值 < 右.稳定主键.键值;
            });
            std::sort(高水位组.begin(), 高水位组.end(), [](const auto& 左, const auto& 右) {
                return 左.命名域 < 右.命名域;
            });
            投影组.reserve(记录组.size());
        } catch (...) {
            结果.状态 = 节点直接恢复仓操作状态::资源失败;
            return 结果;
        }

        for (std::size_t 序号 = 0; 序号 < 记录组.size(); ++序号) {
            const auto& 记录 = 记录组[序号];
            if (记录.节点编号 == 0 || 记录.节点编号 >= 材料.下个节点编号
                || 记录.创建序号 == 0 || 记录.创建序号 >= 材料.下个创建序号
                || 记录.版本号 == 0 || !稳定主键有效(记录.稳定主键)
                || !命名域与节点类型匹配(
                    static_cast<节点稳定主键命名域>(记录.稳定主键.命名域), 记录.类型)
                || (记录.状态 != 记录状态::有效 && 记录.状态 != 记录状态::已删除)
                || (序号 != 0 && 记录组[序号 - 1].节点编号 == 记录.节点编号)) {
                结果.状态 = 节点直接恢复仓操作状态::材料不完整;
                return 结果;
            }
        }
        for (std::size_t 序号 = 0; 序号 < 占用组.size(); ++序号) {
            const auto& 占用 = 占用组[序号];
            if (!稳定主键有效(占用.稳定主键) || 占用.节点编号 == 0 || !占用.曾发布
                || !命名域与节点类型匹配(
                    static_cast<节点稳定主键命名域>(占用.稳定主键.命名域), 占用.类型)
                || (序号 != 0 && 占用组[序号 - 1].稳定主键 == 占用.稳定主键)) {
                结果.状态 = 节点直接恢复仓操作状态::身份冲突;
                return 结果;
            }
            const auto 记录 = std::find_if(记录组.begin(), 记录组.end(), [&](const auto& 项) {
                return 项.节点编号 == 占用.节点编号;
            });
            if (记录 != 记录组.end()
                && (记录->稳定主键 != 占用.稳定主键 || 记录->类型 != 占用.类型)) {
                结果.状态 = 节点直接恢复仓操作状态::身份冲突;
                return 结果;
            }
        }
        for (const auto& 记录 : 记录组) {
            const auto 占用 = std::find_if(占用组.begin(), 占用组.end(), [&](const auto& 项) {
                return 项.稳定主键 == 记录.稳定主键;
            });
            if (占用 == 占用组.end() || 占用->节点编号 != 记录.节点编号) {
                结果.状态 = 节点直接恢复仓操作状态::材料不完整;
                return 结果;
            }
            投影组.push_back({
                {记录.稳定主键, 记录.类型, 记录.版本号},
                {仓库编号_, 记录.节点编号, 记录.版本号},
                记录.状态 == 记录状态::有效});
        }
        for (std::size_t 序号 = 0; 序号 < 高水位组.size(); ++序号) {
            const auto& 高水位 = 高水位组[序号];
            if (高水位.命名域 == 0
                || !命名域已定义(static_cast<节点稳定主键命名域>(高水位.命名域))
                || (序号 != 0 && 高水位组[序号 - 1].命名域 == 高水位.命名域)) {
                结果.状态 = 节点直接恢复仓操作状态::材料不完整;
                return 结果;
            }
            const auto 最大占用 = std::find_if(占用组.rbegin(), 占用组.rend(), [&](const auto& 项) {
                return 项.稳定主键.命名域 == 高水位.命名域;
            });
            if (最大占用 != 占用组.rend() && 高水位.键值 < 最大占用->稳定主键.键值) {
        ...
        if (!节点表_.empty() || !主键占用表_.empty() || !每域高水位_.empty()
            || 恢复事务序号_ != 0) {
            结果.状态 = 节点直接恢复仓操作状态::身份冲突;
            return 结果;
        }
        try {
            节点直接身份仓库权威材料 排序材料{
                材料.仓库编号, 材料.下个节点编号, 材料.下个创建序号,
                std::move(高水位组), std::move(占用组), std::move(记录组)};
            节点直接身份恢复候选 新候选(
                this, 事务序号, std::move(排序材料), std::move(投影组));
            结果.候选.emplace(std::move(新候选));
            恢复事务序号_ = 事务序号;
    ...
    节点直接恢复仓操作状态 确认恢复候选(
        ...
        if (候选.仓库_ != this || 候选.事务序号_ != 事务序号 || 事务序号 == 0
            || 恢复事务序号_ != 事务序号) return 节点直接恢复仓操作状态::入口拒绝;
        if (候选.阶段_ == 节点直接身份恢复候选::阶段::已确认待发布)
            return 节点直接恢复仓操作状态::已确认待发布;
        if (候选.阶段_ != 节点直接身份恢复候选::阶段::未发布)
            return 节点直接恢复仓操作状态::入口拒绝;
        候选.阶段_ = 节点直接身份恢复候选::阶段::已确认待发布;
        return 节点直接恢复仓操作状态::已确认待发布;
    }
    ...
    节点直接恢复仓操作状态 撤销恢复候选(
        ...
        if (候选.仓库_ != this || 候选.事务序号_ != 事务序号 || 事务序号 == 0
            || 恢复事务序号_ != 事务序号
            || (候选.阶段_ != 节点直接身份恢复候选::阶段::未发布
                && 候选.阶段_ != 节点直接身份恢复候选::阶段::已确认待发布))
            return 节点直接恢复仓操作状态::入口拒绝;
        恢复事务序号_ = 0;
        候选.阶段_ = 节点直接身份恢复候选::阶段::已撤销;
    ...
    节点直接恢复仓操作状态 完成发布恢复候选(
        ...
        if (候选.仓库_ != this || 候选.事务序号_ != 事务序号 || 事务序号 == 0
            ...
            return 节点直接恢复仓操作状态::入口拒绝;
        try {
            std::unordered_map<std::uint64_t, 节点内部条目> 新节点表;
            std::unordered_map<节点稳定主键, 主键占用条目, 稳定主键哈希> 新占用表;
            std::unordered_map<std::uint64_t, std::uint64_t> 新高水位;
            新节点表.reserve(候选.材料_.记录组.size());
            新占用表.reserve(候选.材料_.历史占用.size());
            新高水位.reserve(候选.材料_.每域高水位.size());
            for (const auto& 记录 : 候选.材料_.记录组)
                新节点表.emplace(记录.节点编号, 节点内部条目{记录, true, 0, 0, false});
            for (const auto& 占用 : 候选.材料_.历史占用)
                新占用表.emplace(占用.稳定主键,
                    主键占用条目{占用.类型, 占用.节点编号, 占用.曾发布});
            for (const auto& 高水位 : 候选.材料_.每域高水位)
                新高水位.emplace(高水位.命名域, 高水位.键值);
            std::unique_lock<std::shared_mutex> 锁(仓库锁_);
            if (恢复事务序号_ != 事务序号 || !节点表_.empty()
                || !主键占用表_.empty() || !每域高水位_.empty())
                return 节点直接恢复仓操作状态::内部不一致;
            节点表_.swap(新节点表);
            主键占用表_.swap(新占用表);
            每域高水位_.swap(新高水位);
            下个节点编号_ = 候选.材料_.下个节点编号;
            下个创建序号_ = 候选.材料_.下个创建序号;
            恢复事务序号_ = 0;
            候选.阶段_ = 节点直接身份恢复候选::阶段::已发布;
            return 节点直接恢复仓操作状态::已发布;
        } catch (...) {
            return 节点直接恢复仓操作状态::资源失败;
        }
    }

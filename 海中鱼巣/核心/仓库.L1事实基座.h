...
public:
    ...
    L1所有者范围空域读取结果 读取所有者范围空域(
        ...
        using 读取状态 = L1所有者范围空域读取状态;
        std::uint64_t 读取代次 = 0;
        const auto 返回 = [&](读取状态 状态, std::optional<bool> 空域 = std::nullopt) {
            return L1所有者范围空域读取结果{状态,
                L1所有者范围空域读取合同版本, 请求.所有者,
                请求.期望事实代次, 读取代次, 空域};
        };
        if (请求.合同版本 != L1所有者范围空域读取合同版本
            || !有效(请求.所有者) || 请求.期望事实代次 == 0
            || 请求.最大核验事实数 == 0) return 返回(读取状态::入口拒绝);
        try {
            std::shared_lock<std::shared_mutex> 锁(锁_);
            读取代次 = 状态_.事实代次;
            ...
            const auto 历史 = 状态_.历史所有者.find(编码);
            const auto 所有者完整 = [&](const L1结构所有者事实& 事实, bool 已退出) {
                return 事实.所有者 == 请求.所有者 && 编码 < 状态_.下个编码
                    && 事实.创建事实代次 != 0 && 事实.创建事实代次 <= 读取代次
                    && (事实.范围种类 == L1所有者范围种类::独占结构范围
                        || 事实.范围种类 == L1所有者范围种类::旧共享范围)
                    && (已退出
                        ? 事实.退出事实代次 && *事实.退出事实代次 >= 事实.创建事实代次
                            && *事实.退出事实代次 <= 读取代次
                        : !事实.退出事实代次);
            };
            if (当前 == 状态_.当前所有者.end()) {
                if (历史 == 状态_.历史所有者.end()) return 返回(读取状态::未找到);
                return 返回(所有者完整(历史->second, true)
                    ? 读取状态::已退出 : 读取状态::内部不一致);
            }
            if (历史 != 状态_.历史所有者.end() || !所有者完整(当前->second, false))
                return 返回(读取状态::内部不一致);
            ...
            std::uint64_t 剩余预算 = 请求.最大核验事实数;
            for (const auto 数量 : {状态_.当前节点.size(), 状态_.当前关系.size(),
                    状态_.当前值.size(), 状态_.历史.size(), 状态_.物理清理墓碑.size()}) {
                if (数量 > 剩余预算) return 返回(读取状态::数量预算不足);
                剩余预算 -= static_cast<std::uint64_t>(数量);
            }
            const auto 证据完整 = [&](std::uint64_t 键, const auto& 事实, bool 已退出) {
                if (键 == 0 || 键 >= 状态_.下个编码 || 事实.编码.值 != 键
                    || 事实.写入所有者 != 请求.所有者
                    || 事实.创建事实代次 < 当前->second.创建事实代次
                    || 事实.创建事实代次 > 读取代次
                    || (已退出 ? !事实.退出事实代次
                            || *事实.退出事实代次 < 事实.创建事实代次
                            || *事实.退出事实代次 > 读取代次
                        : 事实.退出事实代次.has_value())) return false;
                using 事实类型 = std::decay_t<decltype(事实)>;
                if constexpr (std::is_same_v<事实类型, 节点事实>) {
                    return (事实.种类 == 节点种类::普通 && !事实.属性类型表示)
                        || (事实.种类 == 节点种类::属性类型 && 事实.属性类型表示
                            && *事实.属性类型表示 >= 值表示种类::I64
                            && *事实.属性类型表示 <= 值表示种类::独立材料引用);
                } else if constexpr (std::is_same_v<事实类型, 关系事实>) {
                    return 有效(事实.源节点) && 有效(事实.目标节点)
                        && 有效(事实.关系类型节点);
                } else {
                    return 有效(事实.所属节点) && 有效(事实.属性类型节点)
                        && 有效(事实.来源节点) && !事实.材料.valueless_by_exception()
                        && std::visit([](const auto& 材料) {
                            using 材料类型 = std::decay_t<decltype(材料)>;
                            if constexpr (std::is_same_v<材料类型, std::int64_t>) return true;
                            else if constexpr (std::is_same_v<材料类型, 独立材料引用>)
                                return 有效(材料.编码);
                            else return !材料.empty();
                        }, 事实.材料);
                }
            };
            const auto 查当前 = [&](const auto& 表) -> std::optional<读取状态> {
                for (const auto& [键, 事实] : 表) {
                    if (事实.写入所有者 != 请求.所有者) continue;
                    return 证据完整(键, 事实, false)
                        && (static_cast<unsigned>(状态_.当前节点.contains(键))
                            + static_cast<unsigned>(状态_.当前关系.contains(键))
                            + static_cast<unsigned>(状态_.当前值.contains(键)) == 1)
                        && !状态_.历史.contains(键) && !状态_.物理清理墓碑.contains(键)
                        ? 读取状态::成功 : 读取状态::内部不一致;
                }
                return std::nullopt;
            };
            if (const auto 结论 = 查当前(状态_.当前节点))
                return *结论 == 读取状态::成功 ? 返回(*结论, false) : 返回(*结论);
            if (const auto 结论 = 查当前(状态_.当前关系))
                return *结论 == 读取状态::成功 ? 返回(*结论, false) : 返回(*结论);
            if (const auto 结论 = 查当前(状态_.当前值))
                return *结论 == 读取状态::成功 ? 返回(*结论, false) : 返回(*结论);
            for (const auto& [键, 投影] : 状态_.历史) {
                const auto 结论 = std::visit([&](const auto& 事实) -> std::optional<bool> {
                    if (事实.写入所有者 != 请求.所有者) return std::nullopt;
                    return 证据完整(键, 事实, true) && 投影.查询编码.值 == 键
                        && !投影.当前有效 && !状态_.当前节点.contains(键)
                        && !状态_.当前关系.contains(键) && !状态_.当前值.contains(键)
                        && !状态_.物理清理墓碑.contains(键);
                }, 投影.事实);
                if (结论) return *结论 ? 返回(读取状态::成功, false)
                                      : 返回(读取状态::内部不一致);
            }
            for (const auto& [键, 墓碑] : 状态_.物理清理墓碑) {
                if (墓碑.内部结构分区 != 请求.所有者.编码) continue;
                if (键 == 0 || 键 >= 状态_.下个编码 || 墓碑.编码.值 != 键
                    || !L1物理清理事实身份有效({墓碑.事实种类, 墓碑.编码})
                    || 墓碑.创建事实代次 < 当前->second.创建事实代次
                    || 墓碑.退出事实代次 < 墓碑.创建事实代次
                    || 墓碑.物理清理事实代次 <= 墓碑.退出事实代次
                    || 墓碑.物理清理事实代次 > 读取代次
                    || 状态_.当前节点.contains(键) || 状态_.当前关系.contains(键)
                    || 状态_.当前值.contains(键) || 状态_.历史.contains(键)
                    || !状态_.永久占用.contains(键)) return 返回(读取状态::内部不一致);
                return 返回(读取状态::成功, false);
            }
    ...
    L1节点当前引用读取结果 读取节点全部当前引用(
        ...
        using 读取状态 = L1节点当前引用读取状态;
        std::uint64_t 读取代次 = 0;
        const auto 失败 = [&](读取状态 状态) {
            return L1节点当前引用读取结果{状态, L1节点当前引用读取合同版本,
                请求.节点, 请求.期望事实代次, 读取代次, {}};
        };
        ...
        try {
            ...
            const auto 当前事实头完整 = [&](const auto& 事实) {
                if (!有效(事实.写入所有者) || 事实.创建事实代次 == 0
                    || 事实.创建事实代次 > 读取代次 || 事实.退出事实代次) return false;
                ...
                return 所有者 != 状态_.当前所有者.end()
                    ...
                    && !所有者->second.退出事实代次
                    && !状态_.历史所有者.contains(事实.写入所有者.编码.值)
                    && (所有者->second.范围种类 == L1所有者范围种类::独占结构范围
                        || 所有者->second.范围种类 == L1所有者范围种类::旧共享范围);
            };
            ...
            const auto 历史 = 状态_.历史.find(编码);
            const auto 墓碑 = 状态_.物理清理墓碑.find(编码);
            if (当前 == 状态_.当前节点.end()) {
                if (状态_.当前关系.contains(编码) || 状态_.当前值.contains(编码)
                    || 状态_.当前所有者.contains(编码) || 状态_.历史所有者.contains(编码))
                    return 失败(读取状态::入口拒绝);
                if (历史 != 状态_.历史.end()) {
                    const auto* 节点 = std::get_if<节点事实>(&历史->second.事实);
                    if (!节点) return 失败(历史->second.事实.valueless_by_exception()
                        ? 读取状态::内部不一致 : 读取状态::入口拒绝);
                    if (墓碑 != 状态_.物理清理墓碑.end()
                        || 历史->second.查询编码 != 请求.节点 || 历史->second.当前有效
                        || 节点->编码 != 请求.节点 || !有效(节点->写入所有者)
                ...
                if (墓碑 != 状态_.物理清理墓碑.end()) {
                    const auto& 事实 = 墓碑->second;
                    if (!L1物理清理事实身份有效({事实.事实种类, 事实.编码})
                        || 事实.编码 != 请求.节点 || !有效(事实.内部结构分区)
                        || 事实.创建事实代次 == 0 || 事实.退出事实代次 < 事实.创建事实代次
                        || 事实.物理清理事实代次 <= 事实.退出事实代次
                        || 事实.物理清理事实代次 > 读取代次)
                        return 失败(读取状态::内部不一致);
                    return 失败(事实.事实种类 == L1物理清理事实种类::节点
                        ? 读取状态::已退出 : 读取状态::入口拒绝);
                }
                return 失败(读取状态::未找到);
            }
            ...
            if (节点.编码 != 请求.节点 || 编码 >= 状态_.下个编码
                || 历史 != 状态_.历史.end() || 墓碑 != 状态_.物理清理墓碑.end()
            ...
            if (候选->second.size() > 请求.最大数量)
                return 失败(读取状态::数量预算不足);
            ...
            for (const auto 事实编码 : 候选->second) {
                ...
                if (事实编码 == 0 || 事实编码 <= 前一编码
                    ...
                    || 状态_.当前节点.contains(事实编码) || 状态_.历史.contains(事实编码)
                    || 状态_.物理清理墓碑.contains(事实编码))
                    return 失败(读取状态::内部不一致);
                if (关系命中) {
                    const auto& 事实 = 关系->second;
            ...
            if (!结果.成功(请求)) return 失败(读取状态::内部不一致);
            return 结果;
        } catch (const std::bad_alloc&) {
            return 失败(读取状态::资源失败);
        } catch (const std::length_error&) {
    ...
    L1所有者范围有界历史关系组读取结果 读取所有者范围有界历史关系组(
        const L1所有者范围有界历史关系组读取请求& 请求) const {
        L1所有者范围有界历史关系组读取结果 结果{
            L1所有者范围读取状态::入口拒绝,
            L1所有者范围有界历史组读取合同版本, 请求.方向,
            请求.端点节点, 请求.关系类型节点, 请求.期望读取事实代次,
            请求.历史截止事实代次, 请求.最大数量, 0, {}};
        const bool 方向有效 = 请求.方向 == L1所有者范围关系端点方向::源
            || 请求.方向 == L1所有者范围关系端点方向::目标;
        if (请求.合同版本 != L1所有者范围有界历史组读取合同版本
            || !方向有效 || !有效(请求.端点节点)
            || !有效(请求.关系类型节点) || !请求.期望读取事实代次
            || !请求.历史截止事实代次
            || 请求.历史截止事实代次 > 请求.期望读取事实代次
            || !请求.最大数量) return 结果;
        try {
            std::shared_lock<std::shared_mutex> 锁(锁_);
            if (状态_.隔离 || !状态完整(状态_)) {
            ...
            结果.读取事实代次 = 状态_.事实代次;
            if (状态_.事实代次 != 请求.期望读取事实代次) {
                结果.状态 = L1所有者范围读取状态::事实代次漂移; return 结果;
            }
            auto 核验节点 = [&](稳定编码 编码) {
                const 节点事实* 事实 = nullptr;
                if (const auto 当前 = 状态_.当前节点.find(编码.值);
                    当前 != 状态_.当前节点.end()) 事实 = &当前->second;
                else if (const auto 历史 = 状态_.历史.find(编码.值);
                    历史 != 状态_.历史.end())
                    事实 = std::get_if<节点事实>(&历史->second.事实);
                if (!事实) {
                    return 状态_.物理清理墓碑.contains(编码.值)
                        ? L1所有者范围读取状态::历史材料已清理
                        : L1所有者范围读取状态::未找到;
                }
                if (事实->编码 != 编码 || !事实->写入所有者.编码.值
                    || !事实->创建事实代次
                    || (事实->退出事实代次
                        && *事实->退出事实代次 <= 事实->创建事实代次))
                    return L1所有者范围读取状态::内部不一致;
                if (事实->创建事实代次 > 请求.历史截止事实代次)
                    return L1所有者范围读取状态::未找到;
                if (事实->退出事实代次
                    && 请求.历史截止事实代次 >= *事实->退出事实代次)
                    return L1所有者范围读取状态::已退出;
                return L1所有者范围读取状态::成功;
            };
            if (const auto s = 核验节点(请求.端点节点);
                s != L1所有者范围读取状态::成功) {
                结果.状态 = s; return 结果;
            }
            if (const auto s = 核验节点(请求.关系类型节点);
                s != L1所有者范围读取状态::成功) {
                结果.状态 = s; return 结果;
            }
            const auto& 索引 = 请求.方向 == L1所有者范围关系端点方向::源
                ? 状态_.历史源关系候选索引 : 状态_.历史目标关系候选索引;
            const std::vector<std::uint64_t>* 候选 = nullptr;
            if (const auto 端点 = 索引.find(请求.端点节点.值); 端点 != 索引.end())
                if (const auto 类型 = 端点->second.find(请求.关系类型节点.值);
                    类型 != 端点->second.end()) 候选 = &类型->second;
            if (候选 && 候选->size() > 请求.最大数量) {
                结果.状态 = L1所有者范围读取状态::数量预算不足; return 结果;
            }
            if (候选) {
                结果.关系组.reserve(候选->size());
                for (const auto 编码 : *候选) {
                    ...
                    if (const auto 当前 = 状态_.当前关系.find(编码);
                        ...
                        事实 = std::get_if<关系事实>(&历史->second.事实);
                    if (!事实) {
                        结果.状态 = 状态_.物理清理墓碑.contains(编码)
                            ? L1所有者范围读取状态::历史材料已清理
                            : L1所有者范围读取状态::内部不一致;
                        结果.关系组.clear(); return 结果;
                    }
                    const bool 端点一致 = 请求.方向 == L1所有者范围关系端点方向::源
                        ? 事实->源节点 == 请求.端点节点
                        : 事实->目标节点 == 请求.端点节点;
                    if (事实->编码.值 != 编码 || !端点一致
                        || 事实->关系类型节点 != 请求.关系类型节点
                        || !事实->写入所有者.编码.值 || !事实->创建事实代次
                        || (事实->退出事实代次
                            && *事实->退出事实代次 <= 事实->创建事实代次)) {
                        结果.状态 = L1所有者范围读取状态::内部不一致;
                        结果.关系组.clear(); return 结果;
    ...
    L1所有者范围有界历史属性值组读取结果 读取所有者范围有界历史属性值组(
        const L1所有者范围有界历史属性值组读取请求& 请求) const {
        L1所有者范围有界历史属性值组读取结果 结果{
            L1所有者范围读取状态::入口拒绝,
            L1所有者范围有界历史组读取合同版本, 请求.所属节点,
            请求.期望读取事实代次, 请求.历史截止事实代次,
            请求.最大数量, 0, {}};
        if (请求.合同版本 != L1所有者范围有界历史组读取合同版本
            || !有效(请求.所属节点) || !请求.期望读取事实代次
            || !请求.历史截止事实代次
            || 请求.历史截止事实代次 > 请求.期望读取事实代次
            || !请求.最大数量) return 结果;
        try {
            ...
            const 节点事实* 所属事实 = nullptr;
            if (const auto 当前 = 状态_.当前节点.find(请求.所属节点.值);
                当前 != 状态_.当前节点.end()) 所属事实 = &当前->second;
            else if (const auto 历史 = 状态_.历史.find(请求.所属节点.值);
                历史 != 状态_.历史.end())
                所属事实 = std::get_if<节点事实>(&历史->second.事实);
            if (!所属事实) {
                结果.状态 = 状态_.物理清理墓碑.contains(请求.所属节点.值)
                    ? L1所有者范围读取状态::历史材料已清理
                    : L1所有者范围读取状态::未找到;
                return 结果;
            }
            if (所属事实->编码 != 请求.所属节点
                || !所属事实->写入所有者.编码.值 || !所属事实->创建事实代次
                || (所属事实->退出事实代次
                    && *所属事实->退出事实代次 <= 所属事实->创建事实代次)) {
                结果.状态 = L1所有者范围读取状态::内部不一致; return 结果;
            }
            if (所属事实->创建事实代次 > 请求.历史截止事实代次) {
                结果.状态 = L1所有者范围读取状态::未找到; return 结果;
            }
            if (所属事实->退出事实代次
                && 请求.历史截止事实代次 >= *所属事实->退出事实代次) {
                结果.状态 = L1所有者范围读取状态::已退出; return 结果;
            }
            std::vector<std::uint64_t> 候选;
            if (const auto 所属 = 状态_.历史属性槽值候选索引.find(
                    请求.所属节点.值); 所属 != 状态_.历史属性槽值候选索引.end()) {
                std::uint64_t 候选数量 = 0;
                for (const auto& [_, 编码组] : 所属->second) {
                    if (编码组.size() > 请求.最大数量 - 候选数量) {
                        结果.状态 = L1所有者范围读取状态::数量预算不足;
                        return 结果;
                    }
                    候选数量 += static_cast<std::uint64_t>(编码组.size());
                }
                候选.reserve(static_cast<std::size_t>(候选数量));
                for (const auto& [_, 编码组] : 所属->second)
                    候选.insert(候选.end(), 编码组.begin(), 编码组.end());
            }
            std::sort(候选.begin(), 候选.end());
            if (std::adjacent_find(候选.begin(), 候选.end()) != 候选.end()) {
                结果.状态 = L1所有者范围读取状态::内部不一致; return 结果;
            }
            结果.属性值组.reserve(候选.size());
            for (const auto 编码 : 候选) {
                const 值事实* 事实 = nullptr;
                if (const auto 当前 = 状态_.当前值.find(编码);
                    当前 != 状态_.当前值.end()) 事实 = &当前->second;
                ...
                if (事实->编码.值 != 编码 || 事实->所属节点 != 请求.所属节点
                    || !事实->属性类型节点.值 || !事实->写入所有者.编码.值
                    || !事实->创建事实代次
                    || (事实->退出事实代次
                        && *事实->退出事实代次 <= 事实->创建事实代次)) {
                    结果.状态 = L1所有者范围读取状态::内部不一致;
...
private:
    ...
    static bool 状态完整(const 状态& 值) {
        ...
        for (const auto& [值编码, _] : 期望属性候选)
            if (!已登记属性候选.contains(值编码)) return false;
        for (const auto& [编码, 节点] : 值.当前节点) {
            for (const auto& 槽 : 节点.当前属性) {

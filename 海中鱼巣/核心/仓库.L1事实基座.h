...
public:
    ...
    读取所有者范围来源当前完整值组(
        ...
        try {
            ...
            if (所有者 == 状态_.当前所有者.end()) {
                结果.状态 = 状态_.历史所有者.contains(请求.所有者.编码.值)
                    ? 读取状态::已退出 : 读取状态::未找到;
                return 结果;
            }
            if (所有者->second.所有者 != 请求.所有者
            ...
            if (来源 == 状态_.当前节点.end()) {
                ...
                if(状态_.当前关系.contains(编码)||状态_.当前值.contains(编码)
                    ||状态_.当前所有者.contains(编码)||状态_.历史所有者.contains(编码))
                    结果.状态=读取状态::入口拒绝;
                else if(const auto 历史=状态_.历史.find(编码);历史!=状态_.历史.end())
                    结果.状态=std::holds_alternative<节点事实>(历史->second.事实)
                        ?读取状态::已退出:读取状态::入口拒绝;
                else if(const auto 墓碑=状态_.物理清理墓碑.find(编码);
                    墓碑!=状态_.物理清理墓碑.end())
                    结果.状态=墓碑->second.事实种类==L1物理清理事实种类::节点
                        ?读取状态::已退出:读取状态::入口拒绝;
                else 结果.状态=读取状态::未找到;
                return 结果;
            }
    ...
    L1所有者范围历史完整关系组读取结果_v2 读取所有者范围历史完整关系组(
        const L1所有者范围历史完整关系组读取请求_v2& 请求) const noexcept {
        using 读取状态 = L1所有者范围历史完整关系组读取状态_v2;
        L1所有者范围历史完整关系组读取结果_v2 结果{
            读取状态::入口拒绝, L1所有者范围历史完整关系组读取合同版本,
            请求.方向, 请求.端点节点, 请求.关系类型节点,
            请求.期望读取事实代次, 请求.历史截止事实代次, 0, {}};
        const bool 方向有效 = 请求.方向 == L1所有者范围关系端点方向::源
            || 请求.方向 == L1所有者范围关系端点方向::目标;
        ...
        try {
            ...
            const auto 核验节点 = [&](稳定编码 编码) {
                ...
                if (const auto 当前 = 状态_.当前节点.find(编码.值);
                    ...
                    if (!事实) return 读取状态::入口拒绝;
                }
                if (!事实) {
                    if (状态_.当前关系.contains(编码.值)
                        || 状态_.当前值.contains(编码.值)
                        || 状态_.当前所有者.contains(编码.值)
                        || 状态_.历史所有者.contains(编码.值))
                        return 读取状态::入口拒绝;
                    if (const auto 墓碑 = 状态_.物理清理墓碑.find(编码.值);
                        墓碑 != 状态_.物理清理墓碑.end())
                        return 墓碑->second.事实种类 == L1物理清理事实种类::节点
                            ? 读取状态::历史材料已清理 : 读取状态::入口拒绝;
                    return 读取状态::未找到;
                }
                if (事实->编码 != 编码 || !有效(事实->写入所有者)
                    || !事实->创建事实代次
                    || (事实->退出事实代次
                        && *事实->退出事实代次 <= 事实->创建事实代次))
                    return 读取状态::内部不一致;
            ...
            if (const auto 状态 = 核验节点(请求.端点节点); 状态 != 读取状态::成功) {
                结果.状态 = 状态; return 结果;
            }
            if (const auto 状态 = 核验节点(请求.关系类型节点);
                状态 != 读取状态::成功) {
                结果.状态 = 状态; return 结果;
            }
            const auto& 索引 = 请求.方向 == L1所有者范围关系端点方向::源
            ...
            if (候选) {
                ...
                for (const auto 编码 : *候选) {
                    ...
                    const bool 当前命中 = 当前 != 状态_.当前关系.end();
                    const bool 历史命中 = 历史 != 状态_.历史.end();
                    const bool 墓碑命中 = 墓碑 != 状态_.物理清理墓碑.end();
                    if (当前命中) 事实 = &当前->second;
                    else if (历史命中) 事实 = std::get_if<关系事实>(&历史->second.事实);
                    if (!编码 || 编码 <= 前一编码
                        || static_cast<unsigned>(当前命中)
                            + static_cast<unsigned>(历史命中)
                            + static_cast<unsigned>(墓碑命中) != 1
                        || (历史命中 && !事实)) {
                        结果.状态 = 读取状态::内部不一致;
                        结果.关系组.clear(); return 结果;
                    }
                    if (墓碑命中) {
                        const auto& 清理 = 墓碑->second;
                        if (清理.事实种类 != L1物理清理事实种类::关系
                            || 清理.编码.值 != 编码 || !有效(清理.内部结构分区)
                            || !清理.创建事实代次 || !清理.退出事实代次
                            || !清理.物理清理事实代次
                            || 清理.创建事实代次 > 清理.退出事实代次
                            || 清理.退出事实代次 >= 清理.物理清理事实代次
                            || 清理.物理清理事实代次 > 状态_.事实代次) {
                            结果.状态 = 读取状态::内部不一致;
                            结果.关系组.clear(); return 结果;
                        }
                        if (清理.创建事实代次 <= 请求.历史截止事实代次
                            && 请求.历史截止事实代次 < 清理.退出事实代次) {
                            结果.状态 = 读取状态::历史材料已清理;
                            结果.关系组.clear(); return 结果;
                        }
                        前一编码 = 编码;
    ...
    L1所有者范围历史完整属性值组读取结果_v2 读取所有者范围历史完整属性值组(
        const L1所有者范围历史完整属性值组读取请求_v2& 请求) const noexcept {
        using 读取状态 = L1所有者范围历史完整属性值组读取状态_v2;
        L1所有者范围历史完整属性值组读取结果_v2 结果{
            读取状态::入口拒绝, L1所有者范围历史完整属性值组读取合同版本,
            请求.所属节点, 请求.期望读取事实代次,
            请求.历史截止事实代次, 0, {}};
        ...
        try {
            ...
            if (!所属事实) {
                const auto 编码 = 请求.所属节点.值;
                if (状态_.当前关系.contains(编码) || 状态_.当前值.contains(编码)
                    || 状态_.当前所有者.contains(编码)
                    || 状态_.历史所有者.contains(编码))
                    结果.状态 = 读取状态::入口拒绝;
                else if (const auto 墓碑 = 状态_.物理清理墓碑.find(编码);
                    墓碑 != 状态_.物理清理墓碑.end())
                    结果.状态 = 墓碑->second.事实种类 == L1物理清理事实种类::节点
            ...
            if (所属事实->编码 != 请求.所属节点 || !有效(所属事实->写入所有者)
                || !所属事实->创建事实代次
                || (所属事实->退出事实代次
            ...
            if (所属事实->创建事实代次 > 请求.历史截止事实代次) {
                结果.状态 = 读取状态::未找到; return 结果;
            }
            if (所属事实->退出事实代次
                && 请求.历史截止事实代次 >= *所属事实->退出事实代次) {
                结果.状态 = 读取状态::已退出; return 结果;
            }
            std::vector<std::pair<std::uint64_t, std::uint64_t>> 候选;
            if (const auto 所属 = 状态_.历史属性槽值候选索引.find(请求.所属节点.值);
                所属 != 状态_.历史属性槽值候选索引.end()) {
                std::size_t 候选数量 = 0;
                for (const auto& [_, 编码组] : 所属->second) {
                    if (编码组.size() > 候选.max_size() - 候选数量) {
                        结果.状态 = 读取状态::资源失败; return 结果;
                    }
                    候选数量 += 编码组.size();
                }
                候选.reserve(候选数量);
                for (const auto& [类型编码, 编码组] : 所属->second)
                    for (const auto 编码 : 编码组)
                        候选.emplace_back(编码, 类型编码);
            }
            std::sort(候选.begin(), 候选.end(), [](const auto& 左, const auto& 右) {
                return 左.first < 右.first
                    || (左.first == 右.first && 左.second < 右.second);
            });
            if (std::adjacent_find(候选.begin(), 候选.end(),
            ...
            for (const auto& [编码, 类型编码] : 候选) {
                ...
                const bool 墓碑命中 = 墓碑 != 状态_.物理清理墓碑.end();
                if (当前命中) 事实 = &当前->second;
                else if (历史命中) 事实 = std::get_if<值事实>(&历史->second.事实);
                if (!编码 || !类型编码
                    || static_cast<unsigned>(当前命中)
                        + static_cast<unsigned>(历史命中)
                        + static_cast<unsigned>(墓碑命中) != 1
                ...
                if (事实->编码.值 != 编码 || 事实->所属节点 != 请求.所属节点
                    || 事实->属性类型节点.值 != 类型编码
                    || !有效(事实->写入所有者) || !有效(事实->属性类型节点)
                    || !有效(事实->来源节点) || 事实->材料.valueless_by_exception()
                    || !std::visit([](const auto& 材料) noexcept {
                        using 材料类型 = std::decay_t<decltype(材料)>;
                        if constexpr (std::is_same_v<材料类型, std::int64_t>) return true;
                        else if constexpr (std::is_same_v<材料类型, 独立材料引用>)
                            return 有效(材料.编码);
            ...
            if (候选) {
                ...
                for (const auto 编码 : *候选) {
                    ...
                    if (当前命中) 事实 = &当前->second;
                        ...
                        事实 = std::get_if<关系事实>(&历史->second.事实);
                    if (!编码 || static_cast<unsigned>(当前命中)
                            + static_cast<unsigned>(历史命中)
                            + static_cast<unsigned>(墓碑命中) != 1
                        || (历史命中 && !事实)) {
                        结果.状态 = L1所有者范围读取状态::内部不一致;
        ...
        try {
            ...
            for (const auto& [编码, 类型编码] : 候选) {
                ...
                if (当前命中) 事实 = &当前->second;
                    ...
                    事实 = std::get_if<值事实>(&历史->second.事实);
                if (!编码 || !类型编码 || static_cast<unsigned>(当前命中)
                        + static_cast<unsigned>(历史命中)
                        + static_cast<unsigned>(墓碑命中) != 1
                    || (历史命中 && !事实)) {
                    结果.状态 = L1所有者范围读取状态::内部不一致;
...
private:
    ...
    template<class 索引类型>
    static bool 插入或核对历史定位(索引类型& 索引, std::uint64_t 第一,
        std::uint64_t 第二, std::uint64_t 编码) {
        if (!第一 || !第二 || !编码) return false;
        auto& 编码组 = 索引[第一][第二];
        const auto 位置 = std::lower_bound(编码组.begin(), 编码组.end(), 编码);
        if (位置 != 编码组.end() && *位置 == 编码) return true;
        编码组.insert(位置, 编码);
        return true;
    }

    // 三类历史定位索引只从会跨进程恢复的普通首次写账确定重建。
    // 进程维护账仅含退出事实且不参与定位；外层原子事务账不作为第二来源。
    static bool 重建普通首次账历史定位(const 状态& 来源, 状态& 目标) {
        目标.历史源关系候选索引.clear();
        目标.历史目标关系候选索引.clear();
        目标.历史属性槽值候选索引.clear();

        const auto 处理首次账 = [&](const L1中性写集请求& 写集,
            std::uint64_t 发布代次,
            const std::vector<std::pair<L1中性写集本地键, 稳定编码>>& 映射,
            L1结构所有者身份 所有者) {
            if (是进程维护幂等键(写集.幂等键.值))
                return 仅含非空退出事实(写集);
            if (!有效(所有者) || !发布代次 || 发布代次 > 来源.事实代次)
                return false;
            const auto 查映射 = [&](L1中性写集本地键 本地键)
                -> std::optional<稳定编码> {
                const auto 位置 = std::lower_bound(映射.begin(), 映射.end(), 本地键,
                    [](const auto& 项, L1中性写集本地键 键) {
                        return 项.first < 键;
                    });
                return 位置 != 映射.end() && 位置->first == 本地键
                    ? std::optional<稳定编码>{位置->second} : std::nullopt;
            ...
            const auto 解析 = [&](const L1中性事实引用& 引用)
                ...
                return std::visit([&](const auto& 项) -> std::optional<稳定编码> {
                    using T = std::decay_t<decltype(项)>;
                    if constexpr (std::is_same_v<T, 稳定编码>) return 项;
                    else return 查映射(项);
                }, 引用);
            };
            const auto 关系见证完整 = [&](稳定编码 编码, 稳定编码 源,
                稳定编码 目标节点, 稳定编码 类型) {
                const auto 当前 = 来源.当前关系.find(编码.值);
                const auto 历史 = 来源.历史.find(编码.值);
                const auto 墓碑 = 来源.物理清理墓碑.find(编码.值);
                const bool 当前命中 = 当前 != 来源.当前关系.end();
                const bool 历史命中 = 历史 != 来源.历史.end();
                const auto* 历史关系 = 历史命中
                    ? std::get_if<关系事实>(&历史->second.事实) : nullptr;
                const bool 墓碑命中 = 墓碑 != 来源.物理清理墓碑.end();
                if (static_cast<unsigned>(当前命中)
                        + static_cast<unsigned>(历史命中)
                        + static_cast<unsigned>(墓碑命中) != 1
                    || (历史命中 && !历史关系)) return false;
                if (墓碑命中)
                    return 墓碑->second.事实种类 == L1物理清理事实种类::关系
                        && 墓碑->second.编码 == 编码
                        && 墓碑->second.内部结构分区 == 所有者.编码
                        && 墓碑->second.创建事实代次 == 发布代次
                        && 墓碑->second.退出事实代次 >= 发布代次
                        && 墓碑->second.物理清理事实代次
                            > 墓碑->second.退出事实代次;
                const 关系事实& 事实 = 当前命中 ? 当前->second : *历史关系;
                return 事实.编码 == 编码 && 事实.源节点 == 源
                    && 事实.目标节点 == 目标节点 && 事实.关系类型节点 == 类型
                    && 事实.写入所有者 == 所有者
                    && 事实.创建事实代次 == 发布代次
                    && (当前命中 ? !事实.退出事实代次
                        : 事实.退出事实代次.has_value());
            };
            const auto 值见证完整 = [&](稳定编码 编码, 稳定编码 所属,
                稳定编码 类型) {
                const auto 当前 = 来源.当前值.find(编码.值);
                const auto 历史 = 来源.历史.find(编码.值);
                const auto 墓碑 = 来源.物理清理墓碑.find(编码.值);
                const bool 当前命中 = 当前 != 来源.当前值.end();
                const bool 历史命中 = 历史 != 来源.历史.end();
                const auto* 历史值 = 历史命中
                    ? std::get_if<值事实>(&历史->second.事实) : nullptr;
                const bool 墓碑命中 = 墓碑 != 来源.物理清理墓碑.end();
                if (static_cast<unsigned>(当前命中)
                        + static_cast<unsigned>(历史命中)
                        + static_cast<unsigned>(墓碑命中) != 1
                    || (历史命中 && !历史值)) return false;
                if (墓碑命中)
                    return 墓碑->second.事实种类 == L1物理清理事实种类::值
                        && 墓碑->second.编码 == 编码
                        && 墓碑->second.内部结构分区 == 所有者.编码
                        && 墓碑->second.创建事实代次 == 发布代次
                        && 墓碑->second.退出事实代次 >= 发布代次
            ...
            for (const auto& 关系 : 写集.关系) {
                const auto 编码 = 查映射(关系.本地键);
                const auto 源 = 解析(关系.源节点);
                const auto 目标节点 = 解析(关系.目标节点);
                const auto 类型 = 解析(关系.关系类型节点);
                if (!编码 || !源 || !目标节点 || !类型
                    || !关系见证完整(*编码, *源, *目标节点, *类型)
                    || !插入或核对历史定位(目标.历史源关系候选索引,
                        源->值, 类型->值, 编码->值)
                    || !插入或核对历史定位(目标.历史目标关系候选索引,
                        目标节点->值, 类型->值, 编码->值)) return false;
            }
            for (const auto& 槽 : 写集.属性槽变更) {
                const auto 所属 = 解析(槽.所属节点);
                const auto 类型 = 解析(槽.属性类型节点);
                const auto 编码 = 查映射(槽.新当前值);
                if (!所属 || !类型 || !编码
                    || !值见证完整(*编码, *所属, *类型)
                    || !插入或核对历史定位(目标.历史属性槽值候选索引,
                        所属->值, 类型->值, 编码->值)) return false;
            }
            return true;
        };

        if (!来源.中性幂等账.empty() && !来源.旧共享所有者定位)
            return false;
        for (const auto& [_, 账] : 来源.中性幂等账)
            if (!处理首次账(账.首次规范化写集, 账.首次发布事实代次,
                    账.首次新编码映射, *来源.旧共享所有者定位)) return false;
        for (const auto& [所有者编码, 账组] : 来源.所有者范围幂等账) {
            const L1结构所有者身份 所有者{稳定编码{所有者编码}};
            for (const auto& [_, 账] : 账组)
                if (!处理首次账(账.首次规范化写集, 账.首次发布事实代次,
                        账.首次新编码映射, 所有者)) return false;
        }
        return true;
    }
    ...
    static bool 状态完整(const 状态& 值) {
        ...
        状态 期望定位;
        if (!重建普通首次账历史定位(值, 期望定位)
            || 值.历史源关系候选索引 != 期望定位.历史源关系候选索引
            || 值.历史目标关系候选索引 != 期望定位.历史目标关系候选索引
            || 值.历史属性槽值候选索引 != 期望定位.历史属性槽值候选索引)
            return false;
        return true;
    }

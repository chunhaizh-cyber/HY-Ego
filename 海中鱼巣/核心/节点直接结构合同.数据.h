...
struct 节点直接事务幂等仓库恢复材料 {
    std::vector<节点直接事务幂等记录> 记录组;
    std::vector<节点直接持久证据侧账记录> 持久证据侧账组;
};
...
enum class 节点直接恢复仓操作状态 : std::uint8_t {
    已形成候选 = 1, 已确认待发布 = 2, 已撤销 = 3, 已发布 = 4,
    精确同义 = 5, 入口拒绝 = 6, 材料不完整 = 7, 身份冲突 = 8,
    版本漂移 = 9, 关系矛盾 = 10, 资源失败 = 11, 内部不一致 = 12
};

...
std::optional<发布安全根定义请求_v1> 形成发布请求(
    ...
    return 发布安全根定义请求_v1{
        ...
        1, 稳定编码{0x5341'4645'4445'4601ULL}};
}
...
int 运行安全根定义与当前值端到端测试() noexcept {
    ...
    try {
        ...
        const auto 未登记组 = 原子服务.按特征实例与UTC范围读取完整变化组_v2({
            ...
            1, 1000, 8});
        const auto 未登记代次组 =
            原子服务.按特征实例与事实代次范围读取完整变化组_v2({
                L2特征当前值变化永久账合同版本_v2,
                {L2结构合同版本, *未登记G},
                会话->本能根.安全根特征实例, *未登记G, 8});
        ...
        if (未登记代次组.状态
                != L2特征当前值变化事实代次组读取状态_v2::账未登记
            || 未登记代次组.成功() || !未登记代次组.完整变化组.empty()
            || 未登记代次组.完整集合见证
            || 未登记代次组.本次正式读回截止 != 0)
            return 失败("S12F", "fact-generation read requires registration");
        通过("S12F", "unregistered fact-generation range cannot be a legal empty set");
        ...
        const auto 覆盖边界不可用 =
            原子服务.按特征实例与事实代次范围读取完整变化组_v2({
                ...
                会话->本能根.安全根特征实例,
                登记.首次登记事实代次 - 1, 8});
        auto 坏代次请求 = L2特征当前值变化事实代次组读取请求_v2{
            L2特征当前值变化永久账合同版本_v2,
            {L2结构合同版本, *空组G},
            会话->本能根.安全根特征实例, 0, 8};
        const auto 零起点 =
        ...
        const auto 零预算 =
            原子服务.按特征实例与事实代次范围读取完整变化组_v2(
                坏代次请求);
        if (登记.首次登记事实代次 <= 1
            || !代次空组.成功() || !代次空组.完整变化组.empty()
            || !代次空组.完整集合见证
            || 代次空组.完整集合见证->排除起始事实代次
                != 登记.首次登记事实代次
            || !同G空组.成功() || !同G空组.完整变化组.empty()
            || 覆盖边界不可用.状态
                != L2特征当前值变化事实代次组读取状态_v2::覆盖边界不可用
            || 零起点.状态
            ...
            || 零预算.状态
                != L2特征当前值变化事实代次组读取状态_v2::入口拒绝)
            return 失败("S13F", "fact-generation registration boundary matrix");
        通过("S13F", "registration boundary and same-G empty coverage are closed");
        ...
        const auto 代次预算不足 =
            原子服务.按特征实例与事实代次范围读取完整变化组_v2({
                ...
                会话->本能根.安全根特征实例,
                登记.首次登记事实代次, 2});
        if (!代次变化组.成功() || 代次变化组.完整变化组.size() != 3
            || 代次变化组.完整变化组[0] != *变化.变化账事实
            || 代次变化组.完整变化组[1] != *第二变化.变化账事实
            || 代次变化组.完整变化组[2] != *第三变化.变化账事实
            || !排除首项组.成功() || 排除首项组.完整变化组.size() != 2
            || 排除首项组.完整变化组[0] != *第二变化.变化账事实
            || 排除首项组.完整变化组[1] != *第三变化.变化账事实
            || !当前边界空组.成功()
            || !当前边界空组.完整变化组.empty()
            || 代次预算不足.状态
                != L2特征当前值变化事实代次组读取状态_v2::数量预算不足
            || 代次预算不足.成功()
            || !代次预算不足.完整变化组.empty()
            || 代次预算不足.完整集合见证
            || 代次预算不足.本次正式读回截止 != 0)
            return 失败("S15F", "fact-generation 0/N range, ordering and budget");
        通过("S15F", "fact-generation range is open-left closed-right and ordered");

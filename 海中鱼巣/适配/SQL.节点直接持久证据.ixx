...
DECLARE @状态 tinyint=(SELECT 实例状态 FROM dbo.节点直接安装实例证据_v1 WITH(UPDLOCK,HOLDLOCK) WHERE 安装实例命名域=)SQL"
    ...
    节点直接恢复材料读取结果 读取安装实例(节点直接事务幂等身份 安装实例身份) override {
        using namespace 节点直接持久证据SQL内部;
        std::scoped_lock 锁(互斥量_);
        节点直接恢复材料读取结果 结果;
        if (!节点直接事务幂等身份完整(安装实例身份)) { 结果.安装状态 = 节点直接安装实例证据状态::损坏; return 结果; }
        if (!基础适配_.初始化数据库().成功) { 结果.安装状态 = 节点直接安装实例证据状态::资源失败; return 结果; }
        短连接 连接;
        if (!连接.打开(基础适配_.读取配置())) { 结果.安装状态 = 节点直接安装实例证据状态::资源失败; return 结果; }
        auto 安装域 = 网络序(安装实例身份.命名域);
        auto 安装键 = 网络序(安装实例身份.键值);
        std::array<SQLLEN, 2> 参数指示{};
        短语句 语句(连接.句柄());
        constexpr std::wstring_view 实例文本 =
            L"SELECT 实例状态 FROM dbo.节点直接安装实例证据_v1 WHERE 安装实例命名域=? AND 安装实例键值=?";
        if (!语句.可用() || !准备语句(语句.句柄(), 实例文本)
            || !绑定定长二进制(语句.句柄(), 1, 安装域, 参数指示[0])
            || !绑定定长二进制(语句.句柄(), 2, 安装键, 参数指示[1])
            || !ODBC成功(SQLExecute(语句.句柄()))) { 结果.安装状态 = 节点直接安装实例证据状态::资源失败; return 结果; }
        if (SQLFetch(语句.句柄()) == SQL_NO_DATA) return 结果;
        SQLCHAR 实例状态 = 0; SQLLEN 长度 = 0;
        if (!ODBC成功(SQLGetData(语句.句柄(), 1, SQL_C_UTINYINT, &实例状态, sizeof(实例状态), &长度)) || (实例状态 != 1 && 实例状态 != 2)) { 结果.安装状态 = 节点直接安装实例证据状态::损坏; return 结果; }
        if (SQLFetch(语句.句柄()) != SQL_NO_DATA) { 结果.安装状态 = 节点直接安装实例证据状态::损坏; return 结果; }
        短语句 事务语句(连接.句柄());
        constexpr std::wstring_view 事务文本 =
            L"SELECT 事务命名域,事务键值,尝试序号,材料格式版本,请求意图摘要,执行证据摘要,DATALENGTH(写集材料),写集材料,证据状态,发布代次,结果摘要 FROM dbo.节点直接结构事务证据_v1 WHERE 安装实例命名域=? AND 安装实例键值=? ORDER BY 事务命名域,事务键值,尝试序号";
        if (!事务语句.可用() || !准备语句(事务语句.句柄(), 事务文本)
            || !绑定定长二进制(事务语句.句柄(), 1, 安装域, 参数指示[0])
            || !绑定定长二进制(事务语句.句柄(), 2, 安装键, 参数指示[1])
            || !ODBC成功(SQLExecute(事务语句.句柄()))) { 结果.安装状态 = 节点直接安装实例证据状态::资源失败; return 结果; }
        bool 全部撤销 = true;
        while (true) {
            const auto 获取 = SQLFetch(事务语句.句柄());
            if (获取 == SQL_NO_DATA) break;
            if (!ODBC成功(获取)) { 结果.安装状态 = 节点直接安装实例证据状态::资源失败; return 结果; }
            节点直接持久事务证据 项; SQLLEN 指示 = 0; SQLUINTEGER 格式 = 0; SQLINTEGER 材料长度 = -1; SQLCHAR 状态 = 0;
            std::array<std::uint8_t, 8> 域{}, 键{};
            if (!读取定长(事务语句.句柄(), 1, 域) || !读取定长(事务语句.句柄(), 2, 键)
                || !读取整数(事务语句.句柄(), 3, 项.尝试序号)
                || !ODBC成功(SQLGetData(事务语句.句柄(), 4, SQL_C_ULONG, &格式, sizeof(格式), &指示))
                || !读取定长(事务语句.句柄(), 5, 项.请求意图摘要)
                || !读取定长(事务语句.句柄(), 6, 项.执行证据摘要)
                || !ODBC成功(SQLGetData(事务语句.句柄(), 7, SQL_C_SLONG, &材料长度, sizeof(材料长度), &指示))
                || !读取字节组(事务语句.句柄(), 8, 材料长度, 项.写集材料)
                || !ODBC成功(SQLGetData(事务语句.句柄(), 9, SQL_C_UTINYINT, &状态, sizeof(状态), &指示))) { 结果.安装状态 = 节点直接安装实例证据状态::损坏; return 结果; }
            项.事务身份 = {读取网络序(域), 读取网络序(键)}; 项.材料格式版本 = 格式;
            项.状态 = 状态 == 1 ? 节点直接持久端口状态::已准备 : 状态 == 2 ? 节点直接持久端口状态::已见证 : 状态 == 3 ? 节点直接持久端口状态::结果未知 : 状态 == 4 ? 节点直接持久端口状态::损坏 : 状态 == 5 ? 节点直接持久端口状态::已撤销未发布 : 节点直接持久端口状态::损坏;
            全部撤销 = 全部撤销 && 项.状态 == 节点直接持久端口状态::已撤销未发布;
            std::array<std::uint8_t, 8> 代次{};
            const auto 代次结果 = SQLGetData(事务语句.句柄(), 10, SQL_C_BINARY, 代次.data(), 代次.size(), &指示);
            if (ODBC成功(代次结果) && 指示 == 8) 项.发布代次 = 读取网络序(代次);
            else if (!(ODBC成功(代次结果) && 指示 == SQL_NULL_DATA)) { 结果.安装状态 = 节点直接安装实例证据状态::损坏; return 结果; }
            std::array<std::uint8_t, 32> 摘要{};
            const auto 摘要结果 = SQLGetData(事务语句.句柄(), 11, SQL_C_BINARY, 摘要.data(), 摘要.size(), &指示);
            if (ODBC成功(摘要结果) && 指示 == 32) 项.结果摘要 = 摘要;
            else if (!(ODBC成功(摘要结果) && 指示 == SQL_NULL_DATA)) { 结果.安装状态 = 节点直接安装实例证据状态::损坏; return 结果; }
            const bool 已发布字段完整 = 项.发布代次.has_value() && *项.发布代次 != 0
                && 项.结果摘要.has_value() && 摘要非零(*项.结果摘要);
            const bool 未发布字段为空 = !项.发布代次.has_value() && !项.结果摘要.has_value();
            if (!节点直接事务幂等身份完整(项.事务身份) || 项.尝试序号 == 0
                || !摘要非零(项.请求意图摘要) || !摘要非零(项.执行证据摘要)
                || (项.状态 == 节点直接持久端口状态::已见证 && !已发布字段完整)
                || ((项.状态 == 节点直接持久端口状态::已准备
                        || 项.状态 == 节点直接持久端口状态::已撤销未发布) && !未发布字段为空)) {
                结果.安装状态 = 节点直接安装实例证据状态::损坏;
                return 结果;
            }
            结果.事务组.push_back(std::move(项));
        }
        结果.安装状态 = 实例状态 == 1 || (实例状态 == 2 && !结果.事务组.empty() && 全部撤销)
            ? 节点直接安装实例证据状态::确认从未发布
            : 节点直接安装实例证据状态::存在事务证据;
        return 结果;
    }

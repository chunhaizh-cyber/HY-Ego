...
场景组织历史结果 场景类数据服务::读路径(std::uint64_t g, std::uint64_t h,
                                               场景根角色 role,
                                               稳定编码 instance,
...
场景角色历史结果 场景类数据服务::读角色(std::uint64_t g, std::uint64_t h,
                                               稳定编码 c) const {
...
场景直接包含单项结果 场景类数据服务::读取直接存在成员历史(
    const 场景直接包含历史读取请求 &r) const {
  return 读取直接包含历史(r, 场景直接包含种类::存在成员);
}
场景直接包含单项结果 场景类数据服务::读取直接子场景历史(
    const 场景直接包含历史读取请求 &r) const {

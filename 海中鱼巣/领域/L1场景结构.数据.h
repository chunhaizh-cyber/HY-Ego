...
inline bool 直接子场景结构事实完整(
    const 直接子场景结构事实& 事实) noexcept {
    return 世界结构节点见证有效(事实.子场景)
        && 世界结构值见证有效(事实.场景标记)
        && 世界结构关系见证有效(事实.直接父关系)
        && 事实.子场景.类型 == 节点种类::普通
        && 事实.场景标记.所属节点 == 事实.子场景
...
inline bool 直接存在成员结构事实完整(
    const 直接存在成员结构事实& 事实) noexcept {
    return 世界结构节点见证有效(事实.成员)
        && 世界结构关系见证有效(事实.归属关系)
        && 事实.成员.类型 == 节点种类::普通
        && 事实.归属关系.目标端 == 事实.成员
        && 事实.归属关系.角色或顺序 == 0;
...
inline bool 初始场景完整结构事实完整(
    const 初始场景完整结构事实& 事实) noexcept {
    if (事实.合同版本 != L1场景结构合同版本
        || 事实.世界规则版本 != 世界登记规则版本
        || 事实.事实截止代次 == 0 || !初始场景事实完整(事实.场景)
        || 事实.世界根 != 事实.场景.世界根
        || 事实.场景.事实截止代次 != 事实.事实截止代次)
        return false;
    std::uint64_t 前一子场景 = 0;
    for (std::size_t i = 0; i < 事实.直接子场景组.size(); ++i) {
        const auto& 子项 = 事实.直接子场景组[i];
        if (!直接子场景结构事实完整(子项)
            || 子项.子场景 == 事实.世界根
            || 子项.子场景 == 事实.场景.初始场景
            || 子项.场景标记.属性类型 != 事实.场景.场景标记.属性类型
            || 子项.场景标记.来源 != 事实.场景.场景标记.来源
            || 子项.场景标记.创建事实代次 > 事实.事实截止代次
            || 子项.直接父关系.关系类型
                != 事实.场景.直接父关系.关系类型
            || 子项.直接父关系.源端 != 事实.场景.初始场景
            || 子项.子场景.编码.值 <= 前一子场景)
            return false;
        for (std::size_t j = 0; j < i; ++j)
            if (事实.直接子场景组[j].直接父关系.编码
                == 子项.直接父关系.编码) return false;
        前一子场景 = 子项.子场景.编码.值;
    }
    std::uint64_t 前一成员 = 0;
    std::optional<世界结构节点身份见证> 成员关系类型;
    for (std::size_t i = 0; i < 事实.直接存在成员组.size(); ++i) {
        const auto& 成员项 = 事实.直接存在成员组[i];
        if (!直接存在成员结构事实完整(成员项)
            || 成员项.成员 == 事实.世界根
            || 成员项.成员 == 事实.场景.初始场景
            || 成员项.归属关系.源端 != 事实.场景.初始场景
            || 成员项.成员.编码.值 <= 前一成员
            || (成员关系类型.has_value()
                && 成员项.归属关系.关系类型 != *成员关系类型))
            return false;
        for (std::size_t j = 0; j < i; ++j)
            if (事实.直接存在成员组[j].归属关系.编码
                == 成员项.归属关系.编码) return false;
        for (const auto& 子项 : 事实.直接子场景组)
            if (成员项.成员 == 子项.子场景) return false;
        成员关系类型 = 成员项.归属关系.关系类型;
        前一成员 = 成员项.成员.编码.值;
    }
    return true;
}

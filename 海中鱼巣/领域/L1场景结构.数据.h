...
inline constexpr std::uint32_t L1场景结构合同版本 = 3;
...
inline bool 初始场景创建请求有效(const 初始场景创建请求& 请求) noexcept {
...
inline bool 初始场景读取请求有效(const 初始场景读取请求& 请求) noexcept {
    return 请求.合同版本 == L1场景结构合同版本
        && 请求.世界规则版本 == 世界登记规则版本
        && 世界结构节点见证有效(请求.世界根)
        && 请求.世界根.类型 == 节点种类::普通
...
inline bool 初始场景结果::成功() const noexcept {
    return (状态 == 世界结构状态::已提交
            || 状态 == 世界结构状态::幂等读回
            || 状态 == 世界结构状态::已读取)
        && 事实.has_value() && 初始场景事实完整(*事实);
}

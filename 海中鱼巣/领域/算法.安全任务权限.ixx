...
template<class 层类型>
inline std::optional<std::int64_t> 计算释放倍率(const 层类型& 层) noexcept {
    if (层.值域下界 >= 层.值域上界
        || 层.低位阈值 < 层.值域下界
        || 层.高位阈值 > 层.值域上界
        || 层.低位阈值 > 层.高位阈值
...
权限读取结果 计算安全五组权限(
    ...
    try {
        ...
        for (std::size_t 序号 = 0; 序号 < 请求.安全层来源选择组.size(); ++序号) {
            const auto& 选择 = 请求.安全层来源选择组[序号];
            if (!句柄有效(选择.来源需求)
                || !句柄有效(选择.维护族)
                || !句柄有效(选择.来源因果信息)
                || !句柄有效(选择.最终安全结果)
        ...
        for (std::size_t 序号 = 0; 序号 < 有序层组.size(); ++序号) {
            ...
            if (序号 != 0
                && (层.值版本 != 有序层组.front().值版本
                    || 层.阈值版本 != 有序层组.front().阈值版本
                    || 层.规则版本 != 有序层组.front().规则版本)) {
                return 无权限载荷(安全结果分类::版本漂移);
            }
        }
        for (std::size_t 序号 = 0; 序号 < 有序材料缺口组.size(); ++序号) {
            const auto& 缺口 = 有序材料缺口组[序号];
            if (缺口.首个受影响组 < 1 || 缺口.首个受影响组 > 5
                || !句柄有效(缺口.缺口来源) || 缺口.期望版本 == 0
                || (序号 != 0
                    && (有序材料缺口组[序号 - 1].首个受影响组
                            > 缺口.首个受影响组
                        || (有序材料缺口组[序号 - 1].首个受影响组
        ...
        for (std::size_t 序号 = 0; 序号 < 有序未归层来源组.size(); ++序号) {
            const auto& 选择 = 有序未归层来源组[序号];
            if (!句柄有效(选择.来源需求)
                || !句柄有效(选择.维护族)
                || !句柄有效(选择.来源因果信息)
                || !句柄有效(选择.最终安全结果)
                || 选择.期望当前值版本 == 0
                || (序号 != 0
                    && !来源选择小于(有序未归层来源组[序号 - 1], 选择))) {
                return 无权限载荷(安全结果分类::结构拒绝);
            }
        }

        std::vector<bool> 已分区(请求.安全层来源选择组.size(), false);
        const auto 标记分区 = [&](const 安全权限层来源选择& 选择) {
            for (std::size_t 序号 = 0; 序号 < 请求.安全层来源选择组.size(); ++序号) {
                if (!已分区[序号]
                    && 来源选择同义(请求.安全层来源选择组[序号], 选择)) {
                    已分区[序号] = true;
                    return true;
                }
            }
        ...
        for (const auto& 选择 : 有序未归层来源组) {
            if (!标记分区(选择)) return 无权限载荷(安全结果分类::结构拒绝);
        }
        for (const auto& 缺口 : 有序材料缺口组) {
            bool 已找到 = false;
            for (std::size_t 序号 = 0; 序号 < 请求.安全层来源选择组.size(); ++序号) {
                const auto& 选择 = 请求.安全层来源选择组[序号];
                if (!已分区[序号]
            ...
            if (!已找到) return 无权限载荷(安全结果分类::结构拒绝);
        }
        if (!std::all_of(已分区.begin(), 已分区.end(), [](bool 值) { return 值; })) {
            return 无权限载荷(安全结果分类::结构拒绝);
        }
        ...
        if (!有序层组.empty()) {
            载荷.全部来源版本.值集合版本 = 有序层组.front().值版本;
            载荷.全部来源版本.阈值集合版本 = 有序层组.front().阈值版本;
            载荷.全部来源版本.维护规则版本 = 有序层组.front().规则版本;
        }
        载荷.权限规则版本 = 请求.权限规则版本;
        载荷.收到权限.fill(0);
        载荷.保留权限.fill(0);
        ...
        if (有序层组.empty() && 有序材料缺口组.empty()) {
            载荷.五组已形成权限总量 = 0;
            return {安全结果分类::权限为零, std::move(载荷)};
        }

        std::optional<std::int64_t> 首个缺口组;
        for (const auto& 缺口 : 有序材料缺口组) {
            if (!首个缺口组 || 缺口.首个受影响组 < *首个缺口组) {
                首个缺口组 = 缺口.首个受影响组;
            }
        }
        std::array<std::vector<const 安全组层快照*>, 5> 分组;
        for (const auto& 层 : 载荷.全部安全组层快照) {
            分组[static_cast<std::size_t>(层.优先级组 - 1)].push_back(&层);
        }
...
任务权限结果 计算单任务安全权限(
    ...
    try {
        ...
        if (任务事实.自我 != 请求.自我
            || 任务事实.适用场景 != 请求.适用场景) {
            return 无任务载荷(安全结果分类::内部不一致);
        }
        if (任务事实.事实截止版本 != 请求.事实截止版本
            || 任务事实.任务集合版本 != 请求.任务集合版本
            || 任务事实.来源版本.事实截止版本 != 请求.事实截止版本
            || 任务事实.来源版本.任务集合版本 != 请求.任务集合版本
            || 权限.全部来源版本.事实截止版本 != 请求.事实截止版本
            || 权限.全部来源版本.图版本 != 请求.图版本
            || 权限.全部来源版本.权限规则版本 != 请求.权限规则版本
            || 权限.权限规则版本 != 请求.权限规则版本
            || !任务事实.来源版本.任务集合版本) {
...
任务排序结果 排序安全任务执行候选(
    ...
    try {
        ...
        任务排序载荷 载荷;
        载荷.全部输入版本 = 任务事实.来源版本;
        载荷.全部输入版本.图版本 = 请求.图版本;
        载荷.全部输入版本.权限规则版本 = 请求.权限规则版本;
        载荷.全部输入版本.排序规则版本 = 请求.排序规则版本;
        载荷.排序规则版本 = 请求.排序规则版本;

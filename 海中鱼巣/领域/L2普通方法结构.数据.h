...
struct L2方法动作入口事实_v2 final {
    ...
    稳定编码 动作归因合同;
    L2生命周期 生命周期;
    friend bool operator==(const L2方法动作入口事实_v2&,
        const L2方法动作入口事实_v2&) = default;
};
...
struct L2方法动作入口写入请求_v2 final {
    ...
    稳定编码 结果验证合同;
    稳定编码 动作归因合同;
    friend bool operator==(const L2方法动作入口写入请求_v2&,
        const L2方法动作入口写入请求_v2&) = default;
};
...
struct L2方法动作入口读取请求_v2 final {
    ...
    std::uint64_t 顺序 = 0;
    L2方法稳定动作键 稳定动作键;
    friend bool operator==(const L2方法动作入口读取请求_v2&,
        const L2方法动作入口读取请求_v2&) = default;
};
...
struct L2方法动作入口提交见证 final {
    ...
    friend bool operator==(const L2方法动作入口提交见证&,
        const L2方法动作入口提交见证&) = default;
};
...
struct L2方法动作入口写入结果 final {
    ...
    friend bool operator==(const L2方法动作入口写入结果&,
        const L2方法动作入口写入结果&) = default;
};
...
struct L2方法动作入口读取结果 final {
    ...
    std::uint64_t 本次正式读回截止 = 0;
    bool 成功() const noexcept;
    friend bool operator==(const L2方法动作入口读取结果&,
        const L2方法动作入口读取结果&) = default;
};

inline constexpr std::uint32_t L2方法完整有序动作组读取合同版本_v2 = 2;

struct L2方法完整有序动作组读取请求_v2 final {
    std::uint32_t 合同版本 = L2方法完整有序动作组读取合同版本_v2;
    L2结构请求头 请求头;
    L2方法身份 方法;
    L2方法内容版本 方法内容版本;
    friend bool operator==(const L2方法完整有序动作组读取请求_v2&,
        const L2方法完整有序动作组读取请求_v2&) = default;
};

enum class L2方法完整有序动作组读取状态 : std::uint8_t {
    已读取 = 1,
    未找到 = 2,
    入口拒绝 = 3,
    许可拒绝 = 4,
...
struct L2方法完整有序动作组读取结果 final {
    L2方法完整有序动作组读取状态 状态 =
        L2方法完整有序动作组读取状态::内部错误;
    std::vector<L2方法动作入口事实_v2> 完整有序动作入口组;
    std::uint64_t 本次正式读回截止 = 0;
    bool 成功() const noexcept;
    friend bool operator==(const L2方法完整有序动作组读取结果&,
        const L2方法完整有序动作组读取结果&) = default;
};
...
inline bool L2方法动作作用范围有效(L2方法动作作用范围 值) noexcept {
    return 值 == L2方法动作作用范围::内部治理
        || 值 == L2方法动作作用范围::纯现实观察
        || 值 == L2方法动作作用范围::现实改变;
}
inline bool L2方法动作入口写入请求_v2有效(
    const L2方法动作入口写入请求_v2& 请求) noexcept {
    return 请求.合同版本 == L2方法动作入口合同版本_v2
        && L2结构请求头合同有效(请求.请求头)
        && 请求.请求头.期望事实代次 != 0
        && L2结构幂等身份有效(请求.幂等身份)
        && 有效(请求.方法.值)
        ...
        && 请求.顺序 != 0
        && L2方法稳定动作键有效(请求.稳定动作键)
        && L2方法动作作用范围有效(请求.作用范围)
        && 有效(请求.作用主体约束.值)
        && (!请求.场景约束 || 有效(请求.场景约束->值))
        && 有效(请求.动作范围材料)
        && 请求.技术许可合同版本 != 0
        && 有效(请求.预期结果材料)
        && 有效(请求.结果验证合同)
...
inline bool L2方法动作入口读取请求_v2有效(
    const L2方法动作入口读取请求_v2& 请求) noexcept {
    return 请求.合同版本 == L2方法动作入口合同版本_v2
        && L2结构请求头合同有效(请求.请求头)
        && 请求.请求头.期望事实代次 != 0
        && 有效(请求.方法.值)
...
inline bool L2方法完整有序动作组读取请求_v2有效(
    const L2方法完整有序动作组读取请求_v2& 请求) noexcept {
    return 请求.合同版本 == L2方法完整有序动作组读取合同版本_v2
        && L2结构请求头合同有效(请求.请求头)
        && 请求.请求头.期望事实代次 != 0
        && 有效(请求.方法.值)
        && L2方法内容版本有效(请求.方法内容版本);
...
inline bool L2方法动作入口写入结果::成功() const noexcept {
    return (状态 == L2方法动作入口写入状态::已形成
            || 状态 == L2方法动作入口写入状态::精确重复)
        && 动作入口事实 && !提交见证 && 本次正式读回截止 != 0
        && 动作入口事实->生命周期.创建事实代次 != 0
        && 动作入口事实->生命周期.创建事实代次
            <= 本次正式读回截止
        && !动作入口事实->生命周期.退出事实代次;
}
inline bool L2方法动作入口读取结果::成功() const noexcept {
    return 状态 == L2方法动作入口读取状态::已读取
        && 动作入口事实 && 本次正式读回截止 != 0
        && 动作入口事实->生命周期.创建事实代次 != 0
        && 动作入口事实->生命周期.创建事实代次
            <= 本次正式读回截止
...
inline bool L2方法完整有序动作组读取结果::成功() const noexcept {
    if (状态 != L2方法完整有序动作组读取状态::已读取
        || 完整有序动作入口组.empty() || 本次正式读回截止 == 0)
        return false;
    const auto 方法 = 完整有序动作入口组.front().方法;
    const auto 内容版本 = 完整有序动作入口组.front().方法内容版本;
    for (std::size_t i = 0; i < 完整有序动作入口组.size(); ++i) {
        const auto& 当前 = 完整有序动作入口组[i];
        if (!L2方法动作入口身份有效(当前.身份)
            || 当前.方法 != 方法 || !有效(当前.方法.值)
            || 当前.方法内容版本 != 内容版本
            || !L2方法内容版本有效(当前.方法内容版本)
            || 当前.顺序 != i + 1U
            || !L2方法稳定动作键有效(当前.稳定动作键)
            || !L2方法动作作用范围有效(当前.作用范围)
            || !有效(当前.作用主体约束.值)
            || (当前.场景约束 && !有效(当前.场景约束->值))
            || !有效(当前.动作范围材料)
            || 当前.技术许可合同版本 == 0
            || !有效(当前.预期结果材料)
            || !有效(当前.结果验证合同)
            || !有效(当前.动作归因合同)
            || 当前.生命周期.创建事实代次 == 0
            || 当前.生命周期.创建事实代次 > 本次正式读回截止
            || (当前.生命周期.退出事实代次
                && *当前.生命周期.退出事实代次 <= 本次正式读回截止))
            return false;
        for (std::size_t j = i + 1; j < 完整有序动作入口组.size(); ++j) {
            if (当前.身份 == 完整有序动作入口组[j].身份
                || 当前.稳定动作键 == 完整有序动作入口组[j].稳定动作键)
                return false;
        }
    }
    return true;

...
template<class 写字段>
L1领域意图凭证 形成G3凭证(std::uint32_t 标签, 写字段&& 写入字段) {
    std::vector<std::uint8_t> 编码{'H','Z','Y','-','L','1','G','3'};
    L1确定性编码内部::写U32(编码, L1请求意图格式版本);
    L1确定性编码内部::写U8(编码, 3);
    L1确定性编码内部::写U32(编码, 标签);
    写入字段(编码);
    return {3, L1请求意图格式版本, 标签,
        L1确定性编码内部::哈希(std::move(编码))};
}

L1领域意图凭证 形成G3特征定义意图凭证(const 特征定义登记请求& 请求) {
    return 形成G3凭证(0x15000501U, [&](auto& 编码) {
...
L1领域意图凭证 形成G3I64特征定义意图凭证(const I64特征定义建立请求& 请求) {
    return 形成G3凭证(0x15000502U, [&](auto& 编码) {
        L1确定性编码内部::写U32(编码, 请求.合同版本);
        L1确定性编码内部::写U32(编码, 请求.规则版本);
        L1确定性编码内部::写U64(编码, 请求.幂等身份.值);
        L1确定性编码内部::写U64(编码, 请求.期望事实代次);
        L1确定性编码内部::写I64(编码, 请求.下界);
        L1确定性编码内部::写I64(编码, 请求.上界);
    });
}
...
public:
    ...
    特征定义登记结果 建立登记(const 特征定义登记请求& 请求) {
        ...
        try {
            ...
            const auto 凭证 = L1特征定义内部::形成G3特征定义意图凭证(请求);
            ...
            if (探测.状态 == L1幂等探测状态::同义已发布) {
                return 首次登记请求_.has_value() && *首次登记请求_ == 请求
                    && 登记_.has_value()
                    ? 特征定义登记结果{特征定义状态::幂等读回, 登记_}
                    : 特征定义登记结果{特征定义状态::内部不一致, std::nullopt};
            }
            ...
            const auto 写入 = L1_.提交写集(写集, 0x15000501U);
    ...
    I64特征定义结果 建立I64特征定义(const I64特征定义建立请求& 请求) {
        ...
        try {
            ...
            const auto 凭证 = L1特征定义内部::形成G3I64特征定义意图凭证(请求);
            const auto 幂等键 = L1特征定义内部::映射幂等键(
                L1特征定义内部::定义幂等域, 请求.幂等身份);
            const auto 探测 = L1_.探测幂等({L1事实基座合同版本,
                L1幂等摘要合同版本, 幂等键, 凭证.请求意图摘要, 凭证});
            if (探测.状态 == L1幂等探测状态::同义已发布) {
                if (!定义请求_.has_value() || *定义请求_ != 请求 || !定义事实_)
                    return {特征定义状态::内部不一致, std::nullopt, {}, {}, {}};
                return {特征定义状态::幂等读回, 定义事实_,
                    定义编码_[1], 定义编码_[2], 定义编码_[3]};
            }
            if (探测.状态 == L1幂等探测状态::异义冲突)
                return {特征定义状态::幂等冲突, std::nullopt, {}, {}, {}};
            if (探测.状态 == L1幂等探测状态::已隔离
            ...
            const auto 写入 = L1_.提交写集(写集, 0x15000502U);
            const auto 映射状态 = L1特征定义内部::映射写入状态(写入.状态);
            if (映射状态 != 特征定义状态::已提交
                && 映射状态 != 特征定义状态::幂等读回) {
                return {映射状态, std::nullopt, {}, {}, {}};

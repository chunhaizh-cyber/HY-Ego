...
定义L2任务结构类型身份(L2任务来源需求关系类型身份);
定义L2任务结构类型身份(L2任务正式存在关系类型身份);
...
inline bool L2按任务读取来源需求关系组请求有效(
    const L2按任务读取来源需求关系组请求& 请求) noexcept {
    return 请求.合同版本 == L2按任务读取来源需求关系组合同版本
...
struct L2按任务读取来源需求关系组结果 final {
    ...
    bool 成功() const noexcept {
        if (结果头.合同版本 != L2结构合同版本
            ...
            || !有效(任务.值)
            || 本次正式读回截止 != 结果头.事实截止代次) return false;
...
struct L2按建立幂等身份读取任务轮次核心结果 final {
    ...
    bool 成功() const noexcept {
        if (合同版本 != L2按建立幂等身份读取任务轮次核心合同版本
            ...
            || !L2任务身份来源事实完整(*任务身份来源)
            || 任务身份来源->任务节点 != 任务->身份.值
            || 任务身份来源->任务节点生命周期.创建事实代次 > 本次正式读回截止
            || !任务身份来源->任务节点生命周期.创建事实代次
            || 任务身份来源->任务节点生命周期.退出事实代次
            || 任务身份来源->族归属关系生命周期.创建事实代次 > 本次正式读回截止
...
enum class L2按任务读取当前任务与轮次状态 : std::uint8_t {
    已读取 = 1, 任务未找到 = 2, 当前轮次未找到 = 3, 任务已退出 = 4,
    入口拒绝 = 8, 许可拒绝 = 9, 事实代次漂移 = 10, 关系多义 = 11,

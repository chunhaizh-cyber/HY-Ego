...
public:
    ...
    存在当前身份确认结果 确认当前存在结构身份(
        ...
        try {
            ...
            if (!目标 || 目标->写入所有者 != 所有者_
                || 身份 == 存在族锚点_ || 身份 == 存在族归属关系类型_)
                return 当前身份失败(存在结构身份只读状态::入口拒绝, G0);

            const auto* 关系组 = 查找一致源关系组(
                读取, 身份, 存在族归属关系类型_);
            if (!关系组 || 关系组->成员.empty())
                return 当前身份失败(存在结构身份只读状态::入口拒绝, G0);
            if (!普通空节点当前有效(*目标, 身份))
...
public:
    ...
    存在身份来源历史见证读取结果 读取存在身份来源历史见证(
        ...
        try {
            ...
            if (前.second != Gread)
                return 历史身份失败(存在结构身份只读状态::事实代次漂移,
                    前.second, H);
            读取守卫已成立 = true;

            const auto owner = 第一层服务_.读取历史结构所有者(
                {L1所有者范围CRUD合同版本, 所有者_});
            const auto e = 第一层服务_.读取所有者范围历史事实(
                {L1所有者范围CRUD合同版本, 身份});
            ...
            const auto 读取错误 = 历史来源读取错误(
                owner, e, anchor, type, links, Gread, H, 身份);
            if (读取错误) return 历史身份失败(*读取错误, Gread, H);
            ...
            存在身份来源历史见证读取结果 结果{
                存在结构身份只读状态::已读取,
                存在结构身份只读合同版本, Gread, H, std::move(见证)};
            return 结果.成功(Gread, H, 身份)
                ? 结果
                : 历史身份失败(存在结构身份只读状态::内部不一致,
                    Gread, H);
        } catch (const std::bad_alloc&) {
    ...
    存在类结点结果 删除存在(const 存在类删除请求& 请求) {
        ...
        try {
            ...
            const auto 族关系组 = 读取当前源关系组(
                请求.存在结点, 存在族归属关系类型_, 请求.期望事实代次);
            if (!族关系组.成功)
                return 失败(族关系组.状态, 族关系组.事实代次);
            if (族关系组.关系组.size() != 1
                || !族归属关系有效(族关系组.关系组.front(),
                    请求.存在结点, std::nullopt))
                return 失败(存在类数据状态::内部不一致,
                    请求.期望事实代次);
...
private:
    ...
    static 存在当前身份确认结果 当前身份失败(
        存在结构身份只读状态 状态,
        std::uint64_t Gread = 0) noexcept {
        return {状态, 存在结构身份只读合同版本, Gread};
    }

    static 存在身份来源历史见证读取结果 历史身份失败(
        存在结构身份只读状态 状态,
        std::uint64_t Gread = 0,
        std::uint64_t H = 0) noexcept {
        return {状态, 存在结构身份只读合同版本,
            Gread, H, std::nullopt};
    }

    std::pair<存在结构身份只读状态, std::uint64_t>
    读取当前事实代次() const {
    ...
    static const 一致节点项* 查找一致节点(
        const L1所有者范围一致当前读取结果& 读取,
        稳定编码 编码) noexcept {
        const auto it = std::find_if(读取.节点.begin(), 读取.节点.end(),
            [&](const auto& v) noexcept { return v.查询编码 == 编码; });
        return it == 读取.节点.end() ? nullptr : &*it;
    }

    static const 一致源关系组项* 查找一致源关系组(
        const L1所有者范围一致当前读取结果& 读取,
        稳定编码 源节点,
        稳定编码 关系类型) noexcept {
        const auto it = std::find_if(读取.源关系组.begin(), 读取.源关系组.end(),
    ...
    static const L1所有者范围节点事实* 当前节点事实(
        const 一致节点项& 项) noexcept {
    ...
    bool 族归属关系有效(const L1所有者范围关系事实& 关系,
        ...
        return 有效(关系.编码) && 关系.源节点 == 身份
            ...
            && 关系.角色或顺序 == 1 && 关系.写入所有者 == 所有者_
            && 关系.创建事实代次 != 0
            && 关系.退出事实代次 == 要求退出;
    }

    static 存在结构身份只读状态 映射窄读取状态(
    ...
    static 存在类数据状态 映射窄数据状态(
        存在结构身份只读状态 状态) noexcept {
        switch (状态) {
        case 存在结构身份只读状态::已读取:
            return 存在类数据状态::已读取;
        case 存在结构身份只读状态::未找到:
    ...
    std::optional<存在结构身份只读状态> 历史来源读取错误(
        const L1结构所有者历史读取结果& owner,
        const L1所有者范围历史读取结果& e,
        const L1所有者范围历史读取结果& anchor,
        const L1所有者范围历史读取结果& type,
        const L1所有者范围历史关系组读取结果& links,
        std::uint64_t Gread,
        std::uint64_t H,
        稳定编码 身份) const noexcept {
        if (e.读取事实代次 != Gread) return 存在结构身份只读状态::事实代次漂移;
        if (e.状态 != L1所有者范围读取状态::成功)
            return 映射窄读取状态(e.状态);
        const auto* eFact = e.事实
            ? std::get_if<L1所有者范围节点事实>(&*e.事实) : nullptr;
        if (!eFact || e.合同版本 != L1所有者范围CRUD合同版本
            || e.查询编码 != 身份)
            return 存在结构身份只读状态::内部不一致;
        ...
        if (links.合同版本 != L1所有者范围CRUD合同版本
            || links.方向 != L1所有者范围关系端点方向::源
            || links.端点节点 != 身份
            || links.关系类型节点 != 存在族归属关系类型_
            || links.历史截止事实代次 != H)
            return 存在结构身份只读状态::内部不一致;
        if (links.关系组.empty()) return 存在结构身份只读状态::入口拒绝;
    ...
    void 接受存在族首次材料(
        ...
        if (投影.状态 != L1所有者范围一致当前读取状态::成功
            ...
            || 投影.期望事实代次 != 首次.读取事实代次
            || 投影.读取事实代次 != 首次.读取事实代次
            || 投影.所有者.size() != 1 || 投影.节点.size() != 2
            || !投影.关系.empty() || !投影.值.empty()
    ...
    存在类结点结果 提交新增存在(
        ...
        try {
            ...
            if (!结点 || !族关系) return 失败(存在类数据状态::已可能发布,
                写入.事实代次);
    ...
    存在类结点结果 读取历史存在快照(稳定编码 存在结点,
        ...
        if (读取守卫事实代次 == 0) 读取守卫事实代次 = 结果事实代次;
        const auto 身份 = 读取存在身份来源历史见证(
            读取守卫事实代次, 历史截止事实代次, 存在结点);
        if (!身份.成功(读取守卫事实代次,
                历史截止事实代次, 存在结点))
            return 失败(映射窄数据状态(身份.状态), 身份.Gread);
    ...
    bool 删除存在写集闭合(const L1所有者范围写集请求& 写集,
        ...
        if (!找到存在结点 || 退出事实代次 <= 1) return false;
        const auto 当前 = 读取当前事实代次();
        if (当前.first != 存在结构身份只读状态::已读取)
            return false;
        const auto 来源 = 读取存在身份来源历史见证(
            当前.second, 退出事实代次 - 1, 存在结点);
        return 来源.成功(当前.second, 退出事实代次 - 1, 存在结点)
            && 来源.见证
            && std::count(写集.退出事实.begin(), 写集.退出事实.end(),
                来源.见证->族归属关系) == 1;
    }

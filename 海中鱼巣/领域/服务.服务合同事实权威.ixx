...
public:
    ...
    std::optional<L2存在身份> ARCH_建立验证样本_v1(
        ...
        bool 终态干扰 = false, bool 损坏载荷 = false) noexcept {
        std::lock_guard<std::mutex> 锁(互斥体_);
        try {
            ...
            const auto 节点写入 = 写入端口_.提交所有者范围中性写集(
                形成验证节点写集(当前.事实代次, 幂等身份, 合同数, 事件数));
            if ((节点写入.状态 != L1所有者范围写入状态::成功
                    && 节点写入.状态 != L1所有者范围写入状态::精确重复)
                || 节点写入.事实代次 == 0)
                return std::nullopt;
            const auto 自我 = 服务合同事实权威内部::查找映射(
                节点写入.新编码映射, 1U);
            if (!自我) return std::nullopt;
            if (合同数 == 0 && 事件数 == 0) {
                ARCH_验证样本记录_.emplace(幂等身份, ARCH_验证样本记录{
                    合同数, 事件数, 终态干扰, 损坏载荷,
                    当前.事实代次, 节点写入.事实代次, 节点写入.事实代次,
                    节点写入.新编码映射, L2存在身份{*自我}});
                return L2存在身份{*自我};
            }

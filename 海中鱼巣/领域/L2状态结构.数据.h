...
inline constexpr std::uint32_t L2目标状态合同幂等首次完整读取合同版本_v1 = 1;

struct L2目标状态合同幂等首次完整读取请求_v1 final {
    std::uint32_t 合同版本 = L2目标状态合同幂等首次完整读取合同版本_v1;
    L2结构请求头 请求头;
    L2结构幂等身份 幂等身份;
...
// 关系 19 的强类型读回投影。状态本体继续只保存类型、值与时间；
// 本投影只保存可由节点关系表达的四项上下文，不形成第二状态权威。
struct L2中性状态上下文事实_v1 final {
    L2状态身份 状态{};
    L2存在身份 主体存在{};
    L2场景身份 共同场景{};
    L2特征实例身份 特征实例{};
    ...
    稳定编码 来源存在关系稳定编码{};
    L2生命周期 生命周期{};
    friend bool operator==(const L2中性状态上下文事实_v1&,
        const L2中性状态上下文事实_v1&) = default;
};

struct L2中性状态上下文读取请求_v1 final {
    L2结构请求头 请求头{};
    std::uint32_t 专属合同版本 = L2中性状态动态合同版本_v1;
    L2读取类别 读取类别 = L2读取类别::当前;
    L2状态身份 状态{};
    std::uint64_t 历史截止事实代次 = 0;
    friend bool operator==(const L2中性状态上下文读取请求_v1&,
        const L2中性状态上下文读取请求_v1&) = default;
};

struct L2中性状态上下文读取结果_v1 final {
    L2中性结构结果头_v1 结果头{};
    L2读取类别 读取类别 = L2读取类别::当前;
    std::uint64_t 历史截止事实代次 = 0;
    std::optional<L2中性状态上下文事实_v1> 上下文{};
    bool 成功() const noexcept;
    friend bool operator==(const L2中性状态上下文读取结果_v1&,
        const L2中性状态上下文读取结果_v1&) = default;
};
...
inline constexpr std::uint32_t L2特征首值初始状态原子形成合同版本_v1 = 1;

struct L2特征首值初始状态原子形成请求_v1 final {
    std::uint32_t 合同版本 = L2特征首值初始状态原子形成合同版本_v1;
    L2结构请求头 请求头;
...
inline bool L2目标状态合同幂等首次完整读取请求有效_v1(
    const L2目标状态合同幂等首次完整读取请求_v1& 请求) noexcept {
    return 请求.合同版本 == L2目标状态合同幂等首次完整读取合同版本_v1
        && L2结构请求头合同有效(请求.请求头)
        && 请求.请求头.期望事实代次 != 0
        && L2结构幂等身份有效(请求.幂等身份);
}
...
inline bool L2特征首值初始状态原子形成请求有效_v1(
    const L2特征首值初始状态原子形成请求_v1& 请求) noexcept {
    return 请求.合同版本 == L2特征首值初始状态原子形成合同版本_v1
        && L2结构请求头合同有效(请求.请求头)
        && 请求.请求头.期望事实代次 != 0
        && 有效(请求.组合幂等身份)
        && L2结构幂等身份有效(请求.首值幂等身份)
        && L2结构幂等身份有效(请求.首态幂等身份)
        && 请求.首值幂等身份 != 请求.首态幂等身份
        && 有效(请求.主体存在.值) && 有效(请求.特征实例.值)
        && L2状态原始值材料有效(请求.初始值材料)
        && 请求.初始状态UTC纳秒 != 0
        && 请求.初始状态UTC纳秒
...
inline bool L2目标状态合同幂等首次完整读取结果_v1::成功() const noexcept {
    return 合同版本 == L2目标状态合同幂等首次完整读取合同版本_v1
        && 状态 == L2目标状态合同幂等首次完整读取状态_v1::已读取
        && 本次正式读回截止 != 0 && 合同
        && !合同->生命周期.退出事实代次
        && L2目标状态合同事实完整(*合同, 本次正式读回截止);
}
...
inline bool L2特征首值初始状态原子形成结果_v1::成功() const noexcept {
    const bool 成功状态 = 状态 == L2特征首值初始状态原子形成状态_v1::已提交
        || 状态 == L2特征首值初始状态原子形成状态_v1::精确重复;
    if (合同版本 != L2特征首值初始状态原子形成合同版本_v1
        || !成功状态 || 共同事实代次 == 0 || !当前值 || !初始状态
        || !当前状态选择) return false;
    const auto* 时间 = std::get_if<std::int64_t>(
        &初始状态->时间值.类型化不可变材料);
    return 时间 && *时间 > 0
        && L2特征值事实截止投影完整(*当前值, 共同事实代次)
        && 当前值->生命周期.创建事实代次 == 共同事实代次
        && !当前值->生命周期.退出事实代次
        && 初始状态->生命周期.创建事实代次 == 共同事实代次
        && !初始状态->生命周期.退出事实代次
        && 初始状态->精确值.创建事实代次 == 共同事实代次
        && 初始状态->时间值.创建事实代次 == 共同事实代次
        && !初始状态->精确值.退出事实代次
        && !初始状态->时间值.退出事实代次
        && 初始状态->特征实例 == 当前值->特征实例
        && 初始状态->精确值.类型化不可变材料
            == 当前值->类型化不可变材料
        && 初始状态->来源稳定编码 == 当前值->来源稳定编码
        && 初始状态->精确值.来源稳定编码 == 当前值->来源稳定编码
        && 初始状态->时间值.来源稳定编码 == 当前值->来源稳定编码
        && L2状态当前选择关系引用完整(*当前状态选择, 共同事实代次)
        && 当前状态选择->生命周期.创建事实代次 == 共同事实代次
        && !当前状态选择->生命周期.退出事实代次
        && 当前状态选择->特征实例 == 当前值->特征实例
        && 当前状态选择->状态 == 初始状态->身份;
}

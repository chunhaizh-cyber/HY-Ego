...
void 绑定存在数据服务::要求(bool ok, S s) {
  if (!ok) throw s;
}

void 绑定存在数据服务::核验现实见证(
...
bool 绑定存在数据服务::使用存在提供者(const 绑定存在内容参与者 &p) const noexcept {
  return &p == &content_;
}

bool 绑定存在数据服务::使用场景提供者(const 绑定存在场景参与者 &p) const noexcept {
  return &p == &scene_;
}

void 绑定存在数据服务::完整要求(bool ok, W s) {
  if (!ok) throw s;
}

绑定存在数据服务::W 绑定存在数据服务::映射状态(S s) noexcept {
  switch (s) {
...
绑定存在数据服务::W 绑定存在数据服务::映射状态(P s) noexcept {
  switch (s) {
...
绑定存在数据服务::W 绑定存在数据服务::映射状态(C s) noexcept {
  switch (s) {
...
绑定存在数据服务::W 绑定存在数据服务::映射角色(存在单例角色状态 s) noexcept {
  using R = 存在单例角色状态;
  switch (s) {
...
绑定存在创建请求 绑定存在数据服务::旧片段请求(const 已发布概念绑定创建请求 &r) {
...
void 绑定存在数据服务::核验映射(const Maps &maps, K k, bool role) {
...
bool 绑定存在数据服务::投影完整(const 已发布概念绑定投影 &p,
...
已发布概念绑定创建结果 绑定存在数据服务::创建绑定存在并引用概念(
...
已发布概念绑定恢复结果 绑定存在数据服务::读取已发布概念绑定创建(

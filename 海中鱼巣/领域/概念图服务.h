...
struct 概念图清理准备包 {
    节点句柄 目标概念;
    std::uint64_t 基准活动版本 = 0;
    std::uint64_t 新活动版本 = 0;
    关系句柄 生命周期关系;
    std::vector<关系句柄> 专用关系组;
    std::vector<概念活动签名材料> 完整剩余签名组;
    std::vector<概念删除后直接边> 删除后直接边组;
    std::uint64_t 写集身份 = 0;
    struct 专用关系登记材料 {
    ...
    std::vector<专用关系登记材料> 新专用关系登记组;
    std::vector<生命周期关系登记材料> 新生命周期关系登记组;
    活动快照材料 新活动快照;

    bool 完整() const {
        return 句柄有效(目标概念)
            && 基准活动版本 != 0
            && 基准活动版本 != std::numeric_limits<std::uint64_t>::max()
            && 新活动版本 == 基准活动版本 + 1
            && 句柄有效(生命周期关系)
            && !完整剩余签名组.empty()
            && std::none_of(完整剩余签名组.begin(), 完整剩余签名组.end(), [this](const auto& 签名) {
                return !签名.完整() || 签名.概念 == 目标概念;
            })
            && std::all_of(删除后直接边组.begin(), 删除后直接边组.end(), [](const auto& 边) {
                return 边.完整();
            })
            && 写集身份 != 0;
    }
    ...
    bool 活动快照材料完整() const {
        return 完整()
            ...
            && 新活动快照.根组.size() == 4
            && !新活动快照.概念节点组.empty()
            && 新活动快照.签名组.size() == 新活动快照.概念节点组.size()
            && std::find(新活动快照.概念节点组.begin(), 新活动快照.概念节点组.end(), 目标概念)
                == 新活动快照.概念节点组.end()
            && std::none_of(新活动快照.关系组.begin(), 新活动快照.关系组.end(), [this](const auto& 关系) {
                return 关系.源节点 == 目标概念 || 关系.目标节点 == 目标概念;
            })
            && std::none_of(新活动快照.签名组.begin(), 新活动快照.签名组.end(), [this](const auto& 签名) {
                return 签名.概念 == 目标概念 || !签名.完整();
            });
    }

    bool 提交材料完整() const {
        return 活动快照材料完整()
            && std::none_of(新专用关系登记组.begin(), 新专用关系登记组.end(), [this](const auto& 登记) {
                return 登记.源节点 == 目标概念 || 登记.目标节点 == 目标概念;
            })
            && std::none_of(新生命周期关系登记组.begin(), 新生命周期关系登记组.end(), [this](const auto& 登记) {
                return 登记.概念 == 目标概念;
            });
    }
};
...
struct 概念结构写集已补全候选 {
    ...
    bool 完整() const {
        ...
        const auto 写集身份 = 节点包.写集身份;
        return 完备阶段 == 删除候选完备阶段::结构写集已补全
            && 领域影响.完整()
            && 节点包.完整() && 主信息包.完整() && 关系包.完整() && 索引包.完整()
            && 所有者授权.完整() && 概念图包.完整() && 统计缓存包.完整()
            && 节点包.目标 == 目标
            && 主信息包.目标节点 == 目标
            && 关系包.目标 == 目标
            && 索引包.目标 == 目标
...
public:
    ...
    std::optional<概念图清理准备包> 准备概念登记与活动快照清理包(
        const 概念删除影响候选& 候选,
        const 结构事务令牌& 令牌) {
        if (!候选.完整()
            || 令牌.类型 != 结构许可类型::独占
        ...
        return 准备概念登记与活动快照清理包_已加锁(候选, 令牌, nullptr);
    }

    std::optional<概念结构写集已补全候选> 补全概念删除结构写集(
        const 概念删除影响候选& 候选,
        const 节点删除准备包& 节点包,
        const 主信息删除准备包& 主信息包,
        const 节点关系失效准备包& 关系包,
        const 节点主键删除准备包& 索引包,
        const 概念结构所有者清理授权& 所有者授权,
        const 概念缓存清理准备包& 统计缓存包,
        const 结构事务令牌& 令牌) {
        const auto 概念图包 = 准备概念登记与活动快照清理包(候选, 令牌);
        const 概念结构所有者类型 预期所有者 =
            候选.根类别 == 概念根类别::存在 ? 概念结构所有者类型::存在服务
            : 候选.根类别 == 概念根类别::动态 ? 概念结构所有者类型::动态服务
            : 候选.根类别 == 概念根类别::关系 ? 概念结构所有者类型::二次特征服务
            : 候选.根类别 == 概念根类别::因果 ? 概念结构所有者类型::因果服务
            : 概念结构所有者类型::未定义;
        if (!概念图包.has_value()
            || 所有者授权.所有者 != 预期所有者
            || 候选.待补全结构所有者组.size() != 4
            || std::find(候选.待补全结构所有者组.begin(), 候选.待补全结构所有者组.end(),
                概念删除待补全结构所有者::索引反查) == 候选.待补全结构所有者组.end()
            || std::find(候选.待补全结构所有者组.begin(), 候选.待补全结构所有者组.end(),
                概念删除待补全结构所有者::概念图登记) == 候选.待补全结构所有者组.end()
            || std::find(候选.待补全结构所有者组.begin(), 候选.待补全结构所有者组.end(),
                概念删除待补全结构所有者::统计缓存) == 候选.待补全结构所有者组.end()) return std::nullopt;
        概念结构写集已补全候选 结果{
            删除候选完备阶段::结构写集已补全,
            候选,
            节点包,
        ...
        return 结果.完整() ? std::optional<概念结构写集已补全候选>{std::move(结果)} : std::nullopt;
    }
...
private:
    ...
    using 专用关系登记材料 = 概念图清理准备包::专用关系登记材料;
    using 生命周期关系登记材料 = 概念图清理准备包::生命周期关系登记材料;
    using 概念活动快照 = 概念图清理准备包::活动快照材料;

    struct 概念图冻结上下文 {
        概念图冻结上下文(std::shared_mutex& 活动锁, std::mutex& 图锁)
            : 活动锁定(活动锁, std::try_to_lock),
              图锁定(图锁, std::defer_lock) {
            if (活动锁定.owns_lock() && !图锁定.try_lock()) {
            }
        }

        概念图冻结上下文(const 概念图冻结上下文&) = delete;
        概念图冻结上下文& operator=(const 概念图冻结上下文&) = delete;
        概念图冻结上下文(概念图冻结上下文&&) noexcept = default;
        概念图冻结上下文& operator=(概念图冻结上下文&&) noexcept = default;

        bool 已锁定() const { return 活动锁定.owns_lock() && 图锁定.owns_lock(); }

        std::unique_lock<std::shared_mutex> 活动锁定;
        std::unique_lock<std::mutex> 图锁定;
    };

    std::optional<概念图冻结上下文> 尝试冻结概念图() {
        概念图冻结上下文 上下文(活动图锁_, 图写锁_);
        return 上下文.已锁定()
            ? std::optional<概念图冻结上下文>{std::move(上下文)}
            : std::nullopt;
    }

    std::optional<概念图清理准备包> 准备概念登记与活动快照清理包_已加锁(
        const 概念删除影响候选& 候选,
        const 结构事务令牌& 令牌,
        const 概念图冻结上下文* 上下文) const {
        if ((上下文 != nullptr && !上下文->已锁定())
            || !候选.完整()
            || 令牌.类型 != 结构许可类型::独占
            || 令牌.许可序号 == 0) return std::nullopt;
        const auto 节点读回 = 节点_.读取节点(候选.目标概念, 令牌);
        if (!节点读回.has_value()
            || 节点读回->主信息 != 候选.目标主信息
            || 节点读回->类型 != 候选.目标节点记录.类型
            || !活动快照_.has_value()
            || 活动快照_->活动版本 != 候选.基准活动版本
            || 活动快照_->活动版本 == std::numeric_limits<std::uint64_t>::max()
            || std::find(活动快照_->根组.begin(), 活动快照_->根组.end(), 候选.目标概念)
                != 活动快照_->根组.end()) return std::nullopt;

        const auto 生命周期位置 = std::find_if(
            生命周期关系登记组_.begin(), 生命周期关系登记组_.end(),
            [&候选](const auto& 登记) { return 登记.概念 == 候选.目标概念; });
        if (生命周期位置 == 生命周期关系登记组_.end()
            || 生命周期位置->关系 != 候选.目标生命周期.关系
            || 候选.目标生命周期.阶段 != 概念生命周期阶段::退役) return std::nullopt;
        ...
        包.新活动版本 = 候选.基准活动版本 + 1;
        包.生命周期关系 = 生命周期位置->关系;
        包.完整剩余签名组 = 候选.完整剩余签名组;
        包.删除后直接边组 = 候选.删除后直接边组;
        包.写集身份 = 令牌.许可序号;
        包.新专用关系登记组.reserve(专用关系登记组_.size());
        for (const auto& 登记 : 专用关系登记组_) {
            if (登记.源节点 == 候选.目标概念 || 登记.目标节点 == 候选.目标概念) {
                包.专用关系组.push_back(登记.关系);
            } else {
                包.新专用关系登记组.push_back(登记);
            }
        }
        包.新生命周期关系登记组.reserve(生命周期关系登记组_.size());
        for (const auto& 登记 : 生命周期关系登记组_) {
            if (登记.概念 != 候选.目标概念) 包.新生命周期关系登记组.push_back(登记);
        }
        包.新活动快照 = 活动快照_.value();
        包.新活动快照.活动版本 = 包.新活动版本;
        包.新活动快照.概念节点组.erase(
            std::remove(包.新活动快照.概念节点组.begin(), 包.新活动快照.概念节点组.end(), 候选.目标概念),
            包.新活动快照.概念节点组.end());
        包.新活动快照.关系组.erase(
            std::remove_if(包.新活动快照.关系组.begin(), 包.新活动快照.关系组.end(), [&候选](const auto& 关系) {
                return 关系.源节点 == 候选.目标概念 || 关系.目标节点 == 候选.目标概念;
            }),
            包.新活动快照.关系组.end());
        包.新活动快照.签名组 = 候选.完整剩余签名组;
        规范化活动快照(包.新活动快照);
        std::sort(包.专用关系组.begin(), 包.专用关系组.end(), [](const auto& 左, const auto& 右) {
            return 左.关系编号 < 右.关系编号;
        });
        return 包.提交材料完整()
            ? std::optional<概念图清理准备包>{std::move(包)}
            : std::nullopt;
    }
    ...
    void 提交概念登记清理包_已加锁(
        ...
        if (!上下文.已锁定()
            ...
            || 包.写集身份 != 令牌.许可序号
            || 令牌.类型 != 结构许可类型::独占
            || 会话.读取阶段() != 概念安全删除提交阶段::概念登记) {
            throw std::logic_error("概念安全删除登记提交能力不匹配");
        }
        const auto 生命周期位置 = std::find_if(
            生命周期关系登记组_.begin(), 生命周期关系登记组_.end(),
            [&包](const auto& 登记) { return 登记.概念 == 包.目标概念 && 登记.关系 == 包.生命周期关系; });
        if (生命周期位置 == 生命周期关系登记组_.end()) {
            throw std::logic_error("概念安全删除登记提交前生命周期漂移");
        }
        专用关系登记组_.swap(包.新专用关系登记组);
        生命周期关系登记组_.swap(包.新生命周期关系登记组);
    }

    void 提交概念活动快照清理包_已加锁(
        概念图清理准备包& 包,
        const 结构事务令牌& 令牌,
        const 概念安全删除提交会话& 会话,
        const 概念图冻结上下文& 上下文) {
        if (!上下文.已锁定()
            ...
            || 包.写集身份 != 令牌.许可序号
            || 令牌.类型 != 结构许可类型::独占
            || 会话.读取阶段() != 概念安全删除提交阶段::活动快照
            || !活动快照_.has_value()
            || 活动快照_->活动版本 != 包.基准活动版本) {
            throw std::logic_error("概念安全删除活动快照提交能力不匹配");
        }
        using std::swap;
        swap(活动快照_.value(), 包.新活动快照);
        上一候选图_.reset();
    }

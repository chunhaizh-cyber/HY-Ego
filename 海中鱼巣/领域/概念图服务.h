...
public:
    ...
    std::optional<概念图清理准备包> 准备概念登记与活动快照清理包(
        ...
        if (!节点读回.has_value()
            || 节点读回->主信息 != 候选.目标主信息
            || 节点读回->类型 != 候选.目标节点记录.类型) return std::nullopt;

        std::shared_lock<std::shared_mutex> 活动锁(活动图锁_);
        std::lock_guard<std::mutex> 图锁(图写锁_);
        std::shared_lock<std::shared_mutex> 根锁(根登记锁_);
        if (!活动快照_.has_value()
            || 活动快照_->活动版本 != 候选.基准活动版本
            || 活动快照_->活动版本 == std::numeric_limits<std::uint64_t>::max()
            || std::any_of(根登记组_.begin(), 根登记组_.end(), [&候选](const auto& 根) {
                return 根.has_value() && 根->根节点 == 候选.目标概念;
            })) return std::nullopt;
        ...
        return 包.完整() ? std::optional<概念图清理准备包>{std::move(包)} : std::nullopt;
    }

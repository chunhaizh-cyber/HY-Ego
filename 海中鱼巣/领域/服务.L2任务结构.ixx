...
inline constexpr L1所有者范围写入幂等身份 任务后继核心类型登记幂等身份{
    0x4C32'5452'434F'5245ULL};
...
// 后继核心类型使用独立幂等身份登记，禁止改变既有十项类型登记请求。
void 初始化任务后继核心类型登记(
    const L1事实基座服务& 第一层服务,
    L1所有者范围写端口& 写入端口,
    任务结构类型定位& 类型定位) {
    ...
    写集.合同版本 = L1所有者范围CRUD合同版本;
    写集.期望事实代次 = 读取固定登记期望代次(
        第一层服务, 写入端口, 任务后继核心类型登记幂等身份);
    写集.写入幂等身份 = 任务后继核心类型登记幂等身份;
    写集.节点 = {
        {{任务来源需求关系类型键值}, 节点种类::普通, std::nullopt},
        {{任务正式存在关系类型键值}, 节点种类::普通, std::nullopt}};
    ...
    if (写入.合同版本 != L1所有者范围CRUD合同版本
        || 写入.所有者 != 写入端口.所有者身份()
        || 写入.写入幂等身份 != 任务后继核心类型登记幂等身份
        || 写入.事实代次 == 0 || (!首次完整 && !重复完整)
        || 写入.新编码映射.size() != 2)
        throw std::runtime_error("L2 task successor core type registry failed");

    for (const auto& [本地键, 编码] : 写入.新编码映射) {
        if (!有效(编码))
            throw std::runtime_error("L2 task successor core type code invalid");
        if (本地键.值 == 任务来源需求关系类型键值) {
            if (有效(类型定位.任务来源需求关系类型))
                throw std::runtime_error("L2 task source-demand type duplicate");
            类型定位.任务来源需求关系类型 = 编码;
        } else if (本地键.值 == 任务正式存在关系类型键值) {
            if (有效(类型定位.任务正式存在关系类型))
                throw std::runtime_error("L2 task existence type duplicate");
            类型定位.任务正式存在关系类型 = 编码;
        } else {
            throw std::runtime_error("L2 task successor core type key unknown");
        }
    }
    if (!有效(类型定位.任务来源需求关系类型)
        || !有效(类型定位.任务正式存在关系类型)
        || 类型定位.任务来源需求关系类型
            == 类型定位.任务正式存在关系类型)
        throw std::runtime_error("L2 task successor core type mapping incomplete");

    const 稳定编码 编码组[] = {类型定位.任务来源需求关系类型,
        类型定位.任务正式存在关系类型};
    for (const auto 编码 : 编码组) {
        ...
        if (读取.状态 != L1所有者范围读取状态::成功
            ...
            || 节点->退出事实代次)
            throw std::runtime_error(
                "L2 task successor core type readback invalid");
    }
}
...
L2任务读取结果 读取任务当前投影(
    ...
    // 形状：1 所有者、3 节点、0 关系、0 值、4 源关系组、0 目标关系组
    if (读取.所有者.size() != 1 || 读取.节点.size() != 3
        || !读取.关系.empty() || !读取.值.empty()
        || !读取.属性值.empty() || 读取.源关系组.size() != 4
        || !读取.目标关系组.empty())
        return 失败(L2结构状态::内部不一致);
...
L2结构状态 验证任务退出事实闭包当前有效(
    ...
    const auto* 来源需求组项 = 查找唯一任务来源关系组(
        读取, 任务编码, 类型定位.任务来源需求关系类型);
    if (!族归属组项 || !列表项引用组项
        || !虚拟存在归属组项 || !来源需求组项)
        return L2结构状态::内部不一致;
    ...
    for (const auto& 成员 : 来源需求组项->成员) {
        const auto& 关系 = 成员.关系;
        const auto& 需求节点 = 成员.对端节点;
        if (关系.写入所有者 != 来源定位.所有者
            || 关系.退出事实代次 || 关系.源节点 != 任务编码
            || 关系.目标节点 != 需求节点.编码
            || 关系.关系类型节点 != 类型定位.任务来源需求关系类型
            || 关系.角色或顺序 != 1
            || 关系.创建事实代次 < 节点.创建事实代次
            || 关系.创建事实代次 > 读取.读取事实代次
            || !有效(需求节点.编码)
            || 需求节点.种类 != 节点种类::普通
            || 需求节点.属性类型表示 || 需求节点.退出事实代次
            || 需求节点.创建事实代次 > 读取.读取事实代次)
            return L2结构状态::内部不一致;
    }
...
L2任务退出结果 读回退出任务(
    ...
    稳定编码 列表项引用编码,
    稳定编码 虚拟存在归属编码,
    const std::vector<稳定编码>& 来源需求关系编码组,
    std::uint64_t 写入代次) {
    const auto 失败 = [&](L2结构状态 状态) noexcept {
        L2任务退出结果 结果;
        结果.结果头.合同版本 = L2结构合同版本;
    ...
    for (const auto 编码 : 来源需求关系编码组) {
        const auto 来源需求 = 读回关系(编码);
        if (!来源需求 || 来源需求->源节点 != 请求.任务.值
            || 来源需求->关系类型节点 != 类型定位.任务来源需求关系类型
            || 来源需求->角色或顺序 != 1
            || !有效(来源需求->目标节点))
            return 失败(L2结构状态::内部不一致);
    }
...
std::vector<std::uint64_t> 编码正式选择完整请求(
    ...
    const auto& 授权 = 冻结.授权安全材料;
    写.写(static_cast<std::uint64_t>(授权.结论)); 写.写(授权.来源需求组.size());
    for (const auto& 需求 : 授权.来源需求组) 写.写编码(需求.值);
    写.写有符号(授权.最高权限); 写.写(授权.授权版本);
    写.写(授权.安全图版本); 写.写(授权.阈值版本);
    写.写(授权.权限规则版本); 写.写(授权.适用性分类规则版本);
    写.写(授权.强类型来源证据组.size());
    for (const auto& 证据 : 授权.强类型来源证据组) {
        写.写(证据.index());
        std::visit([&](const auto& 身份) { 写.写编码(身份.值); }, 证据);
    }
...
std::optional<L2发布任务筹办正式选择请求> 解码正式选择完整请求(
    ...
    auto& 授权 = 冻结.授权安全材料;
    const auto 结论 = 读.读(); const auto 授权来源数 = 读.读();
    if (!结论 || !授权来源数 || *授权来源数 > 1'000'000) return std::nullopt;
    授权.结论 = static_cast<L2任务授权安全复判结论>(*结论);
    for (std::uint64_t i = 0; i < *授权来源数; ++i) { const auto 身份 = 读.读编码(); if (!身份) return std::nullopt; 授权.来源需求组.emplace_back(*身份); }
    const auto 权限 = 读.读有符号(); const auto 授权版本 = 读.读(); const auto 安全图 = 读.读();
    const auto 阈值 = 读.读(); const auto 权限规则 = 读.读(); const auto 分类规则 = 读.读();
    if (!权限 || !授权版本 || !安全图 || !阈值 || !权限规则 || !分类规则) return std::nullopt;
    授权.最高权限 = *权限; 授权.授权版本 = *授权版本; 授权.安全图版本 = *安全图;
    授权.阈值版本 = *阈值; 授权.权限规则版本 = static_cast<std::uint32_t>(*权限规则);
    授权.适用性分类规则版本 = static_cast<std::uint32_t>(*分类规则);
    const auto 授权证据数 = 读.读(); if (!授权证据数 || *授权证据数 > 1'000'000) return std::nullopt;
    for (std::uint64_t i = 0; i < *授权证据数; ++i) {
        const auto 分支 = 读.读(); const auto 身份 = 读.读编码(); if (!分支 || !身份) return std::nullopt;
        switch (*分支) {
        case 0: 授权.强类型来源证据组.emplace_back(L2来源需求授权事实身份{*身份}); break;
        case 1: 授权.强类型来源证据组.emplace_back(L2安全图事实身份{*身份}); break;
        case 2: 授权.强类型来源证据组.emplace_back(L2安全阈值事实身份{*身份}); break;
        case 3: 授权.强类型来源证据组.emplace_back(L2权限规则事实身份{*身份}); break;
        case 4: 授权.强类型来源证据组.emplace_back(L2适用性分类规则事实身份{*身份}); break;
...
std::optional<初次筹办融合写入编码映射> 查找初次筹办融合写入编码映射(
    ...
    if (写入.新编码映射.size() != 13
        && 写入.新编码映射.size() != 21
        && 写入.新编码映射.size() != 22) return std::nullopt;
    初次筹办融合写入编码映射 映射;
    std::uint32_t 已找到 = 0;
    ...
    for (const auto& [本地键, 编码] : 写入.新编码映射) {
        ...
        else if (键 == 初次筹办任务来源需求关系写入键) 合法 = 设置(映射.任务来源需求关系, 编码);
...
std::optional<std::vector<std::uint64_t>> 编码初次筹办完整请求(
    ...
    return std::vector<std::uint64_t>{材料.外层合同版本,
        ...
        材料.需求.值.值, 材料.任务建立幂等身份.值};
}

std::optional<L2任务初次筹办准备完整请求材料> 解码初次筹办完整请求(
    const L1所有者范围原始值材料& 原始) noexcept {
    const auto* 值 = std::get_if<std::vector<std::uint64_t>>(&原始);
    if (!值 || 值->size() != 8) return std::nullopt;
    L2任务初次筹办准备完整请求材料 材料;
    ...
    材料.任务建立幂等身份 = L2结构幂等身份{(*值)[7]};
    return L2任务初次筹办准备完整请求材料完整(材料)
        ? std::optional{材料} : std::nullopt;
}
...
L2新增任务与首次初次筹办准备记录结果 读回初次筹办融合首次结果(
    ...
    const auto& 来源需求 = 关系[3];
    const auto& 记录族 = 关系[4];
    const auto& 记录任务 = 关系[5];
    const auto& 记录虚拟 = 关系[6];
    const auto& 记录列表项 = 关系[7];
    if (任务族.源节点 != 映射.任务节点
        || 任务族.目标节点 != 来源.任务锚点
...
按初次筹办准备幂等身份读取首次记录投影(
    ...
    const auto 映射 = 查找初次筹办融合写入编码映射(*首次.首次写入结果);
    if (!映射) {
...
L2按任务读取筹办轮次分区结果 读取筹办轮次分区投影(
    ...
    std::optional<L2任务初次筹办准备完整请求材料> 完整请求;
    if (值读取.状态 == L1所有者范围读取状态::成功
        && 值读取.历史截止事实代次 == 截止
        && 值读取.读取事实代次 == 观察代次) {
        for (const auto& 值 : 值读取.属性值组) {
    ...
    const auto 首次读取 = 按初次筹办准备幂等身份读取首次记录投影(
        第一层服务, 写入端口, 来源, 类型, 初次定位, 轮次定位,
        {{L2结构合同版本, 观察代次}, 完整请求->准备幂等身份},
        观察代次);
...
public:
    ...
    L2按任务读取来源需求关系组结果 按任务读取来源需求关系组(
        ...
        const auto 失败 = [&](L2结构状态 状态) noexcept {
            ...
            结果.合同版本 = L2按任务读取来源需求关系组合同版本;
            return 结果;
        };
        if (!L2按任务读取来源需求关系组请求有效(请求))
            return 失败(L2结构状态::入口拒绝);

        try {
            const auto G0 = 请求.请求头.期望事实代次;
            const auto 读前代次 = L2任务结构内部::读取中性当前事实代次(L1_);
            if (读前代次 == 0)
                return 失败(L2结构状态::内部不一致);
            if (读前代次 != G0)
                return 失败(L2结构状态::事实代次漂移);
            ...
            L1请求.源关系组 = {
                ...
                {请求.任务.值, 类型定位_.任务来源需求关系类型}};
            const auto 读取 = L1_.尝试读取所有者范围一致当前投影(L1请求);
            const auto 顶层 = L2任务结构内部::解释任务身份来源一致读取顶层(
                读取, G0);
            if (!顶层.成功()) return 失败(顶层.状态);

            if (读取.所有者.size() != 1 || 读取.节点.size() != 4
                || !读取.关系.empty() || !读取.值.empty()
                || !读取.属性值.empty() || 读取.源关系组.size() != 2
                || !读取.目标关系组.empty())
                return 失败(L2结构状态::内部不一致);

            const auto* 任务项 = L2任务结构内部::查找唯一任务节点项(
                读取, 请求.任务.值);
            const auto* 锚点项 = L2任务结构内部::查找唯一任务节点项(
                读取, 身份来源定位_.任务锚点);
            const auto* 族类型项 = L2任务结构内部::查找唯一任务节点项(
                读取, 身份来源定位_.任务族归属关系类型);
            const auto* 来源类型项 = L2任务结构内部::查找唯一任务节点项(
                读取, 类型定位_.任务来源需求关系类型);
            const auto* 族关系组 = L2任务结构内部::查找唯一任务来源关系组(
                读取, 请求.任务.值, 身份来源定位_.任务族归属关系类型);
            const auto* 来源需求组 = L2任务结构内部::查找唯一任务来源关系组(
                读取, 请求.任务.值, 类型定位_.任务来源需求关系类型);
            if (!任务项 || !锚点项 || !族类型项 || !来源类型项
                || !族关系组 || !来源需求组)
                return 失败(L2结构状态::内部不一致);

            if (任务项->状态 != L1所有者范围一致当前读取项目状态::成功
                || !任务项->事实) {
                const auto 状态 = L2任务结构内部::映射任务身份来源当前项目状态(
                    任务项->状态);
                return 失败(状态 == L2结构状态::已读取
                    ? L2结构状态::内部不一致 : 状态);
            }
            const auto& 任务节点 = *任务项->事实;
            if (任务节点.写入所有者 != 身份来源定位_.所有者
                || 任务节点.种类 != 节点种类::普通
                || 任务节点.属性类型表示 || 任务节点.退出事实代次)
                return 失败(L2结构状态::引用冲突);

            const auto 内部节点完整 = [&](const auto* 项,
                稳定编码 期望编码) noexcept {
                return 项->状态 == L1所有者范围一致当前读取项目状态::成功
                    && 项->事实 && 项->事实->编码 == 期望编码
                    && 项->事实->写入所有者 == 身份来源定位_.所有者
                    && 项->事实->种类 == 节点种类::普通
                    && !项->事实->属性类型表示
                    && !项->事实->退出事实代次
                    && 项->事实->当前属性.empty()
            ...
            if (族关系组->成员.size() != 1)
                return 失败(L2结构状态::引用冲突);
            const auto& 族关系 = 族关系组->成员.front().关系;
            if (族关系.写入所有者 != 身份来源定位_.所有者
                || 族关系.源节点 != 请求.任务.值
                || 族关系.目标节点 != 身份来源定位_.任务锚点
                || 族关系.关系类型节点
                    != 身份来源定位_.任务族归属关系类型
                || 族关系.角色或顺序 != 1 || 族关系.退出事实代次
                || 族关系.创建事实代次 != 任务节点.创建事实代次)
                return 失败(L2结构状态::引用冲突);

            std::vector<L2任务来源需求关系事实> 关系组;
            关系组.reserve(来源需求组->成员.size());
            for (const auto& 成员 : 来源需求组->成员) {
                const auto& 关系 = 成员.关系;
                const auto& 需求节点 = 成员.对端节点;
                if (关系.写入所有者 != 身份来源定位_.所有者
                    || !有效(关系.编码) || 关系.源节点 != 请求.任务.值
                    || 关系.目标节点 != 需求节点.编码
                    || 关系.关系类型节点 != 类型定位_.任务来源需求关系类型
                    || 关系.角色或顺序 != 1 || 关系.退出事实代次
                    || 关系.创建事实代次 < 任务节点.创建事实代次
                    || 关系.创建事实代次 > G0
                    || !有效(需求节点.编码)
                    || 需求节点.种类 != 节点种类::普通
                    || 需求节点.属性类型表示 || 需求节点.退出事实代次
                ...
                const auto 需求读取 = 需求结构服务_.读取需求身份(
                    {L2结构合同版本, L2需求身份{关系.目标节点}, std::nullopt});
                if (需求读取.状态 == L2需求身份读取状态::许可拒绝)
                    return 失败(L2结构状态::许可拒绝);
                if (需求读取.状态 == L2需求身份读取状态::事实代次漂移
                    || (需求读取.成功() && 需求读取.事实截止代次 != G0))
                    return 失败(L2结构状态::事实代次漂移);
                if (需求读取.状态 == L2需求身份读取状态::资源失败)
                    return 失败(L2结构状态::资源失败);
                if (!需求读取.成功()
                    || 需求读取.需求.值 != 关系.目标节点)
                    return 失败(需求读取.状态
            ...
            std::sort(关系组.begin(), 关系组.end(),
                [](const auto& 左, const auto& 右) noexcept {
                    if (左.来源需求.值 != 右.来源需求.值)
                        return 左.来源需求.值 < 右.来源需求.值;
                    return 左.关系稳定编码 < 右.关系稳定编码;
                });
            for (std::size_t i = 0; i < 关系组.size(); ++i)
                for (std::size_t j = 0; j < i; ++j)
                    if (关系组[i].关系稳定编码 == 关系组[j].关系稳定编码)
                        return 失败(L2结构状态::引用冲突);
    ...
    L2任务退出结果 退出任务(
        ...
        try {
            ...
            if (!来源需求组)
                return 失败(L2结构状态::内部不一致,
                    请求.请求头.期望事实代次);
            std::vector<稳定编码> 来源需求关系编码组;
            来源需求关系编码组.reserve(来源需求组->成员.size());
            for (const auto& 成员 : 来源需求组->成员) {
                const auto 需求读取 = 需求结构服务_.读取需求身份(
                    {L2结构合同版本,
                        L2需求身份{成员.关系.目标节点}, std::nullopt});
                if (需求读取.状态 == L2需求身份读取状态::许可拒绝)
                    return 失败(L2结构状态::许可拒绝);
                if (需求读取.状态 == L2需求身份读取状态::事实代次漂移
                    || (需求读取.成功() && 需求读取.事实截止代次
                ...
                if (需求读取.状态 == L2需求身份读取状态::资源失败)
                    return 失败(L2结构状态::资源失败);
                if (!需求读取.成功()
                    || 需求读取.需求.值 != 成员.关系.目标节点)
                    return 失败(需求读取.状态
                            == L2需求身份读取状态::内部不一致
                        ? L2结构状态::内部不一致
                        : L2结构状态::引用冲突);
                来源需求关系编码组.push_back(成员.关系.编码);
            }
            std::sort(来源需求关系编码组.begin(), 来源需求关系编码组.end());
            if (std::adjacent_find(来源需求关系编码组.begin(),
                    来源需求关系编码组.end())
                != 来源需求关系编码组.end())
                return 失败(L2结构状态::引用冲突,
                    请求.请求头.期望事实代次);
...
public:
    ...
    L2发布任务筹办正式选择结果 发布任务筹办正式选择(
        ...
        try {
            if (!L2发布任务筹办正式选择请求有效(请求))
                return 失败(L2结构状态::入口拒绝);
            if (请求.来源需求组.size() > 1024
                || 请求.稳定证据组.size() > 1024)
                return 失败(L2结构状态::数量预算不足);
            const auto 来源幂等身份 =
                L2取得任务筹办来源幂等身份(请求.来源材料);
            const auto 首次账 = L2任务结构内部::定位任务筹办来源首次账(
                第一层写入端口_, 来源幂等身份);
            if (首次账.状态 != L2结构状态::未找到
                && 首次账.状态 != L2结构状态::已读取)
                return 失败(首次账.状态);
            const auto 当前代次 = L2任务结构内部::读取中性当前事实代次(L1_);
            if (当前代次 == 0) return 失败(L2结构状态::内部不一致);
            if (当前代次 != 请求.请求头.期望事实代次) {
                if (首次账.状态 != L2结构状态::已读取
                    || !首次账.首次请求 || !首次账.首次规范化写集
                    || !首次账.首次写入结果)
                    return 失败(L2结构状态::事实代次漂移, 当前代次);
                if (*首次账.首次请求 != 请求)
                    return 失败(L2结构状态::幂等冲突, 当前代次);
                ...
                if (!选择编码) return 失败(L2结构状态::内部不一致, 当前代次);
                const auto 读回 = 读取正式选择当前投影(
                    L2任务方法选择记录身份{*选择编码}, 当前代次);
                if (!读回.成功()) return 失败(读回.结果头.状态, 当前代次);
                L2发布任务筹办正式选择结果 结果;
                结果.结果头 = {L2结构合同版本, L2结构状态::精确重复,
                    当前代次, 首次账.首次写入结果->事实代次};
                结果.正式选择 = 读回.正式选择;
                结果.冻结材料 = 读回.冻结材料;
                结果.实例方法 = 读回.实例方法;
                return 结果;
            ...
            if (首次账.状态 == L2结构状态::已读取)
                return 失败(首次账.首次请求 && *首次账.首次请求 == 请求
                    ? L2结构状态::事实代次漂移 : L2结构状态::幂等冲突,
                    当前代次);
            const auto G = 当前代次;
            const auto 来源任务 = L2取得任务筹办任务(请求.来源材料);
            const auto 新筹办轮次 = L2取得任务筹办轮次(请求.来源材料);
            const auto 身份来源 = L2任务结构内部::读取任务身份来源当前投影(
                L1_, 身份来源定位_, {{L2结构合同版本, G},
                    L2读取类别::当前, 来源任务, 0}, G);
            if (!身份来源.成功() || !身份来源.身份来源
                || *身份来源.身份来源 != 请求.任务身份来源)
                return 失败(L2结构状态::引用冲突, G);
            ...
            if (!任务.成功() || !任务.任务)
                return 失败(L2结构状态::引用冲突, G);
            const auto* 初次来源 = std::get_if<
                L2任务初次筹办规范化来源材料>(&请求.来源材料);
            const auto* 重筹办来源 = std::get_if<
                L2任务重筹办规范化来源材料>(&请求.来源材料);
            std::optional<L2任务方法路径身份> 旧路径身份;
            std::optional<L2实例方法身份> 旧实例身份;
            std::uint64_t 新执行轮次 = 1;
            if (初次来源) {
                const auto 节点读取 = L1_.读取所有者范围历史事实({
                    L1所有者范围CRUD合同版本, 初次来源->首次准备记录.值});
                const auto* 节点 = 节点读取.事实
                    ? std::get_if<L1所有者范围节点事实>(&*节点读取.事实)
                    : nullptr;
                if (节点读取.状态 != L1所有者范围读取状态::成功 || !节点
                    || 节点->写入所有者 != 身份来源定位_.所有者
                    || 节点->创建事实代次 > G
                    || (节点->退出事实代次 && *节点->退出事实代次 <= G))
                    return 失败(L2结构状态::引用冲突, G);
                const auto 值读取 = L1_.读取所有者范围历史属性值组({
                    L1所有者范围CRUD合同版本,
                    初次来源->首次准备记录.值, G});
                ...
                if (值读取.状态 == L1所有者范围读取状态::成功
                    && 值读取.历史截止事实代次 == G) {
                    for (const auto& 值项 : 值读取.属性值组) {
                        if (值项.属性类型节点
                            == 初次筹办记录定位_.完整请求属性类型) {
                            if (完整请求) return 失败(L2结构状态::内部不一致, G);
                            完整请求 = L2任务结构内部::解码初次筹办完整请求(
                                值项.材料);
                        }
                    }
                }
                if (!完整请求) return 失败(L2结构状态::内部不一致, G);
                const auto 首次读取 = L2任务结构内部::
                    按初次筹办准备幂等身份读取首次记录投影(
                        L1_, 第一层写入端口_, 身份来源定位_, 类型定位_,
                        初次筹办记录定位_, 筹办轮次定位_,
                        {{L2结构合同版本, G}, 完整请求->准备幂等身份}, G);
                if (!首次读取.成功() || !首次读取.首次准备记录
                    || !首次读取.首次任务 || !首次读取.首次任务身份来源)
                    return 失败(L2结构状态::引用冲突, G);
                const auto& 首次 = *首次读取.首次准备记录;
                if (首次.身份 != 初次来源->首次准备记录
                    || 首次.任务 != 初次来源->任务
                    || 首次.首次需求裁决.需求事实.身份 != 初次来源->来源需求
                    || 首次.需求列表项 != 初次来源->需求列表项
                    || 首次.首次需求裁决.首次共同事实截止G0
                        != 初次来源->首次目标比较共同事实截止G0
                    || 首次.首次任务建立代次G1 != 初次来源->任务建立事实代次G1
                    || 首次.筹办轮次 != 初次来源->筹办轮次
                    || 首次.完整请求.运行代次 != 初次来源->运行代次
                    || 首次读取.首次任务->身份 != 来源任务
                    || 首次读取.首次任务->需求列表项 != 初次来源->需求列表项
                    || 首次读取.首次任务->任务虚拟存在 != 首次.任务虚拟存在
                    || *首次读取.首次任务身份来源 != 请求.任务身份来源
                    || 请求.稳定证据组.size() != 1
                    || 请求.稳定证据组.front().类别
                        != L2任务选择稳定证据类别::初次筹办准备记录
                    || 请求.稳定证据组.front().身份 != 初次来源->首次准备记录.值
                    || !std::holds_alternative<L2任务初次筹办准备记录身份>(
                        请求.冻结材料.归因证据)
                    || std::get<L2任务初次筹办准备记录身份>(
                        请求.冻结材料.归因证据) != 初次来源->首次准备记录)
                    return 失败(L2结构状态::引用冲突, G);
            } else if (重筹办来源) {
                const auto 旧路径 = L2任务结构内部::读取任务方法路径当前投影(
                    L1_, 身份来源定位_, 类型定位_,
                    {{L2结构合同版本, G}, L2读取类别::当前,
                        重筹办来源->旧路径, 0}, G, &正式选择定位_);
                const auto 旧实例 = L2任务结构内部::读取实例方法当前投影(
                    L1_, 身份来源定位_, 类型定位_, 重筹办来源->旧实例, G,
                    &正式选择定位_);
                const auto 旧结果 = L2任务结构内部::读取任务实际结果当前投影(
                    L1_, 身份来源定位_, 类型定位_, 重筹办来源->旧实际结果, G);
                const auto 治理 = L2任务结构内部::读取任务治理状态历史投影(
                    L1_, 身份来源定位_, 类型定位_,
                    重筹办来源->正式待重筹办状态, G, G);
                if (!旧路径.成功() || !旧路径.路径 || !旧实例 || !旧结果 || !治理
                    || 旧路径.路径->任务 != 重筹办来源->任务
                    || 旧路径.路径->身份 != 重筹办来源->旧路径
                    || 旧路径.路径->筹办轮次 != 重筹办来源->旧筹办轮次
                    || 旧实例->任务 != 重筹办来源->任务
                    || 旧实例->身份 != 重筹办来源->旧实例
                    || 旧实例->执行轮次 != 重筹办来源->旧执行轮次
                    || 旧结果->任务 != 重筹办来源->任务
                    || 旧结果->身份 != 重筹办来源->旧实际结果
                    || 治理->状态.任务 != 重筹办来源->任务
                    || 治理->状态.身份 != 重筹办来源->正式待重筹办状态
                    || 治理->证据.身份 != 重筹办来源->正式目标裁决证据
                    || 请求.稳定证据组.size() != 2
                    || 请求.稳定证据组[0].类别
                        != L2任务选择稳定证据类别::目标裁决证据
                    || 请求.稳定证据组[0].身份
                    ...
                    || 请求.稳定证据组[1].身份
                        != 重筹办来源->正式待重筹办状态.值
                    || !std::holds_alternative<L2任务目标裁决证据身份>(
                        请求.冻结材料.归因证据)
                    || std::get<L2任务目标裁决证据身份>(
                        请求.冻结材料.归因证据)
                        != 重筹办来源->正式目标裁决证据)
                    return 失败(L2结构状态::引用冲突, G);
                旧路径身份 = 重筹办来源->旧路径;
                旧实例身份 = 重筹办来源->旧实例;
                新执行轮次 = 重筹办来源->旧执行轮次 + 1;
            } else return 失败(L2结构状态::入口拒绝, G);
            const auto 成员读取 = 需求结构服务_.读取需求列表成员({
                {L2结构合同版本, G}, L2读取类别::当前,
                任务.任务->需求列表项, 0});
            if (!成员读取.成功()
                || 成员读取.结果头.事实截止代次 != G)
                return 失败(L2结构状态::引用冲突, G);
            std::vector<L2需求身份> 实际来源;
            for (const auto& 成员 : 成员读取.成员关系组)
                实际来源.push_back(成员.需求);
            std::sort(实际来源.begin(), 实际来源.end(), [](const auto& 左, const auto& 右) {
                return 左.值.值 < 右.值.值;
            });
            if (!std::includes(实际来源.begin(), 实际来源.end(),
                    请求.来源需求组.begin(), 请求.来源需求组.end(),
                    [](const auto& 左, const auto& 右) noexcept {
                        return 左.值.值 < 右.值.值;
                    }))
                return 失败(L2结构状态::引用冲突, G);
            if (初次来源 && std::find(实际来源.begin(), 实际来源.end(),
                    初次来源->来源需求) == 实际来源.end())
                return 失败(L2结构状态::引用冲突, G);

            L1所有者范围一致当前读取请求 当前关系请求;
            当前关系请求.合同版本 = L1所有者范围一致当前读取合同版本;
            当前关系请求.期望事实代次 = G;
            当前关系请求.所有者 = {身份来源定位_.所有者};
            当前关系请求.源关系组 = {
                {来源任务.值, 类型定位_.当前选中路径关系类型},
                {来源任务.值, 类型定位_.实例方法当前关系类型},
                {来源任务.值,
                    正式选择定位_.任务当前正式选择关系类型}};
            const auto 当前关系读取 = L1_.尝试读取所有者范围一致当前投影(
                当前关系请求);
            if (当前关系读取.状态
                    != L1所有者范围一致当前读取状态::成功
                || 当前关系读取.读取事实代次 != G
                || 当前关系读取.源关系组.size() != 3)
                return 失败(L2结构状态::内部不一致, G);
            const L1所有者范围一致源关系组读取结果项* 当前路径组 = nullptr;
            const L1所有者范围一致源关系组读取结果项* 当前实例组 = nullptr;
            const L1所有者范围一致源关系组读取结果项* 当前选择组 = nullptr;
            for (const auto& 组 : 当前关系读取.源关系组) {
                if (组.关系类型节点 == 类型定位_.当前选中路径关系类型) 当前路径组 = &组;
                else if (组.关系类型节点 == 类型定位_.实例方法当前关系类型) 当前实例组 = &组;
                else if (组.关系类型节点 == 正式选择定位_.任务当前正式选择关系类型) 当前选择组 = &组;
            }
            if (!当前路径组 || !当前实例组 || !当前选择组)
                return 失败(L2结构状态::内部不一致, G);
            const bool 当前关系符合 = 初次来源
                ? 当前路径组->成员.empty() && 当前实例组->成员.empty()
                    && 当前选择组->成员.empty()
                : 当前路径组->成员.size() == 1
                    && 当前实例组->成员.size() == 1
                    && 当前选择组->成员.size() == 1
                    && 当前路径组->成员.front().关系.目标节点
                        == 旧路径身份->值
                    && 当前实例组->成员.front().关系.目标节点
                        == 旧实例身份->值;
            if (!当前关系符合)
                return 失败(L2结构状态::引用冲突, G);
            ...
            写集.合同版本 = L1所有者范围CRUD合同版本;
            写集.期望事实代次 = G;
            写集.写入幂等身份 = 首次账.写入幂等身份;
            写集.节点 = {
                {选择键, 节点种类::普通, std::nullopt},
                {冻结键, 节点种类::普通, std::nullopt},
                {路径键, 节点种类::普通, std::nullopt},
                {实例键, 节点种类::普通, std::nullopt}};
            写集.关系 = {
                {键(L2任务结构内部::选择记录族归属关系写入本地键值),
                    选择键, 正式选择定位_.选择记录锚点,
                    正式选择定位_.选择记录族归属关系类型, 1},
                {键(L2任务结构内部::冻结材料族归属关系写入本地键值),
                    冻结键, 正式选择定位_.冻结材料锚点,
                    正式选择定位_.冻结材料族归属关系类型, 1},
                {键(L2任务结构内部::任务当前正式选择关系写入本地键值),
                    来源任务.值, 选择键,
                    正式选择定位_.任务当前正式选择关系类型, 1},
                {键(L2任务结构内部::选择记录方法引用关系写入本地键值),
                    选择键, 请求.冻结材料.方法.值,
                    正式选择定位_.选择记录方法引用关系类型, 1},
                {键(L2任务结构内部::选择记录路径引用关系写入本地键值),
                    选择键, 路径键,
                    正式选择定位_.选择记录路径引用关系类型, 1},
                {键(L2任务结构内部::选择记录冻结材料关系写入本地键值),
                    选择键, 冻结键,
                    正式选择定位_.选择记录冻结材料关系类型, 1},
                {键(L2任务结构内部::路径族归属关系写入本地键值),
                    路径键, 身份来源定位_.路径族归属关系,
                    类型定位_.路径族归属关系类型, 1},
                {键(L2任务结构内部::路径任务引用关系写入本地键值),
                    路径键, 来源任务.值,
                    类型定位_.路径任务引用关系类型, 1},
                {键(L2任务结构内部::当前选中路径关系写入本地键值),
                    来源任务.值, 路径键,
                    类型定位_.当前选中路径关系类型, 1},
                {键(L2任务结构内部::实例方法族归属关系写入本地键值),
                    实例键, 身份来源定位_.实例方法族归属关系,
                    类型定位_.实例方法族归属关系类型, 1},
                {键(L2任务结构内部::实例方法任务引用关系写入本地键值),
                    实例键, 来源任务.值,
                    类型定位_.实例方法任务引用关系类型, 1},
                {键(L2任务结构内部::实例方法路径引用关系写入本地键值),
                    实例键, 路径键,
                    类型定位_.实例方法路径引用关系类型, 1},
                {键(L2任务结构内部::实例方法当前关系写入本地键值),
                    来源任务.值, 实例键,
                    类型定位_.实例方法当前关系类型, 1}};
            for (std::size_t 索引 = 0; 索引 < 请求.来源需求组.size(); ++索引)
                写集.关系.push_back({键(
                    L2任务结构内部::选择记录来源需求关系写入基值
                        + static_cast<std::uint32_t>(索引)),
                    选择键, 请求.来源需求组[索引].值,
                    正式选择定位_.选择记录来源需求关系类型,
                    static_cast<std::int64_t>(索引 + 1)});
            for (std::size_t 索引 = 0; 索引 < 请求.稳定证据组.size(); ++索引)
                写集.关系.push_back({键(
                    L2任务结构内部::选择记录稳定证据关系写入基值
                        + static_cast<std::uint32_t>(索引)),
                    选择键, 请求.稳定证据组[索引].身份,
                    正式选择定位_.选择记录稳定证据关系类型,
                    static_cast<std::int64_t>(请求.稳定证据组[索引].类别)});
            写集.值 = {
                {键(L2任务结构内部::正式选择完整请求值写入本地键值),
                    选择键, 正式选择定位_.完整请求属性类型,
                    L2任务结构内部::编码正式选择完整请求(请求), 选择键},
                {键(L2任务结构内部::路径筹办轮次值写入本地键值),
                    路径键, 身份来源定位_.路径筹办轮次属性类型,
                    static_cast<std::int64_t>(新筹办轮次), 路径键},
                {键(L2任务结构内部::路径来源方法值写入本地键值),
                    路径键, 身份来源定位_.路径来源方法属性类型,
                    static_cast<std::int64_t>(请求.冻结材料.方法.值.值), 路径键},
                {键(L2任务结构内部::路径来源方法内容版本值写入本地键值),
                    路径键, 身份来源定位_.路径来源方法内容版本属性类型,
                    static_cast<std::int64_t>(请求.冻结材料.方法内容版本.值), 路径键},
                {键(L2任务结构内部::路径动作入口值写入本地键值),
                    路径键, 身份来源定位_.路径动作入口属性类型,
                    L1所有者范围独立材料引用{
                        请求.冻结材料.动作入口.身份.值}, 路径键},
                {键(L2任务结构内部::路径预期结果值写入本地键值),
                    路径键, 身份来源定位_.路径预期结果属性类型,
                    L1所有者范围独立材料引用{
                        请求.冻结材料.主轴结果.来源稳定编码}, 路径键},
                {键(L2任务结构内部::路径验证合同值写入本地键值),
                    路径键, 身份来源定位_.路径验证合同属性类型,
                    L1所有者范围独立材料引用{
                        请求.冻结材料.验证合同.值}, 路径键},
                {键(L2任务结构内部::实例方法执行轮次属性类型本地键值),
                    实例键, 身份来源定位_.实例方法执行轮次属性类型,
                    static_cast<std::int64_t>(新执行轮次),
                    实例键}};
            写集.属性槽变更 = {
                {选择键, 正式选择定位_.完整请求属性类型,
                    键(L2任务结构内部::正式选择完整请求值写入本地键值)},
            ...
            if (重筹办来源) {
                写集.退出事实 = {
                    当前路径组->成员.front().关系.编码,
                    当前实例组->成员.front().关系.编码,
                    重筹办来源->旧路径.值,
                    重筹办来源->旧实例.值};
            }
            std::optional<L2任务方法选择记录身份> 旧当前选择;
            if (!当前选择组->成员.empty()) {
                写集.退出事实.push_back(当前选择组->成员.front().关系.编码);
                旧当前选择 = L2任务方法选择记录身份{
                    当前选择组->成员.front().关系.目标节点};
            }
            const auto 写入 = 第一层写入端口_.提交所有者范围中性写集(写集);
            const auto 写入状态 = L2任务结构内部::映射任务写入状态(写入.状态);
            if (写入状态 != L2结构状态::已提交
                && 写入状态 != L2结构状态::精确重复)
                return 失败(写入状态, 写入.事实代次);
            const auto 选择编码 = L2任务结构内部::查找治理写入编码(
                写入, L2任务结构内部::正式选择记录节点写入本地键值);
            if (!选择编码) return 失败(L2结构状态::内部不一致, 写入.事实代次);
            const auto 读回 = 读取正式选择当前投影(
                L2任务方法选择记录身份{*选择编码}, 写入.事实代次);
            if (!读回.成功()) return 失败(读回.结果头.状态, 写入.事实代次);
            L2发布任务筹办正式选择结果 结果;
            结果.结果头 = {L2结构合同版本, 写入状态,
                写入.事实代次, 写入.事实代次};
            结果.正式选择 = 读回.正式选择;
            结果.冻结材料 = 读回.冻结材料;
            结果.实例方法 = 读回.实例方法;
    ...
    L2提交目标未达成待重筹办迁移结果 提交目标未达成待重筹办迁移(
        ...
        try {
                    ...
                    if (请求.轮次分区 ==
                        ...
                        const auto 前态 =
                            L2任务结构内部::读取任务治理状态历史投影(
                                ...
                                请求.前一治理状态,
                                请求.请求头.期望事实代次, 当前代次);
                        if (前态 && 前态->状态.任务 == 请求.任务) {
                            const auto 已消费组 = L1_.读取所有者范围历史关系组({
                                L1所有者范围CRUD合同版本,
                                L1所有者范围关系端点方向::目标,
                                前态->状态.来源任务实际结果.值,
                                筹办轮次定位_.关系类型[12], 当前代次});
                            if (已消费组.状态 == L1所有者范围读取状态::成功
                                && 已消费组.关系组.size() == 1) {
                                L2提交目标未达成待重筹办迁移结果 已消费;
                                已消费.结果头 = {L2结构合同版本,
                                    L2结构状态::引用冲突, 当前代次, std::nullopt};
                                已消费.轮次分区 =
                                    L2任务筹办轮次分区::统一轮次权威;
                                已消费.后继准备记录 =
                                    L2任务结构内部::读取后继筹办准备历史投影(
            ...
            const auto 分区 = L2任务结构内部::读取筹办轮次分区投影(
                ...
                {{L2结构合同版本, Gs}, L2读取类别::当前,
                    请求.任务, 0}, Gs, Gs);
            if (分区.结果头.状态 == L2结构状态::内部不一致
                || 分区.分区 == L2任务筹办轮次分区::内部不一致)
                return 失败(L2结构状态::内部不一致, Gs);
            if (分区.分区 != 请求.轮次分区)
                return 失败(L2结构状态::引用冲突, Gs);
            std::optional<稳定编码> 当前筹办关系编码;
            if (请求.轮次分区 == L2任务筹办轮次分区::统一轮次权威) {
                if (!分区.统一当前准备 || !请求.统一轮次推进)
                    return 失败(L2结构状态::引用冲突, Gs);
                const auto& 当前准备 = *分区.统一当前准备;
                const auto& 推进 = *请求.统一轮次推进;
                const auto 当前后继身份 = 当前准备.当前后继准备记录
                    ? std::optional<L2任务后继筹办准备记录身份>{
                        当前准备.当前后继准备记录->身份}
                    : std::nullopt;
                if (推进.轮次权威 != 当前准备.轮次权威.身份
                    || 推进.首次准备记录 != 当前准备.首次准备记录.身份
                    || 推进.期望前一后继准备记录 != 当前后继身份
                    || 推进.期望前一筹办轮次 != 当前准备.当前筹办轮次
                    || 旧实例->筹办轮次 != 当前准备.当前筹办轮次)
                    return 失败(L2结构状态::引用冲突, Gs);
                if (当前准备.当前筹办轮次 ==
                    (std::numeric_limits<std::uint64_t>::max)())
                    return 失败(L2结构状态::内部不一致, Gs);
                const auto 当前筹办关系组 = L1_.读取所有者范围历史关系组({
                    L1所有者范围CRUD合同版本,
                    L1所有者范围关系端点方向::源, 请求.任务.值,
                    筹办轮次定位_.关系类型[17], Gs});
                const auto 期望目标 = 当前后继身份
                    ? 当前后继身份->值 : 当前准备.首次准备记录.身份.值;
                if (当前筹办关系组.状态 != L1所有者范围读取状态::成功
                    || 当前筹办关系组.读取事实代次 != Gs
                    || 当前筹办关系组.关系组.size() != 1
                    || 当前筹办关系组.关系组.front().目标节点 != 期望目标
                    || 当前筹办关系组.关系组.front().角色或顺序 != 1)
                    return 失败(L2结构状态::内部不一致, Gs);
                当前筹办关系编码 = 当前筹办关系组.关系组.front().编码;
                const auto 已消费组 = L1_.读取所有者范围历史关系组({
                    L1所有者范围CRUD合同版本,
                    L1所有者范围关系端点方向::目标,
                    前态->状态.来源任务实际结果.值,
                ...
                if (!已消费组.关系组.empty()) {
                    L2提交目标未达成待重筹办迁移结果 已消费;
                    已消费.结果头 = {L2结构合同版本, L2结构状态::引用冲突,
                        Gs, std::nullopt};
                    已消费.轮次分区 = L2任务筹办轮次分区::统一轮次权威;
                    已消费.后继准备记录 =
                        L2任务结构内部::读取后继筹办准备历史投影(
                            L1_, 筹办轮次定位_,
                            L2任务后继筹办准备记录身份{
                                已消费组.关系组.front().源节点}, Gs, Gs);
                    if (!已消费.后继准备记录)
                        return 失败(L2结构状态::内部不一致, Gs);
                    return 已消费;
                }
            }
            const auto 写集 = 请求.轮次分区 ==
                    L2任务筹办轮次分区::统一轮次权威
                ? L2任务结构内部::形成统一目标未达成迁移写集(
                    请求, 前态->状态, 前态->证据, 当前关系.编码,
                    当前路径关系.编码, 当前实例关系.编码,
                    *当前筹办关系编码, *分区.统一当前准备,
                    身份来源定位_, 类型定位_, 筹办轮次定位_)
                : L2任务结构内部::形成目标未达成迁移写集(
                    请求, 前态->状态, 前态->证据, 当前关系.编码,
                    当前路径关系.编码, 当前实例关系.编码,
                    身份来源定位_, 类型定位_);
            const auto 写入 = 第一层写入端口_.提交所有者范围中性写集(写集);

...
任务正式选择登记定位 初始化任务正式选择登记(
    ...
    if (当前 == 0) throw std::runtime_error("task selection generation unavailable");
    const auto 键 = [](std::uint32_t 值) noexcept {
        return L1所有者范围写集本地键{值};
    };
    L1所有者范围写集请求 写集;
...
任务筹办轮次定位 初始化任务筹办轮次登记(
    const L1事实基座服务& 第一层服务,
    L1所有者范围写端口& 写入端口) {
    const auto 当前读取 = 第一层服务.读取中性当前事实代次(
        {L1中性CRUD合同版本});
    const auto 当前代次 = 当前读取.状态 == L1中性读取状态::成功
        ? 当前读取.事实代次 : 0;
    if (当前代次 == 0)
        throw std::runtime_error("L2 task planning round registry has no generation");
    L1所有者范围写集请求 写集;
    写集.合同版本 = L1所有者范围CRUD合同版本;
    写集.期望事实代次 = 当前代次;
...
任务初次筹办准备记录定位 初始化任务初次筹办准备记录登记(
    const L1事实基座服务& 第一层服务,
    L1所有者范围写端口& 写入端口) {
    const auto 当前代次读取 = 第一层服务.读取中性当前事实代次(
        {L1中性CRUD合同版本});
    const auto 当前代次 = 当前代次读取.状态 == L1中性读取状态::成功
        ? 当前代次读取.事实代次 : 0;
    if (当前代次 == 0)
        throw std::runtime_error("L2 task initial planning record init has no fact generation");
    L1所有者范围写集请求 写集;
    写集.合同版本 = L1所有者范围CRUD合同版本;
    写集.期望事实代次 = 当前代次;

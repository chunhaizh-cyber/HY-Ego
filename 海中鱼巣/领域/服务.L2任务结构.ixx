...
std::optional<L2任务后继筹办准备记录事实> 读取后继筹办准备历史投影(
    ...
    const auto 旧路径 = 读唯一(13);
    const auto 旧实例 = 读唯一(14);
    const auto 裁决 = 读唯一(15);
    const auto 正式状态 = 读唯一(16);
    if (!族 || *族 != 定位.后继记录锚点 || !任务 || !虚拟 || !列表项
        || !权威 || !首次 || !触发 || !旧路径 || !旧实例 || !裁决
        || !正式状态 || 前一组.状态 != L1所有者范围读取状态::成功
        || 前一组.历史截止事实代次 != 截止
    ...
    事实.首次准备记录 = L2任务初次筹办准备记录身份{*首次};
    事实.前一后继准备记录 = 前一;
    事实.触发类型 = static_cast<L2任务后继筹办触发类型>(触发类型->front());
    事实.触发事实 = *触发;
    事实.旧路径 = L2任务方法路径身份{*旧路径};
    事实.旧实例 = L2实例方法身份{*旧实例};
    事实.目标裁决证据 = L2任务目标裁决证据身份{*裁决};

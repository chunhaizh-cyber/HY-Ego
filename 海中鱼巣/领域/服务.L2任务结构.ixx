...
public:
    ...
    L2新增执行结果待验证状态结果 新增执行结果待验证状态(
        ...
        try {
            ...
            // legacy 入口只重放已经存在的首次账，不再建立 proof 事实。
            {
                ...
                const auto 重放 = 第一层写入端口_.提交所有者范围中性写集(
                    *首次.首次规范化写集);
                if (重放.状态 != L1所有者范围写入状态::精确重复)
                    return 失败(重放.状态 == L1所有者范围写入状态::资源失败
                        ? L2结构状态::资源失败
                        : 重放.状态 == L1所有者范围写入状态::许可拒绝
                            ? L2结构状态::许可拒绝
                            : 重放.状态 == L1所有者范围写入状态::幂等冲突
                                ? L2结构状态::幂等冲突
                                : L2结构状态::内部不一致);

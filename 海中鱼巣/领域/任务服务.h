...
public:
    ...
    节点句柄 按需求创建任务(节点句柄 需求节点, const 需求服务& 需求, 存在服务& 存在,
        ...
        const auto 目标状态 = 需求.读取需求目标状态(需求节点);
        if (!目标状态.has_value() || !节点类型匹配(目标状态.value(), 节点类型::状态)) {
            return {};
        }
        ...
        const auto 任务运行场景关系 = 关系_.创建关系(关系类型::引用, 任务节点, 任务场景);
        const auto 任务目标状态关系 = 关系_.创建关系(关系类型::模板, 任务节点, 目标状态.value());
        if (!句柄有效(需求拥有任务关系)
            || !句柄有效(任务来源需求关系)
            || !句柄有效(任务虚拟存在关系)
            || !句柄有效(任务运行场景关系)
        ...
        const auto 创建状态 = 记录任务生命周期状态(
            任务节点, 任务虚拟存在, 任务场景, 任务生命周期状态::已创建, 发生时间戳, 状态);

...
bool 项目9() {
    L1事实基座服务 L1;
    L1写集请求 写集;
    写集.幂等键 = {0x0100'0000'0000'0009ULL};
    写集.节点 = {{{1}, 节点种类::普通, std::nullopt}};
    const auto 写入 = L1.提交写集(写集, 0x15000B01U);
    世界登记服务 服务(L1);
    const auto 前 = L1.读取完整快照({});
...
bool 项目11() {
    ...
    零.幂等身份 = {};
    auto 越界 = 请求(1);
    越界.幂等身份 = {0x0100'0000'0000'0000ULL};
    const auto A = 服务.建立空世界(零);
    const auto B = 服务.建立空世界(越界);
    const auto 快照 = L1.读取完整快照({});
    return A.状态 == 世界登记状态::入口拒绝
        && B.状态 == 世界登记状态::入口拒绝
        && 快照.快照.has_value() && 快照.快照->事实代次 == 0
...
bool 项目12() {
    L1事实基座服务 L1;
    世界登记服务 登记服务(L1);
    L1场景结构服务 场景服务(L1, 登记服务);
    const auto 身份 = 请求(12);
    const auto 登记 = 登记服务.建立空世界(身份);
    if (登记.状态 != 世界登记状态::已提交 || !登记.登记) return false;
    初始场景创建请求 场景请求;
    场景请求.世界根 = {登记.登记->世界根场景, 节点种类::普通};
    场景请求.幂等身份 = 身份.幂等身份;
    场景请求.期望事实代次 = 1;
    const auto 场景 = 场景服务.创建初始场景(场景请求);
    const auto 登记审计 = L1.读取审计({L1事实基座合同版本, {0x0100'0000'0000'000CULL}});
    const auto 场景审计 = L1.读取审计({L1事实基座合同版本, {0x0200'0000'0000'000CULL}});
    return 场景.状态 == 世界结构状态::已提交
        && 登记审计.状态 == L1读取状态::成功
        && 场景审计.状态 == L1读取状态::成功
        && !登记审计.记录组.empty() && !场景审计.记录组.empty()
        && 登记审计.幂等键.值 != 场景审计.幂等键.值;
}

世界登记自检报告 执行() {
    const std::array<bool, 12> 结果{
        项目1(), 项目2(), 项目3(), 项目4(), 项目5(),

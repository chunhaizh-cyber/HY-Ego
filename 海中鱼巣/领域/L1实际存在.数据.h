...
inline bool 实际存在资格定位读取请求有效(
    const 实际存在资格定位读取请求& 请求) noexcept {
    return 请求.合同版本 == L1实际存在合同版本
        && 请求.规则版本 == L1实际存在规则版本
        && 有效(请求.存在);
}
...
// 诊断责任：无适用错误分支；纯值移除请求判断不读取状态、不产生副作用。
inline bool 实际存在当前场景移除请求有效(
    ...
    return 请求.合同版本 == L1实际存在合同版本
        ...
        && 请求.期望事实代次 != 0
        && 实际存在读取请求有效(请求.当前事实)
        && 请求.当前事实.合同版本 == 请求.合同版本
        && 请求.当前事实.规则版本 == 请求.规则版本;
}
...
inline bool 实际存在已退出归属关系见证完整(
    const 实际存在已退出归属关系见证& 见证) noexcept {
    return 世界结构关系见证有效(见证.关系)
        && 见证.创建事实代次 != 0
        && 见证.退出事实代次 >= 见证.创建事实代次;
}

inline bool 实际存在当前场景移除事实完整(
    const 实际存在当前场景移除事实& 事实) noexcept {
    return 事实.合同版本 == L1实际存在合同版本
        && 事实.规则版本 == L1实际存在规则版本
        && 事实.发布事实代次 != 0
        && 事实.事实截止代次 >= 事实.发布事实代次
        ...
        && 世界结构节点见证有效(事实.原场景)
        && 世界结构值见证有效(事实.资格)
        && 实际存在已退出归属关系见证完整(事实.已退出归属关系)
        && 事实.世界根.类型 == 节点种类::普通
        && 事实.存在.类型 == 节点种类::普通
        && 事实.原场景.类型 == 节点种类::普通
        && 事实.资格.所属节点 == 事实.存在
        && 事实.资格.I64值 == 实际存在资格值
        && 事实.资格.创建事实代次 <= 事实.事实截止代次
...
// 诊断责任：无适用错误分支；纯值移除结果成功判断不读取状态、不产生副作用。
inline bool 实际存在当前场景移除结果::成功() const noexcept {
    return (状态 == 实际存在状态::已提交
            || 状态 == 实际存在状态::幂等读回)
        && 事实.has_value() && 实际存在当前场景移除事实完整(*事实);
}

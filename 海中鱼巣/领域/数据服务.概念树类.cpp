...
存在概念使用当前完整读取结果_v3
概念树类数据服务::读取存在概念使用当前完整_v3(
    const 存在概念使用当前完整读取请求_v3 &r) const noexcept {
    存在概念使用当前完整读取结果_v3 out;
    out.Gread=r.Gread;
    try {
        if(!pure_layout_||r.版本!=存在概念使用当前完整读取合同版本_v3||
           !r.Gread||!有效(r.E.值)) {
            out.状态=存在概念使用当前完整读取状态_v3::入口拒绝;
            return out;
        }
        核验外部节点(r.E.值,r.Gread);
        const L1节点当前完整引用读取请求_v2 request{
            L1节点当前完整引用读取合同版本,r.E.值,r.Gread};
        const auto read=l1_.读取节点全部当前引用_v2(request);
        if(read.合同版本!=L1节点当前完整引用读取合同版本||
           read.节点!=r.E.值||read.期望事实代次!=r.Gread||
           read.读取事实代次!=r.Gread) {
            out.状态=存在概念使用当前完整读取状态_v3::事实代次漂移;
            return out;
        }
        if(read.状态!=L1节点当前完整引用读取状态_v2::成功) {
            if(read.状态==L1节点当前完整引用读取状态_v2::未找到)
                out.状态=存在概念使用当前完整读取状态_v3::未找到;
            else if(read.状态==L1节点当前完整引用读取状态_v2::事实代次漂移)
                out.状态=存在概念使用当前完整读取状态_v3::事实代次漂移;
            else if(read.状态==L1节点当前完整引用读取状态_v2::资源失败)
                out.状态=存在概念使用当前完整读取状态_v3::资源失败;
            else out.状态=存在概念使用当前完整读取状态_v3::内部不一致;
            return out;
        }
        std::optional<L1所有者范围关系事实> edge;
        for(const auto &item:read.引用) {
            const auto *relation=std::get_if<L1所有者范围关系事实>(&item);
            if(!relation||relation->源节点!=r.E.值||
               relation->关系类型节点!=pure_layout_->类型.存在概念使用)continue;
            if(edge){out.状态=存在概念使用当前完整读取状态_v3::内部不一致;return out;}
            edge=*relation;
        }
        if(!edge){out.状态=存在概念使用当前完整读取状态_v3::未找到;return out;}
        if(edge->写入所有者!=port_.所有者身份()||edge->角色或顺序!=1||
           !edge->创建事实代次||edge->创建事实代次>r.Gread) {
            out.状态=存在概念使用当前完整读取状态_v3::内部不一致;return out;
        }
        const 纯概念完整读取请求_v3 概念请求{
            3,r.Gread,概念树概念身份{edge->目标节点}};
        const auto 概念结果=读取纯概念完整(概念请求);
        if(!概念结果.成功(概念请求)||!概念结果.事实) {
            if(概念结果.状态==纯概念状态::未找到)
                out.状态=存在概念使用当前完整读取状态_v3::未找到;
            else if(概念结果.状态==纯概念状态::类别冲突)
                out.状态=存在概念使用当前完整读取状态_v3::类别冲突;
            else if(概念结果.状态==纯概念状态::定义不相容)
                out.状态=存在概念使用当前完整读取状态_v3::定义不相容;
            else if(概念结果.状态==纯概念状态::事实代次漂移)
                out.状态=存在概念使用当前完整读取状态_v3::事实代次漂移;
            else if(概念结果.状态==纯概念状态::资源失败)
                out.状态=存在概念使用当前完整读取状态_v3::资源失败;
            else out.状态=存在概念使用当前完整读取状态_v3::内部不一致;
            return out;
        }
        if(概念结果.事实->类别!=相关概念类别::存在){out.状态=存在概念使用当前完整读取状态_v3::类别冲突;return out;}
        out.使用=存在概念使用事实{edge->编码,r.E.值,
            概念树概念身份{edge->目标节点},{edge->创建事实代次}};
        out.概念=*概念结果.事实;
        out.状态=存在概念使用当前完整读取状态_v3::已读取;
        守卫代次(r.Gread);
    } catch(const 纯失败&e) {
        if(e.状态==纯概念状态::未找到)
            out.状态=存在概念使用当前完整读取状态_v3::未找到;
        else if(e.状态==纯概念状态::类别冲突)
            out.状态=存在概念使用当前完整读取状态_v3::类别冲突;
        else if(e.状态==纯概念状态::定义不相容)
            out.状态=存在概念使用当前完整读取状态_v3::定义不相容;
        else if(e.状态==纯概念状态::事实代次漂移)
            out.状态=存在概念使用当前完整读取状态_v3::事实代次漂移;
        else if(e.状态==纯概念状态::资源失败)
            out.状态=存在概念使用当前完整读取状态_v3::资源失败;
        else out.状态=存在概念使用当前完整读取状态_v3::内部不一致;
        out.使用.reset();out.概念.reset();
    } catch(const 失败&e) {
        ...
        out.状态=存在概念使用当前完整读取状态_v3::资源失败;out.使用.reset();out.概念.reset();
    } catch(...) {
        out.状态=存在概念使用当前完整读取状态_v3::内部不一致;out.使用.reset();out.概念.reset();
    }
    return out;
}

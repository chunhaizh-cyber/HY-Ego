...
bool 完整定义布局独立(const 海中鱼巣::完整存在概念结构交付_v1& x,
                     const 海中鱼巣::纯概念结构交付_v2& p) noexcept {
    if(x.版本!=1)return false;
    const auto&t=x.类型;
    const std::array<海中鱼巣::稳定编码,9> ids{x.格式锚点,t.完整定义成员,t.完整定义自身特征,
      t.完整定义组成项,t.完整定义组成子概念,t.完整定义关系项,t.完整定义关系参与角色,
      t.组成角色属性,t.关系类型属性};
    std::set<std::uint64_t> used;
    for(const auto id:ids)if(!海中鱼巣::有效(id)||!used.insert(id.值).second)return false;
    const auto&q=p.类型;
    const std::array<海中鱼巣::稳定编码,15> pure{p.格式锚点,p.概念族锚点,q.类型登记,q.概念族成员,
      q.格式版本,q.概念类别,q.定义成员,q.定义种类,q.定义特征类型,q.定义模板,q.I64域,
      q.通用规则,q.直接上位,q.生命周期,q.存在概念使用};
    return std::none_of(pure.begin(),pure.end(),[&](auto id){return used.contains(id.值);});
}
bool 可登记完整定义(const 海中鱼巣::完整存在概念结构登记请求_v1& r) noexcept {
    return r.版本==1&&r.G0&&海中鱼巣::有效(r.幂等身份)&&r.最大首次材料项数>=18&&
      r.纯概念结构.版本==2&&海中鱼巣::有效(r.纯概念结构.格式锚点)&&
      海中鱼巣::有效(r.纯概念结构.类型.类型登记)&&
      海中鱼巣::有效(r.纯概念结构.类型.格式版本);
}
}
...
void 概念树类数据服务::校验布局(std::uint64_t g) const {
    ...
    plain(旧布局().签名规则.值);
    const auto &t = 旧布局().类型;
    const auto registrations = 关系(旧布局().格式锚点, t.类型登记, false, g, g, 28);
    if (registrations.size() != 28)
        throw 失败{S::引用冲突};
    for (std::size_t i = 0; i < 28; ++i)
        if (registrations[i].目标节点 != types[i] || registrations[i].角色或顺序 != i + 1)
            throw 失败{S::引用冲突};
    const auto rootEdges = 关系(旧布局().格式锚点, t.根成员, false, g, g, 4);
    if (rootEdges.size() != 4)
        throw 失败{S::引用冲突};
    ...
    if (唯一关系(旧布局().格式锚点, t.规则归属, g, g).目标节点 != 旧布局().签名规则.值)
        throw 失败{S::引用冲突};
    const auto anchorValues = 属性(旧布局().格式锚点, g, g);
    const auto ruleValues = 属性(旧布局().签名规则.值, g, g);
...
稳定编码 概念树类数据服务::支持关系类型(const 概念树世界引用 &w) const {
    if (w.valueless_by_exception())
        throw 失败{S::入口拒绝};
    return std::holds_alternative<概念树存在引用>(w) ? 旧布局().类型.支持存在 : 旧布局().类型.支持特征;
}
...
概念树支持事实 概念树类数据服务::支持记录(稳定编码 id, std::uint64_t g, std::uint64_t h,
    ...
    if (anchor.size() != 1 || anchor.front().源节点 != 旧布局().格式锚点 || anchor.front().角色或顺序 != 1)
        throw 失败{S::内部不一致};
...
std::vector<概念树支持事实> 概念树类数据服务::支持组(std::optional<概念树概念身份> conceptId,
    ...
    if (conceptId)
        ...
        members = rel(旧布局().格式锚点, t.支持归属, false, g, h, b.最大支持数);
...
概念树图结果 概念树类数据服务::图(概念树根角色 role, bool withSupport, std::uint64_t g, std::uint64_t h,
    ...
    const auto members = rel(out.根身份.值, 旧布局().类型.概念根归属, true, g, h, b.最大概念数);
    ...
    for (const auto &f : out.概念组)
        for (const auto &e : rel(f.概念.值, 旧布局().类型.直接上位, false, g, h, b.最大关系数))
    ...
    std::visit(
        [&](const auto &r) {
            ...
            if constexpr (std::is_same_v<T, 概念树概念建立请求>) {
                ...
                newEdge(cn, Ref{旧布局().签名规则.值}, t.概念规则);
...
概念树写入结果 概念树类数据服务::发布结果(const 概念树业务写请求 &request, const L1所有者范围写集请求 &ws,
    ...
    for (const auto &edge : ws.关系)
        if (edge.关系类型节点 == Ref{旧布局().类型.直接上位})
...
void 概念树类数据服务::校验共享扩展(std::uint64_t g) const {
    ...
    if (av.size() != 1 || std::get<std::int64_t>(唯一属性(av, 旧布局().类型.格式版本).材料) != 2)
        throw 失败{S::引用冲突};
    using R = L1所有者范围值表示种类;
    ...
    const auto registrations = 关系(x.锚点, 旧布局().类型.类型登记, false, g, g, 21);
...
概念树应用读取结果<旧特征概念审计事实> 概念树类数据服务::读取旧特征概念审计(
    ...
    return 共享读取封装<旧特征概念审计事实>(r.读取头,r.预算,[&](auto& c) {
        ...
        const auto& t=旧布局().类型;const auto& x=shared_->类型;
        ...
        旧特征概念审计事实 out;
        out.C=r.概念;out.本体根=旧布局().根组.特征根;out.规则=旧布局().签名规则;
...
void 概念树类数据服务::校验存在扩展(std::uint64_t g) const {
    ...
    distinct(旧布局().格式锚点);distinct(旧布局().签名规则.值);
...
void 概念树类数据服务::校验特征扩展(std::uint64_t g) const {
    ...
    unique(旧布局().格式锚点);unique(旧布局().签名规则.值);
    ...
    const auto* format=std::get_if<std::int64_t>(&唯一属性(av,旧布局().类型.格式版本).材料);
    ...
    const auto es=关系(x.锚点,旧布局().类型.类型登记,false,g,g,24);
...
概念树类数据服务::共享展开 概念树类数据服务::展开共享定义(概念树概念身份 id,共享读取上下文& c) const {
    ...
    const auto& t=旧布局().类型; const auto& x=feature_layout_->类型;
    ...
    f.身份=id; f.本体根=旧布局().根组.特征根; f.规则=旧布局().签名规则;
...
概念树应用定义事实 概念树类数据服务::应用定义(概念树概念身份 id, 共享读取上下文 &c) const {
    ...
    if (std::find(roots.begin(), roots.end(), id) == roots.end()) {
        const auto av = 属性(id.值, c.G, c.H);
        const auto kind = std::get<std::int64_t>(唯一属性(av, 旧布局().类型.概念种类).材料);
        if (kind == 4)
...
概念树应用图事实 概念树类数据服务::应用图(概念树根角色 role, bool support,
    ...
    const auto members = 共享关系(out.根身份.值, 旧布局().类型.概念根归属, true, c.H, c);
    ...
    for (const auto &[id, degree] : degrees) {
        for (const auto &e : 共享关系(稳定编码{id}, 旧布局().类型.直接上位, false, c.H, c))
...
void 概念树类数据服务::准备父组(
    ...
    for(const auto& change:changes) {
        ...
        if(addition) {
            ...
            b.关系(addition->first,Ref{f.身份.值},旧布局().类型.直接上位);
        }
        ...
        for(auto parent:ids)
            if(std::none_of(actual.begin(),actual.end(),[&](const auto& e){return e.上位==parent;}))
                b.关系(Ref{parent.值},Ref{f.身份.值},旧布局().类型.直接上位);
    }
...
概念树类数据服务::特征准备结果 概念树类数据服务::准备特征写入(
    ...
    const auto& x=feature_layout_->类型;const auto& t=旧布局().类型;
    ...
    std::visit([&](const auto& r) {
        ...
        if constexpr(std::is_same_v<T,类型观察登记请求>) {
            ...
            if(typeRoot) {
                if(parents.size()!=1||parents.front()!=旧布局().根组.特征根)throw 失败{S::引用冲突};
            } else for(auto id:parents) {
                if(id==旧布局().根组.特征根)throw 失败{S::引用冲突};
            ...
            b.关系(Ref{node},Ref{旧布局().根组.特征根.值},t.概念根归属);
            b.关系(Ref{node},Ref{旧布局().签名规则.值},t.概念规则);
...
概念树类数据服务::存在展开_v2 概念树类数据服务::展开存在定义_v2(
    ...
    if (n.种类 != 节点种类::普通 || n.属性类型表示 || id == 旧布局().根组.存在根)
        throw 失败{S::引用冲突};
    const auto av = 属性(id.值, c.G, c.H);
    ...
    存在展开_v2 out;
    auto &f = out.事实;
    f.概念 = id; f.本体根 = 旧布局().根组.存在根; f.规则 = 旧布局().签名规则;
...
概念树类数据服务::存在准备结果_v2 概念树类数据服务::准备存在写入(
    ...
    auto parent = [&](概念树概念身份 id) {
        ...
        if (std::visit([&](const auto &v) { return v.本体根 != 旧布局().根组.存在根; }, fact)) throw 失败{S::引用冲突};
        if (id != 旧布局().根组.存在根 && !std::holds_alternative<概念树应用存在事实>(fact)) throw 失败{S::引用冲突};
    };
    std::visit([&](const auto &r) {
        using T = std::decay_t<decltype(r)>;
        if constexpr (std::is_same_v<T, 概念树存在建立请求_v2>) {
            ...
            builder.关系(Ref{cn}, Ref{旧布局().根组.存在根.值}, t.概念根归属);
            builder.关系(Ref{cn}, Ref{旧布局().签名规则.值}, t.概念规则);
            ...
            if (id == 旧布局().根组.存在根) throw 失败{S::根不可退出};
        ...
        const auto prepare = [&] {
            ...
            std::visit([&](const auto &r) {
                ...
                if constexpr (std::is_same_v<T, 概念树上位操作请求> || std::is_same_v<T, 概念树生命周期迁移请求> ||
                    ...
                    if (id == 旧布局().根组.特征根 || id == 旧布局().根组.动态根 || id == 旧布局().根组.存在根 || id == 旧布局().根组.因果链根)
                        throw 失败{S::根不可退出};
                    const auto av = 属性(id.值, c.G, c.H);
                    const auto kind = std::get<std::int64_t>(唯一属性(av, 旧布局().类型.概念种类).材料);
                    if (kind == 4) {
                    ...
                    const auto root = 唯一关系(id.值, 旧布局().类型.概念根归属, c.G, c.H);
                    if (root.目标节点 != 旧布局().根组.特征根.值) throw 失败{S::引用冲突};
                }
            }, request.操作);
...
概念树类数据服务::概念树类数据服务(
    ...
    const 存在类数据服务 &existences, const 特征值类数据服务 &values,
    const 场景类数据服务 &scenes, L1所有者范围写端口 &&port,
    const 概念树结构交付 &oldLayout, const 相关概念结构交付 &layout)
    : 概念树类数据服务(l1, features, existences, values, std::move(port), oldLayout) {
    if (!scenes.绑定于(l1_)) throw std::invalid_argument("related concept scene binding mismatch");
    scenes_ = &scenes;
    related_layout_ = layout;
    try { 校验相关概念布局(当前代次()); }
    catch (const 失败 &e) {
        if (e.状态 == S::资源失败) throw std::bad_alloc{};
        throw std::invalid_argument("related concept structure delivery is invalid");
...
void 概念树类数据服务::校验相关概念布局(std::uint64_t g) const {
    ...
    for(std::size_t i=0;i<types.size();++i){
        std::optional<R> rep;
        switch(i+1){case 3:case 5:case 9:case 12:case 13:case 21:rep=R::I64;break;
        case 10:rep=R::I64组;break;case 11:case 20:rep=R::U64组;break;default:break;}
        const auto n=节点(types[i],g,g);
        if(n.种类!=(rep?节点种类::属性类型:节点种类::普通)||n.属性类型表示!=rep||!属性(types[i],g,g).empty())
            throw 失败{S::引用冲突};
    }
    const auto registrations=关系(x.格式锚点,t.类型登记,false,g,g,21);
    if(registrations.size()!=21)throw 失败{S::引用冲突};
...
相关概念参与状态 概念树类数据服务::映射相关状态(S s) const noexcept {
    ...
    case S::目标已退出:return R::概念已退出;case S::形成环:return R::上位成环;
    case S::事实代次漂移:return R::事实代次漂移;case S::幂等冲突:return R::幂等冲突;
    case S::数量预算不足:return R::数量预算不足;case S::历史材料不可用:return R::历史材料不可用;
    ...
    case S::已创建:case S::已添加支持:case S::已退出支持:case S::已添加上位:
    case S::已替换上位:case S::已退出上位:case S::已迁移生命周期:case S::已退出概念:
    case S::已释放引用:case S::无须变更:case S::根不可退出:case S::已创建共享定义:
    case S::已绑定共享名称:case S::已退出共享名称:case S::已记录共享用途:case S::不支持:
...
void 概念树类数据服务::验证相关世界(const 相关概念世界挂靠 &binding,std::uint64_t g,
    ...
    if(binding.世界事实.valueless_by_exception()||!binding.证据截止||binding.证据截止>g||
    ...
    try{
        if(const auto *scene=std::get_if<概念树场景引用>(&binding.世界事实)){
            ...
            const 场景历史身份请求 request{2,g,binding.证据截止,scene->值};
            const auto result=scenes_->读取场景角色历史(request);
            if(!result.成功(request)){
                if(result.状态==场景角色数据状态::资源失败)throw 相关失败{相关概念参与状态::资源失败};
                if(result.状态==场景角色数据状态::历史材料已清理)throw 相关失败{相关概念参与状态::历史材料不可用};
                if(result.状态==场景角色数据状态::事实代次漂移)throw 相关失败{相关概念参与状态::事实代次漂移};
                throw 相关失败{相关概念参与状态::挂靠无效};
            }
            return;
        }
        验证世界(binding.世界事实,g,binding.证据截止,budget);
    }catch(const 相关失败&){throw;}catch(const 失败 &e){
        const auto mapped=映射相关状态(e.状态);
        if(mapped==相关概念参与状态::概念未找到||mapped==相关概念参与状态::概念已退出||
...
概念树类数据服务::相关概念展开 概念树类数据服务::展开相关概念(
    ...
    if (!有效(id.值) || !h || h > g) throw 相关失败{相关概念参与状态::入口拒绝};
    try {
        const auto &t = related_layout_->类型;
        相关概念展开 out;
        out.概念 = id;
        const auto conceptNodeFact = 节点(id.值, g, h);
        if (conceptNodeFact.种类 != 节点种类::普通 || conceptNodeFact.属性类型表示)
            throw 相关失败{相关概念参与状态::签名冲突};
        out.生命周期 = 投影生命(conceptNodeFact, h);

        const auto members = 关系(id.值, t.概念族成员, true, g, h, 1);
        if (members.size() != 1 || members.front().源节点 != related_layout_->概念族锚点 ||
            members.front().角色或顺序 != 1)
            throw 相关失败{相关概念参与状态::签名冲突};
        const auto conceptValues = 属性(id.值, g, h);
        ...
        if (kind == 1) {
            ...
            概念树特征定义 definition;
            const auto hosts = 关系(definitionNode.编码, t.定义宿主, false, g, h, 1);
            const auto types = 关系(definitionNode.编码, t.定义特征类型, false, g, h, 1);
        ...
        std::set<std::uint64_t> parentIds;
        for (const auto &edge : 关系(id.值, t.直接上位, true, g, h, budget.最大关系数)) {
            if (edge.角色或顺序 != 1 || edge.源节点 == id.值 ||
                !parentIds.insert(edge.源节点.值).second)
                throw 相关失败{相关概念参与状态::上位成环};
            const auto parentValues = 属性(edge.源节点, g, h);
        ...
        for (const auto &member : 关系(id.值, t.形成成员, false, g, h, budget.最大来源数)) {
            if (member.角色或顺序 != 1) throw 相关失败{相关概念参与状态::内部不一致};
            const auto record = 节点(member.目标节点, g, h);
            const auto recordValues = 属性(record.编码, g, h);
            if (record.种类 != 节点种类::普通 || record.属性类型表示 || recordValues.size() != 1)
                throw 相关失败{相关概念参与状态::内部不一致};
            const auto &evidence = 唯一属性(recordValues, t.证据截止);
            const auto &cutoffs = std::get<std::vector<std::uint64_t>>(evidence.材料);
            if (cutoffs.size() != 1 || !cutoffs.front() || cutoffs.front() > record.创建事实代次)
                throw 相关失败{相关概念参与状态::内部不一致};
            std::optional<E> endpoint;
            std::size_t branch = 0;
            for (const auto type : {t.形成存在, t.形成特征, t.形成特征类型, t.形成场景}) {
                const auto found = 关系(record.编码, type, false, g, h, 1);
            ...
            out.挂接.push_back({endpoint->编码, record.编码, id, world, 投影生命(*endpoint, h)});
        }
        return out;
...
相关概念完整事实 概念树类数据服务::读取相关概念内部(
    ...
    const auto expanded = 展开相关概念(id, g, h, request.预算);
...
L1有限N分区原子参与者写集_v3 概念树类数据服务::形成相关概念写集(
    ...
    newValue(formationNode, t.证据截止,
             std::vector<std::uint64_t>{request.挂靠.证据截止});
...
相关概念参与片段 概念树类数据服务::准备相关概念片段(
    ...
    try {
        ...
        auto currentBinding = request.挂靠; currentBinding.证据截止 = g;
        验证相关世界(currentBinding, g, request.预算);
        const auto category = std::visit([](const auto &v) { return v.类别; }, request.方案);
        ...
        if (const auto *feature = std::get_if<概念树特征定义>(&definition)) {
            验证相关世界({feature->形成宿主, g}, g, request.预算);
            验证相关世界({feature->特征类型, g}, g, request.预算);
        } else {
            for (const auto templateId : std::get<概念树存在定义>(definition).特征模板组) {
                const auto feature = 展开相关概念(templateId, g, g, request.预算);
        ...
        if (const auto *reuse = std::get_if<相关概念精确复用方案>(&request.方案)) {
            const auto found = 展开相关概念(reuse->概念, g, g, request.预算);
            ...
            const auto family = 关系(related_layout_->概念族锚点,
                                     ...
                                     g, g, request.预算.最大概念数);
            for (const auto &member : family) {
                const auto found = 展开相关概念(概念树概念身份{member.目标节点}, g, g, request.预算);
        ...
        if (const auto *create = std::get_if<相关概念创建方案>(&request.方案)) {
            ...
            for (const auto parent : parents) {
                const auto value = 展开相关概念(parent, g, g, request.预算);
        ...
        if (existing) {
            const auto expanded = 展开相关概念(existing->概念, g, g, request.预算);
            const auto target = 世界编码(request.挂靠.世界事实);
            for (const auto &attachment : expanded.挂接) {
                if (attachment.世界引用.index() != request.挂靠.世界事实.index() ||
                    世界编码(attachment.世界引用) != target) continue;
                const auto values = 属性(attachment.记录, g, g);
                const auto &cutoff = std::get<std::vector<std::uint64_t>>(
                    唯一属性(values, related_layout_->类型.证据截止).材料);
                if (cutoff.size() != 1 || cutoff.front() != request.挂靠.证据截止)
                    throw 相关失败{相关概念参与状态::挂靠无效};
...
相关概念参与读回 概念树类数据服务::读取相关概念结果(
    const 相关概念参与请求 &request, std::uint64_t g, std::uint64_t h) const noexcept {
    相关概念参与读回 out;
    out.Gread = g;
    try {
        if (!related_layout_) throw 相关失败{相关概念参与状态::旧格式不支持};
        if (request.版本 != 1 || !g || !h || h > g || request.G0 > g ||
        ...
        if (const auto *reuse = std::get_if<相关概念精确复用方案>(&request.方案)) id = reuse->概念;
            ...
            const auto family = 关系(related_layout_->概念族锚点,
                                     related_layout_->类型.概念族成员, false,
                                     g, h, request.预算.最大概念数);
            for (const auto &member : family) {
                const auto found = 展开相关概念(概念树概念身份{member.目标节点}, g, h, request.预算);
        ...
        out.概念 = 读取相关概念内部(request, id, g, h);
        out.H = h;
        out.状态 = 相关概念参与状态::已读取;
        守卫代次(g);
        return out;
    ...
    try {
        ...
        if (first.状态 == L1所有者范围读取状态::成功) {
            ...
            const L1所有者范围空域读取请求 emptyRequest{
                L1所有者范围空域读取合同版本,port.所有者身份(),r.G0,
                r.最大空域核验事实数};
            const auto empty=l1.读取所有者范围空域(emptyRequest);
            out.Gread=empty.读取事实代次;
            if(!empty.成功(emptyRequest)) {
                switch(empty.状态) {
                case L1所有者范围空域读取状态::入口拒绝:
                    out.状态=纯概念状态::入口拒绝;break;
                case L1所有者范围空域读取状态::事实代次漂移:
                    out.状态=纯概念状态::事实代次漂移;break;
                case L1所有者范围空域读取状态::数量预算不足:
                    out.状态=纯概念状态::数量预算不足;break;
                case L1所有者范围空域读取状态::资源失败:
                    out.状态=纯概念状态::资源失败;break;
                case L1所有者范围空域读取状态::范围不支持:
                    out.状态=纯概念状态::旧格式不支持;break;
                default:out.状态=纯概念状态::内部不一致;break;
                }
                return out;
            }
            if(!empty.确认空域(emptyRequest)) {
                out.状态=纯概念状态::旧格式不支持;
                return out;
            }
        } else {
        ...
        auto raw=[&](稳定编码 id) {
            ...
            if(read.状态!=L1所有者范围读取状态::成功||read.读取事实代次!=out.Gread||
               !read.事实||read.物理清理墓碑||read.物理清理事实代次) {
        ...
        for(std::uint64_t i=0;i<13;++i) {
            ...
            if(!edge||edge->写入所有者!=port.所有者身份()||
               ...
               edge->创建事实代次!=saved.事实代次||edge->退出事实代次)
                throw 纯失败{纯概念状态::内部不一致};
        }
...
bool 特征概念出生使用结构登记结果::成功(
    ...
    return x.锚点.编码 == 交付->锚点 && x.F到FCv出生使用关系类型.编码 == 交付->F到FCv出生使用关系类型 &&
        ...
        x.锚点.种类 == 节点种类::普通 && !x.锚点.属性类型表示 && !x.锚点.退出事实代次 &&
        x.F到FCv出生使用关系类型.种类 == 节点种类::普通 && !x.F到FCv出生使用关系类型.属性类型表示 && !x.F到FCv出生使用关系类型.退出事实代次 &&
        x.类型登记关系.源节点 == 交付->锚点 &&
        x.类型登记关系.目标节点 == 交付->F到FCv出生使用关系类型 &&
        x.类型登记关系.关系类型节点 == r.纯概念结构.类型.类型登记 &&
        x.类型登记关系.角色或顺序 == 1 && !x.类型登记关系.退出事实代次 &&
        x.格式值.所属节点 == 交付->锚点 && x.格式值.来源节点 == 交付->锚点 &&
        x.格式值.属性类型节点 == r.纯概念结构.类型.格式版本 &&
        std::holds_alternative<std::int64_t>(x.格式值.材料) &&
        std::get<std::int64_t>(x.格式值.材料) == 1 && !x.格式值.退出事实代次 &&
        x.锚点.创建事实代次 == *首次发布H &&
        x.F到FCv出生使用关系类型.创建事实代次 == *首次发布H &&
        x.类型登记关系.创建事实代次 == *首次发布H && x.格式值.创建事实代次 == *首次发布H;
}

完整存在概念结构登记结果_v1 概念树类数据服务::登记完整存在概念结构_v1(
    const L1事实基座服务& l1, L1所有者范围写端口& port,
    const 完整存在概念结构登记请求_v1& r) noexcept {
    完整存在概念结构登记结果_v1 out; out.原请求=r; bool entered=false;
    try {
        if(!可登记完整定义(r)||r.G0==UINT64_MAX||!port.有效()||!port.绑定于(l1))return out;
        const auto& p=r.纯概念结构; const auto&t=p.类型;
        const auto prior=port.读取首次写入材料({L1所有者范围首次写入读取合同版本,r.幂等身份});
        out.Gread=prior.读取事实代次;
        if(prior.合同版本!=L1所有者范围首次写入读取合同版本||prior.所有者!=port.所有者身份()||
           prior.写入幂等身份!=r.幂等身份) { out.状态=纯概念状态::内部不一致; return out; }
        const bool replay=prior.状态==L1所有者范围读取状态::成功;
        if(!replay&&prior.状态!=L1所有者范围读取状态::未找到) {
            out.状态=prior.状态==L1所有者范围读取状态::资源失败?纯概念状态::资源失败:纯概念状态::内部不一致;return out;
        }
        L1所有者范围写集请求 ws{L1所有者范围CRUD合同版本,r.G0,r.幂等身份};
        for(std::uint32_t i=1;i<=9;++i) {
            const bool valueType=i>=8;
            ws.节点.push_back({{i},valueType?节点种类::属性类型:节点种类::普通,
                valueType?std::optional<L1所有者范围值表示种类>{L1所有者范围值表示种类::I64}:std::nullopt});
        }
        for(std::uint32_t i=0;i<8;++i)
            ws.关系.push_back({{0x10001U+i},Key{1},Key{2U+i},t.类型登记,static_cast<std::int64_t>(i+1)});
        ws.值.push_back({{0x20001},Key{1},t.格式版本,std::int64_t{1},Key{1}});
        ws.属性槽变更.push_back({Key{1},t.格式版本,Key{0x20001}});
        if(replay) {
            if(!prior.首次规范化写集||*prior.首次规范化写集!=ws) {out.状态=纯概念状态::幂等冲突;return out;}
            if(!prior.首次写入结果||prior.首次写入结果->状态!=L1所有者范围写入状态::成功||
               prior.首次写入结果->事实代次!=r.G0+1||!prior.首次写入结果->是否形成内存权威发布) {
                out.状态=纯概念状态::已可能发布;out.发布=纯概念发布状态::可能发布;return out;
            }
        } else if(prior.读取事实代次!=r.G0||prior.首次规范化写集||prior.首次写入结果) {
            out.状态=prior.读取事实代次==r.G0?纯概念状态::内部不一致:纯概念状态::事实代次漂移;return out;
        }
        entered=true; const auto saved=port.提交所有者范围中性写集(ws); out.Gread=saved.事实代次;
        if(saved.状态!=(replay?L1所有者范围写入状态::精确重复:L1所有者范围写入状态::成功)||
           saved.合同版本!=L1所有者范围CRUD合同版本||saved.所有者!=port.所有者身份()||
           saved.写入幂等身份!=r.幂等身份||saved.事实代次!=r.G0+1||saved.新编码映射.size()!=18) {
            out.状态=saved.状态==L1所有者范围写入状态::事实代次漂移?纯概念状态::事实代次漂移:
              (saved.状态==L1所有者范围写入状态::幂等冲突?纯概念状态::幂等冲突:纯概念状态::已可能发布);
            out.发布=saved.是否形成内存权威发布||replay?纯概念发布状态::可能发布:纯概念发布状态::确认未发布;return out;
        }
        std::array<稳定编码,9> ids{}; std::array<稳定编码,8> edges{}; 稳定编码 value{};
        for(const auto&[key,id]:saved.新编码映射) {
            if(key.值>=1&&key.值<=9)ids[key.值-1]=id;
            else if(key.值>=0x10001&&key.值<=0x10008)edges[key.值-0x10001]=id;
            else if(key.值==0x20001)value=id;
        }
        if(!有效(value)||std::ranges::any_of(ids,[](auto x){return !有效(x);})||
           std::ranges::any_of(edges,[](auto x){return !有效(x);}))throw 纯失败{纯概念状态::已可能发布};
        out.交付=完整存在概念结构交付_v1{1,ids[0],{ids[1],ids[2],ids[3],ids[4],ids[5],ids[6],ids[7],ids[8]}};
        if(!完整定义布局独立(*out.交付,p))throw 纯失败{纯概念状态::内部不一致};
        const auto tail=l1.读取中性当前事实代次({L1中性CRUD合同版本});
        if(tail.状态!=L1中性读取状态::成功||tail.事实代次<saved.事实代次)throw 纯失败{纯概念状态::已可能发布};
        out.Gread=tail.事实代次;out.首次发布H=saved.事实代次;完整存在概念结构首次材料_v1 material;
        auto raw=[&](稳定编码 id)->const L1所有者范围事实& { const auto x=l1.读取所有者范围历史事实({L1所有者范围CRUD合同版本,id});
            if(x.状态!=L1所有者范围读取状态::成功||x.读取事实代次!=out.Gread||!x.事实)throw 纯失败{纯概念状态::已可能发布};return *x.事实; };
        for(std::size_t i=0;i<ids.size();++i){const auto*node=std::get_if<L1所有者范围节点事实>(&raw(ids[i]));if(!node)throw 纯失败{纯概念状态::内部不一致};material.节点[i]=*node;}
        for(std::size_t i=0;i<edges.size();++i){const auto*edge=std::get_if<L1所有者范围关系事实>(&raw(edges[i]));if(!edge)throw 纯失败{纯概念状态::内部不一致};material.类型登记关系[i]=*edge;}
        const auto*format=std::get_if<L1所有者范围值事实>(&raw(value));if(!format)throw 纯失败{纯概念状态::内部不一致};material.格式值=*format;
        out.首次材料=std::move(material);out.状态=replay?纯概念状态::精确重复:纯概念状态::已创建;out.发布=纯概念发布状态::确认发布;
        if(!out.成功(r))throw 纯失败{纯概念状态::内部不一致};
    } catch(const 纯失败&e) {out.状态=entered?纯概念状态::已可能发布:e.状态;out.发布=entered?纯概念发布状态::可能发布:纯概念发布状态::确认未发布;out.首次发布H.reset();out.交付.reset();out.首次材料.reset();}
      catch(const std::bad_alloc&) {out.状态=entered?纯概念状态::已可能发布:纯概念状态::资源失败;out.发布=entered?纯概念发布状态::可能发布:纯概念发布状态::确认未发布;out.首次发布H.reset();out.交付.reset();out.首次材料.reset();}
      catch(...) {out.状态=entered?纯概念状态::已可能发布:纯概念状态::内部不一致;out.发布=entered?纯概念发布状态::可能发布:纯概念发布状态::确认未发布;out.首次发布H.reset();out.交付.reset();out.首次材料.reset();}
    return out;
}
    ...
    try {
        ...
        for (std::size_t index=0; index<pure.size(); ++index) {
            ...
            if (read.状态 != L1所有者范围读取状态::成功 || read.读取事实代次 != r.G0 ||
                !node || node->退出事实代次 || node->写入所有者 != port.所有者身份() ||
                node->种类 != (representation ? 节点种类::属性类型 : 节点种类::普通) ||
                node->属性类型表示 != representation) {
        ...
        for (const auto& e : registrations.关系组) {
            if (e.源节点 != r.纯概念结构.格式锚点 || e.关系类型节点 != t.类型登记 ||
                ...
                e.目标节点 != pure[static_cast<std::size_t>(e.角色或顺序+1)] || e.退出事实代次) {
        ...
        auto rawNode=[&](稳定编码 id) {
            const auto read=l1.读取所有者范围历史事实({L1所有者范围CRUD合同版本,id});
            if (read.状态 != L1所有者范围读取状态::成功 || read.读取事实代次 != out.Gread || !read.事实 || read.物理清理墓碑 || read.物理清理事实代次)
            ...
            if (!node || node->创建事实代次 != *out.首次发布H ||
                (node->退出事实代次 && *node->退出事实代次 <= *out.首次发布H))
                throw 纯失败{纯概念状态::已可能发布};
            return *node;
        };
        const auto an=rawNode(anchor), rtn=rawNode(relationType);
        const auto edgeRead=l1.读取所有者范围历史关系组(
            {L1所有者范围CRUD合同版本,L1所有者范围关系端点方向::源,anchor,t.类型登记,*out.首次发布H});
        const auto valueRead=l1.读取所有者范围历史属性值组(
            {L1所有者范围CRUD合同版本,anchor,*out.首次发布H});
        if (edgeRead.状态 != L1所有者范围读取状态::成功 || edgeRead.读取事实代次 != out.Gread ||
            edgeRead.历史截止事实代次 != *out.首次发布H || edgeRead.关系组.size() != 1 ||
            valueRead.状态 != L1所有者范围读取状态::成功 || valueRead.读取事实代次 != out.Gread ||
            valueRead.历史截止事实代次 != *out.首次发布H || valueRead.属性值组.size() != 1)
            throw 纯失败{纯概念状态::已可能发布};
        const auto& edge=edgeRead.关系组.front(); const auto& value=valueRead.属性值组.front();
        if (edge.编码 != registration || value.编码 != format || an.写入所有者 != port.所有者身份() || rtn.写入所有者 != port.所有者身份() ||
            an.种类 != 节点种类::普通 || an.属性类型表示 || an.退出事实代次 ||
            rtn.种类 != 节点种类::普通 || rtn.属性类型表示 || rtn.退出事实代次 ||
            edge.写入所有者 != port.所有者身份() || edge.源节点 != anchor || edge.目标节点 != relationType || edge.关系类型节点 != t.类型登记 || edge.角色或顺序 != 1 || edge.创建事实代次 != *out.首次发布H || edge.退出事实代次 ||
            value.写入所有者 != port.所有者身份() || value.所属节点 != anchor || value.来源节点 != anchor || value.属性类型节点 != t.格式版本 || value.创建事实代次 != *out.首次发布H || value.退出事实代次 ||
...
概念树类数据服务::概念树类数据服务(
    ...
    const 完整存在概念结构交付_v1 &completeLayout)
    : l1_(l1), features_(features), existences_(existences), values_(values),
      port_(std::move(port)), scenes_(&scenes), pure_layout_(layout), feature_birth_layout_(birthLayout),
      complete_definition_layout_(completeLayout) {
    if (!port_.有效() || !port_.绑定于(l1_) || !features_.绑定于(l1_) ||
        !existences_.绑定于(l1_) || !values_.绑定于(l1_) || !scenes.绑定于(l1_) ||
        layout.版本 != 2)
        throw std::invalid_argument("pure concept services");
    std::array<稳定编码, 15> ids{layout.格式锚点, layout.概念族锚点,
        layout.类型.类型登记, layout.类型.概念族成员, layout.类型.格式版本,
        layout.类型.概念类别, layout.类型.定义成员, layout.类型.定义种类,
        layout.类型.定义特征类型, layout.类型.定义模板, layout.类型.I64域,
        layout.类型.通用规则, layout.类型.直接上位, layout.类型.生命周期,
        layout.类型.存在概念使用};
    std::set<std::uint64_t> unique;
    for (const auto id : ids)
        if (!有效(id) || !unique.insert(id.值).second)
            throw std::invalid_argument("pure concept layout");
    const auto g=当前代次();
    for(std::size_t i=0;i<ids.size();++i) {
        ...
        if (key == 5 || key == 6 || key == 8 || key == 12 || key == 14)
            ...
            representation = L1所有者范围值表示种类::U64组;
        if(n.种类!=(representation ? 节点种类::属性类型 : 节点种类::普通)||
           n.属性类型表示!=representation||n.写入所有者!=port_.所有者身份())
            throw std::invalid_argument("pure concept layout facts");
    }
    const auto registrations=关系(layout.格式锚点,layout.类型.类型登记,false,g,g,13);
    if(registrations.size()!=13) throw std::invalid_argument("pure concept registrations");
    for(std::size_t i=0;i<registrations.size();++i)
        if(registrations[i].目标节点!=ids[i+2]||registrations[i].角色或顺序!=static_cast<std::int64_t>(i+1))
            throw std::invalid_argument("pure concept registration mapping");
    const auto attrs=属性(layout.格式锚点,g,g);
    if(attrs.size()!=1||唯一属性(attrs,layout.类型.格式版本).材料!=L1所有者范围原始值材料{std::int64_t{2}})
        throw std::invalid_argument("pure concept format value");
    if (birthLayout.格式 != 1 || !有效(birthLayout.锚点) ||
        !有效(birthLayout.F到FCv出生使用关系类型) ||
        birthLayout.锚点 == birthLayout.F到FCv出生使用关系类型 ||
        unique.contains(birthLayout.锚点.值) || unique.contains(birthLayout.F到FCv出生使用关系类型.值))
        throw std::invalid_argument("feature birth layout");
    const auto anchor=节点(birthLayout.锚点,g,g);
    const auto relationType=节点(birthLayout.F到FCv出生使用关系类型,g,g);
    if (anchor.种类 != 节点种类::普通 || anchor.属性类型表示 ||
        relationType.种类 != 节点种类::普通 || relationType.属性类型表示 ||
        anchor.写入所有者 != port_.所有者身份() || relationType.写入所有者 != port_.所有者身份())
        throw std::invalid_argument("feature birth layout facts");
    const auto typeRegistration=关系(birthLayout.锚点,layout.类型.类型登记,false,g,g,1);
    if (typeRegistration.size()!=1 || typeRegistration.front().目标节点 != birthLayout.F到FCv出生使用关系类型 ||
        typeRegistration.front().角色或顺序 != 1)
        throw std::invalid_argument("feature birth registration");
    const auto birthAttrs=属性(birthLayout.锚点,g,g);
    if (birthAttrs.size()!=1 || 唯一属性(birthAttrs,layout.类型.格式版本).材料 != L1所有者范围原始值材料{std::int64_t{1}})
        throw std::invalid_argument("feature birth format value");
    if(!完整定义布局独立(completeLayout,layout))
        throw std::invalid_argument("complete definition layout");
    const auto&ct=completeLayout.类型;
    const std::array<稳定编码,9> complete{completeLayout.格式锚点,ct.完整定义成员,ct.完整定义自身特征,
      ct.完整定义组成项,ct.完整定义组成子概念,ct.完整定义关系项,ct.完整定义关系参与角色,
      ct.组成角色属性,ct.关系类型属性};
    for(std::size_t i=0;i<complete.size();++i) {
        const auto n=节点(complete[i],g,g); const bool attribute=i>=7;
        const auto rep=attribute?std::optional<L1所有者范围值表示种类>{L1所有者范围值表示种类::I64}:std::nullopt;
        if(n.写入所有者!=port_.所有者身份()||n.种类!=(attribute?节点种类::属性类型:节点种类::普通)||n.属性类型表示!=rep)
            throw std::invalid_argument("complete definition layout facts");
    }
    const auto registered=关系(completeLayout.格式锚点,layout.类型.类型登记,false,g,g,8);
    if(registered.size()!=8)throw std::invalid_argument("complete definition registration");
    for(std::size_t i=0;i<registered.size();++i)
        if(registered[i].目标节点!=complete[i+1]||registered[i].角色或顺序!=static_cast<std::int64_t>(i+1))
            throw std::invalid_argument("complete definition registration mapping");
    const auto completeAttrs=属性(completeLayout.格式锚点,g,g);
    if(completeAttrs.size()!=1||唯一属性(completeAttrs,layout.类型.格式版本).材料!=L1所有者范围原始值材料{std::int64_t{1}})
        throw std::invalid_argument("complete definition format value");
}

纯概念定义 概念树类数据服务::规范化纯概念定义(
    const 纯概念定义 &input, std::uint64_t g, std::uint64_t h,
    const 概念树预算 &budget) const {
    if (!pure_layout_ || !g || !h || h > g || !budget.最大概念数 ||
    ...
    if (const auto *fc = std::get_if<纯I64特征概念定义>(&input)) {
        ...
        const auto normalized = 特征结果(features_.规范化I64特征域(
            {{1,g,h,{fc->特征类型.值}},raw}));
        if (normalized.Gread != g || normalized.H != h || normalized.数据.区间.empty())
            throw 纯失败{纯概念状态::定义不相容};
    ...
    if (const auto *ec = std::get_if<纯合取存在概念定义>(&input)) {
        ...
        for (const auto id : out.特征模板组) {
            const auto fact = 读取纯概念内部(id,g,h,budget);
    ...
    if(const auto*complete=std::get_if<完整存在概念定义>(&input))
        return 规范化完整定义内部(*complete,g,h,budget);
...
完整存在概念定义 概念树类数据服务::规范化完整定义内部(
    const 完整存在概念定义& input,std::uint64_t g,std::uint64_t h,
    const 概念树预算& budget) const {
    if(!pure_layout_||!complete_definition_layout_)throw 纯失败{纯概念状态::旧格式不支持};
    if(!g||!h||h>g||!budget.最大概念数||!budget.最大关系数||!budget.最大特征属性数||
       input.完整性!=存在概念完整性声明::本次定义范围已知事实完整)
        throw 纯失败{纯概念状态::入口拒绝};
    auto out=input;
    std::sort(out.自身特征.begin(),out.自身特征.end(),[](const auto&a,const auto&b){return a.特征概念.值.值<b.特征概念.值.值;});
    if(std::adjacent_find(out.自身特征.begin(),out.自身特征.end(),[](const auto&a,const auto&b){return a.特征概念==b.特征概念;})!=out.自身特征.end())
        throw 纯失败{纯概念状态::定义不相容};
    for(const auto&item:out.自身特征) {const auto fact=读取纯概念内部(item.特征概念,g,h,budget);
        if(fact.类别!=相关概念类别::特征||fact.治理状态==概念树生命周期状态::退役)throw 纯失败{纯概念状态::定义不相容};}
    std::sort(out.组成.begin(),out.组成.end(),[](const auto&a,const auto&b){
        if(a.子存在概念!=b.子存在概念)return a.子存在概念.值.值<b.子存在概念.值.值;
        return a.组成角色.值<b.组成角色.值;});
    if(std::adjacent_find(out.组成.begin(),out.组成.end())!=out.组成.end())throw 纯失败{纯概念状态::定义不相容};
    for(const auto&item:out.组成) {if(!有效(item.子存在概念.值)||!有效(item.组成角色))throw 纯失败{纯概念状态::定义不相容};
        const auto fact=读取纯概念内部(item.子存在概念,g,h,budget);if(fact.类别!=相关概念类别::存在||fact.治理状态==概念树生命周期状态::退役)throw 纯失败{纯概念状态::定义不相容};}
    for(auto&item:out.关系) {
        if(!有效(item.关系类型)||item.有序参与角色.empty())throw 纯失败{纯概念状态::定义不相容};
        std::sort(item.有序参与角色.begin(),item.有序参与角色.end(),[](const auto&a,const auto&b){return a.顺序<b.顺序;});
        for(std::size_t i=0;i<item.有序参与角色.size();++i) {const auto&role=item.有序参与角色[i];
            if(role.顺序!=i+1||!有效(role.概念约束.值))throw 纯失败{纯概念状态::定义不相容};
            const auto fact=读取纯概念内部(role.概念约束,g,h,budget);if(fact.类别!=相关概念类别::存在||fact.治理状态==概念树生命周期状态::退役)throw 纯失败{纯概念状态::定义不相容};}
    }
    std::sort(out.关系.begin(),out.关系.end(),[](const auto&a,const auto&b){
        if(a.关系类型!=b.关系类型)return a.关系类型.值<b.关系类型.值;
        return std::lexicographical_compare(a.有序参与角色.begin(),a.有序参与角色.end(),b.有序参与角色.begin(),b.有序参与角色.end(),
            [](const auto&x,const auto&y){return x.顺序!=y.顺序?x.顺序<y.顺序:x.概念约束.值.值<y.概念约束.值.值;});});
    if(std::adjacent_find(out.关系.begin(),out.关系.end())!=out.关系.end())throw 纯失败{纯概念状态::定义不相容};
    return out;
}
...
纯概念事实 概念树类数据服务::读取纯概念内部(
    ...
    const 概念树预算 &budget) const {
    if (!pure_layout_) throw 纯失败{纯概念状态::旧格式不支持};
    if (!有效(id.值) || !g || !h || h > g || !budget.最大关系数 ||
    ...
    const auto defs=关系(id.值,t.定义成员,false,g,h,1);
    ...
    out.定义关系组.push_back({defs.front().编码,defs.front().源节点,
        ...
        纯概念定义关系种类::定义成员,1,投影纯生命(defs.front(),g)});
    const auto dv=属性(out.定义记录,g,h);
    const auto kind=std::get<std::int64_t>(唯一属性(dv,t.定义种类).材料);
    if(kind==1) {
        if(out.类别!=相关概念类别::特征||dv.size()!=2)
            throw 纯失败{纯概念状态::类别冲突};
        const auto ft=关系(out.定义记录,t.定义特征类型,false,g,h,1);
        ...
        const auto normalized=规范化纯概念定义(def,g,h,budget);
        if(normalized!=纯概念定义{def})
            throw 纯失败{纯概念状态::内部不一致};
        out.定义=std::move(def);
        out.定义关系组.push_back({ft.front().编码,ft.front().源节点,
            ft.front().目标节点,ft.front().关系类型节点,
            纯概念定义关系种类::定义特征类型,1,投影纯生命(ft.front(),g)});
    } else if(kind==2) {
        if(out.类别!=相关概念类别::存在||dv.size()!=1)
            throw 纯失败{纯概念状态::类别冲突};
        const auto edges=关系(out.定义记录,t.定义模板,false,g,h,budget.最大关系数);
        ...
        for(std::size_t i=0;i<edges.size();++i) {
            ...
            out.定义关系组.push_back({edges[i].编码,edges[i].源节点,
                ...
                纯概念定义关系种类::定义模板,i+1,投影纯生命(edges[i],g)});
        }
        ...
        if(!complete_definition_layout_||out.类别!=相关概念类别::存在||dv.size()!=1)
            throw 纯失败{纯概念状态::旧格式不支持};
        const auto&ct=complete_definition_layout_->类型; 完整存在概念定义 def;
        def.完整性=存在概念完整性声明::本次定义范围已知事实完整;
        const auto own=关系(out.定义记录,ct.完整定义自身特征,false,g,h,budget.最大关系数);
        for(const auto&e:own) { if(e.角色或顺序<1)throw 纯失败{纯概念状态::内部不一致};
            def.自身特征.push_back({概念树概念身份{e.目标节点}}); }
        const auto composition=关系(out.定义记录,ct.完整定义组成项,false,g,h,budget.最大关系数);
        for(const auto&e:composition) {
            const auto child=关系(e.目标节点,ct.完整定义组成子概念,false,g,h,1);
            const auto values=属性(e.目标节点,g,h);
            if(child.size()!=1||values.size()!=1)throw 纯失败{纯概念状态::内部不一致};
            const auto role=std::get<std::int64_t>(唯一属性(values,ct.组成角色属性).材料);
            const auto roleCode=std::bit_cast<std::uint64_t>(role);
            if(!roleCode)throw 纯失败{纯概念状态::内部不一致};
            def.组成.push_back({概念树概念身份{child.front().目标节点},稳定编码{roleCode}});
        }
        const auto relations=关系(out.定义记录,ct.完整定义关系项,false,g,h,budget.最大关系数);
        for(const auto&e:relations) {
            const auto values=属性(e.目标节点,g,h);
            if(values.size()!=1)throw 纯失败{纯概念状态::内部不一致};
            const auto type=std::get<std::int64_t>(唯一属性(values,ct.关系类型属性).材料);
            const auto typeCode=std::bit_cast<std::uint64_t>(type);
            if(!typeCode)throw 纯失败{纯概念状态::内部不一致};
            完整存在概念关系约束 item{稳定编码{typeCode}};
            const auto roles=关系(e.目标节点,ct.完整定义关系参与角色,false,g,h,budget.最大关系数);
            for(const auto&r:roles) { if(r.角色或顺序<1)throw 纯失败{纯概念状态::内部不一致};
                item.有序参与角色.push_back({static_cast<std::uint64_t>(r.角色或顺序),概念树概念身份{r.目标节点}}); }
            def.关系.push_back(std::move(item));
        }
        out.定义=规范化完整定义内部(def,g,h,budget);
    } else throw 纯失败{纯概念状态::定义不支持};
    const auto parents=关系(id.值,t.直接上位,true,g,h,budget.最大关系数);
    std::set<std::uint64_t> seen;
    for(const auto &e:parents) {
        if(e.角色或顺序!=1||e.源节点==id.值||!seen.insert(e.源节点.值).second)
            throw 纯失败{纯概念状态::上位成环};
        out.直接上位.push_back({e.编码,概念树概念身份{e.源节点},id,投影纯生命(e,g)});
    }
    return out;
}

完整存在概念事实 概念树类数据服务::读取完整定义内部(
    概念树概念身份 id,std::uint64_t g,std::uint64_t h,const 概念树预算& budget) const {
    if(!complete_definition_layout_)throw 纯失败{纯概念状态::旧格式不支持};
    const auto base=读取纯概念内部(id,g,h,budget);
    const auto*definition=std::get_if<完整存在概念定义>(&base.定义);
    if(!definition||base.类别!=相关概念类别::存在)throw 纯失败{纯概念状态::类别冲突};
    完整存在概念事实 out;out.概念=base.概念;out.类别=base.类别;out.定义记录=base.定义记录;
    out.定义记录生命周期=base.定义记录生命周期;out.治理状态=base.治理状态;out.生命周期值事实=base.生命周期值事实;
    out.生命周期值生命周期=base.生命周期值生命周期;out.生命周期=base.生命周期;out.定义=*definition;
    const auto&ct=complete_definition_layout_->类型;
    const auto own=关系(base.定义记录,ct.完整定义自身特征,false,g,h,budget.最大关系数);
    for(const auto&e:own)out.自身特征关系组.push_back({e.编码,e.源节点,e.目标节点,e.关系类型节点,
        纯概念定义关系种类::定义模板,static_cast<std::uint64_t>(e.角色或顺序),投影纯生命(e,g)});
    const auto composition=关系(base.定义记录,ct.完整定义组成项,false,g,h,budget.最大关系数);
    for(const auto&e:composition) {
        const auto child=关系(e.目标节点,ct.完整定义组成子概念,false,g,h,1);
        const auto values=属性(e.目标节点,g,h);if(child.size()!=1||values.size()!=1)throw 纯失败{纯概念状态::内部不一致};
        const auto&role=唯一属性(values,ct.组成角色属性);
        out.组成项组.push_back({e.目标节点,投影纯生命(节点(e.目标节点,g,h),g),
          {child.front().编码,child.front().源节点,child.front().目标节点,child.front().关系类型节点,纯概念定义关系种类::定义模板,1,投影纯生命(child.front(),g)},
          role.编码,投影纯生命(role,g),稳定编码{std::bit_cast<std::uint64_t>(std::get<std::int64_t>(role.材料))}});
    }
    const auto relations=关系(base.定义记录,ct.完整定义关系项,false,g,h,budget.最大关系数);
    for(const auto&e:relations) {
        const auto values=属性(e.目标节点,g,h);if(values.size()!=1)throw 纯失败{纯概念状态::内部不一致};
        const auto&type=唯一属性(values,ct.关系类型属性);完整存在概念关系项事实 item;
        item.节点=e.目标节点;item.节点生命周期=投影纯生命(节点(e.目标节点,g,h),g);item.关系类型值事实=type.编码;
        item.关系类型值生命周期=投影纯生命(type,g);item.关系类型=稳定编码{std::bit_cast<std::uint64_t>(std::get<std::int64_t>(type.材料))};
        const auto roles=关系(e.目标节点,ct.完整定义关系参与角色,false,g,h,budget.最大关系数);
        for(const auto&role:roles)item.参与角色关系组.push_back({role.编码,role.源节点,role.目标节点,role.关系类型节点,
          纯概念定义关系种类::定义模板,static_cast<std::uint64_t>(role.角色或顺序),投影纯生命(role,g)});
        out.关系项组.push_back(std::move(item));
    }
    return out;
}

完整存在概念规范化结果 概念树类数据服务::规范化完整存在概念定义(
    const 完整存在概念规范化请求& r) const noexcept {
    完整存在概念规范化结果 out;out.Gread=r.Gread;
    try {if(r.版本!=1||!r.Gread||!r.H||r.H>r.Gread)throw 纯失败{纯概念状态::入口拒绝};
        守卫代次(r.Gread);out.定义=规范化完整定义内部(r.定义,r.Gread,r.H,r.预算);out.H=r.H;out.状态=完整存在概念状态::已规范化;守卫代次(r.Gread);
    }catch(const 纯失败&e){out.状态=e.状态==纯概念状态::旧格式不支持?完整存在概念状态::旧格式不支持:(e.状态==纯概念状态::定义不相容?完整存在概念状态::定义不相容:完整存在概念状态::入口拒绝);out.H=0;out.定义.reset();}
     catch(...){out.状态=完整存在概念状态::内部不一致;out.H=0;out.定义.reset();}return out;
}

完整存在概念读取结果 概念树类数据服务::读取完整存在概念(
    const 完整存在概念读取请求& r) const noexcept {
    完整存在概念读取结果 out;out.Gread=r.Gread;
    try {if(!complete_definition_layout_)throw 纯失败{纯概念状态::旧格式不支持};
        if(r.版本!=1||!r.Gread||!r.H||r.H>r.Gread||!有效(r.概念.值))throw 纯失败{纯概念状态::入口拒绝};
        守卫代次(r.Gread);out.事实=读取完整定义内部(r.概念,r.Gread,r.H,r.预算);out.H=r.H;out.状态=完整存在概念状态::已读取;守卫代次(r.Gread);
    }catch(const 纯失败&e){out.状态=e.状态==纯概念状态::旧格式不支持?完整存在概念状态::旧格式不支持:
      (e.状态==纯概念状态::类别冲突?完整存在概念状态::类别冲突:完整存在概念状态::内部不一致);out.H=0;out.事实.reset();}
     catch(...){out.状态=完整存在概念状态::内部不一致;out.H=0;out.事实.reset();}return out;
}

完整存在概念查询结果 概念树类数据服务::精确查询完整存在概念(
    const 完整存在概念查询请求& r) const noexcept {
    完整存在概念查询结果 out;out.Gread=r.Gread;
    try {if(!complete_definition_layout_)throw 纯失败{纯概念状态::旧格式不支持};
        if(r.版本!=1||!r.Gread||!r.H||r.H>r.Gread)throw 纯失败{纯概念状态::入口拒绝};
        const auto normalized=规范化完整定义内部(r.定义,r.Gread,r.H,r.预算);
        const auto pure=精确查询纯概念({2,r.Gread,r.H,纯概念定义{normalized},r.预算});
        if(pure.状态==纯概念状态::未找到){out.H=r.H;out.状态=完整存在概念状态::未找到;return out;}
        if(!pure.事实||pure.状态!=纯概念状态::已读取)throw 纯失败{pure.状态};
        out.事实=读取完整定义内部(pure.事实->概念,r.Gread,r.H,r.预算);out.H=r.H;out.状态=完整存在概念状态::已读取;
    }catch(const 纯失败&e){out.状态=e.状态==纯概念状态::旧格式不支持?完整存在概念状态::旧格式不支持:
      (e.状态==纯概念状态::定义不相容?完整存在概念状态::定义不相容:完整存在概念状态::内部不一致);out.H=0;out.事实.reset();}
     catch(...){out.状态=完整存在概念状态::内部不一致;out.H=0;out.事实.reset();}return out;
}

完整存在概念写入结果 概念树类数据服务::创建或复用完整存在概念(
    const 完整存在概念创建请求& r) noexcept {
    完整存在概念写入结果 out;out.原请求=r;
    try {if(!complete_definition_layout_||r.版本!=1||!r.G0||r.G0==UINT64_MAX||!有效(r.幂等身份))throw 纯失败{pure_layout_?纯概念状态::入口拒绝:纯概念状态::旧格式不支持};
        const auto normalized=规范化完整定义内部(r.定义,r.G0,r.G0,r.预算);
        const auto pure=创建或复用纯概念({2,r.G0,r.幂等身份,纯概念定义{normalized},概念初始组织指定::显式顶层,{},r.预算});
        out.Gread=pure.Gread;out.H=pure.H;out.首次发布H=pure.首次发布H;
        if((pure.状态!=纯概念状态::已创建&&pure.状态!=纯概念状态::精确重复)||!pure.事实)throw 纯失败{pure.状态};
        out.事实=读取完整定义内部(pure.事实->概念,pure.Gread,pure.H,r.预算);
        out.状态=pure.状态==纯概念状态::已创建?完整存在概念状态::已创建:完整存在概念状态::精确重复;
        out.发布=pure.发布==纯概念发布状态::确认发布?完整存在概念发布状态::确认发布:完整存在概念发布状态::确认未发布;
    }catch(const 纯失败&e){out.状态=e.状态==纯概念状态::幂等冲突?完整存在概念状态::幂等冲突:
      (e.状态==纯概念状态::事实代次漂移?完整存在概念状态::事实代次漂移:
      (e.状态==纯概念状态::已可能发布?完整存在概念状态::已可能发布:完整存在概念状态::内部不一致));out.发布=e.状态==纯概念状态::已可能发布?完整存在概念发布状态::可能发布:完整存在概念发布状态::确认未发布;out.H=0;out.首次发布H.reset();out.事实.reset();}
     catch(...){out.状态=完整存在概念状态::内部不一致;out.发布=完整存在概念发布状态::确认未发布;out.H=0;out.首次发布H.reset();out.事实.reset();}return out;
}
...
纯概念读取结果 概念树类数据服务::读取纯概念(
    ...
    try {
        if(r.版本!=2||!r.Gread||!r.H||r.H>r.Gread||!有效(r.概念.值))
            throw 纯失败{纯概念状态::入口拒绝};
        守卫代次(r.Gread);
        out.事实=读取纯概念内部(r.概念,r.Gread,r.H,r.预算);
        out.H=r.H; out.状态=纯概念状态::已读取;
        守卫代次(r.Gread);
    } catch(const 纯失败&e){out.状态=e.状态;out.H=0;out.事实.reset();}
      catch(const 失败&e){
        switch(e.状态) {
...
纯概念查询结果 概念树类数据服务::精确查询纯概念(
    ...
    try {
        if(r.版本!=2||!r.Gread||!r.H||r.H>r.Gread)
            throw 纯失败{纯概念状态::入口拒绝};
        守卫代次(r.Gread);
        const auto expected=规范化纯概念定义(r.定义,r.Gread,r.H,r.预算);
        const auto members=关系(pure_layout_->概念族锚点,pure_layout_->类型.概念族成员,
                                false,r.Gread,r.H,r.预算.最大概念数);
        ...
        if(!found) {
            out.H=r.H; out.状态=纯概念状态::未找到;
            守卫代次(r.Gread); return out;
        }
        if(found->治理状态==概念树生命周期状态::退役)
            throw 纯失败{纯概念状态::概念已退役};
        out.事实=std::move(found);out.H=r.H;out.状态=纯概念状态::已读取;
        守卫代次(r.Gread);
    } catch(const 纯失败&e){out.状态=e.状态;out.H=0;out.事实.reset();}
      catch(const 失败&e){
        switch(e.状态) {
...
I64特征概念组织读取结果 概念树类数据服务::读取当前I64特征概念(
    ...
    try {
        if(r.版本!=2||!r.Gread||!r.H||r.H>r.Gread||!有效(r.FT.编码)||
        ...
        if(std::adjacent_find(out.概念组.begin(),out.概念组.end(),[](const auto&a,const auto&b){return a.概念==b.概念;})!=out.概念组.end())
            throw 纯失败{纯概念状态::内部不一致};
        out.H=r.H;out.状态=纯概念状态::已读取;守卫代次(r.Gread);
    }catch(const 纯失败&e){out.状态=e.状态;out.H=0;out.概念组.clear();}
     catch(const 失败&e){
        switch(e.状态){case S::未找到:out.状态=纯概念状态::未找到;break;case S::目标已退出:out.状态=纯概念状态::目标已退出;break;
        case S::事实代次漂移:out.状态=纯概念状态::事实代次漂移;break;case S::数量预算不足:out.状态=纯概念状态::数量预算不足;break;
        case S::历史材料不可用:out.状态=纯概念状态::历史材料不可用;break;case S::资源失败:out.状态=纯概念状态::资源失败;break;
        default:out.状态=纯概念状态::内部不一致;break;}out.H=0;out.概念组.clear();}
     catch(const std::bad_alloc&){out.状态=纯概念状态::资源失败;out.H=0;out.概念组.clear();}
     catch(...){out.状态=纯概念状态::内部不一致;out.H=0;out.概念组.clear();}
    return out;
}
    ...
    if(const auto *d=std::get_if<纯I64特征概念定义>(&definition)) {
        ...
        for(std::size_t i=0;i<d->特征模板组.size();++i)
            edge(Ref{Key{2}},Ref{d->特征模板组[i].值},t.定义模板,
                 static_cast<std::int64_t>(i+1));
    } else if(const auto*d=std::get_if<完整存在概念定义>(&definition)) {
        value(Ref{Key{2}},t.定义种类,std::int64_t{4});
        const auto&ct=complete_definition_layout_->类型;
        std::uint32_t nodeKey=3;
        for(std::size_t i=0;i<d->自身特征.size();++i)
            edge(Ref{Key{2}},Ref{d->自身特征[i].特征概念.值},ct.完整定义自身特征,static_cast<std::int64_t>(i+1));
        for(std::size_t i=0;i<d->组成.size();++i) {
            const Key item{nodeKey++}; ws.节点.push_back({item,节点种类::普通,std::nullopt});
            edge(Ref{Key{2}},Ref{item},ct.完整定义组成项,static_cast<std::int64_t>(i+1));
            edge(Ref{item},Ref{d->组成[i].子存在概念.值},ct.完整定义组成子概念);
            value(Ref{item},ct.组成角色属性,std::bit_cast<std::int64_t>(d->组成[i].组成角色.值));
        }
        for(std::size_t i=0;i<d->关系.size();++i) {
            const Key item{nodeKey++}; ws.节点.push_back({item,节点种类::普通,std::nullopt});
            edge(Ref{Key{2}},Ref{item},ct.完整定义关系项,static_cast<std::int64_t>(i+1));
            value(Ref{item},ct.关系类型属性,std::bit_cast<std::int64_t>(d->关系[i].关系类型.值));
            for(const auto&role:d->关系[i].有序参与角色)
                edge(Ref{item},Ref{role.概念约束.值},ct.完整定义关系参与角色,static_cast<std::int64_t>(role.顺序));
        }
    } else {
...
纯概念写入结果 概念树类数据服务::创建或复用纯概念(
    ...
    try {
        ...
        const auto definition=规范化纯概念定义(r.定义,current.Gread,r.G0,r.预算);
        auto parents=r.直接上位;
        std::sort(parents.begin(),parents.end(),[](auto a,auto b){return a.值.值<b.值.值;});
        if(std::adjacent_find(parents.begin(),parents.end())!=parents.end()||
        ...
        if(first.状态==L1所有者范围读取状态::成功) {
            ...
            if(restored.成功(rr)) {
                out.H=*restored.首次发布H;out.事实=restored.事实;
                ...
                out.状态=restored.状态==纯概念恢复状态::幂等冲突 ? 纯概念状态::幂等冲突 :
                    (restored.状态==纯概念恢复状态::已可能发布 ? 纯概念状态::已可能发布 :
                     (restored.状态==纯概念恢复状态::资源失败 ? 纯概念状态::资源失败 :
                      (restored.状态==纯概念恢复状态::目标已退出 ? 纯概念状态::目标已退出 :
                       (restored.状态==纯概念恢复状态::事实代次漂移 ? 纯概念状态::事实代次漂移 :
                        (restored.状态==纯概念恢复状态::数量预算不足 ? 纯概念状态::数量预算不足 :
                         (restored.状态==纯概念恢复状态::历史材料不可用 ? 纯概念状态::历史材料不可用 :
                          纯概念状态::内部不一致))))));
        ...
        const auto found=精确查询纯概念({2,r.G0,r.G0,definition,r.预算});
        if(found.成功({2,r.G0,r.G0,definition,r.预算})) {
            ...
            out.Gread=r.G0;out.H=r.G0;out.事实=found.事实;return out;
        }
        if(!found.确认未找到({2,r.G0,r.G0,definition,r.预算})) throw 纯失败{found.状态};
        const auto category=std::holds_alternative<纯I64特征概念定义>(definition) ?
            相关概念类别::特征 : 相关概念类别::存在;
        for(const auto parent:parents) {
            const auto pf=读取纯概念内部(parent,r.G0,r.G0,r.预算);
        ...
        out.Gread=tail.Gread;out.H=saved.事实代次;
        out.事实=读取纯概念内部(概念树概念身份{concept_id},out.Gread,out.H,r.预算);
        out.状态=纯概念状态::已创建;out.发布=纯概念发布状态::确认发布;
    } catch(const 纯失败&e){
        out.状态=entered ? 纯概念状态::已可能发布 : e.状态;
        out.发布=entered ? 纯概念发布状态::可能发布 : 纯概念发布状态::确认未发布;
        if(out.状态!=纯概念状态::已可能发布){out.首次发布H.reset();out.H=0;out.事实.reset();}
    } catch(const 失败&e) {
        switch(e.状态) {
        case S::入口拒绝:out.状态=纯概念状态::入口拒绝;break;
        case S::未找到:out.状态=纯概念状态::未找到;break;
        ...
        if(out.状态!=纯概念状态::已可能发布) {
            out.首次发布H.reset();out.H=0;out.事实.reset();
        }
...
纯概念创建恢复结果 概念树类数据服务::读取纯概念创建首次结果(
    ...
    try {
        ...
        if(std::adjacent_find(parents.begin(),parents.end())!=parents.end()||
           ...
           (r.组织==概念初始组织指定::具名上位&&parents.empty()))
            throw 纯失败{纯概念状态::组织冲突};
        const auto definition=规范化纯概念定义(r.定义,r.Gread,g0,r.预算);
        ...
        case 纯概念状态::历史材料不可用:out.状态=纯概念恢复状态::历史材料不可用;break;
        case 纯概念状态::已可能发布:out.状态=纯概念恢复状态::已可能发布;break;
        case 纯概念状态::资源失败:out.状态=纯概念恢复状态::资源失败;break;
        case 纯概念状态::目标已退出:out.状态=纯概念恢复状态::目标已退出;break;
        ...
        else if(e.状态==S::历史材料不可用) out.状态=纯概念恢复状态::历史材料不可用;
        else if(e.状态==S::目标已退出) out.状态=纯概念恢复状态::目标已退出;
...
void 概念树类数据服务::核验纯概念无保护引用(
    ...
    const auto member=关系(fact.概念.值,pure_layout_->类型.概念族成员,true,g,g,1);
    ...
    for(const auto&v:属性(fact.概念.值,g,g))add(v.编码);
    for(const auto&v:属性(fact.定义记录,g,g))add(v.编码);

    auto mapFailure=[](L1节点当前引用读取状态 s) {
        switch(s) {
        case L1节点当前引用读取状态::入口拒绝:return 纯概念状态::入口拒绝;
        case L1节点当前引用读取状态::未找到:return 纯概念状态::未找到;
        case L1节点当前引用读取状态::已退出:return 纯概念状态::目标已退出;
        case L1节点当前引用读取状态::事实代次漂移:return 纯概念状态::事实代次漂移;
        case L1节点当前引用读取状态::数量预算不足:return 纯概念状态::数量预算不足;
        case L1节点当前引用读取状态::资源失败:return 纯概念状态::资源失败;
    ...
    for(const auto node:{fact.概念.值,fact.定义记录}) {
        const L1节点当前引用读取请求 request{
            L1节点当前引用读取合同版本,node,g,limit};
        const auto read=l1_.读取节点全部当前引用(request);
        if(!read.成功(request))throw 纯失败{mapFailure(read.状态)};
...
纯概念生命周期结果 概念树类数据服务::迁移纯概念生命周期(
    ...
    try {
        ...
        const auto before=读取纯概念内部(r.概念,current.Gread,r.G0,r.预算);
        if(!replay&&before.生命周期.退出事实代次&&*before.生命周期.退出事实代次<=current.Gread)
            throw 纯失败{纯概念状态::目标已退出};
        ...
        if(!tail.成功()||tail.Gread<saved.事实代次)
            throw 纯失败{纯概念状态::已可能发布};
        knownPublished=true;
        out.Gread=tail.Gread;out.H=saved.事实代次;out.首次发布H=saved.事实代次;
        out.事实=读取纯概念内部(r.概念,out.Gread,out.H,r.预算);
        if(out.事实->治理状态!=r.目标治理状态||
           out.事实->生命周期值事实==r.预期生命周期值事实||
           out.事实->生命周期值生命周期.创建事实代次!=out.H||
...
纯概念退出结果 概念树类数据服务::退出无引用叶概念(
    ...
    try {
        ...
        for(const auto &x:属性(r.概念.值,current.Gread,r.G0))ws.退出事实.push_back(x.编码);
        for(const auto &x:属性(before.定义记录,current.Gread,r.G0))ws.退出事实.push_back(x.编码);
        const auto member=关系(r.概念.值,pure_layout_->类型.概念族成员,
                               true,current.Gread,r.G0,1);
        ...
        if(!tail.成功()||tail.Gread<saved.事实代次)
            throw 纯失败{纯概念状态::已可能发布};
        knownPublished=true;
        out.Gread=tail.Gread;out.H=saved.事实代次-1;out.首次发布H=saved.事实代次;
        out.历史事实=读取纯概念内部(r.概念,out.Gread,out.H,r.预算);
        const auto raw=原始事实(r.概念.值,out.Gread);
        const auto *node=std::get_if<N>(&raw);
        if(!node||node->退出事实代次!=saved.事实代次||
           out.历史事实->治理状态!=概念树生命周期状态::退役||
           out.历史事实->生命周期值事实!=r.预期生命周期值事实||
           out.历史事实->生命周期.退出事实代次!=saved.事实代次||
           out.历史事实->定义记录生命周期.退出事实代次!=saved.事实代次)
            throw 纯失败{纯概念状态::内部不一致};
        for(const auto &x:out.历史事实->定义关系组)
            if(x.生命周期.退出事实代次!=saved.事实代次)
                throw 纯失败{纯概念状态::内部不一致};
        out.状态=replay?纯概念状态::精确重复:纯概念状态::已退出;
        out.发布=纯概念发布状态::确认发布;
    } catch(const 纯失败&e) {
        out.状态=entered?纯概念状态::已可能发布:e.状态;
        out.发布=knownPublished?纯概念发布状态::确认发布:
...
存在概念使用读取结果 概念树类数据服务::读取存在概念使用(
    ...
    存在概念使用读取结果 out; out.Gread=r.Gread; out.H=r.H;
    try {
        if(!pure_layout_||r.版本!=2||!r.Gread||!r.H||r.H>r.Gread||
           !有效(r.E.值)||!r.预算.最大关系数||!r.预算.最大概念数)
            throw 纯失败{pure_layout_ ? 纯概念状态::入口拒绝 : 纯概念状态::旧格式不支持};
        核验外部节点(r.E.值,r.Gread,r.H);
        ...
        const auto fact=读取纯概念内部(概念树概念身份{edge.目标节点},r.Gread,r.H,r.预算);
        if(fact.类别!=相关概念类别::存在)
            throw 纯失败{纯概念状态::类别冲突};
        存在概念使用事实 use;
        use.关系=edge.编码; use.E=r.E.值;
        use.EC=概念树概念身份{edge.目标节点};
        use.生命周期={edge.创建事实代次,edge.退出事实代次};
        out.使用=use;
    ...
    try {
        if(!pure_layout_||r.版本!=2||!g||r.G0!=g||!有效(r.幂等身份)||
           ...
           !有效(r.EC.值)||!r.预算.最大关系数)
            throw 纯失败{pure_layout_ ? 纯概念状态::入口拒绝 : 纯概念状态::旧格式不支持};
        const auto definition=规范化纯概念定义(r.预期定义,g,g,r.预算);
        const auto fact=读取纯概念内部(r.EC,g,g,r.预算);
        ...
        case 纯概念状态::目标已退出:out.状态=已发布概念引用参与状态::目标已退出;break;
        ...
        e.状态==S::目标已退出 ? 已发布概念引用参与状态::目标已退出 : 已发布概念引用参与状态::内部不一致;out.写集.reset();}
...
原子I64特征出生使用读取结果 概念树类数据服务::读取原子I64出生使用(
    ...
    原子I64特征出生使用读取结果 out;out.Gread=r.Gread;out.H=r.H;
    auto map=[&](纯概念状态 s) {
        using A=原子I64特征出生使用读取状态;
        switch(s) { case 纯概念状态::未找到:return A::未找到;case 纯概念状态::目标已退出:
        case 纯概念状态::概念已退役:return A::目标已退出;case 纯概念状态::事实代次漂移:return A::事实代次漂移;
        case 纯概念状态::数量预算不足:return A::预算不足;case 纯概念状态::历史材料不可用:return A::历史材料不可用;
        case 纯概念状态::资源失败:return A::资源失败;default:return A::内部不一致; }
    };
    try {
        if(!原子I64结构已就绪()||r.版本!=1||!r.Gread||!r.H||r.H>r.Gread||!有效(r.F)||
        ...
        guard();
        const auto edges=l1_.读取所有者范围历史关系组({L1所有者范围CRUD合同版本,
            L1所有者范围关系端点方向::源,r.F,feature_birth_layout_->F到FCv出生使用关系类型,r.H});
        if(edges.状态!=L1所有者范围读取状态::成功||edges.读取事实代次!=r.Gread) {
            out.状态=edges.状态==L1所有者范围读取状态::事实代次漂移 ? 原子I64特征出生使用读取状态::事实代次漂移 :
                (edges.状态==L1所有者范围读取状态::资源失败 ? 原子I64特征出生使用读取状态::资源失败 :
                 (edges.状态==L1所有者范围读取状态::历史材料已清理 ? 原子I64特征出生使用读取状态::历史材料不可用 :
                  (edges.状态==L1所有者范围读取状态::已退出 ? 原子I64特征出生使用读取状态::目标已退出 : 原子I64特征出生使用读取状态::内部不一致)));return out;
        }
        ...
        if(edge.写入所有者!=port_.所有者身份()||edge.源节点!=r.F||edge.关系类型节点!=feature_birth_layout_->F到FCv出生使用关系类型||edge.角色或顺序!=1||edge.创建事实代次>r.H||edge.退出事实代次) {
            out.状态=原子I64特征出生使用读取状态::内部不一致;return out;
        }
        const auto fc=读取纯概念({2,r.Gread,r.H,概念树概念身份{edge.目标节点},
        ...
        if(fc.Gread!=r.Gread||fc.H!=r.H||fc.事实->类别!=相关概念类别::特征||!definition||
...
概念树类数据服务::准备原子I64出生片段(const 原子I64特征出生请求& r,std::uint64_t g) const {
    ...
    try {
        ...
        const auto fc=读取纯概念({2,g,r.G0,r.FCv,{r.概念读取预算.最大概念数,r.概念读取预算.最大关系数,0,0,0,r.概念读取预算.最大特征属性数,0,0}});
        if(fc.状态!=纯概念状态::已读取||!fc.事实) {
            switch(fc.状态) {case 纯概念状态::未找到:throw 原子I64特征出生状态::未找到;case 纯概念状态::目标已退出:case 纯概念状态::概念已退役:throw 原子I64特征出生状态::目标已退出;case 纯概念状态::事实代次漂移:throw 原子I64特征出生状态::事实代次漂移;case 纯概念状态::数量预算不足:throw 原子I64特征出生状态::预算不足;case 纯概念状态::历史材料不可用:throw 原子I64特征出生状态::历史材料不可用;case 纯概念状态::资源失败:throw 原子I64特征出生状态::资源失败;default:throw 原子I64特征出生状态::内部不一致;}
        }
        const auto* d=std::get_if<纯I64特征概念定义>(&fc.事实->定义);
        if(fc.Gread!=g||fc.H!=r.G0||fc.事实->类别!=相关概念类别::特征||fc.事实->治理状态!=概念树生命周期状态::活跃||!d||d->特征类型.值!=r.正式特征类型||d->规范域.size()!=1||d->规范域[0].下界!=r.准确I64||d->规范域[0].上界!=r.准确I64)
...
概念树类数据服务::准备原子I64出生使用退出片段(const 原子I64特征出生使用退出片段请求& r,std::uint64_t g) const {
    ...
    const auto use=读取原子I64出生使用({1,g,r.G0,r.F,r.概念读取预算});
...
bool 原子I64特征出生使用读取结果::成功(const 原子I64特征出生使用读取请求& r) const noexcept {
    return 版本==1&&r.版本==1&&状态==原子I64特征出生使用读取状态::已读取&&Gread==r.Gread&&H==r.H&&事实.has_value()&&事实->F.编码==r.F;
}

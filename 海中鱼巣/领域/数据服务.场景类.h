...
public:
    ...
    try {
      ...
      for (const auto id : ids) {
        const L1所有者范围当前事实读取请求_v2 request{
            L1所有者范围当前事实读取合同版本_v2, port.所有者身份(), id,
            out.Gread};
        const auto q = l1.读取所有者范围当前事实(request);
        if (q.状态 != L1所有者范围当前事实读取状态_v2::成功 ||
            q.合同版本 != request.合同版本 || q.所有者 != request.所有者 ||
            q.事实编码 != request.事实编码 || q.期望事实代次 != request.期望事实代次 ||
            q.读取事实代次 != out.Gread || !q.载荷) {
          out.状态 = 场景角色数据状态::已可能发布;
          return out;
        }
        const auto *n = std::get_if<L1所有者范围节点事实>(&*q.载荷);
  ...
  场景角色当前读取结果 读取当前场景角色(const 场景角色当前读取请求 &r) const override;
  ...
  场景组织写结果_v2 组织动态实例(const 场景动态组织请求 &,
                                 const 动态结构只读提供者 &);
  场景组织当前读取结果 读取当前实例组织(const 场景组织当前读取请求 &) const;
  场景动态组织当前读取结果
  读取当前动态场景组织(const 场景动态组织当前读取请求 &) const override;
  场景特征组织写结果_v1 组织特征实例(const 场景特征组织请求 &);
  场景特征组织当前读取结果 读取当前特征组织(const 场景特征组织当前读取请求 &) const;
  ...
  直接归属场景角色当前完整读取结果_v2
  读取当前场景角色位置_v2(
      const 直接归属场景角色当前完整读取请求_v2 &) const override;
  ...
  场景直接包含单项结果 读取当前直接包含(const 场景直接包含当前读取请求 &) const;
...
private:
  ...
  struct 当前父场景语境私有结果_v2 final {
    ...
    bool 成功(std::uint64_t, 稳定编码,
              const 直接归属联合事实 &) const noexcept;
  };
  struct 当前场景树证明私有结果_v2 final {
    场景角色数据状态 状态 = 场景角色数据状态::入口拒绝;
    std::uint64_t Gread = 0;
    std::optional<场景树证明事实> 证明;
    bool 成功(std::uint64_t, 稳定编码, 稳定编码,
              const 直接归属场景角色事实 &) const noexcept;
  };
  ...
  关系读取 查询关系(std::uint64_t, 关系端点方向, 稳定编码, 稳定编码) const;
  节点读取 读节点(稳定编码, std::uint64_t) const;
  关系事实读取 读关系(稳定编码, std::uint64_t) const;
  场景角色当前读取结果 读角色(std::uint64_t, 稳定编码) const;
  场景角色当前读取结果 读当前完整角色_v2(std::uint64_t, 稳定编码) const;
  当前父场景语境私有结果_v2
  读当前父场景语境_v2(std::uint64_t, 稳定编码,
                       const 直接归属联合事实 &) const;
  当前场景树证明私有结果_v2
  读当前场景树证明_v2(std::uint64_t, 稳定编码, 稳定编码,
                      const 直接归属场景角色事实 &) const;
  状态使用绑定读取结果 读B(std::uint64_t, 状态使用绑定身份) const;
  状态使用绑定组查询结果 查B组(std::uint32_t, std::uint64_t, 稳定编码,
                               std::int64_t, std::uint64_t) const;
  场景组织当前读取结果 读路径(std::uint64_t, 场景根角色, 稳定编码,
                           std::uint64_t) const;
  场景特征组织当前读取结果 读特征路径(std::uint64_t, 特征信息身份,
  ...
  准备场景绑定片段(const 绑定存在创建请求 &r, std::uint64_t g) const override {
    return 绑定保护<L1有限N分区原子参与者写集_v3>(g, [&] {
      auto parent = 读角色(g, r.绑定.绑定节点);
      if (!parent.成功({2, g, r.绑定.绑定节点}))
        throw 绑定映射(parent.状态);
      ...
      for (std::uint32_t i = 1; i <= 4; ++i)
        p.写集.关系.push_back({{i + 5},
                               ...
                               layout_.根绑定关系类型,
                               static_cast<std::int64_t>(i)});
      auto markers = 查询关系(g, 关系端点方向::源,
                              r.绑定.绑定节点, includeLayout_.根标记关系类型);
      auto membership =
          查询关系(g, 关系端点方向::源, r.绑定.绑定节点,
  ...
  绑定存在参与者结果<场景绑定出生见证>
  读取场景绑定出生(std::uint64_t g, 稳定编码 e,
                   const 存在初始绑定 &b) const override {
    return 绑定保护<场景绑定出生见证>(g, [&] {
      auto type = b.种类 == 存在初始绑定种类::场景成员
                      ? includeLayout_.直接存在成员关系类型
                      : includeLayout_.直接子场景关系类型;
      auto edges = 查询关系(g, 关系端点方向::目标, e, type);
      if (!edges.成功())
        throw 绑定映射(edges.状态);
      if (edges.关系.empty())
        throw 绑定S::绑定未找到;
      ...
      if (x.源节点 != b.绑定节点 || x.目标节点 != e || x.角色或顺序 != 1)
        throw 绑定S::内部不一致;
      auto bound = 读取当前直接包含({1, g, x.编码});
      if (!bound.成功())
        throw 绑定映射(bound.结果头.状态);
      if (!bound.包含)
        throw 绑定S::内部不一致;
      场景绑定出生见证 out{g, b.种类, b.绑定节点, e, *bound.包含, {}};
      if (b.种类 == 存在初始绑定种类::直接子场景) {
        auto role = 读角色(g, e);
        if (!role.成功({2, g, e}))
          throw 绑定映射(role.状态);
        auto proof = 查询关系(g, 关系端点方向::源, e,
                              includeLayout_.树归属关系类型);
        if (!proof.成功())
          throw 绑定映射(proof.状态);
        if (proof.关系.size() != 1)
        ...
        node.树证明 = {g,
                       ...
                       {edge.编码,
                        ...
                        {edge.创建事实代次}}};
        node.直接父 =
            直接归属联合事实{g, 直接归属来源::直接子场景, x.编码,
                             x.源节点, e, x.创建事实代次};
        node.从上游场景到本场景路径.push_back(*node.直接父);
        const auto contextEdges=查询关系(g,关系端点方向::源,e,
                                           layout_.父场景语境关系类型);
        ...
        node.父语境投影=场景父语境投影事实{
            g,e,b.绑定节点,*node.直接父,转边(contextEdge)};
    ...
    try {
      ...
      const auto map=[](场景角色数据状态 s) {
        ...
        switch(s) {case X::入口拒绝:return B::入口拒绝;case X::未找到:return B::未找到;
          case X::事实代次漂移:return B::事实代次漂移;
          case X::数量预算不足:return B::预算不足;
          case X::资源失败:return B::资源失败;case X::实例未组织:return B::位置冲突;
          default:return B::内部不一致;}
      };
      const auto role=读角色(g,r.位置.场景);
      if(!role.成功({2,g,r.位置.场景}))throw map(role.状态);
      const auto root=role.角色->四根[0].根.编码;
      std::size_t pathLength=0;
      if(r.位置.组织父!=root) {
        bool missingKnown=false;
        const auto path=读特征路径(g,{r.位置.组织父},r.组织读取预算.最大路径长度,&missingKnown);
        if(missingKnown)throw 原子I64特征出生状态::位置冲突;
        if(!path.成功({1,g,{r.位置.组织父},r.组织读取预算.最大路径长度}))throw map(path.状态);
  ...
  原子I64特征窄读取结果<原子I64特征组织事实> 读取原子I64组织(const 原子I64特征组织读取请求& r) const override {
    原子I64特征窄读取结果<原子I64特征组织事实> out;out.Gread=r.Gread;
    ...
    try {if(r.版本!=1||!r.Gread||!有效(r.C)||!有效(r.P)||!有效(r.F)||!r.预算.最大路径长度)return out;
      ...
      for(std::uint64_t depth=0;;++depth){if(!seen.insert(child.值).second)throw 原子I64特征窄读取状态::内部不一致;
        const auto roots=l1_.读取所有者范围当前目标关系组({L1所有者范围CRUD合同版本,child,layout_.根绑定关系类型});
        if(roots.读取事实代次!=r.Gread)throw 原子I64特征窄读取状态::事实代次漂移;
        if(roots.状态!=L1所有者范围读取状态::成功)throw map(roots.状态);
        if(!roots.关系组.empty()){if(roots.关系组.size()!=1)throw 原子I64特征窄读取状态::内部不一致;const auto& e=roots.关系组.front();if(e.写入所有者!=owner_||e.源节点!=r.C||e.目标节点!=child||e.角色或顺序!=static_cast<std::int64_t>(场景根角色::特征)||e.创建事实代次>r.Gread)throw 原子I64特征窄读取状态::位置冲突;break;}
        if(depth>=r.预算.最大路径长度)throw 原子I64特征窄读取状态::预算不足;const auto rows=l1_.读取所有者范围当前目标关系组({L1所有者范围CRUD合同版本,child,featureLayout_.特征组织关系类型});
        if(rows.读取事实代次!=r.Gread)throw 原子I64特征窄读取状态::事实代次漂移;
        if(rows.状态!=L1所有者范围读取状态::成功)throw map(rows.状态);
        if(rows.关系组.size()!=1)throw 原子I64特征窄读取状态::位置冲突;const auto& e=rows.关系组.front();if(e.写入所有者!=owner_||e.目标节点!=child||e.关系类型节点!=featureLayout_.特征组织关系类型||e.角色或顺序!=1||e.创建事实代次>r.Gread)throw 原子I64特征窄读取状态::内部不一致;reverse.push_back({e.编码,e.源节点,e.目标节点,e.关系类型节点,e.角色或顺序,e.创建事实代次});child=e.源节点;
      }
      ...
      const auto& last=reverse.back();out.事实=原子I64特征组织事实{r.C,r.P,r.F,last.关系,last.关系类型,std::move(reverse),last.创建事实代次};out.状态=原子I64特征窄读取状态::已读取;

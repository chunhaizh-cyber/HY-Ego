// 文件规则：因果模式材料只承载纯值复合身份、非权威机会、结果证据和整数概率候选；不得承载事务能力或稳定因果事实。
module;
...
struct 因果完整结构键 {
    ...
    bool 局部完整() const noexcept {
        return 原因动态键.完整() && 结果动态键.完整()
            && 因果规则版本 != 0 && 格式版本 != 0;
    }

    bool 当前版本兼容() const noexcept {
        return 局部完整()
            && 格式版本 == 因果完整结构键格式版本
            && 原因动态键.规则版本 == 动态模式规则版本
            && 结果动态键.规则版本 == 动态模式规则版本
    ...
    bool 原因结果角色不同() const noexcept {
        return 原因动态键.被改变目标.仓库编号 != 结果动态键.被改变目标.仓库编号
            || 原因动态键.被改变目标.节点编号 != 结果动态键.被改变目标.节点编号
            || 原因动态键.被改变目标.版本号 != 结果动态键.被改变目标.版本号
            || 原因动态键.前状态值 != 结果动态键.前状态值
            || 原因动态键.后状态值 != 结果动态键.后状态值
            || 原因动态键.有来源动作 != 结果动态键.有来源动作
            || 原因动态键.来源动作.仓库编号 != 结果动态键.来源动作.仓库编号
            || 原因动态键.来源动作.节点编号 != 结果动态键.来源动作.节点编号
            || 原因动态键.来源动作.版本号 != 结果动态键.来源动作.版本号
            || 原因动态键.规则版本 != 结果动态键.规则版本
            || 原因动态键.二次特征角色 != 结果动态键.二次特征角色;
    }
};
...
struct 因果完整结构键结果 {
    ...
    bool 成功() const noexcept {
        return 状态 == 因果完整结构键处理状态::已形成
            && 拒绝原因 == 因果完整结构键拒绝原因::无
            && 身份.has_value() && 身份->当前版本兼容()
            && 身份->原因结果角色不同();
    }
};

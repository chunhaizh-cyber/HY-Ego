...
public:
   ...
   词节点类* 查找词性节点(const 词节点类* 词节点, 枚举_词性 词性值);
   词节点类* 查找词性节点(const 节点类* 词节点, std::string 词性值);
   ...
   词节点类* 添加词性词(词节点类* 词节点, 枚举_词性 词性);
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    const 词性节点类* 绑定词性词到场景基础信息(const std::string& 词字符串, 枚举_词性 词性, 场景节点类* 场景, 基础信息节点类* 基础信息节点);
    ...
    std::vector<基础信息节点类*> 通过词和场景查找基础信息(const std::string& 词字符串,场景节点类* 当前场景,std::optional<枚举_词性> 词性 = std::nullopt)
    {
        ...
        // 遍历该词的所有词性节点
        const 词性节点类* 词性根节点 = 词节点->子;
        const 词性节点类* 游标 = 词性根节点;
        do {
    ...
    int64_t 添加对应基础信息(const 词性节点类* 词节点, 基础信息节点类* 基础信息);
    std::string 获取词(const 词性节点类* 词性词指针);
private:
    // 纯查找：存在则返回节点，不存在返回 nullptr（不会创建）
    词节点类* 查找词(const std::string& 词字符串);
  
    枚举_主信息类型 映射_词性到主信息类型(枚举_词性 词性);
    static inline std::size_t utf8_char_count(const std::string& s) {
        std::size_t n = 0;
        for (std::size_t i = 0; i < s.size(); ) {
            ...
            else step = 1; // 非法起始字节，按 1 字节跳过（也可改成 throw）

            // 防越界：如果剩余不足 step，就当作非法，跳 1
    ...
    static inline std::size_t utf8_char_len(unsigned char c) {

...
public:
    外设运行基类() = default;
    // 注意：不要在基类析构中调用 停止()。
    // 停止() 内部会调用 停止设备()/抓取数据_阻塞 等虚函数；
    // 在析构阶段（派生类已销毁）调用会触发 __purecall / __debugbreak。
    // 约定：由拥有者在销毁前显式调用 停止()。
    virtual ~外设运行基类() = default;
    ...
    枚举_取数结果 等待下一包(外设数据包<数据T>& out, int 超时毫秒) override {
        ...
        if (!pred()) {
            if (超时毫秒 < 0) {
                ...
                const ULONGLONG 开始时间 = ::GetTickCount64();
                const ULONGLONG 超时上限 = static_cast<ULONGLONG>(超时毫秒);
                while (!pred()) {
                    lk.unlock();
                    const ULONGLONG 已等待 = ::GetTickCount64() - 开始时间;
                    if (已等待 >= 超时上限) {
                        lk.lock();
                        return 枚举_取数结果::超时;
                    }

                    const ULONGLONG 剩余毫秒 = 超时上限 - 已等待;
                    const DWORD 睡眠毫秒 = 剩余毫秒 > 2 ? 2u : static_cast<DWORD>(剩余毫秒);
                    ::Sleep(睡眠毫秒 == 0 ? 1u : 睡眠毫秒);
                    lk.lock();

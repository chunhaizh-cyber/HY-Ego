...
- `流程图/20260722_NODE-TYPED-MIGRATION_NT-P4_统一快照恢复退役施工流程图_v0.1.md`
...
flowchart LR
    ...
    ISO --> FREEZE["冻结协调器::形成统一权威冻结材料"]
    FREEZE --> SEG["具名强类型段：命名域 ABI / 节点、关系 0—23、任务选择、概念签名、领域历史、4170"]
    ...
    CAP --> FIRST["运行期上下文宿主::尝试首次发布已验证上下文"]
...
上述函数表描述 `main@1185e1b...`。P4 开工前必须在 P3 固定提交重新扫描，不能把本表当成届时实际接口。

## 5. 目标函数职责表

| 层级 | 目标函数 | 输入 | 唯一责任 | 输出 / 失败边界 |
| --- | --- | --- | --- | --- |
| 冻结协调器 | `形成统一权威冻结材料` | 当前节点直接上下文、唯一冻结身份 | 在同一代次复制并互证全部必需权威段 | 完整不可变材料；任一段失败不产出完整快照 |
| 段注册表 | `读取当前必需段声明组` | 当前段清单版本 | 返回具名段类型、版本、上限、冻结 / 装载 / 互证处理器 | 未知、重复或缺段声明为设计 / 内部错误 |
| 序列化器 | `序列化统一权威快照` | 统一冻结材料 | 形成规范化二进制编码，不写 C++ 内存布局 | 规范化字节组或资源失败 |
| 文件材料服务 | `原子保存快照材料` | 规范化字节组、目标路径 | 临时写、复读校验并原子替换 | 已保存或资源失败；旧完整文件不被半写覆盖 |
| 严格解析器 | `解析统一权威快照` | 不可信文件字节 | 分配前校验边界、版本、段、摘要并逐字段解码 | 不可变恢复材料包或材料拒绝 |
| 恢复协调器 | `形成隔离恢复候选` | 已解析恢复材料包 | 按固定顺序建立普通路径不可见的新上下文候选 | 隔离候选；失败销毁且宿主不变 |
...
| 运行期上下文宿主 | `尝试首次发布已验证上下文` | 已验证候选 | 只在空宿主中完成一次原子发布 | 已发布或已有上下文；不得替换 / 清空 |
...
候选函数名是设计职责，不锁定最终 ABI。若执行前实际模块方向迫使公开仓库、记录容器、事务令牌或可变上下文，停止并退回设计。
...
P4-FREEZE、P4-RESTORE 和 P4-LEGACY 不得改默认装配、入口、旧句柄 ABI 或物理删除旧文件。四叶固定串行，工程 XML 与统一运行器的新增模块 / 专属自检登记所有权按 FREEZE -> RESTORE -> LEGACY -> CUTOVER 随固定提交移交，CUTOVER 最终统一整理；不存在无编号 `P4-SHARED`。

## 8. 叶子依赖与最终切换原子性

```mermaid
flowchart TD
    P123["P1—P3 固定节点直接新域提交"] --> F["P4-FREEZE"]
    F --> R["P4-RESTORE"]
    F --> L["P4-LEGACY"]
    R --> L
    P123 --> C["P4-CUTOVER"]

...
- `流程图/20260722_NODE-TYPED-MIGRATION_NT-P2C_自我内部世界与事实投影迁移施工流程图_v0.1.md`
...
本叶子活动期共享接线唯一所有者
  工程 XML 和统一自检运行器（只接隔离自检）
  -> #341 已集成且经设计接受后由 #342 接管；入口禁止修改
...
flowchart LR
    ...
    P2A --> P2C
    P2B --> P2C
    P2B工程["#341 已集成固定提交：工程与运行器所有权"] --> P2C
    P2C --> P3["NT-P3：旧调用方、兼容入口和删除审计"]
    P2C --> P4["NT-P4：快照恢复、旧材料策略和物理退役"]
```

P2A/P2B/P2C 固定按 `#340 -> #341 -> #342` 串行：#341 真实消费 #340 的 `复核特征值归属`，#342 真实消费 #340 的 `读取宿主实例特征槽位组` 与 #341 的主体状态 / 动态事实组；工程和统一自检运行器所有权随已集成且经设计接受的固定提交逐项移交。不另立共享接线叶子，也不得提前并行派发。
...
8. #340 / #341 的固定提交、值式接口或工程 / 运行器所有权尚未按顺序移交，或 P2A/P2B/P2C 对同一生产、自检、工程或装配文件发生所有权重叠；
...
- 所有列表都是调用期值式投影，清空可重建；
- 逻辑内返回和内部逻辑错误没有互相降级；
- 旧自我写路径、P2A/P2B 接口和 #341 到 #342 的工程 / 运行器所有权移交均有明确门禁；

...
- `流程图/20260722_NODE-TYPED-MIGRATION_NT-P2A_特征值类型化记录迁移现状流程图_v0.1.md`
- `流程图/20260722_NODE-TYPED-MIGRATION_NT-P2A_特征值类型化记录迁移施工流程图_v0.1.md`
...
- 4170 与值本体保持分账；
- 逻辑内返回与内部逻辑错误无交叉降级；
- P2A、P2B、P2C 固定按 `#340 -> #341 -> #342` 串行；工程和统一自检运行器所有权随每个已集成且经设计接受的固定提交逐项移交，不另立共享接线叶子，也不允许三叶并行派发。
- P2B 写入关系 19 角色 3（特征）和角色 4（值）前，对 P2A `复核特征值归属` 存在真实消费依赖，相关代码不得与接口实现并行派发。
- P2C 形成自我实例特征读取投影前，对 P2A `读取宿主实例特征槽位组` 存在真实消费依赖，相关代码不得与接口实现并行派发。

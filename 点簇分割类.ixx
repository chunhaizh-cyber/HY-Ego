...
public:
 ...
 // 为了保证占据金字塔颜色一一对应，这里从边界像素重新构建对齐数据
            ...
            if (有颜色) {
                double 颜色一致性 = 0.0;
                auto* out颜色一致性 = 参数.计算颜色一致性 ? &颜色一致性 : nullptr;
                res.颜色 = 计算簇平均颜色_边缘降权(帧, res.簇, res.边界, 边界像素, 0.35, out颜色一致性);
                res.颜色一致性 = 参数.计算颜色一致性 ? std::clamp(颜色一致性, 0.0, 1.0) : 0.0;

...
继续创建新计划及其所需的完整设计包：现状扫描、正式规范及目录、冲突登记、流程图、逐节点分析、详细设计、知识图谱、接口冻结、不可变计划合同和计划索引项。不得修改代码 / 工程 / 自检、施工 / 验证记录或智能体协作规则，也不处理执行侧问题。每份完整设计包通过治理验证、精确提交并推送且计划状态成为 `可执行` 后，只向唯一登记的既有执行智能体发送 `PLAN-AVAILABLE-NOTICE`，随后停止修改该精确包并继续下一个新计划；不得发送其它跨智能体消息。

...
不得硬编码仓库或 worktree 路径。用 Git 顶层、`git worktree list --porcelain`、`项目记忆/并行工作树登记表.md` 和 `项目记忆/窗口交互记录.md` 解析当前身份。
...
继续当前规范、流程图、详细设计、计划、索引、队列、设计记录、状态同步和路由治理；不得修改 C++ / 工程 / 自检或运行 C++ 构建与程序。
...
只继续当前任务 worktree 正式分配的唯一计划和执行专属断点；按计划修改允许的代码 / 工程 / 自检和执行专属记录，运行已授权验证，精确提交并只推送同名任务分支。
...
裸 `继续` 永远不构成角色切换。同一物理窗口串行切换也必须由用户明确要求，并在整棵任务树全部后代停止、旧角色记录与验证完成、Git 和占用释放、正式路由提交推送后整体切换；切换后重新读取 Git、索引、队列、登记、目标计划、断点和实际接口。

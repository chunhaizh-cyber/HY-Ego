...
当前目标
-> 当前工作区与 dirty state
-> 当前规则 / 复杂计划列表 / 计划状态 / 具名依赖，或直接代码切片的用户授权与当前声明
-> 当前真实代码入口、调用点、结构承载
-> 缺口、风险、待确认问题
```
...
5. 核对 dirty diff 是否影响所读事实；复杂计划状态只从计划索引读取，直接代码切片范围只从用户直接授权与当前切片声明读取，切片开始、漂移、失败和完成事实只从 Git 与结构化消息读取；不得把不同提交、计划版本或切片范围的事实混合为同一结论。

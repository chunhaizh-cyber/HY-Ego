...
- 用户要求持久化扫描结果时，本技能仍保持只读；扫描结束后的独立写步骤中，交互智能体可按用户直接指令取得明确切片并落盘，计划智能体或计划支撑智能体可写入自己拥有的设计包，执行智能体可写入当前计划冻结的专属施工 / 验证记录并向配对计划支撑智能体发送消息，流程图与知识图谱维护智能体可转用 `hai-zhong-yu-chao-flowchart-code-correction` 写入其当前代码图谱切片但不得写能力证据目录；能力证据目录维护智能体只写其专属目录。不得在本技能内部混入写操作。

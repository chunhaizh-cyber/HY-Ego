...
推理结论不等于规范事实、代码事实或能力完成。交互智能体按用户要求可以只形成目标 / 设计输入，也可以取得明确设计切片，完成正式依据核对、冲突裁决、文件修订和规范目录登记；计划智能体或计划支撑智能体仍只在各自默认拥有的设计包内落盘。
...
用户要求保存时，交互智能体可以按用户直接指令在明确切片内落盘任意项目设计产物并完成正式登记；计划智能体或计划支撑智能体只在自己默认拥有的设计包及授权路径内落盘。计划智能体不修改计划索引、不处理执行退回；计划支撑智能体默认登记计划索引，接管新计划登记或执行侧具名请求且复核成立的既有设计包后，可以在当前包原目标内按需新建或修改必要文件。计划选择、S0 PASS、正常执行或纯代码错误不产生自动窗口的设计包修订权。执行智能体只返回执行事实。

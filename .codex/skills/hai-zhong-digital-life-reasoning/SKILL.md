...
用户要求保存时，只有计划智能体或计划支撑智能体可以在自己拥有的设计包及正式授权路径内落盘；计划智能体只为创建新计划形成所需设计产物，不修改计划索引、不处理执行退回；计划支撑智能体独占登记计划索引，并只在执行侧具名请求且复核成立时修订既有设计包，不替计划智能体创建新计划。计划选择、S0 PASS、正常执行或纯代码错误不产生设计包修订权。交互智能体只返回目标 / 交接文本，执行智能体只返回执行事实。

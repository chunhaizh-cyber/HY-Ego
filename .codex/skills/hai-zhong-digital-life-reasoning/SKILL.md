...
用户要求保存时，交互智能体可以按用户直接指令在明确切片内落盘任意项目设计产物并完成正式登记；计划智能体负责新目标设计包、创建侧验证及对应计划索引登记，不处理执行退回；计划支撑智能体只在执行侧提出具名无法裁决问题且确认属于设计 / 计划后，在当前计划原目标内给出裁决并新建或修改必要设计材料及受影响索引项。计划选择、S0 PASS、正常执行、纯代码错误或环境问题不产生计划支撑写入权。执行智能体只返回执行事实。

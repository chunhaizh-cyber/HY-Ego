...
用户要求保存时，只有计划智能体或计划支撑智能体可以在自己拥有的设计包及正式授权路径内落盘；计划智能体只为创建新计划形成所需设计产物，不处理执行退回；计划支撑智能体只修订执行侧因具名计划 / 设计问题明确请求、且支撑侧复核成立的既有计划，不扩展到原目标外的新设计包。计划选择、S0 PASS、正常执行或纯代码错误不产生设计包写权。交互智能体只返回目标 / 交接文本，执行智能体只返回执行事实。不得另建 Markdown 状态目录。

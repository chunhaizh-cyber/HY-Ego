...
推理结论不等于规范事实、代码事实或能力完成。交互智能体只把结论形成目标或设计输入；正式依据、冲突裁决、文件修订和规范目录登记必须由拥有该设计包的计划智能体或计划支撑智能体按 `AGENTS.md` 完成。
...
用户要求保存时，只有计划智能体或计划支撑智能体可以在自己拥有的设计包及正式授权路径内落盘；计划智能体只为创建新计划形成所需设计产物，不修改计划索引、不处理执行退回；计划支撑智能体独占登记计划索引，接管新计划登记或执行侧具名请求且复核成立的既有设计包后，可以在当前包原目标内按需新建或修改必要文件，但不得独立发起无关新计划或扩大用户目标。计划选择、S0 PASS、正常执行或纯代码错误不产生设计包修订权。交互智能体只返回目标 / 交接文本，执行智能体只返回执行事实。

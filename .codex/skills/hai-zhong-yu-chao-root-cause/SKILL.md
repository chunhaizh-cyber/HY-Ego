...
严格服从仓库根目录 `AGENTS.md`。用 Git 顶层和正式登记解析仓库、worktree、提交和角色，不硬编码路径或分支。把每个弹窗、逻辑错误日志、验证失败、异常状态或崩溃邻近现象按只读证据链追踪：
...
需要持久化时，将报告交给正式设计角色；设计角色据此修订规范 / 设计 / 计划，随后由独立顶层执行任务消费已登记计划。三阶段不得在本技能内自动串成同一写任务。

## 停止边界

- 本技能不得修改代码、规范、设计、计划、断点、项目记忆或 Git 状态，不得构建或运行程序，也不得取得任务执行权。
- 不绕过领域服务，不建议先写错误结构再靠后续校验修复。
- 没有正式设计和已登记计划时，不得把修复方向交给执行；已有计划也必须由独立执行任务重新复核后实施。

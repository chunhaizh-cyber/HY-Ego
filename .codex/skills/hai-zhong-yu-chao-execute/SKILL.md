...
预冻结接口合同只允许派生需求提供者计划进入施工，并允许消费设计提前闭合；提供者计划可以直接实现真实能力，也可以按合同先实现接口骨架。合同不能使提供结果尚不足以满足验证矩阵的消费计划成为 `可执行`。提供者和消费者由唯一执行智能体按计划段顺序在 main 上串行实现。
...
- 派生需求提供者计划的预冻结合同缺项、版本不一致，或需要执行智能体自行决定最终函数身份、DTO、签名、真实行为或未实现语义。
- 需要修改禁止文件或中央治理文件。

...
当前计划正常收口，或计划明确允许的普通代码 WIP 已形成具名断点提交、推送到 `origin/main` 且工作区 / index 恢复 clean 后，必须立即重读计划列表。只要存在候选就禁止输出最终答复，必须建立下一计划段并重新 S0。不等待计划智能体或计划支撑智能体接受 S0 或选择下一项。规则允许的 `DESIGN-DRIFT-WIP` 收口后例外进入 `DESIGN-DRIFT-WAIT`，等待计划支撑修订，不选择其它计划。
...
-> 读取关联规范、流程图、详细设计和实际代码接口 / 接口骨架；计划内提供者计划读取预冻结合同作为待实现目标
...
预冻结接口合同只允许计划内提供者计划进入施工，并允许消费设计提前闭合；提供者计划可以直接实现真实能力，也可以按合同先实现接口骨架。合同不能使提供结果尚不足以满足验证矩阵的消费计划成为 `可执行`。提供者和消费者由唯一执行智能体按计划段顺序在 main 上串行实现。

接口骨架计划必须使用最终模块、函数名、DTO 和签名。仅声明足够时不得增加定义，验证最终声明、DTO / ABI、模块导入 / 头文件消费及不需要链接定义的编译边界；完整链接确需定义时，只能结构化返回“未实现”，不得返回假成功、有效句柄、默认业务对象或伪数据，不得写仓库、关系、索引、数据库或其它机器事实，并追加验证全工程链接、调用返回“未实现”和零机器事实 / 结构变化。执行智能体把完成边界写成“接口骨架已实现”，不得写成真实能力完成。
...
-> 复核具名前置产物与实际接口 / 接口骨架；计划内提供者计划复核预冻结合同
-> 按计划机械阶段实施最小闭合切片
...
- 实际接口与计划假定契约不一致。
- 消费计划只有预冻结合同、接口骨架尚未真实实现，或骨架身份 / 版本与合同不一致。
- 消费计划依赖显式未实现定义，但没有冻结其传播 / 收口路径，或验证矩阵要求真实成功行为。
- 计划内提供者计划的预冻结合同缺项、版本不一致，或需要执行智能体自行决定最终函数身份、DTO、签名、真实行为或未实现语义。
- 需要修改禁止文件或中央治理文件。
...
计划 DRIFT 后，普通 WIP 仍须由计划明确允许。若执行智能体已经明确请求设计包修订，且计划支撑智能体只读复核后发送 `DESIGN-DRIFT-CONFIRMED`，则当前改动全部属于计划白名单并达到单工作区规则定义的安全代码断点时，可以提交并推送 `DESIGN-DRIFT-WIP`，无需叶子计划预先列出该漂移场景。提交正文必须记录计划身份 / blob / 段起点、具名设计缺口、变更文件、已通过与未通过验证和未完成边界；不得宣称计划完成。恢复 clean 后释放占用并进入 `DESIGN-DRIFT-WAIT`。无法形成安全断点时保持现场并以 `DIRTY-BLOCKED` 停止；不得 stash、覆盖、提交破损 main 或切换下一计划。

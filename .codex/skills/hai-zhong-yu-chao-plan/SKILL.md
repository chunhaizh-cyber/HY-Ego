...
随后按交互规范完成两段式真实派发：先推送保持 `待执行` 的派发准备提交，再通过 Codex 任务工具向唯一、独立、用户可见的执行顶层任务实际发送消息并读回接收状态；消息明确要求目标等待派发状态提交。发送与读回成功后，把消息证据和 `已派发待执行回执` 写入中央治理文件，再次验证、提交并非强制推送，最后通知目标读取该派发状态提交。目标只有读到该提交并完成 S0 后才能写入。不得使用本设计任务树的子智能体承担执行角色，也不得把预留 worktree、目标标题、消息草稿或“下一步”当作已经派发。

派发消息至少包含计划 / 队列编号、派发准备提交、权威路径、允许 / 禁止范围、任务 worktree / 分支 / 冻结基线、必须重读的材料、验证、等待派发状态提交和返回条件；状态同步后追加派发状态提交号。消息只负责唤醒，执行窗口仍须从仓库完成 S0。

唯一目标缺失、歧义、工具不可用或消息发送失败时，保持 `待执行`，在窗口交互记录中具名写明缺失的目标身份或失败证据；不得宣称已经派发或执行已经开始。只有消息发送已经成功、只是读回接口不可用时，才可按交互规范使用任务工具返回的持久化投递回执继续同步派发状态。具名门禁解除后立即恢复派发链。

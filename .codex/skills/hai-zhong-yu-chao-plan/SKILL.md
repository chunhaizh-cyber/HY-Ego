...
-> 建立或修订正式规范、规范目录与冲突登记
-> 建立或修订流程图并冻结节点身份、分支和终止路径
-> 按 `.codex/rules/设计执行双窗口交互规则.md` 第 6.1 节逐节点完成实现分析
-> 确认、归并所需函数和共享数据 / 事务 / 版本合同
-> 形成可直接编码的详细设计与知识图谱
-> 核对当前代码和实际接口
-> 核对节点—函数—结构—返回—验证双向覆盖和范围一致性
...
严格执行 `.codex/rules/设计执行双窗口交互规则.md` 第 6.1 节。流程图只定义过程，不是执行智能体直接写代码的依据；先把每个流程节点转换成节点实现分析和已确认函数合同，再写入完整详细设计。

形成详细设计时依次完成：

```text
固定流程图版本和节点身份
-> 读取每个节点的规范、相邻边、当前代码和合同
-> 写清节点条件、输入输出、事实读写、状态变化、分支、调用 / 事务、结果和验证
-> 裁决现有函数复用或新增，并冻结完整函数合同
...
详细设计必须完整记录第 6.1 节列出的文档身份、节点实现表、函数合同表、数据合同、调用与事务、失败合同、代码映射、验证矩阵和空未决项。凡仍需执行智能体决定字段、类型、签名、判断条件、版本、所有权、调用顺序或错误处理的设计，保持 `待激活`。

详细设计生产可以按无重叠流程节点组交给同类型子智能体。顶层先冻结节点、共享合同和唯一所有者；子智能体只写分配的节点分析或独立详细设计切片；共享合同、跨节点去重、双向覆盖、计划索引和 Git 收口由顶层串行完成。不得让两个子智能体分别定义同一函数、DTO、事务、版本或同一文档切片。

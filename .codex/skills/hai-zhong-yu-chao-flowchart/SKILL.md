...
3. 只有计划智能体或计划支撑智能体可以在自己拥有的设计包内落盘：计划智能体只为创建新计划形成所需流程图且不得修改计划索引；计划支撑智能体独占登记计划索引，接管新计划登记后可在当前包原目标内按需新建或修改必要流程图，也可在执行侧报告具名计划 / 设计问题、明确请求修订且复核成立后新建或修订既有计划的关联流程图。计划选择、S0 PASS 和正常执行不触发流程图写入。交互智能体只形成目标 / 草稿输入；执行智能体只返回事实，不写流程图。

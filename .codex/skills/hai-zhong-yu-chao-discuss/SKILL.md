...
严格服从仓库根目录 `AGENTS.md`。用 Git 顶层、当前唯一工作区事实和计划索引解析仓库、分支与当前角色，不硬编码路径或分支。讨论是专家审查，不是实施授权，也不因用户接受某一观点自动切换为写入：
...
- 本技能不修改任何项目文件或 Git 状态。用户确认观点只形成目标或设计治理输入；讨论结束后的独立写步骤中，交互智能体可按用户直接指令取得明确切片并建立或修订正式产物，计划智能体或计划支撑智能体仍只修改自己默认拥有的设计包。

...
严格服从仓库根目录 `AGENTS.md`。用 Git 顶层、当前 worktree 事实和计划索引解析仓库、分支与当前角色，不硬编码路径或分支。讨论是专家审查，不是实施授权，也不因用户接受某一观点自动切换为写入：
...
- 本技能不修改任何项目文件或 Git 状态。用户确认观点只形成目标或设计治理输入；交互智能体需要建立新设计包时只向计划智能体发送目标，计划智能体或计划支撑智能体只能在随后独立写步骤中修改自己拥有的设计包。

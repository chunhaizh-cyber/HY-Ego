...
- 聊天输出默认标明 `设计输入` 或 `草稿`。
- 交互智能体使用本技能时，按用户要求既可以只形成聊天中的目标 / 设计输入，也可以直接建立、修订、登记和发布项目机器规范及关联设计包；落盘时必须完成证据核对、冲突裁决、规范目录登记和相应验证。
- 需要落盘时，计划智能体只能为创建尚未进入既有计划登记 / 修订范围的新计划写入所需设计包且不得修改计划索引；计划支撑智能体独占登记计划索引，接管计划后可在当前包原目标内新建或修改全部必要项目设计材料及计划内提供者设计包 / 施工计划，也可在配对执行智能体具名请求且复核成立时新建或修订关联设计包文件。缺少流程图、详细设计、接口或必需函数时由计划支撑智能体直接补齐，不交回计划智能体。计划选择、S0 PASS、正常执行和纯代码错误都不产生设计包修订权；执行智能体只交接事实，不写设计包。

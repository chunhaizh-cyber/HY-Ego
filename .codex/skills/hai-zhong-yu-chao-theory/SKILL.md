...
- 需要落盘时，计划智能体只能为创建新计划写入所需设计包且不得修改计划索引；计划支撑智能体独占登记计划索引，接管新计划登记后可在当前包原目标内按需新建或修改必要设计文件，也可在配对执行智能体具名请求且复核成立时新建或修订关联设计包文件。计划选择、S0 PASS、正常执行和纯代码错误都不产生设计包修订权；执行智能体只交接事实，不写设计包。

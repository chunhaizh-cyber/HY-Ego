...
- 聊天输出默认标明 `设计输入` 或 `草稿`。
- 交互智能体使用本技能时只形成聊天中的目标或设计输入；确实需要建立项目设计包时只向计划智能体发送目标，不自行落盘项目机器规范。
- 需要落盘时，计划智能体只能写尚未被执行选中的新目标设计包；计划支撑智能体只能写配对执行智能体已选中并交回的关联设计包。执行智能体只交接事实，不写设计包。
- 用户确认只证明口径已明确，仍须由合法设计包所有者完成依据链、冲突检查、正式文件修订、目录登记和验证。

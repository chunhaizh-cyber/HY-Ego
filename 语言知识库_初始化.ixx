...
import 基础数据类型模块; // 用于 词性节点类 / 枚举_词性

// —— 工具：取/建 词性词（根据你的词典实现替换） ——
static const 词性节点类* _取词(const std::string& 面, 枚举_词性 词性) {
...
static std::vector<const 词性节点类*> _线性化_SVO(
    const 结构体_语言模板& 模板,
    const 结构体_线性化上下文& ctx)
{
    std::vector<const 词性节点类*> 序列;
    // 模板.词序列槽位中，约定：索引语义依次为 主语, 谓词, 宾语, 时间?, 地点?
    for (const 词性节点类* slot : 模板.词序列槽位) {
...
static std::vector<const 词性节点类*> _线性化_比较(
    const 结构体_语言模板& 模板,
    const 结构体_线性化上下文& ctx)
{
    std::vector<const 词性节点类*> 序列;
...
static 结构体_语言知识条目 _条目(
    ...
    const std::vector<const 词性节点类*>& 槽位,
...
static void _注册_存在_陈述() {
    ...
    std::vector<const 词性节点类*> 槽位 = { /*E1名*/nullptr, w在, /*地点名*/nullptr };
...
static void _注册_特征_描述() {
    ...
    std::vector<const 词性节点类*> 槽位 = { /*E1名*/nullptr, w的, /*Fx名/值*/nullptr };
...
static void _注册_事件_叙述() {
    ...
    std::vector<const 词性节点类*> 槽位 = { nullptr, /*动词*/nullptr, nullptr, 在, nullptr, 于, nullptr };
...
static void _注册_关系_布局() {
    ...
    auto* 在 = _取词("在", 枚举_词性::介词);
    std::vector<const 词性节点类*> 槽位 = { nullptr, 在, nullptr, /*方位词*/nullptr };
...
static void _注册_比较_排序() {
    ...
    std::vector<const 词性节点类*> 槽位 = { nullptr, 比, nullptr, 更, /*形容词*/nullptr, /*参照*/nullptr };
...
static void _注册_因果_解释() {
    ...
    std::vector<const 词性节点类*> 槽位 = { 因为, /*子句A*/nullptr, 所以, /*子句B*/nullptr };

...
public:
    ...
    // 写入（异步）
    // ============================================================
    void 提交(std::uint64_t key, 图像BGR8 img, 时间戳 ts = 0) {
        if (key == 0) return;
        if (!img.有效()) return;
        if (ts == 0) ts = 当前微秒_();
    ...
    // 读（同步）
    // ============================================================
    bool 取(std::uint64_t key, 缓存条目& out) {
        if (key == 0) return false;
        const 参数 参数快照 = 获取参数();
        std::unique_lock lk(mtxCache_);
        auto it = map_.find(key);
        if (it == map_.end()) return false;
        if (私有_已过期_已加锁(it->second, 参数快照)) {
            私有_移除_已加锁(it->second);
            map_.erase(key);
            return false;
        }
    ...
    // ============================================================
    static std::uint64_t 生成Key_轮廓签名FNV(const VecIU64& sig) {
        // FNV-1a 64
...
private:
    ...
    bool 私有_已过期_已加锁(const std::list<缓存条目>::iterator& it, const 参数& p) const {
...
private:
    // 1240 局部顺序：
    // 1. 参数锁 mtxParam_
    // 2. 队列锁 mtxQ_（cv_ 只与它配对）
    // 3. 缓存锁 mtxCache_
    // 默认单锁化；读缓存前先快照参数，禁止在持有 mtxCache_ 时再回头拿 mtxParam_。
    // 参数
    ...
    std::list<缓存条目> lru_{};
    std::unordered_map<std::uint64_t, std::list<缓存条目>::iterator> map_{};

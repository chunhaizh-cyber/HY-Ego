...
namespace {
template<class T>
inline T& rs句柄引用(void* p) {
    return *static_cast<T*>(p);
}

template<class T>
inline const T& rs句柄引用(const void* p) {
    return *static_cast<const T*>(p);
}

template<class T>
inline void rs释放句柄(void*& p) {
...
public:
    ...
    bool 打开() override {
        try {
            ...
            // IMU 先只做“设备能力缓存”，不并入主 frameset。
            // 经验上把 motion 流直接绑进主 pipeline，容易出现“相机启动成功但视频首帧长时间不返回”。
            cfg.启用IMU = true;
            日志::运行f("[D455相机] IMU 采用独立能力缓存模式：陀螺仪={}fps, 加速度计={}fps（不并入主视频frameset）", cfg.陀螺仪FPS, cfg.加速度计FPS);
            ...
            try {
                ...
                rs句柄引用<rs2::pipeline_profile>(profile句柄_) = rs句柄引用<rs2::pipeline>(管道句柄_).start(c);
            }
            catch (...) {
                // 回退
                ...
                rs句柄引用<rs2::pipeline_profile>(profile句柄_) = rs句柄引用<rs2::pipeline>(管道句柄_).start(c2);
            ...
            // 对齐器（可配置）
            rs释放句柄<rs2::align>(对齐器句柄_);
            对齐器句柄_ = new rs2::align(RS2_STREAM_COLOR);
            日志::运行("[D455相机] 初始化对齐器（对齐到彩色流）");

            // 深度内参
            auto 深度profile = rs句柄引用<rs2::pipeline_profile>(profile句柄_).get_stream(RS2_STREAM_DEPTH).as<rs2::video_stream_profile>();
            rs句柄引用<rs2_intrinsics>(深度内参句柄_) = 深度profile.get_intrinsics();
            日志::运行f("[D455相机] 获取深度内参：fx={}, fy={}, cx={}, cy={}, 宽度={}, 高度={}", 
                rs句柄引用<rs2_intrinsics>(深度内参句柄_).fx, rs句柄引用<rs2_intrinsics>(深度内参句柄_).fy, rs句柄引用<rs2_intrinsics>(深度内参句柄_).ppx, rs句柄引用<rs2_intrinsics>(深度内参句柄_).ppy, rs句柄引用<rs2_intrinsics>(深度内参句柄_).width, rs句柄引用<rs2_intrinsics>(深度内参句柄_).height);

            // 深度尺度 + 设备缓存
            rs句柄引用<rs2::device>(device_句柄_) = rs句柄引用<rs2::pipeline_profile>(profile句柄_).get_device();
            if (auto ds = rs句柄引用<rs2::device>(device_句柄_).first<rs2::depth_sensor>(); ds) {
            ...
            if (!有IMU传感器_) {
                ...
                try { rs句柄引用<rs2::pipeline>(管道句柄_).stop(); }
                catch (...) {}
                已打开 = false;
                return false;
            }
    ...
    void 关闭() override {
        ...
        rs释放句柄<rs2::align>(对齐器句柄_);
        rs句柄引用<rs2::pipeline_profile>(profile句柄_) = rs2::pipeline_profile();
        rs句柄引用<rs2::device>(device_句柄_) = rs2::device();
        rs句柄引用<rs2::sensor>(颜色传感器句柄_) = rs2::sensor();
        rs句柄引用<rs2::sensor>(深度传感器句柄_) = rs2::sensor();
        rs句柄引用<rs2::sensor>(IMU传感器句柄_) = rs2::sensor();
        有颜色传感器_ = false;
        有深度传感器_ = false;
        有IMU传感器_ = false;
        已采到视频帧数_ = 0;
    ...
    bool 采集一帧(结构体_原始场景帧& 输出) override {
        if (!已打开 || !对齐器句柄_) return false;

        try {
            rs2::frameset frames;
            if (!rs句柄引用<rs2::pipeline>(管道句柄_).try_wait_for_frames(&frames, 1000)) {
                ++连续取帧超时次数_;
                if (连续取帧超时次数_ <= 5 || (连续取帧超时次数_ % 30) == 0) {
                    日志::运行f(
                        "[D455相机] 等待视频帧超时：连续超时次数={}",
                        连续取帧超时次数_);
                }
                return false;
            }
            if (连续取帧超时次数_ > 0) {
                日志::运行f(
                    "[D455相机] 视频帧恢复：此前连续超时次数={}",
                    连续取帧超时次数_);
                连续取帧超时次数_ = 0;
            }
            已采到视频帧数_ += 1;
            if (已采到视频帧数_ <= 3 || (已采到视频帧数_ % 60) == 0) {
                日志::运行f(
                    "[D455相机] 已采到视频帧：计数={}, frameset大小={}",
                    已采到视频帧数_,
            ...
            // ===== 1) 对齐 =====
            rs2::frameset aligned = rs句柄引用<rs2::align>(对齐器句柄_).process(frames);
            ...
            rs句柄引用<rs2_intrinsics>(深度内参句柄_) = cprof.get_intrinsics();
            // ===== 2) 设备时间戳（微秒）=====
            ...
            if (cfg.启用视差域处理) {
                filtered = rs句柄引用<rs2::disparity_transform>(深度到视差句柄_).process(filtered);
            }

            if (cfg.启用空间滤波) {
                filtered = rs句柄引用<rs2::spatial_filter>(空间滤波句柄_).process(filtered);
            }

            if (cfg.启用时间滤波) {
                filtered = rs句柄引用<rs2::temporal_filter>(时间滤波句柄_).process(filtered);
            }

            if (cfg.启用视差域处理) {
                filtered = rs句柄引用<rs2::disparity_transform>(视差到深度句柄_).process(filtered);
            }

            if (cfg.启用填洞滤波) {
                filtered = rs句柄引用<rs2::hole_filling_filter>(填洞滤波句柄_).process(filtered);
            }
            ...
            if constexpr (requires { 输出.深度内参; }) {
                输出.深度内参.fx = (double)rs句柄引用<rs2_intrinsics>(深度内参句柄_).fx;
                输出.深度内参.fy = (double)rs句柄引用<rs2_intrinsics>(深度内参句柄_).fy;
                输出.深度内参.cx = (double)rs句柄引用<rs2_intrinsics>(深度内参句柄_).ppx;
                输出.深度内参.cy = (double)rs句柄引用<rs2_intrinsics>(深度内参句柄_).ppy;
                输出.深度内参.宽度 = w;
                输出.深度内参.高度 = h;
                输出.深度内参.有效 = true;
            }
...
public:
    ...
    std::uint64_t 获取最后视角键() const { return 最后视角键; }

    std::string 获取设备序列号() const {
        try {
        if (device_句柄_ && static_cast<bool>(rs句柄引用<rs2::device>(device_句柄_))) {
            auto sn = rs句柄引用<rs2::device>(device_句柄_).get_info(RS2_CAMERA_INFO_SERIAL_NUMBER);
    ...
    外设指令回执 处理外设指令(const 外设指令& cmd) {
        ...
        const bool ok = device_句柄_ && static_cast<bool>(rs句柄引用<rs2::device>(device_句柄_));
...
private:
    ...
    void 释放RealSense句柄() {
        rs释放句柄<rs2::align>(对齐器句柄_);
        rs释放句柄<rs2::sensor>(IMU传感器句柄_);
        rs释放句柄<rs2::sensor>(深度传感器句柄_);
        rs释放句柄<rs2::sensor>(颜色传感器句柄_);
        rs释放句柄<rs2::hole_filling_filter>(填洞滤波句柄_);
        rs释放句柄<rs2::disparity_transform>(视差到深度句柄_);
        rs释放句柄<rs2::temporal_filter>(时间滤波句柄_);
        rs释放句柄<rs2::spatial_filter>(空间滤波句柄_);
        rs释放句柄<rs2::disparity_transform>(深度到视差句柄_);
        rs释放句柄<rs2::decimation_filter>(降采样滤波句柄_);
        rs释放句柄<rs2_intrinsics>(深度内参句柄_);
        rs释放句柄<rs2::device>(device_句柄_);
        rs释放句柄<rs2::pipeline_profile>(profile句柄_);
        rs释放句柄<rs2::pipeline>(管道句柄_);
    }

    void 确保RealSense句柄已创建_() {
        if (!管道句柄_) {
            管道句柄_ = new rs2::pipeline();
        }
        if (!profile句柄_) {
            profile句柄_ = new rs2::pipeline_profile();
        }
        if (!device_句柄_) {
            device_句柄_ = new rs2::device();
        }
        if (!深度内参句柄_) {
            深度内参句柄_ = new rs2_intrinsics();
        }
        if (!降采样滤波句柄_) {
            降采样滤波句柄_ = new rs2::decimation_filter();
        }
        if (!深度到视差句柄_) {
            深度到视差句柄_ = new rs2::disparity_transform(true);
        }
        if (!空间滤波句柄_) {
            空间滤波句柄_ = new rs2::spatial_filter();
        }
        if (!时间滤波句柄_) {
            时间滤波句柄_ = new rs2::temporal_filter();
        }
        if (!视差到深度句柄_) {
            视差到深度句柄_ = new rs2::disparity_transform(false);
        }
        if (!填洞滤波句柄_) {
            填洞滤波句柄_ = new rs2::hole_filling_filter();
        }
        if (!颜色传感器句柄_) {
            颜色传感器句柄_ = new rs2::sensor();
        }
        if (!深度传感器句柄_) {
            深度传感器句柄_ = new rs2::sensor();
        }
        if (!IMU传感器句柄_) {
            IMU传感器句柄_ = new rs2::sensor();
        }
    }

    void* 管道句柄_ = nullptr;
    void* profile句柄_ = nullptr;
    void* device_句柄_ = nullptr;
    void* 对齐器句柄_ = nullptr;
    void* 深度内参句柄_ = nullptr;
    double 深度尺度 = 0.001;

    void* 降采样滤波句柄_ = nullptr;
    void* 深度到视差句柄_ = nullptr;
    void* 空间滤波句柄_ = nullptr;
    void* 时间滤波句柄_ = nullptr;
    void* 视差到深度句柄_ = nullptr;
    void* 填洞滤波句柄_ = nullptr;

    void* 颜色传感器句柄_ = nullptr;
    void* 深度传感器句柄_ = nullptr;
    bool 有颜色传感器_ = false;
    bool 有深度传感器_ = false;

    // ===== IMU / 姿态估计 =====
    void* IMU传感器句柄_ = nullptr;
    bool 有IMU传感器_ = false;

    std::deque<D455_IMU样本> IMU缓冲; // 按时间戳递增
    ...
    结构体_相机状态 最后相机状态{};
    std::uint64_t 最后视角键 = 0;

    时间戳 最后设备时间_微秒 = 0;
    std::uint64_t 已采到视频帧数_ = 0;
    std::uint32_t 连续取帧超时次数_ = 0;
...
private:
    ...
    void 缓存传感器() {
        ...
        有深度传感器_ = false;
        try {
            for (auto&& s : rs句柄引用<rs2::device>(device_句柄_).query_sensors()) {
                bool isColor = false;
                try {
                ...
                if (isColor && !有颜色传感器_) {
                    rs句柄引用<rs2::sensor>(颜色传感器句柄_) = s;
                    有颜色传感器_ = true;
                }
                if (s.is<rs2::depth_sensor>() && !有深度传感器_) {
                    rs句柄引用<rs2::sensor>(深度传感器句柄_) = s;
                    有深度传感器_ = true;
                }
            }
        }
    ...
    void 缓存IMU传感器() {
        有IMU传感器_ = false;
        try {
            auto ms = rs句柄引用<rs2::device>(device_句柄_).first<rs2::motion_sensor>();
            if (ms) {
                rs句柄引用<rs2::sensor>(IMU传感器句柄_) = ms;
                有IMU传感器_ = true;
            }
        }
        catch (...) {
    ...
    void 收集IMU(const void* frames句柄) {
        // 将当前 frameset 内的 motion_frame 收集到缓冲，并按时间排序后逐条融合
        const auto& frames = rs句柄引用<rs2::frameset>(frames句柄);
        std::vector<D455_IMU样本> batch;
        batch.reserve(4);
        ...
        for (auto it = frames.begin(); it != frames.end(); ++it) {
            ...
            try {
                ...
                if (st == RS2_STREAM_GYRO) {
                    batch.push_back({ D455_IMU样本::枚举_类型::Gyro, v.x, v.y, v.z, t_us });
                    // 输出：陀螺样本（rad/s）
                    ...
                    batch.push_back({ D455_IMU样本::枚举_类型::Accel, v.x, v.y, v.z, t_us });
                    // 输出：加速度样本（m/s^2）
        ...
        std::sort(batch.begin(), batch.end(), [](const D455_IMU样本& a, const D455_IMU样本& b) { return a.t_us < b.t_us; });
    ...
    void 处理单个IMU样本(const D455_IMU样本& s) {
        if (s.类型 == D455_IMU样本::枚举_类型::Accel) {
    ...
    static inline std::uint64_t FNV1a64_bytes(const void* data, std::size_t n) {
    ...
    std::uint64_t 计算视角键(const 结构体_相机状态& s) const {
        if (!s.有效) return 0;
        ...
        struct 视角键三元组 { std::int64_t r, p, y; } key_data{ qr, qp, qy };
        return FNV1a64_bytes(&key_data, sizeof(key_data));
    }
    ...
    void 配置传感器() {
        ...
        if (有颜色传感器_) {
            rs2::sensor s = rs句柄引用<rs2::sensor>(颜色传感器句柄_);
        ...
        if (有深度传感器_) {
            rs2::sensor s = rs句柄引用<rs2::sensor>(深度传感器句柄_);
    ...
    void 配置滤波链() {
        ...
        try_set(rs句柄引用<rs2::spatial_filter>(空间滤波句柄_), RS2_OPTION_FILTER_SMOOTH_ALPHA, cfg.空间_平滑系数);
        try_set(rs句柄引用<rs2::spatial_filter>(空间滤波句柄_), RS2_OPTION_FILTER_SMOOTH_DELTA, cfg.空间_平滑阈值);
        try_set(rs句柄引用<rs2::spatial_filter>(空间滤波句柄_), RS2_OPTION_HOLES_FILL, cfg.空间_孔洞填充);

        try_set(rs句柄引用<rs2::temporal_filter>(时间滤波句柄_), RS2_OPTION_FILTER_SMOOTH_ALPHA, cfg.时间_平滑系数);
        try_set(rs句柄引用<rs2::temporal_filter>(时间滤波句柄_), RS2_OPTION_FILTER_SMOOTH_DELTA, cfg.时间_平滑阈值);

        try_set(rs句柄引用<rs2::hole_filling_filter>(填洞滤波句柄_), RS2_OPTION_HOLES_FILL, cfg.填洞_模式);
    }
    ...
    bool 设置运行时参数(const std::string& key, const 外设参数值& v) {
        ...
        if (key == "color_auto_exposure") {
            ...
            cfg.彩色_自动曝光 = *b;
            return try_set_sensor(rs句柄引用<rs2::sensor>(颜色传感器句柄_), RS2_OPTION_ENABLE_AUTO_EXPOSURE, *b ? 1.0 : 0.0);
        }
        if (key == "color_auto_white_balance") {
            if (!有颜色传感器_) return false;
            auto b = 取bool(v); if (!b) return false;
            cfg.彩色_自动白平衡 = *b;
            return try_set_sensor(rs句柄引用<rs2::sensor>(颜色传感器句柄_), RS2_OPTION_ENABLE_AUTO_WHITE_BALANCE, *b ? 1.0 : 0.0);
        }
        if (key == "color_exposure") {
            ...
            cfg.彩色_曝光 = *d;
            cfg.彩色_自动曝光 = false;
            try_set_sensor(rs句柄引用<rs2::sensor>(颜色传感器句柄_), RS2_OPTION_ENABLE_AUTO_EXPOSURE, 0.0);
            return try_set_sensor(rs句柄引用<rs2::sensor>(颜色传感器句柄_), RS2_OPTION_EXPOSURE, *d);
        }
        if (key == "color_gain") {
            if (!有颜色传感器_) return false;
            auto d = 取double(v); if (!d) return false;
            ...
            cfg.彩色_自动曝光 = false;
            try_set_sensor(rs句柄引用<rs2::sensor>(颜色传感器句柄_), RS2_OPTION_ENABLE_AUTO_EXPOSURE, 0.0);
            return try_set_sensor(rs句柄引用<rs2::sensor>(颜色传感器句柄_), RS2_OPTION_GAIN, *d);
        }
        if (key == "color_white_balance") {
            ...
            cfg.彩色_白平衡 = *d;
            cfg.彩色_自动白平衡 = false;
            try_set_sensor(rs句柄引用<rs2::sensor>(颜色传感器句柄_), RS2_OPTION_ENABLE_AUTO_WHITE_BALANCE, 0.0);
            return try_set_sensor(rs句柄引用<rs2::sensor>(颜色传感器句柄_), RS2_OPTION_WHITE_BALANCE, *d);
        }
        ...
        if (key == "depth_emitter") {
            ...
            auto b = 取bool(v); if (!b) return false;
            cfg.深度_启用发射器 = *b;
            return try_set_sensor(rs句柄引用<rs2::sensor>(深度传感器句柄_), RS2_OPTION_EMITTER_ENABLED, *b ? 1.0 : 0.0);
        }
        if (key == "depth_laser_power") {
            if (!有深度传感器_) return false;
            auto d = 取double(v); if (!d) return false;
            cfg.深度_激光功率 = *d;
            return try_set_sensor(rs句柄引用<rs2::sensor>(深度传感器句柄_), RS2_OPTION_LASER_POWER, *d);
        }
        ...
        if (try_set_filter("spatial_alpha", rs句柄引用<rs2::spatial_filter>(空间滤波句柄_), RS2_OPTION_FILTER_SMOOTH_ALPHA, cfg.空间_平滑系数)) return true;
        if (try_set_filter("spatial_delta", rs句柄引用<rs2::spatial_filter>(空间滤波句柄_), RS2_OPTION_FILTER_SMOOTH_DELTA, cfg.空间_平滑阈值)) return true;
        if (try_set_filter("spatial_holes", rs句柄引用<rs2::spatial_filter>(空间滤波句柄_), RS2_OPTION_HOLES_FILL, cfg.空间_孔洞填充)) return true;

        if (try_set_filter("temporal_alpha", rs句柄引用<rs2::temporal_filter>(时间滤波句柄_), RS2_OPTION_FILTER_SMOOTH_ALPHA, cfg.时间_平滑系数)) return true;
        if (try_set_filter("temporal_delta", rs句柄引用<rs2::temporal_filter>(时间滤波句柄_), RS2_OPTION_FILTER_SMOOTH_DELTA, cfg.时间_平滑阈值)) return true;

        if (try_set_filter("holes_fill", rs句柄引用<rs2::hole_filling_filter>(填洞滤波句柄_), RS2_OPTION_HOLES_FILL, cfg.填洞_模式)) return true;

        return false;
    }

    bool 读取对齐彩色(const void* color句柄, 结构体_原始场景帧& out) {
        const auto& color = rs句柄引用<rs2::video_frame>(color句柄);
        const int w = out.宽度;
        const int h = out.高度;
    ...
    void 生成点云(结构体_原始场景帧& 帧) {
        const double fx = rs句柄引用<rs2_intrinsics>(深度内参句柄_).fx;
        const double fy = rs句柄引用<rs2_intrinsics>(深度内参句柄_).fy;
        const double cx = rs句柄引用<rs2_intrinsics>(深度内参句柄_).ppx;
        const double cy = rs句柄引用<rs2_intrinsics>(深度内参句柄_).ppy;

...
$项目路径 = Join-Path $PSScriptRoot '不可变材料结构参数验证.vcxproj'
$临时根 = Join-Path ([System.IO.Path]::GetTempPath()) '海中鱼巣_不可变材料结构参数验证'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "不可变材料结构参数验证项目构建失败，退出码 $LASTEXITCODE。"
    }
    $程序路径 = Join-Path $输出目录 '不可变材料结构参数验证.exe'
    if (-not (Test-Path -LiteralPath $程序路径 -PathType Leaf)) {
        throw "验证程序不存在：$程序路径"
    }
    & $程序路径
    if ($LASTEXITCODE -ne 0) {
        throw "不可变材料结构参数验证失败，退出码 $LASTEXITCODE。"
    }
} finally {

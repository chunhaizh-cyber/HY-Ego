...
$项目路径 = Join-Path $PSScriptRoot 'L2存在单例角色结构参数验证.vcxproj'
$临时根 = Join-Path ([System.IO.Path]::GetTempPath()) '海中鱼巣_L2存在单例角色结构参数验证'
...
if ($LASTEXITCODE -ne 0) {
    throw "L2 存在单例角色结构参数验证项目构建失败，退出码 $LASTEXITCODE。"
}

$程序路径 = Join-Path $输出目录 'L2存在单例角色结构参数验证.exe'
...
& $程序路径
if ($LASTEXITCODE -ne 0) {
    throw "L2 存在单例角色结构参数验证失败，退出码 $LASTEXITCODE。"
}

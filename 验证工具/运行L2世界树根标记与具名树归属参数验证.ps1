...
$项目路径 = Join-Path $PSScriptRoot 'L2世界树根标记与具名树归属参数验证.vcxproj'
$临时根 = Join-Path ([System.IO.Path]::GetTempPath()) '海中鱼巣_L2世界树根标记与具名树归属参数验证'
...
if ($LASTEXITCODE -ne 0) { throw "R1 专项构建失败，退出码 $LASTEXITCODE。" }

$程序路径 = Join-Path $输出目录 'L2世界树根标记与具名树归属参数验证.exe'
if (-not (Test-Path -LiteralPath $程序路径 -PathType Leaf)) { throw "验证程序不存在：$程序路径" }
Write-Host "验证临时目录：$本轮目录"
& $程序路径 '--幂等基值' $幂等基值
if ($LASTEXITCODE -ne 0) { throw "R1 专项验证失败，退出码 $LASTEXITCODE。" }

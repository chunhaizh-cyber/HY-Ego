...
void 验证请求与状态(夹具 &x) {
  ...
  const auto 存在删除后 = x.存在->读取当前组成父_v2({2, x.G(), x.E[5]});
  检查(存在删除后.状态 == 存在组成当前完整读取状态_v2::未找到 &&
             !存在删除后.父 && 存在删除后.子组.empty(),
       "existence-deleted-endpoint-missing");
  const auto 场景删除后 =
      x.场景->读取当前场景包含父组_v2({2, x.G(), x.场景事实.at(9)});
  检查(场景删除后.状态 == 场景直接包含当前完整读取状态_v2::未找到 &&
             场景删除后.包含组.empty(),
       "scene-deleted-endpoint-missing");

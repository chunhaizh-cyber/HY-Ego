...
$项目路径 = Join-Path $PSScriptRoot 'L2因果引用来源换代退出参数验证.vcxproj'
$本轮目录 = Join-Path ([System.IO.Path]::GetTempPath()) ('海中鱼巣_C2_' + [System.Guid]::NewGuid().ToString('N'))
...
if ($LASTEXITCODE -ne 0) { throw "C2 专项工程构建失败：$LASTEXITCODE" }
$程序路径 = Join-Path $输出目录 'L2因果引用来源换代退出参数验证.exe'
& $程序路径 '--幂等基值' $幂等基值 '--条件一顺序' $条件一顺序 '--条件二顺序' $条件二顺序 '--结果一顺序' $结果一顺序 '--结果二顺序' $结果二顺序
if ($LASTEXITCODE -ne 0) { throw "C2 专项验证失败：$LASTEXITCODE" }

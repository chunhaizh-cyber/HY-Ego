...
[[nodiscard]] 自我线程创建请求_v1 建立请求(const std::uint64_t 逻辑身份 = 20) {
    自我线程创建请求_v1 请求;
    请求.运行代次 = { 10 };
    请求.逻辑身份 = { 逻辑身份 };
    请求.邮箱容量 = 2;
...
void 验证基础生命周期与邮箱() {
    ...
    重复请求.上下文预算 = { 8, 8, 32 };
    重复请求.根复核预算 = { 8, 16, 32 };
    const auto 重复 = 线程.创建并停在治理运行门(重复请求, 2000);
    检查(重复.状态 == 自我线程操作状态::精确重复 && 重复.成功(),
        "budget retry reuses physical selection");
    ...
    auto 错代消息 = 建立消息(70, 自我线程复核触发根::安全根, 50);
    错代消息.运行代次 = { 11 };
    检查(线程.提交根需求复核消息(错代消息).状态 == 自我线程操作状态::运行代次不匹配,
        "message generation mismatch rejected");

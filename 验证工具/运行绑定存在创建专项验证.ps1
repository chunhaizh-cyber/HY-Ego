param(
    [string]$OutputRoot = 'D:/TEMP/海中鱼巣/EXISTENCE-BOUND-CREATION/01a06f6b-20260912-code',
    [ValidateSet('Debug','Release','Both')][string]$Configuration = 'Both'
)
$ErrorActionPreference = 'Stop'
$repoRoot = Split-Path $PSScriptRoot -Parent
$project = Join-Path $PSScriptRoot '绑定存在创建专项验证.vcxproj'
$msbuild = 'C:/Program Files/Microsoft Visual Studio/18/Professional/MSBuild/Current/Bin/MSBuild.exe'
$resolvedOutput = [IO.Path]::GetFullPath($OutputRoot).TrimEnd('\','/')
$allowedRoot = [IO.Path]::GetFullPath('D:/TEMP/海中鱼巣/EXISTENCE-BOUND-CREATION/01a06f6b-20260912-code').TrimEnd('\','/')
if (-not $resolvedOutput.Equals($allowedRoot,[StringComparison]::OrdinalIgnoreCase)) { throw 'Unexpected output root' }
...
foreach ($config in $configs) {
    ...
    & $msbuild $project /t:Rebuild /m:1 "/p:Configuration=$config" /p:Platform=x64 "/p:OutDir=$dir/out/" "/p:IntDir=$dir/obj/" /p:TargetName=bound-existence-core /v:minimal *> $buildLog
    ...
    $clock.Restart()
    & (Join-Path $dir 'out/bound-existence-core.exe') *> $runLog

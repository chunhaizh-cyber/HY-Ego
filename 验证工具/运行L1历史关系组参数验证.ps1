...
$项目路径 = Join-Path $PSScriptRoot 'L1历史关系组参数验证.vcxproj'
$临时根 = Join-Path ([System.IO.Path]::GetTempPath()) '海中鱼巣_L1历史关系组参数验证'
...
if ($LASTEXITCODE -ne 0) {
    throw "L1 历史关系组参数验证项目构建失败，退出码 $LASTEXITCODE。"
}

$程序路径 = Join-Path $输出目录 'L1历史关系组参数验证.exe'
...
if ($LASTEXITCODE -ne 0) {
    throw "L1 历史关系组参数验证失败，退出码 $LASTEXITCODE。"
}

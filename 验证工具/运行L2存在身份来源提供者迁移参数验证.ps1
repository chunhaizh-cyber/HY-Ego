...
$项目路径 = Join-Path $PSScriptRoot 'L2存在身份来源提供者迁移参数验证.vcxproj'
$临时根 = Join-Path ([System.IO.Path]::GetTempPath()) '海中鱼巣_L2存在身份来源提供者迁移参数验证'
...
try {
    ...
    $程序路径 = Join-Path $输出目录 'L2存在身份来源提供者迁移参数验证.exe'

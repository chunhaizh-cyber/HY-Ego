# 文件规则：本工具只从施工流程图和当前代码生成文档基线；不得修改代码、计划索引或项目记忆，不得把未匹配施工节点声明为已实现。
...
def 分类(名称: str) -> tuple[str, str]:
    ...
    return "已实现或已形成第一轮入口", "当前代码存在对应入口；完成范围仍以实施记录和现状边界为准。"
...
def 未实现图(标题: str, 原因: str) -> str:
    ...
    return "\n".join([
        "flowchart TD",
        f'    A["对应施工图：{安全标题}"] --> B["扫描当前代码、计划和实施记录"]',
        '    B --> C{"是否存在完整正式实现入口？"}',
        f'    C -- "否" --> D["当前现状：{安全原因}"]',

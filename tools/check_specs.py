...
def 检查任务状态治理() -> list[检查项]:
    ...
    if "#352" in 当前项 and 当前项["#352"].字段["计划版本"] != "v0.1":
        ...
        for 依赖 in ("#347", "#348", "#349", "#350", "#351"):
            if 依赖 not in 当前项 or 当前项[依赖].字段["当前生命周期"] != "已完成 / 已归档":
                结果.append(检查项("ERROR", 文件相对路径, f"#352 升版前 {依赖} 尚未完成 / 归档"))
        ...
        if (
            not re.search(r"^门禁结果[：:]\s*PASS\s*$", 三五二文本, re.MULTILINE)
            or 门禁片段 != ["门禁结果=PASS"]
        ):
            结果.append(检查项("ERROR", 文件相对路径, "#352 升版缺少结构化门禁结果 PASS"))
        证据路径集合: set[str] = set()
        证据Blob集合: set[str] = set()
        证据内容合同 = {
            "调用点映射": ["调用点", "文件", "符号", "处置"],
            "最终删除集": ["删除项", "文件", "对象", "删除依据"],
            "执行白名单": ["文件", "允许动作", "对应删除项", "验证"],
        }
        for 证据名 in ("调用点映射", "最终删除集", "执行白名单"):
            证据模式 = re.compile(
                rf"^{证据名}冻结[：:]\s*`(?P<路径>[^`]+)`\s*/\s*blob\s*`(?P<blob>[0-9a-f]{{40}})`\s*$",
                re.MULTILINE,
            )
            证据匹配 = 证据模式.search(三五二文本)
            if not 证据匹配:
                结果.append(检查项("ERROR", 三五二["计划路径"], f"#352 升版缺少 {证据名} 路径 / blob 冻结字段"))
                continue
            证据路径 = 证据匹配.group("路径")
            证据Blob = 证据匹配.group("blob")
            事件绑定 = [片段 for 片段 in 证据片段 if re.fullmatch(rf"{证据名}=[0-9a-f]{{40}}", 片段)]
            if 事件绑定 != [f"{证据名}={证据Blob}"]:
                结果.append(检查项("ERROR", 文件相对路径, f"#352 最新事件未绑定 {证据名} blob"))
            证据文件, 路径错误 = 解析仓库内路径(证据路径)
            if 路径错误 or 证据文件 is None:
                结果.append(检查项("ERROR", 三五二["计划路径"], f"#352 {证据名} 路径非法: {路径错误}"))
                continue
            实际Blob, Blob错误 = 计算工作树Blob(证据路径)
            if Blob错误 or 实际Blob != 证据Blob:
                结果.append(检查项("ERROR", 三五二["计划路径"], f"#352 {证据名} 产物路径 / blob 与工作树不一致"))
            if 证据路径 in 证据路径集合 or 证据Blob in 证据Blob集合:
                结果.append(检查项("ERROR", 三五二["计划路径"], "#352 三类证据产物必须使用互不相同的路径和内容"))
            证据路径集合.add(证据路径)
            证据Blob集合.add(证据Blob)
            if not 证据文件.is_file():
                结果.append(检查项("ERROR", 三五二["计划路径"], f"#352 {证据名} 产物不存在"))
                continue
            产物文本 = 读取文本(证据文件)
            必需元数据 = [
                rf"^证据类型[：:]\s*{re.escape(证据名)}\s*$",
                r"^任务[：:]\s*#352\s*$",
                rf"^计划版本[：:]\s*{re.escape(三五二['计划版本'])}\s*$",
                r"^证据状态[：:]\s*PASS\s*$",
            ]
            for 元数据模式 in 必需元数据:
                if not re.search(元数据模式, 产物文本, re.MULTILINE):
                    结果.append(检查项("ERROR", 证据路径, f"#352 {证据名} 缺少内容合同字段: {元数据模式}"))
            总数匹配 = re.findall(r"^条目总数[：:]\s*(\d+)\s*$", 产物文本, re.MULTILINE)
            产物行, 产物错误 = 解析Markdown表格(产物文本, 证据内容合同[证据名][0], 证据内容合同[证据名], 证据路径)
            结果.extend(产物错误)
            空内容行 = [
                行号 for 行号, 行字段 in 产物行
                if any(not 值.strip() or 值.strip() == "—" for 值 in 行字段.values())
            ]
            if 空内容行:
                结果.append(检查项("ERROR", 证据路径, f"#352 {证据名} 存在空内容行: {', '.join(map(str, 空内容行))}"))
            唯一内容 = {tuple(行字段[字段名] for 字段名 in 证据内容合同[证据名]) for _, 行字段 in 产物行}
            if len(唯一内容) != len(产物行):
                结果.append(检查项("ERROR", 证据路径, f"#352 {证据名} 存在重复证据行"))
            if len(总数匹配) != 1 or int(总数匹配[0]) < 1:
                结果.append(检查项("ERROR", 证据路径, f"#352 {证据名} 必须声明一个正数条目总数"))
            elif len(产物行) != int(总数匹配[0]):
                结果.append(检查项("ERROR", 证据路径, f"#352 {证据名} 条目总数与结构化表不一致"))
        if "预冻结候选文件（非执行白名单）" in 三五二文本:
            结果.append(检查项("ERROR", 三五二["计划路径"], "#352 升版后仍保留 v0.1 预冻结候选口径"))

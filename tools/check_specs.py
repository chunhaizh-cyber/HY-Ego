...
合法任务迁移 = {
    # 已派发待执行回执仅用于重放 JY-608/E349 及以前的不可变历史；新协议不再生成该迁移。
    "待执行": {"执行中", "已派发待执行回执", "依赖门控待执行", "接口漂移待设计修订"},
    "依赖门控待执行": {"待执行"},
    "已派发待执行回执": {"执行中", "接口漂移待设计修订", "调度权越界退回"},
    "调度权越界退回": {"待执行"},
    "执行中": {"分支完成待集成", "接口漂移待设计修订", "验证失败待设计裁决", "调度权越界退回"},
    # 已形成候选后，独立只读审计仍可能证明冻结共享合同本身错误；
    # 这时必须保留候选 Git 证据并退回设计，不能伪装成可继续集成。
    "分支完成待集成": {"已预留待集成派发", "接口漂移待设计修订"},
    "已预留待集成派发": {"已派发待集成回执"},
...
合法S0结果 = {"不适用", "未开始", "PASS", "PASS待设计接受", "PASS已接受", "DRIFT"}
合法技术绑定阶段 = {"未绑定", "预登记", "已冻结", "通道已绑定"}
状态责任角色 = {
...
长期执行通道身份模式 = re.compile(r"EXEC-[A-Z0-9]+(?:-[A-Z0-9]+)*-R\d+")
集成工作树身份模式 = re.compile(r"INT-[A-Z0-9]+(?:-[A-Z0-9]+)*-R\d+")
合法计划段状态 = {"未开始", "活动中", "候选完成", "具名WIP", "已退出"}
叶子身份模式 = re.compile(
...
def 提取精确允许文件(计划文本: str, 当前计划路径: str | None = None) -> tuple[set[str], str | None]:
    标题匹配 = re.search(
        r"^##\s+\d+(?:\.\d+)*\.\s*(?:精确允许文件(?:与所有权)?|精确允许与禁止范围|执行文件所有权与白名单合同)\s*$",
        计划文本,
        re.MULTILINE,
    )
    if not 标题匹配:
...
def 检查计划索引治理() -> list[检查项]:
    ...
    for 行号, 字段 in 叶子行:
        ...
        计划文件, 计划路径错误 = 解析仓库内路径(字段["计划"])
        if 计划路径错误 or 计划文件 is None:
            结果.append(检查项("ERROR", 相对路径, f"{任务} 计划路径非法: {字段['计划']}（{计划路径错误}）"))
            continue
        if not 计划文件.is_file():
            结果.append(检查项("ERROR", 相对路径, f"{任务} 当前计划路径不存在: {字段['计划']}"))
            continue
        计划文本 = 读取文本(计划文件)
        身份 = 叶子身份模式.search(计划文本)
        if not 身份:
            结果.append(检查项("ERROR", 转为相对路径(计划文件), "计划索引指向的叶子缺少可解析身份"))
            continue
        实际任务 = f"#{身份['任务']}"
        实际版本 = 身份["版本"]
        if 实际任务 != 任务:
            结果.append(检查项("ERROR", 相对路径, f"{任务} 索引路径实际属于 {实际任务}"))
        if f"计划 {实际版本}" not in 字段["任务状态定位键"]:
            结果.append(检查项("ERROR", 相对路径, f"{任务} 任务状态定位键未绑定实际计划版本 {实际版本}"))
        if not 计划文件.name.endswith(f"_{实际版本}.md"):
            结果.append(检查项("ERROR", 相对路径, f"{任务} 计划文件名与正文版本 {实际版本} 不闭合"))
        if 激活状态 != "已退出" and "计划/过期计划/" in 转为相对路径(计划文件):
            结果.append(检查项("ERROR", 相对路径, f"{任务} 活动计划不得指向过期计划目录"))
...
def 检查任务状态治理() -> list[检查项]:
    ...
    for 行号, 字段 in 当前原始:
        ...
        if 绑定阶段 == "未绑定":
            ...
            for 字段名 in 绑定字段:
                if 字段[字段名] == "—":
                    结果.append(检查项("ERROR", 文件相对路径, f"{任务} 通道已绑定阶段缺少 {字段名}"))
            基线是否为空 = [字段[字段名] == "—" for 字段名 in 基线字段]
            if any(基线是否为空) and not all(基线是否为空):
                结果.append(检查项("ERROR", 文件相对路径, f"{任务} 通道计划段基线版本与提交必须同时填写或同时为空"))
            if not any(基线是否为空) and not re.fullmatch(r"[0-9a-f]{40}", 字段["冻结基线提交"]):
                结果.append(检查项("ERROR", 文件相对路径, f"{任务} 通道计划段起点必须是完整 40 位哈希"))

        if 字段["当前生命周期"] in {"待执行", "依赖门控待执行", "已预留待集成派发"}:
            for 字段名 in ("派发状态版本", "派发计划 blob", "S0 合同版本", "S0 计划 blob", "S0 结果版本"):
                if 字段[字段名] != "—":
                    结果.append(检查项("ERROR", 文件相对路径, f"{任务} 在 {字段['当前生命周期']} 阶段不得把旧派发 / S0 字段作为当前许可: {字段名}"))
            if 字段["S0 结果"] not in {"不适用", "未开始"}:
                结果.append(检查项("ERROR", 文件相对路径, f"{任务} 在 {字段['当前生命周期']} 阶段 S0 结果只能为不适用或未开始"))
        ...
        if 字段["当前生命周期"] in 已开始且需要S0状态 | {"已派发待执行回执", "已派发待集成回执"}:
            允许通道绑定 = (
                绑定阶段 == "通道已绑定"
                and 长期执行通道身份模式.fullmatch(字段["worktree 引用"]) is not None
            )
            if 绑定阶段 != "已冻结" and not 允许通道绑定:
                结果.append(检查项("ERROR", 文件相对路径, f"{任务} 在 {字段['当前生命周期']} 前技术身份必须已冻结"))
            for 字段名 in ("执行身份", "worktree 引用", "冻结基线版本", "冻结基线提交"):
                if 字段[字段名] == "—":
                    结果.append(检查项("ERROR", 文件相对路径, f"{任务} 在 {字段['当前生命周期']} 阶段缺少 {字段名}"))
            for 字段名 in ("派发计划 blob", "S0 计划 blob"):
                if 字段[字段名] != "—" and 字段[字段名] != 字段["计划 blob"]:
                    结果.append(检查项("ERROR", 文件相对路径, f"{任务} 历史 {字段名} 与当前计划 blob 不一致"))
        if 字段["当前生命周期"] in {"已派发待执行回执", "已派发待集成回执"} and 字段["S0 结果"] not in {"未开始", "PASS待设计接受"}:
            结果.append(检查项("ERROR", 文件相对路径, f"{任务} 历史派发回执阶段 S0 结果非法"))
        当前状态版本元组 = 解析状态版本(字段["状态版本"])
        S0结果版本元组 = 解析状态版本(字段["S0 结果版本"])
        历史PASS已接受 = (
            字段["S0 结果"] == "PASS已接受"
            and 当前状态版本元组 is not None
            and S0结果版本元组 is not None
            and 当前状态版本元组 <= 历史设计接受截止版本
            and S0结果版本元组 <= 历史设计接受截止版本
        )
        if 字段["当前生命周期"] in 已开始且需要S0状态 and 字段["S0 结果"] != "PASS" and not 历史PASS已接受:
            结果.append(检查项("ERROR", 文件相对路径, f"{任务} 已开始状态必须由执行 / 集成角色自行 S0 PASS；不得等待设计接受"))
        if 字段["S0 结果"] in {"PASS", "PASS待设计接受", "PASS已接受", "DRIFT"}:
            if 字段["S0 结果版本"] == "—":
    ...
    通道字段 = ["通道", "目标路径 / 分支", "建立基线", "执行身份", "技术状态"]
    计划段字段 = [
        "通道", "任务", "计划版本", "计划 blob", "计划路径", "中央设计提交",
        "计划段起点", "结果 / WIP 提交", "段状态",
    ]
    通道标题数量 = len(re.findall(r"^###\s+1\.4\s+长期执行通道结构化登记\s*$", 登记表文本, re.MULTILINE))
    目标标题数量 = len(re.findall(r"^###\s+1\.4\s+长期执行通道目标登记\s*$", 登记表文本, re.MULTILINE))
    计划段标题数量 = len(re.findall(r"^####\s+1\.4\.1\s+计划段登记\s*$", 登记表文本, re.MULTILINE))
    结构化通道已建立 = 通道标题数量 == 1
    if 通道标题数量 + 目标标题数量 != 1:
        结果.append(
            检查项(
                "ERROR", "项目记忆/并行工作树登记表.md",
                "长期执行通道必须且只能处于“目标登记”或“结构化登记”一个阶段",
            )
        )
    if 通道标题数量 > 1 or 目标标题数量 > 1:
        结果.append(检查项("ERROR", "项目记忆/并行工作树登记表.md", "长期执行通道阶段标题重复"))
    if 结构化通道已建立:
        if 计划段标题数量 != 1:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"结构化长期执行通道必须且只能有一个计划段登记，实际 {计划段标题数量} 个",
                )
            )
        通道原始, 通道解析结果 = 解析全部同结构表格(
            登记表文本, 通道字段, "项目记忆/并行工作树登记表.md"
        )
        计划段原始, 计划段解析结果 = 解析全部同结构表格(
            登记表文本, 计划段字段, "项目记忆/并行工作树登记表.md"
        )
        结果.extend(通道解析结果)
        结果.extend(计划段解析结果)
    else:
        ...
        if 计划段标题数量:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    "长期执行通道尚处目标登记阶段，不得提前形成计划段登记",
                )
            )
        目标标题 = re.search(r"^###\s+1\.4\s+长期执行通道目标登记\s*$", 登记表文本, re.MULTILINE)
        if 目标标题:
            后续标题 = re.search(r"^##\s+", 登记表文本[目标标题.end():], re.MULTILINE)
            目标结束 = 目标标题.end() + 后续标题.start() if 后续标题 else len(登记表文本)
            目标文本 = 登记表文本[目标标题.end():目标结束]
            目标合同 = {
                "目标身份": r"^目标身份[：:]\s*(EXEC-[A-Z0-9]+(?:-[A-Z0-9]+)*-R\d+)\s*$",
                "目标路径": r"^目标路径[：:]\s*(.+)\s*$",
                "目标分支": r"^目标分支[：:]\s*(codex/execution-[a-z0-9]+(?:-[a-z0-9]+)*-r\d+)\s*$",
                "目标执行顶层任务": r"^目标执行顶层任务[：:]\s*([0-9a-f-]{36})\s*$",
                "绑定起点": r"^(?:建立|绑定)起点[：:]\s*(.+)\s*$",
                "当前技术事实": r"^当前技术事实[：:]\s*(.+)\s*$",
            }
            目标值: dict[str, str] = {}
            for 字段名, 模式 in 目标合同.items():
                匹配 = re.findall(模式, 目标文本, re.MULTILINE)
                if len(匹配) != 1:
                    结果.append(
                        检查项(
                            "ERROR", "项目记忆/并行工作树登记表.md",
                            f"长期执行通道目标登记必须唯一声明 {字段名}",
                        )
                    )
                else:
                    目标值[字段名] = 匹配[0]
            if "目标路径" in 目标值 and not Path(目标值["目标路径"]).is_absolute():
                结果.append(检查项("ERROR", "项目记忆/并行工作树登记表.md", "长期执行通道目标路径不是绝对路径"))
            if "绑定起点" in 目标值 and not (
                re.search(r"[0-9a-f]{40}", 目标值["绑定起点"])
                or re.search(r"JY-\d+/E\d+", 目标值["绑定起点"])
            ):
                结果.append(
                    检查项(
                        "ERROR", "项目记忆/并行工作树登记表.md",
                        "长期执行通道目标建立起点缺少完整提交或唯一状态版本",
                    )
                )
            if "当前技术事实" in 目标值 and not (
                ("待绑定" in 目标值["当前技术事实"] and "尚无结构化" in 目标值["当前技术事实"])
                or "待建立" in 目标值["当前技术事实"]
            ):
                结果.append(
                    检查项(
                        "ERROR", "项目记忆/并行工作树登记表.md",
                        "目标登记阶段必须明确通道仍待绑定且尚无结构化登记",
                    )
                )

    通道项: dict[str, dict[str, str]] = {}
    通道技术身份: dict[str, tuple[str, str, str]] = {}
    已登记通道路径: dict[str, str] = {}
    已登记通道分支: dict[str, str] = {}
    for 行号, 字段 in 通道原始:
        通道 = 字段["通道"]
        if not 长期执行通道身份模式.fullmatch(通道):
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"第 {行号} 行长期执行通道身份格式非法: {通道}",
                )
            )
            continue
        if 通道 in 通道项:
            结果.append(检查项("ERROR", "项目记忆/并行工作树登记表.md", f"长期执行通道重复: {通道}"))
            continue
        通道项[通道] = 字段
        路径分支 = re.findall(r"`([^`]+)`", 字段["目标路径 / 分支"])
        if len(路径分支) != 2:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} 必须精确登记一个绝对路径和一个分支",
                )
            )
            continue
        目标路径, 目标分支 = 路径分支
        if not Path(目标路径).is_absolute():
            结果.append(检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} 目标路径不是绝对路径"))
        if not re.fullmatch(r"codex/execution-[a-z0-9]+(?:-[a-z0-9]+)*-r\d+", 目标分支):
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} 分支不符合长期执行通道格式: {目标分支}",
                )
            )
        if 目标路径 in 已登记通道路径:
            结果.append(
        ...
        if 目标分支 in 已登记通道分支:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} 与 {已登记通道分支[目标分支]} 重复登记同一分支",
                )
            )
        else:
            已登记通道分支[目标分支] = 通道
        基线匹配 = re.findall(r"(?<![0-9a-f])[0-9a-f]{40}(?![0-9a-f])", 字段["建立基线"])
        if len(基线匹配) != 1:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} 建立基线必须精确包含一个完整提交",
                )
            )
            continue
        建立基线 = 基线匹配[0]
        if 运行Git("cat-file", "-e", f"{建立基线}^{{commit}}").returncode != 0:
            结果.append(检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} 建立基线不是可解析提交"))
        if 字段["执行身份"] == "—":
            结果.append(检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} 缺少长期执行顶层身份"))
        if 字段["技术状态"] == "—":
            结果.append(检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} 缺少 Git 技术状态"))
        通道技术身份[通道] = (目标路径, 目标分支, 建立基线)

    计划段项: dict[tuple[str, str], dict[str, str]] = {}
    每通道计划段: dict[str, list[dict[str, str]]] = {}
    for 行号, 字段 in 计划段原始:
        通道 = 字段["通道"]
        任务 = 字段["任务"]
        定位键 = (通道, 任务)
        if 通道 not in 通道项:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"第 {行号} 行计划段引用未登记通道: {通道}",
                )
            )
        if 任务 not in 受管任务编号 or not 340 <= int(任务.removeprefix("#")) <= 352:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"第 {行号} 行长期通道计划段任务不在 #340—#352: {任务}",
                )
            )
        if 定位键 in 计划段项:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"同一通道中的任务计划段重复: {通道} / {任务}",
                )
            )
            continue
        计划段项[定位键] = 字段
        每通道计划段.setdefault(通道, []).append(字段)
        if 字段["段状态"] not in 合法计划段状态:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} / {任务} 使用非法计划段状态: {字段['段状态']}",
                )
            )
            ...
            for 字段名, 预期 in 对照.items():
                if 字段[字段名] != 预期:
                    结果.append(
                        检查项(
                            "ERROR", "项目记忆/并行工作树登记表.md",
                            f"{通道} / {任务} 的 {字段名} 与任务状态台账不一致",
                        )
                    )
        计划文件, 计划路径错误 = 解析仓库内路径(字段["计划路径"])
        if 计划路径错误 or 计划文件 is None or not 计划文件.is_file():
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} / {任务} 计划路径不可读取: {字段['计划路径']}",
                )
            )
        else:
            计划文本 = 读取文本(计划文件)
            身份 = 叶子身份模式.search(计划文本)
            if not 身份 or f"#{身份['任务']}" != 任务 or 身份["版本"] != 字段["计划版本"]:
                结果.append(
                    检查项(
                        "ERROR", 字段["计划路径"],
                        f"{通道} / {任务} 计划路径、任务和版本没有闭合",
                    )
                )
            工作树Blob, Blob错误 = 计算工作树Blob(字段["计划路径"])
            if Blob错误 or 工作树Blob != 字段["计划 blob"]:
                结果.append(
                    检查项(
                        "ERROR", 字段["计划路径"],
                        f"{通道} / {任务} 计划 blob 与当前不可变施工合同不一致",
                    )
                )
            _, 允许错误 = 提取精确允许文件(计划文本, 字段["计划路径"])
            if 允许错误:
                结果.append(
                    检查项(
                        "ERROR", 字段["计划路径"],
                        f"{通道} / {任务} 无法建立计划段允许范围: {允许错误}",
                    )
                )

        中央设计提交 = 字段["中央设计提交"]
        中央提交哈希: str | None = None
        if re.fullmatch(r"[0-9a-f]{40}", 中央设计提交):
            中央提交哈希 = 中央设计提交
        elif 任务状态版本模式.fullmatch(中央设计提交):
            中央提交哈希 = 核对状态版本(
                中央设计提交,
                "项目记忆/并行工作树登记表.md",
                f"{通道} / {任务} 中央设计状态版本 ",
            ...
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} / {任务} 中央设计提交必须是完整提交或唯一状态版本",
                )
            )
        if 中央提交哈希 and 运行Git("cat-file", "-e", f"{中央提交哈希}^{{commit}}").returncode != 0:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} / {任务} 中央设计提交不可解析",
        ...
        elif 中央提交哈希 and 读取提交Blob(中央提交哈希, 字段["计划路径"]) != 字段["计划 blob"]:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} / {任务} 中央设计提交没有冻结当前计划 blob",
                )
            )

        起点 = 字段["计划段起点"]
        结果提交 = 字段["结果 / WIP 提交"]
        起点有效 = bool(re.fullmatch(r"[0-9a-f]{40}", 起点))
        结果有效 = bool(re.fullmatch(r"[0-9a-f]{40}", 结果提交))
        if 起点 != "—" and not 起点有效:
            结果.append(
                检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} / {任务} 计划段起点格式非法")
            )
        if 结果提交 != "—" and not 结果有效:
            结果.append(
                检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} / {任务} 结果 / WIP 提交格式非法")
            )
        if 起点有效 and 运行Git("cat-file", "-e", f"{起点}^{{commit}}").returncode != 0:
            结果.append(
                检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} / {任务} 计划段起点不可解析")
            )
        if 结果有效 and 运行Git("cat-file", "-e", f"{结果提交}^{{commit}}").returncode != 0:
            结果.append(
                检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} / {任务} 结果 / WIP 提交不可解析")
            )
        if 字段["段状态"] == "未开始" and (起点 != "—" or 结果提交 != "—"):
            结果.append(
                检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} / {任务} 未开始计划段不得提前登记起点或结果")
            )
        if 字段["段状态"] == "活动中" and (not 起点有效 or 结果提交 != "—"):
            结果.append(
                检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} / {任务} 活动计划段必须有起点且不得提前登记结果")
            )
        if 字段["段状态"] in {"候选完成", "具名WIP"} and (not 起点有效 or not 结果有效):
            结果.append(
                检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} / {任务} 已收口计划段必须同时登记起点和结果提交")
            )
        if 起点有效 and 结果有效 and 运行Git("merge-base", "--is-ancestor", 起点, 结果提交).returncode != 0:
            结果.append(
                检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} / {任务} 结果提交不是计划段起点的后代")
            )

    for 通道, 计划段 in 每通道计划段.items():
        活动计划段 = [字段 for 字段 in 计划段 if 字段["段状态"] == "活动中"]
        if len(活动计划段) > 1:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} 同时登记了 {len(活动计划段)} 个活动计划段",
                )
            )
        if 通道 not in 通道技术身份:
            continue
        ...
        for 字段 in 计划段:
            起点 = 字段["计划段起点"]
            结果提交 = 字段["结果 / WIP 提交"]
            if not re.fullmatch(r"[0-9a-f]{40}", 起点) or not re.fullmatch(r"[0-9a-f]{40}", 结果提交):
                continue
            if 起点 in 已收口边:
                结果.append(
                    检查项(
                        "ERROR", "项目记忆/并行工作树登记表.md",
                        f"{通道} 多个已收口计划段共享同一起点 {起点}",
                    )
                )
            已收口边[起点] = 结果提交
            if 结果提交 in 结果到任务:
        ...
        当前提交 = 建立基线
        已访问: set[str] = set()
        while 当前提交 in 已收口边 and 当前提交 not in 已访问:
            已访问.add(当前提交)
            当前提交 = 已收口边[当前提交]
        if 当前提交 in 已访问:
            结果.append(检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} 计划段累计父链形成循环"))
        if len(已访问) != len(已收口边):
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} 已收口计划段没有从建立基线形成单一累计父链",
                )
            )
        if 活动计划段 and 活动计划段[0]["计划段起点"] != 当前提交:
            结果.append(
                检查项(
                    "ERROR", "项目记忆/并行工作树登记表.md",
                    f"{通道} 活动计划段起点不是上一已收口段的终端提交",
                )
            )

        通道状态 = 通道项[通道]["技术状态"]
        登记HEAD = re.findall(r"(?:^|[；;\s])HEAD=([0-9a-f]{40})(?=\s|[；;，,。/]|$)", 通道状态)
        if len(登记HEAD) > 1:
            结果.append(检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} 技术状态重复登记 HEAD"))
        elif len(登记HEAD) == 1:
            if 活动计划段:
                if 活动计划段[0]["计划段起点"] != 登记HEAD[0]:
                    结果.append(
                        检查项(
                            "ERROR", "项目记忆/并行工作树登记表.md",
                            f"{通道} 活动计划段起点与登记 HEAD 不一致",
                        )
                    )
            elif 当前提交 != 登记HEAD[0]:
                结果.append(
                    检查项(
                        "ERROR", "项目记忆/并行工作树登记表.md",
                        f"{通道} 累计计划段终端提交与登记 HEAD 不一致",
                    )
                )

        目标路径, 目标分支, _ = 通道技术身份[通道]
        if Path(目标路径).is_dir():
            分支结果 = subprocess.run(
                ["git", "branch", "--show-current"], cwd=目标路径, text=True, encoding="utf-8",
                errors="replace", capture_output=True, check=False,
            )
            if 分支结果.returncode != 0 or 分支结果.stdout.strip() != 目标分支:
                结果.append(检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} 实际分支与登记不一致"))
            变更状态, 变更错误 = 读取工作树变更状态(目标路径)
            if 变更错误:
                结果.append(
                    检查项("ERROR", "项目记忆/并行工作树登记表.md", f"{通道} 无法读取工作树变更: {变更错误}")
                )
            else:
                Markdown变更 = sorted(路径 for 路径 in 变更状态 if Path(路径).suffix.lower() == ".md")
                if Markdown变更:
                    结果.append(
                        检查项(
                            "ERROR", "项目记忆/并行工作树登记表.md",
                            f"{通道} 执行工作树含 Markdown 变更: {', '.join(Markdown变更)}",
                        )
                    )
                非Markdown变更 = {路径 for 路径 in 变更状态 if Path(路径).suffix.lower() != ".md"}
                if 活动计划段:
                    活动任务 = 活动计划段[0]["任务"]
                    活动计划路径 = 活动计划段[0]["计划路径"]
                    活动计划文件, 路径错误 = 解析仓库内路径(活动计划路径)
                    if 路径错误 or 活动计划文件 is None or not 活动计划文件.is_file():
                        结果.append(
                            检查项(
                                "ERROR", "项目记忆/并行工作树登记表.md",
                                f"{通道} 无法读取活动计划段 {活动任务} 的允许范围",
                            )
                        )
                    else:
                        允许文件, 允许错误 = 提取精确允许文件(读取文本(活动计划文件), 活动计划路径)
                        if 允许错误:
                            结果.append(检查项("ERROR", 活动计划路径, f"无法建立活动计划段 dirty 白名单: {允许错误}"))
                        else:
                            越界路径 = sorted(非Markdown变更 - 允许文件)
                            if 越界路径:
                                结果.append(
                                    检查项(
                                        "ERROR", "项目记忆/并行工作树登记表.md",
                                        f"{通道} 活动计划段 {活动任务} dirty 文件越出允许范围: {', '.join(越界路径)}",
                                    )
                                )
                elif 非Markdown变更:
                    结果.append(
                        检查项(
                            "ERROR", "项目记忆/并行工作树登记表.md",
                            f"{通道} 没有活动计划段时必须保持 clean",
                        )
                    )

    索引治理文本 = 读取文本(计划索引路径)
    长期通道映射已启用 = "长期执行通道映射" in 索引治理文本 and "#340—#351" in 索引治理文本
    if 长期通道映射已启用:
        if 结构化通道已建立:
            共享通道: set[str] = set()
            for 编号 in range(340, 352):
                任务 = f"#{编号}"
                匹配段 = [(通道, 字段) for (通道, 当前任务), 字段 in 计划段项.items() if 当前任务 == 任务]
                if len(匹配段) != 1:
                    结果.append(
                        检查项(
                            "ERROR", "项目记忆/并行工作树登记表.md",
                            f"{任务} 必须且只能登记一个当前版本长期执行计划段，实际 {len(匹配段)} 个",
                        )
                    )
                else:
                    共享通道.add(匹配段[0][0])
            if len(共享通道) != 1:
                结果.append(
                    检查项(
                        "ERROR", "项目记忆/并行工作树登记表.md",
                        f"#340—#351 必须共享同一长期执行通道，实际涉及 {len(共享通道)} 个",
                    )
                )
        else:
            for 编号 in range(340, 352):
                ...
                if (
                    当前任务.字段["技术绑定阶段"] != "未绑定"
                    or 当前任务.字段["执行身份"] != "—"
                    or 当前任务.字段["worktree 引用"] != "—"
                ):
                    结果.append(
                        检查项(
                            "ERROR", 文件相对路径,
                            f"{任务} 在长期执行通道仅作目标登记时必须保持技术未绑定",
                        )
                    )
        for _, 技术登记 in 登记原始:
            定位键 = 技术登记["计划 / 状态定位键"]
            身份匹配 = re.fullmatch(r"WT-(?P<编号>34[0-9]|35[01])-R\d+", 技术登记["身份"])
            当前任务 = 当前项.get(f"#{身份匹配['编号']}") if 身份匹配 else None
            if 身份匹配 and 当前任务 and 当前任务.字段["计划版本"] in 定位键:
                结果.append(
                    检查项(
                        "ERROR", "项目记忆/并行工作树登记表.md",
                        f"{技术登记['身份']} 仍把当前计划版本登记为逐任务 worktree",
                    )
                )
    for 任务, 当前 in 当前项.items():
        字段 = 当前.字段
        ...
        _, 允许错误 = 提取精确允许文件(计划文本, 计划路径)
        if 允许错误:
            结果.append(检查项("ERROR", 计划路径, f"执行允许文件合同非法: {允许错误}"))
        ...
        长期执行通道匹配 = 长期执行通道身份模式.fullmatch(字段["worktree 引用"])
        集成工作树匹配 = 集成工作树身份模式.fullmatch(字段["worktree 引用"])
        集成生命周期 = {
            "已预留待集成派发", "已派发待集成回执", "集成中", "集成身份冲突待设计重建",
            "集成失败退回任务", "集成分支验证通过待主线发布", "已集成待设计同步",
        }
        if 字段["当前生命周期"] in 集成生命周期 and (任务工作树匹配 or 长期执行通道匹配):
            结果.append(检查项("ERROR", 文件相对路径, f"{任务} 集成生命周期不得继续引用执行 worktree / 长期通道身份"))
        ...
        if 长期执行通道匹配:
            ...
            计划段 = 计划段项.get((通道, 任务))
            if not 技术登记:
                结果.append(检查项("ERROR", 文件相对路径, f"{任务} 长期执行通道未登记: {通道}"))
            if not 计划段:
                结果.append(检查项("ERROR", 文件相对路径, f"{任务} 在 {通道} 中缺少结构化计划段"))
            else:
                对照 = {
                    "计划版本": 字段["计划版本"],
                    "计划 blob": 字段["计划 blob"],
                ...
                允许段状态 = 状态对应.get(字段["当前生命周期"])
                if 允许段状态 and 计划段["段状态"] not in 允许段状态:
                    结果.append(
                        检查项(
                            "ERROR", 文件相对路径,
                            f"{任务} 生命周期 {字段['当前生命周期']} 与计划段状态 {计划段['段状态']} 不一致",
                        )
                    )
                if 任务 == "#352" and 字段["当前生命周期"] == "依赖门控待执行" and 计划段["段状态"] not in {"未开始", "已退出"}:
                    结果.append(
                        检查项(
                            "ERROR", 文件相对路径,
                            "#352 仍处于 CUTOVER-GATE 真实依赖门控，不得在长期通道提前形成活动或结果计划段",
                        )
                    )
            ...
            if 字段["技术绑定阶段"] != "通道已绑定":
                结果.append(
                    检查项(
                        "ERROR", 文件相对路径,
                        f"{任务} 引用长期执行通道时技术绑定阶段必须为通道已绑定",
                    )
                )
            else:
                基线提交 = 字段["冻结基线提交"]
                if 计划段 and 计划段["段状态"] == "未开始":
                    for 字段名 in ("冻结基线版本", "冻结基线提交"):
                        if 字段[字段名] != "—":
                            结果.append(
                                检查项(
                                    "ERROR", 文件相对路径,
                                    f"{任务} 未开始计划段不得提前填写 {字段名}",
                                )
                            )
                elif 计划段 and 基线提交 != 计划段["计划段起点"]:
                    结果.append(
                        检查项(
                            "ERROR", 文件相对路径,
                            f"{任务} 冻结基线提交必须等于当前计划段起点",
                        )
                    )
                if 字段["冻结基线版本"] != "—":
                    核对状态版本(字段["冻结基线版本"], 文件相对路径, f"{任务} 计划段冻结状态版本 ")

            if 技术登记 and 通道 in 通道技术身份:
                目标路径, 目标分支, _ = 通道技术身份[通道]
                技术状态 = 技术登记["技术状态"]
                已回收 = "已回收" in 技术状态
                待建立 = "待建立" in 技术状态 or "未创建" in 技术状态
                if not Path(目标路径).is_dir() and not (已回收 or 待建立):
                    结果.append(检查项("ERROR", 文件相对路径, f"{通道} 登记为可用但物理 worktree 不存在"))
                elif Path(目标路径).is_dir():
                    def 通道Git(*参数: str) -> subprocess.CompletedProcess[str]:
                        return subprocess.run(
                            ["git", *参数], cwd=目标路径, text=True, encoding="utf-8",
                            errors="replace", capture_output=True, check=False,
                        )

                    分支结果 = 通道Git("branch", "--show-current")
                    HEAD结果 = 通道Git("rev-parse", "HEAD")
                    if 分支结果.returncode != 0 or 分支结果.stdout.strip() != 目标分支:
                        结果.append(检查项("ERROR", 文件相对路径, f"{通道} 实际分支与登记不一致"))
                    实际HEAD = HEAD结果.stdout.strip() if HEAD结果.returncode == 0 else ""
                    if 计划段:
                        段起点 = 计划段["计划段起点"]
                        if re.fullmatch(r"[0-9a-f]{40}", 段起点):
                            祖先结果 = 通道Git("merge-base", "--is-ancestor", 段起点, 实际HEAD)
                            if not 实际HEAD or 祖先结果.returncode != 0:
                                结果.append(
                                    检查项(
                                        "ERROR", 文件相对路径,
                                        f"{任务} 计划段起点不是长期通道当前 HEAD 的祖先",
                                    )
                                )
        elif worktree匹配:
    ...
    if "#352" in 当前项 and 当前项["#352"].字段["计划版本"] != "v0.1":
        ...
        提前冻结合同 = {
            "计划合同冻结": (r"^计划合同冻结[：:]\s*PASS\s*$", ()),
            "执行门禁": (
                r"^执行门禁[：:]\s*(?P<值>.+)$",
                ("#339—#351", "固定候选", "#347", "调用点映射", "白名单", "删除集"),
            ),
            "调用点映射合同": (
                r"^调用点映射合同[：:]\s*(?P<值>.+)$",
                ("#347", "逐调用点", "#352 S0", "固定提交", "内容", "本文路径", "不生成"),
            ),
            "最终删除集合同": (
                r"^最终删除集合同[：:]\s*(?P<值>.+)$",
                ("第 6 节", "CUTOVER-DELETE/v1", "精确路径", "#352 S0", "不得增删"),
            ),
            "执行白名单合同": (
                r"^执行白名单合同[：:]\s*(?P<值>.+)$",
                ("第 5 节", "CUTOVER-WHITELIST/v1", "精确路径全集", "S0", "不", "生成白名单"),
            ),
        }
        ...
        三五二允许文件, 三五二允许错误 = 提取精确允许文件(三五二文本, 三五二["计划路径"])
        if 三五二允许错误:
            结果.append(检查项("ERROR", 三五二["计划路径"], f"#352 无法解析固定执行白名单: {三五二允许错误}"))
        elif len(三五二允许文件) != 127:
            结果.append(
                检查项(
                    "ERROR",
                    三五二["计划路径"],
                    f"#352 固定执行白名单应为 127 个唯一路径，实际 {len(三五二允许文件)}",
                )
            )

        删除标题 = re.search(r"^##\s+6\.\s*固定物理删除集\s+`CUTOVER-DELETE/v1`\s*$", 三五二文本, re.MULTILINE)
        三五二删除原始: list[str] = []
        if not 删除标题:
            结果.append(检查项("ERROR", 三五二["计划路径"], "#352 缺少固定物理删除集章节"))
        else:
            后续标题 = re.search(r"^##\s+", 三五二文本[删除标题.end():], re.MULTILINE)
            删除结束 = 删除标题.end() + 后续标题.start() if 后续标题 else len(三五二文本)
            删除章节 = 三五二文本[删除标题.end():删除结束]
            在代码块 = False
            for 行内容 in 删除章节.splitlines():
                去空 = 行内容.strip()
                if 去空.startswith("```"):
                    在代码块 = not 在代码块
                    continue
                if not 在代码块 or not re.search(r"\.(?:h|cpp|ixx)$", 去空, re.IGNORECASE):
                    continue
                路径, 错误 = 解析仓库内路径(去空)
                if 错误 or 路径 is None:
                    结果.append(检查项("ERROR", 三五二["计划路径"], f"#352 删除路径非法: {去空}（{错误}）"))
                    continue
                三五二删除原始.append(路径.relative_to(根目录.resolve()).as_posix())
        三五二删除文件 = set(三五二删除原始)
        if len(三五二删除原始) != 39 or len(三五二删除文件) != 39:
            结果.append(
                检查项(
                    "ERROR",
                    三五二["计划路径"],
                    f"#352 固定删除集应为 39 个互不重复路径，实际总数 {len(三五二删除原始)}、唯一数 {len(三五二删除文件)}",
                )
            )
        if not 三五二允许错误 and not 三五二删除文件.issubset(三五二允许文件):
            结果.append(检查项("ERROR", 三五二["计划路径"], "#352 固定删除集存在白名单之外路径"))

        唯一新建路径 = "海中鱼巣/自检.节点直接完整恢复与退役.ixx"
        if not 三五二允许错误 and 唯一新建路径 not in 三五二允许文件:
            结果.append(检查项("ERROR", 三五二["计划路径"], "#352 唯一新建自检路径未进入固定白名单"))
        唯一新建合同 = (
            唯一新建路径 in 三五二文本
            and "唯一允许新建的路径" in 三五二文本
            and "不存在且无同名模块 / 工程项 / 所有者冲突" in 三五二文本
        )
        if not 唯一新建合同:
            结果.append(检查项("ERROR", 三五二["计划路径"], "#352 唯一新建路径的 S0 不存在性合同不完整"))
        ...
        if not 三五二已解除门控:
            ...
            证据内容合同 = {
                ...
                "合同接受矩阵": ("CUTOVER-ACCEPT/v1", ("NT-C1/v0.3", "NT-C4/v0.3", "签名", "ABI", "所有权", "生命周期", "结果")),
    ...
    if "具名有序执行序列" in 索引文本 or re.search(r"\|\s*序列名\s*\|", 索引文本):
        结果.append(检查项("ERROR", "计划/计划索引.md", "计划索引不得恢复由设计窗口控制的执行序列"))
    ...
    自主规则要求 = {
        "AGENTS.md": [
            ...
            "自行核对唯一执行占用并完成 S0",
            "当前工作区没有第二活动计划段",
            "长期执行通道只登记一次顶层执行身份、持久 worktree 和持久分支",
        ],
        "规范/设计执行双窗口交互规范.md": [
            "设计窗口不得用任务工具发送",
            "PASS：直接实施并发送事实消息",
        ...
        "规范/多工作树并发与集成规范.md": [
            "同一时刻只有一个活动计划段",
            "通道 HEAD 可以随着计划段提交累计前进",
        ],
        ".codex/skills/hai-zhong-yu-chao-plan/SKILL.md": [
            "设计发布到此结束",
            "不得通过 Codex 任务工具向执行窗口发送计划",
            "不得为每份计划重复建立 R2、派发状态或执行许可",
        ],
        ".codex/skills/hai-zhong-yu-chao-execute/SKILL.md": [
            "不等待设计窗口派发、确认冻结、接受 S0 或选择下一项",
            "执行窗口自行完成 S0，PASS 后直接写入",
        ],
    }
    for 相对路径, 必需片段 in 自主规则要求.items():
        当前文本 = 读取文本(根目录 / 相对路径)
        for 片段 in 必需片段:
            if 片段 not in 当前文本:
                结果.append(检查项("ERROR", 相对路径, f"缺少角色自主边界: {片段}"))

    执行技能文本 = 读取文本(根目录 / ".codex" / "skills" / "hai-zhong-yu-chao-execute" / "SKILL.md")
    for 禁止片段 in (
        "只有台账当前状态已由设计窗口接受到允许写入的阶段",
        "设计窗口确认当前计划冻结后",
        "只有设计确认冻结后才越过当前项",
    ):
        if 禁止片段 in 执行技能文本:
            结果.append(检查项("ERROR", ".codex/skills/hai-zhong-yu-chao-execute/SKILL.md", f"仍保留设计调度通道: {禁止片段}"))

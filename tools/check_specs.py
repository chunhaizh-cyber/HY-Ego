...
def 检查任务状态治理() -> list[检查项]:
    ...
    自主规则要求 = {
        "AGENTS.md": ["设计窗口不得创建执行许可", "自行核对唯一执行占用并完成 S0"],
        "规范/设计执行双窗口交互规范.md": ["设计窗口不得用任务工具发送", "PASS：直接实施并发送事实消息"],
        ".codex/skills/hai-zhong-yu-chao-plan/SKILL.md": ["设计发布到此结束", "不得通过 Codex 任务工具向执行窗口发送计划"],

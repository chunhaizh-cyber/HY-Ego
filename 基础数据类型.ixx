...
export enum class 枚举_比较模式 : std::uint8_t {
    ...
    海明距离 = 10,
    // 适用：U64 / VecIU64（位签名、体素块、轮廓块等）
    // 语义：按 bit 计数差异（xor 后 popcount）；越小越相似
    // 常见：Bits64签名、体素占据签名、二值轮廓编码

    L1距离 = 11,
    // 适用：I64/U64（标量时=|a-b|）、VecI64/VecIU64（逐维绝对差求和）
    // 语义：曼哈顿距离；对异常值比 L2 更“稳”，适合粗匹配/鲁棒聚合

    L2距离 = 12,
    // 适用：I64/U64（标量时=(a-b)^2 或 |a-b| 再开方取决于实现）、VecI64/VecIU64
...
// =========================
export using VecI64 = std::vector<std::int64_t>;
...
export struct 结构体_存在观测 {
    ...
    std::vector<U64>  轮廓编码;      // 8×8×n×n 的封闭内部二值图块
    std::vector<颜色_BGR8> 轮廓编码点颜色;
    // 平时不填（empty），仅当本帧触发雕刻时填
    std::vector<U64> 三维模型_体素立方体;     // blocks
...
export struct 点簇增强结果 {
    ...
    // ===== 轮廓与形状摘要 =====
    std::vector<U64> 轮廓编码;                          //原始轮廓编码,小于8*8的忽略
    std::vector<Vector3D>    轮廓3D;                   // 深度边缘反投影得到的3D轮廓点（可选，调试/精细匹配用）

    std::vector<U64> 边界点序列;                  // 可选：全图坐标打包的边缘像素序列 (v<<32|u)

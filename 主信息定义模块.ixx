...
public:
    因果主信息类() : 基础信息基类(nullptr, nullptr, 枚举_主信息类型::因果) {}
    因果主信息类(const 词性节点类* 名, const 词性节点类* 型) : 基础信息基类(名, 型, 枚举_主信息类型::因果) {}
...
// -------------------- 工具：比较 --------------------
// 已迁移到 通用函数类（比较指针_条件 / 比较I64_条件 / 比较U64_条件）。
...
export struct 结构体_动作句柄 {
    ...
    // 三选一使用：
    std::uint64_t 本能ID_U64 = 0; // 当 类型==本能函数ID
    std::string 外部实现主键;            // 当 类型==外部实现主键
    std::string 动作序列主键;            // 当 类型==动作序列主键

    bool 有效() const {
        switch (类型) {
        case 枚举_动作句柄类型::本能函数ID: return 本能ID_U64 != 0;
...
inline 特征值主信息类* 特征值主信息类::反序列化(std::istream& is)
{
    ...
    if (ver == 1) {
        // 兼容旧存档：旧版特征值链可能混入 I64/U64/字符串等值。
...
inline void 基础信息基类::序列化(std::ostream& os) const
{
    ...
    else if (dynamic_cast<const 因果主信息类*>(this)) tag = 枚举_主信息类型::因果;
    ...
    case 枚举_主信息类型::存在: {
        ...
        // 指向其他节点的指针只存主键
        写字符串(os, p ? (p->概念模板 ? 节点主键(p->概念模板) : p->概念模板->获取主键()) : std::string{});
        写字符串(os, p ? (p->内部世界 ? 节点主键(p->内部世界) : p->内部世界->获取主键()) : std::string{});

        写POD(os, p ? p->创建时间 : 0);
        写POD(os, p ? p->最后观测时间 : 0);
        写POD(os, p ? p->命中次数 : 0u);
        写POD(os, p ? p->可信度 : 1.0f);
        // 其他统计/历史较大，这里先不做全量存档（可按需扩展）
        break;
    }
    case 枚举_主信息类型::场景: {
        auto* p = dynamic_cast<const 场景节点主信息类*>(this);
        写字符串(os, p ? (p->宿主存在 ? 节点主键(p->宿主存在) : p->宿主存在->获取主键()) : std::string{});
        写枚举(os, p ? p->世界类型 : 枚举_世界类型::现实世界);
        写字符串(os, p ? (p->来源文章 ? 节点主键(p->来源文章) : p->来源文章->获取主键()) : std::string{});
...
inline 基础信息基类* 基础信息基类::反序列化(std::istream& is)
{
    ...
    case 枚举_主信息类型::存在: {
        ...
        out->概念模板 = nullptr;
        out->内部世界 = nullptr;
        out->概念模板->获取主键() = std::move(kConcept);
        out->内部世界->获取主键() = std::move(kInner);
        out->创建时间 = createT;
        out->最后观测时间 = lastT;
        out->命中次数 = hit;
        return out;

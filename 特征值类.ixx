...
private:
   ...
   // std::unordered_map<uint64_t, std::vector<特征值节点类*>> 矢量金字塔索引[最大层级 + 1];
    ...
    /// </summary>
    static uint64_t 计算矢量压缩哈希(const std::vector<std::int64_t>& 原始矢量, int 目标层级);
...
public:
    ...
    // 1. 标量特征值（int64_t + 单位）
    //==================================================================
    特征值节点类* 获取或创建标量特征值(语素节点类* 单位指针, std::int64_t 数值);
    ...
    // 3. 矢量特征值（任意维度）
    //==================================================================
    特征值节点类* 获取或创建矢量特征值(const std::vector<std::int64_t>& 矢量值);
   

    // 方便的单值快捷方式
    特征值节点类* 获取或创建矢量特征值(std::int64_t 单值);
    ...
    相似矢量结果 查找最相似矢量(const std::vector<std::int64_t>& 目标矢量, int 使用层级 = 4) const;
    //==================================================================
...
public:
    ...
    特征值节点类* 查找标量特征值(语素节点类* 单位指针, std::int64_t 数值) const;

    // 字符串特征值：会做【小写化 + 去空格】之后再查索引
    特征值节点类* 查找字符串特征值(const std::string& 原始文本) const;

    // 矢量特征值：使用最高层级的哈希做索引，再逐元素比对确认
    特征值节点类* 查找矢量特征值(const std::vector<std::int64_t>& 矢量值) const;

...
std::uint64_t 特征值类::FNV1a64_MixU64(std::uint64_t h, std::uint64_t x) noexcept {
    for (int i = 0; i < 8; ++i) {
...
std::uint64_t 特征值类::哈希VecIU64(const VecIU64& v) noexcept {
    std::uint64_t h = 1469598103934665603ull;
    h = FNV1a64_MixU64(h, (std::uint64_t)v.size());
    for (auto x : v) h = FNV1a64_MixU64(h, x);
    return h;
}
...
static inline std::uint64_t FNV1a64_MixU64_local(std::uint64_t h, std::uint64_t x) noexcept {
    for (int i = 0; i < 8; ++i) {
        const std::uint8_t b = (std::uint8_t)((x >> (i * 8)) & 0xFF);
        h ^= (std::uint64_t)b;
        h *= 1099511628211ull;
...
static inline std::uint64_t 哈希层(const VecIU64& layer) {
    std::uint64_t h = 1469598103934665603ull;
    h = FNV1a64_MixU64_local(h, (std::uint64_t)layer.size());
    for (auto x : layer) h = FNV1a64_MixU64_local(h, x);
    return h;
}

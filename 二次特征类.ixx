...
public:
    ...
    {
        if (!场景 || !新状态) return;
        if (auto* 新动态 = 动态集.状态记录后刷新动态主信息(场景, 新状态, now, 调用点 + "/动态")) {
            const bool 验收关注 = (调用点.find("安全值") != std::string::npos) || (调用点.find("记录安全状态") != std::string::npos);
            if (验收关注) {
                日志::运行f("[验收安全链路] 动态已生成(状态触发/运行时): 场景={}, 动态={}, 来源状态={}",
                    static_cast<void*>(场景),
                    static_cast<void*>(新动态),
                    static_cast<void*>(新状态));
            }
            因果集.动态记录后刷新因果(场景, 新动态, now, 调用点 + "/因果");
            刷新新状态的二次特征(场景, 新状态, now, 调用点 + "/状态");
            return;
        }
        ...
        const std::string& 调用点 = "二次特征类::刷新新状态的二次特征")
    {
        ...
        smi->清理空指针();
        const std::size_t 关系数量起始 = smi->关系列表.size();
        // 刷新过程中会为关系节点继续追加状态，先拍一份快照，避免遍历状态列表时
        // 因 vector 扩容/重排触发未定义行为。
        const std::vector<状态节点类*> 状态列表快照 = smi->状态列表;

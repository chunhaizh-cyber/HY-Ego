...
bool 自然语言知识库类::融合_空间位置(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
...
bool 自然语言知识库类::融合_时间定位(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
...
bool 自然语言知识库类::融合_比较排序(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    if (词序列.size() < 3) return false;
    ...
    const 词性节点类* A词 = 词序列[比索引 - 1];
    const 词性节点类* B词 = 词序列[比索引 + 1];

    // 简单：最后一个词当作“比较维度（形容词）”
    const 词性节点类* 维度词 = 词序列.back();
...
bool 自然语言知识库类::融合_数量范围(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    if (词序列.size() < 3) return false;
...
//5. 否定与排除：不 / 没 / 不是 / 没有
bool 自然语言知识库类::融合_否定排除(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    int 否定索引 = -1;
    for (size_t i = 0; i < 词序列.size(); ++i) {
        if (是否定词(词序列[i])) {
...
//6. 疑问与信息需求：吗 / 什么 / 谁 / 哪儿 / …？
bool 自然语言知识库类::融合_疑问需求(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    bool 有疑问 = false;
    bool 是是非问 = false;
...
//7. 条件与假设：如果 A，(就) B
bool 自然语言知识库类::融合_条件假设(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    int 条件索引 = -1;
    int 就索引 = -1;
...
//8. 因果与解释：因为 A，所以 B
bool 自然语言知识库类::融合_因果解释(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    int 因索引 = -1;
    int 果索引 = -1;
...
bool 自然语言知识库类::融合_时间体貌(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    std::string 体貌标记;
    for (auto* 词 : 词序列) {
        auto t = 取词文本(词);
...
//10. 评价与立场：好 / 坏 / 喜欢 / 讨厌
bool 自然语言知识库类::融合_评价立场(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    int 评价索引 = -1;
    int 极性 = 0;
...
bool 自然语言知识库类::融合_来源引述(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    int 说索引 = -1;
    for (size_t i = 0; i < 词序列.size(); ++i) {
        auto t = 取词文本(词序列[i]);
...
bool 自然语言知识库类::融合_定义分类(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    int 系词索引 = -1;
    for (size_t i = 0; i < 词序列.size(); ++i) {
        auto t = 取词文本(词序列[i]);
...
bool 自然语言知识库类::融合_角色标注(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    ...
    const 词性节点类* 施事词 = 词序列[0];               // 简化：第一个词
    const 词性节点类* 受益者词 = 词序列[给索引 + 1];
    const 词性节点类* 主题词 = 词序列.back();           // 简化：最后一个
...
bool 自然语言知识库类::融合_约束义务(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    int 模态索引 = -1;
    for (size_t i = 0; i < 词序列.size(); ++i) {
        if (是模态词(词序列[i])) {
...
//15. 例外与对照：除了 A 以外，B
bool 自然语言知识库类::融合_例外对照(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    int 除了索引 = -1;
    int 以外索引 = -1;
...
bool 自然语言知识库类::融合_单位度量(const std::vector<const 词性节点类*>& 词序列,世界树类& 世界树,场景节点类* 场景)
{
    if (词序列.size() < 3) return false;
...
bool 自然语言知识库类::融合_指代省略( const std::vector<const 词性节点类*>& 词序列, 世界树类& 世界树, 场景节点类* 场景)
{

...
全部精确函数、类型、字段来源、物理映射、依赖方向、原子参与者次序、原请求恢复、失败结果和验收条件冻结在[详细设计 v0.3](../规范/详细设计/20260913_WORLD-TREE-CONCEPT-FIRST-A_概念前置与世界添加详细设计_v0.1.md)。本文不再复制其 DTO 与流程。
...
详细设计第8节八组、v0.2对第6—7组及v0.3对第4组的增补为必测，本批Debug/Release都须运行。代码构建不是业务验收：探针须从正常 `运行普通程序`/`初始化普通应用自我` 及公开数据服务进入，并通过 `读取普通应用自我`和各owner公开读回证明实际终态。普通二进制退出1且阶段19是后继未实现的真实输出；只有同时证明self角色/同节点/根/概念闭包，才能称A完成。

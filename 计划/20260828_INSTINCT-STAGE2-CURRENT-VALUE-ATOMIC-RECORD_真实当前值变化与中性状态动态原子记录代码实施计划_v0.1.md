...
4. 普通应用唯一构造、getter、独立读回和本能根重启恢复接线完成；持久恢复后的第二次完整构造中，全部固定登记复用首次 G0 / G1 和编码映射。
...
- 每个固定登记必须区分首次账成功、未找到、许可 / 资源 / 内部失败和账内缺项；成功恢复原样重放首次规范化写集，未找到才按当前 G 形成首次请求，提交后均独立读回。
...
- 使用受控临时持久根验证首次、重放、连续迁移和两次普通应用完整构造；逐个固定登记核对首次规范化写集、首次结果、编码映射和独立读回。

...
- `验证工具/世界树根启动专项验证.cpp`（仅修正 `实例特征结构登记请求` 为版本2和幂等键 `0x4946525354525632`；不得删除或弱化既有断言）
...
10. 将世界树根启动专项中唯一旧实例特征结构夹具更新为版本2和 `0x4946525354525632`，不改生产接口、不恢复 v1 兼容、不删除测试断言。
11. 完成最低验证矩阵，按真实结果修复本计划范围内代码问题。
12. 写实施/验证记录；实施记录包含函数级代码知识变更清单。
13. 精确暂存允许文件，复核 staged diff，普通提交并推送。

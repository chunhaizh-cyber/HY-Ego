...
详细设计：`规范/详细设计/20260924_INSTINCT-ROOT-RC-PRODUCTION-INIT_安全服务双根RC生产初始化与稳定定位详细设计_v0.1.md`
...
4. 实现每根 K 的当前路由读取→定义核验→固定键建立→发布未知按完整原请求重放收敛→身份和当前路由双读回。
5. 实现每根 RC 的完整定义查找→固定键建立→原请求收敛→身份完整读回。
6. 冻结最终 G，重读两根 FT/FC、共同 EC、双 K/当前路由和双 RC，形成角色化交付。
7. 按详细设计冻结的数值15、原因字段、每次公开调用独立使用的三组具名生产预算接入普通应用上下文、重复装配核验和只读访问器；不修改启动阶段顺序。
...
专项必须逐项报告详细设计第9节的十二类验证；未实际覆盖发布未知或跨进程时不得写为 PASS，也不得以构建替代。

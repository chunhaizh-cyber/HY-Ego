    ...
    bool 本能_观察场景(场景节点类* 输入场景, 场景节点类* 输出场景)
    {
        ...
        if (!gotFacts) {
            gotFacts = 读帧输出
                ...
                && 方法集.调用本能函数(枚举_本能动作ID::外设_提取前景存在候选事实, 输入场景, 读帧输出).成功;
        }
        if (!gotFacts) {
            auto* 取帧输出 = 建场景("观察_取帧输出", "观察过程场景", "本能_观察场景/取帧输出");
            bool gotFrame = 取帧输出 && 方法集.调用本能函数(枚举_本能动作ID::外设_获取帧信息, 输入场景, 取帧输出).成功;
            if (!gotFrame) {
                日志::运行(
            ...
            if (!方法集.调用本能函数(枚举_本能动作ID::自我_读取帧特征信息, 读帧输入, 读帧输出).成功) {
                守卫.设置结果(false, -14);
                写回执(输出场景, false, -14);
                return false;
            }
        ...
        if (识别率输出 && 方法集.调用本能函数(枚举_本能动作ID::自我_获取画面识别率, 读帧输出, 识别率输出).成功) {
        ...
        for (auto* cand : 场景类::获取子存在(candScene)) {
            ...
            const bool found = 方法集.调用本能函数(枚举_本能动作ID::自我_查找存在, 查找输入, 查找输出).成功;
            ...
            if (target) {
                ...
                if (方法集.调用本能函数(枚举_本能动作ID::自我_更新存在, 更新输入, 更新输出).成功) {
                    updated += 1;
                ...
                if (方法集.调用本能函数(枚举_本能动作ID::自我_生成存在, 生成输入, 生成输出).成功) {
                    created += 1;
